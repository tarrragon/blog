...
**收集的項目**：頁面路徑、來源網址（`document.referrer`）、瀏覽器語言、裝置類別（僅 mobile / tablet / desktop 三選一）、兩組隨機識別碼、事件型別（進入或離開）、頁面停留秒數、是否曾捲動或點擊，以及一個自動化訪客標記——那個標記只包含比對命中的關鍵字（例如 `googlebot`）與環境旗標，不含完整 User-Agent。

**不收集的項目**：IP 位址、完整 User-Agent、螢幕解析度或其他指紋特徵、任何頁面輸入內容。IP 在技術上也讀不到 — 接收端是 Google Apps Script，它的請求物件不提供 HTTP header。

兩組識別碼是瀏覽器產生的隨機值（優先用 `crypto.randomUUID()`，不可用時退回時間戳加亂數），不含任何個人資訊，用途是區分「同一次瀏覽讀了幾篇」與「是不是回訪」。其中一組存在 `sessionStorage`，關閉分頁即消失。
...
設定存在瀏覽器的 `localStorage`（不是 cookie、不會隨請求送出），綁定「這個瀏覽器 + 這個站」。所以換瀏覽器或換裝置要各設定一次，清除網站資料後也需要重新設定。要恢復統計改用 `?noanalytics=0`。

...
- **寫的當下就用內容名**：分組用它回答的問題（「語意模型」），步驟用動作（「選寫法」），分級用它的判準（「當場報錯」），並排的做法用描述（「日期差自連接」）。段標不帶序號，引用用節名或內容。標題與 slug 裡的計數一起改（舊網址用 `aliases` 保留）。
...
**判定者不代填槽位。** 單位要寫在句子裡；從上下文推得出來不算。判準是是非題「有沒有寫在句子裡」，寫成「推不推得出來」它永遠通過——一次實測判定者替「完全一樣」補了單位然後放行。

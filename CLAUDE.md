...
技術文章的每一句讓讀者知道四件事裡的至少兩件：**誰**在做（操作者、工具、系統元件）、**對什麼**做（哪一份資料、哪一段輸出）、**做了什麼**（轉換、比對、寫入）、**結果是什麼**（輸出變什麼、狀態變什麼）。這條標準獨立於文章的定位，適用於 `content/`、`.claude/skills/`、AGENTS.md 的所有書面產出。

兩條規則：

1. **主詞是人或系統元件，不是指令片段**。指令片段（`tr -d ''`、`-G 16M`、`exec-out`）當主詞時語法完整，但做事的人與被操作的資料都消失了。

   改前：「`tr -d ''` 處理的是行尾」（誰在跑 tr？對什麼做？）

   改後：「腳本裡每一行 `getprop` 的結尾都接了 `tr -d ''`，作用是去掉輸出裡的歸位字元。」（操作者=腳本、工具=tr、受詞=輸出裡的歸位字元）

2. **強調句式用直述事實替代**。「一個 X 都沒有 Y」「根本沒」「完全一樣」這些口語強調在文字上讓讀者多一步拆解。

   改前：「遠端一個字都沒有執行」

   改後：「指令沒有被送到遠端執行」

寫每一段操作說明時，按讀者的問題序排列：讀者會看到什麼 → 這個工具做什麼 → 為什麼需要它（機制，系統元件指名）→ 不做的具體後果 → 副作用。主詞換對之後段落順序通常自動修正。

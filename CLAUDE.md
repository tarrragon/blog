...
- **文章不連 `/<分類>/#錨點`**：那些錨點在渲染後的頁面上不存在，連結看起來正常而點下去什麼都沒有。掃法：`scripts/index-leak-check.py --anchors`（任何層數的路徑都比對）。先前寫在這裡的 `rg '\]\(/[a-z0-9-]+/#'` 只比對單層路徑，`/testing/06-agent-authored-code/#…` 這種多層的全數漏掉，一次全站掃描補出七處。「寫在[模組入口]」這類連到目錄頁本身、宣稱內容在那裡的句子是同一個問題，腳本抓不到，寫到模組入口時自己問一次：那段內容讀者看得到嗎。

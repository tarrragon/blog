...
1. **開場交代範圍，一句話**。讀者由搜尋進來、帶著自己的問題，標題與 description 已經告訴他這篇講什麼。開場的職責是交代適用範圍（什麼版本、什麼環境、從哪一步開始），用一句話完成。
2. **每個指令與每段說明寫完整的操作主體**。文章的讀者從任意段落落進來，手上只有這一段的文字。「回答的卻是沒問的那個問題」——是什麼工具回答的、回答了什麼、讀者該看哪個欄位，全部寫出來。
3. **正確的做法與讀法直接寫在指令旁邊**。某個輸出有多種讀法時，從一開始就引導讀者看對的欄位、給出正確的讀法。每段說明自帶脈絡，讀者照著段落順序讀就能理解，不依賴對前文的記憶。

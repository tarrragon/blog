...
AGENTS.md 原則十一說 `content/` 底下一律教學定位，而「教學」被 LLM 讀成「教室講課」是本專案反覆出現的同一類問題。三個高頻形態：

1. **情境故事開場**——讀者由搜尋進來、知道自己在排查什麼，不需要一段故事替他建構情境。範圍交代用一句話，不用一段事件敘事。
2. **省略主詞受詞**——教室裡師生共享脈絡所以主詞可以省，文章的讀者從任意位置落進來、手上沒有作者腦中的地圖。每個指令與每段說明的操作主體寫完整。
3. **模擬教室互動**——「這個問題該問誰」「這裡容易讀反」「前面提到的四種」「先確認 A 再做 B」。直接把正確做法與讀法寫清楚，如果某個輸出容易被讀反就一開始引導讀者看對的欄位，而不是寫完之後告訴他「剛剛那段容易搞錯」。

判讀徵兆與根因分析在 [#355](/report/teaching-is-not-lecturing/)。寫 `content/` 的時候先過一次這三條再提交。

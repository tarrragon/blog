...
體例分兩種，判別用一句話：**讀者是在掃列表決定要不要進去，還是已經決定要讀而需要知道範圍。** 前者寫成觸發條件（什麼情境下需要回來讀），後者寫成涵蓋交代（這一篇涵蓋什麼、交付哪一段觀念）。這個問句取代枚舉，因為枚舉一定漏——枚舉外的檔案在規範裡沒有答案，而三個最大的分類都在枚舉外。

- **涵蓋交代**：`content/` 下規劃在序列裡的文章與章節（教材）。讀者是照路線走到這裡的，`description` 補上標題說不完的部分。
- **觸發條件**：排錯記錄、工具筆記、知識卡的定義層、`report/` 與 `record/` 的卡（歸納層，讀者帶著一個問題來找有沒有人處理過）、以及各分類的 `_index.md`（那是掃列表的位置本身，「我要不要進去」正是它主要的作用）。
...
2. **論證用得到的材料就地寫進去**——承重術語當場定義、承重機制寫進正文、每一句的指涉能從已讀文字解出來。這三項作者看不見，成因是熟悉度。檢查：跑 `scripts/loadbearing-terms.py`，把它列出的高頻實詞逐個回全文查有沒有一處說過它指什麼。**那支腳本每檔只回前三名**（`TOP_K = 3`），所以它是抽樣不是全集，第四名以後結構上看不到——「三個都查過」與「這篇只有三個高頻詞」在報告上長得一樣。**這支腳本只涵蓋高頻的那一半**——實測它對本站自己的一次違規（「承重」在四個檔用了十九次而全庫零定義）命中率是零，因為那個詞在單篇裡只出現五次、排在候選清單十名之後。跨檔高頻而單篇中頻的自創詞要另外掃：把候選換成「這一批反覆出現的詞」，再逐個查它在別處有沒有被定義過。
3. **往外的連結綁在本篇模型的一個可動項上**——可動項指本篇建立的模型裡可以獨立改變的那幾件事，每一條連結是把其中一個換掉會走到哪一篇，而不是「遇到 X 去讀 Y」，也不只是列舉相鄰主題。這裡有兩個獨立的檢查：遮住目的地看讀者預不預測得出那一篇給什麼（測連結有沒有交代目的地，見 [#303](/report/cross-links-eat-the-article-they-live-in/)），以及那一句綁在什麼上（綁在本篇的一個可動項上才是第三種）。第二個檢查的前置動作是**先把本篇的可動項列出來**：列得出兩個以上走第三種；列得出一個是單軸（判斷標準篇與術語卡多屬這一類，換掉它得到的是同一篇的另一半而不是隔壁那一篇），第二種在那裡就完整；**列不出來是訊號，不是豁免**——那代表本篇還沒建立帶可動項的模型。豁免與訊號在報告上分開寫，否則豁免會把訊號收掉。另外三種形態只沿同層鄰篇這個軸排，往回問來源、往前問下游的連結是另一個軸，不必綁在可動項上。

...
兩個例外落在相反的那一側，而且理由相同：它們的使用者不是來理解一件事的，是來查一條規則或找一個落點的。

**`_index.md`** 是分類的操作手冊：成員表、場景路徑、Backlog、路由。使用者是要找落點的人與維護這個分類的 agent，動作是照著它走到某一篇或某一列。

**`report/`** 是本專案的規則語料。卡的消費者是即將動手寫或審而需要查一條規則的人或 agent——`判讀徵兆` 是症狀清單、`修法` 是可執行的處置，而 skill 直接把卡複製成 principle。它解釋理由是為了讓規則套得準，不是為了讓人讀懂一個主題。

這兩處該有的正是文章不該有的：編號、清單、步驟、查表用的對照欄。掃體例的時候把 `_index.md` 與 `content/report/` 排除在外。
...
- [ ] 體例是教學而不是操作手冊（`_index.md` 與 `report/` 不適用，那兩處本來就是手冊）：逐節問「讀者讀完之後那個動作落在哪裡」，在世界上動手就要改框架；七種裝置（表徵對映清單 / 檢查順序指示 / 配比數字 / 顛倒的後果 / 操作判準 / 器具作法 / 序列連接詞）掃過（見原則十一與 [#302](/report/positioning-decides-form-before-any-rule-applies/)）
- [ ] 每段首句是核心原則，不是例子
- [ ] 負向陳述已降到必要最小
- [ ] 每個抽象名詞都有情境化說明

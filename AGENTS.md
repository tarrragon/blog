...
這個問句取代枚舉，因為枚舉一定漏——枚舉外的檔案在規範裡沒有答案，而三個最大的分類都在枚舉外。

- **涵蓋交代**：`content/` 下規劃在序列裡的文章與章節（教材）。讀者是照路線走到這裡的，`description` 補上標題說不完的部分。
- **觸發條件**：排錯記錄、工具筆記、知識卡、`report/` 與 `record/` 的卡（歸納層，讀者帶著一個問題來找有沒有人處理過）、以及各分類的 `_index.md`（那是掃列表的位置本身，「我要不要進去」正是它主要的作用）。
...
寫或改任何一篇教材之前，先讀 [教學文章的脈絡](/record/teaching-article-context-method/)。它是料理與 SQL 兩次全分類改寫抽出來的方法，SSoT 在那一篇，本節只列它的三個支柱與各自的檢查：
...
- [ ] 體例是教學而不是操作手冊（`_index.md` 與 `report/` 不適用，前者是手冊、後者是歸納層）：逐節問「讀者讀完之後那個動作落在哪裡」，在世界上動手就要改框架；七種裝置（表徵對映清單 / 檢查順序指示 / 配比數字 / 顛倒的後果 / 操作判準 / 器具作法 / 序列連接詞）掃過（見原則十一與 [#302](/report/positioning-decides-form-before-any-rule-applies/)）
- [ ] 每段首句是核心原則，不是例子
- [ ] 負向陳述已降到必要最小
- [ ] 每個抽象名詞都有情境化說明
...
- [ ] 稿件裡反覆出現而從未被定義的用詞、以及讀起來偏口語的譬喻，已用術語探針量過，且報告上記了三件事：通用性控制詞收不收斂、測試詞在規範檔的詞頻表、這一批有沒有合格的同域安慰劑詞（撈不到就記「無污染對照」）；判定非通用的詞先列詞形分佈再按義項替換，有英文詞而無可靠中譯的保留英文並連回知識卡（見 §5 的第七種用途與 [#289](/report/term-probe-measures-register-not-invention/)）

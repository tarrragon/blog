...
- 表格中承載推導的每個「情境/判斷」都要有出口，出口有三個：**就地展開**（表格下方加延伸子段落，回答這情境在真實服務長什麼樣、為何成立、常見陷阱是什麼）、**拆卡外部化**（概念與範例住進知識卡、文章保留「什麼情境用哪張卡」的判讀敘事）、**換成連結**（那個概念已經有卡或有專章承載時，格內不寫自撰的簡述、直接指過去）。壓縮判準去遷就格寬不在出口之列。
...
- 選出口之前先查這個概念在別篇有沒有不相容的版本（兩篇各給一套階段表或欄位組時，三個出口都會把矛盾保留下來），判準與收斂做法見 [#263](/report/incompatible-decompositions-look-complementary/)。
...
- 標題與段標是承載結論的直述句；正文不設懸念、也不灌輸——案例是推導的材料，判準在推導走完的位置浮現，讀者是讀完自己長出這個概念、不是被交付結論。把判準壓到文末扣住是懸念，把結論抽成開頭一段要讀者先記住是灌輸，兩者都讓結論與推導脫節。操作型自問句（判準的執行步驟，如「唸出型別名稱後還剩什麼」）可用問句。
...
- 建卡判準是教學需求：讀者若缺少某個術語的知識就難以理解教材，這個術語就值得建卡，即使它目前只在單篇文章出現、即使它不直接驅動某個實作選擇。出現頻率與是否影響實作判斷都只是補充訊號，不參與「是否要建卡」的必要判準。
- 優先使用情境精確命名，避免傘狀詞。
...
驗一段文字會不會被誤解時，判準放在**多份獨立理解實際帶走了什麼**，不放在任何一個 reviewer 的判斷上。做法是把同一個低階模型（Haiku 這類）獨立跑 N 次——同一個模型、N 個互不通訊的實例，不是 N 個不同的模型——每一次都用逐字相同的指令讀同一份稿件，回報他們讀到什麼。跨模型家族的對照沒有測過，換模型就換掉了分歧的來源。

低階在這個用途上是條件、不是妥協。推理能力越強，越容易在讀到殘缺的句子時自動補完，缺漏因此被填平而不現形；成本與速度則決定能不能大量同步派發，而這個方法要多份才成立。

五個設計條件缺一即退化：

- **指令逐字相同**：差異才只來自模型的理解，分歧才可歸因。
...
換問題就換用途、派發方式不變：理解正確性 / 重點涵蓋度 / 主旨明確度 / 閱讀負擔 / 語氣與讀者定位 / 翻成英文（詳見下段）。其中重點涵蓋度的判準與其餘四項不同，比對的是作者的意圖清單與讀者的回報清單之間的差集——讀者不知道自己漏了什麼，所以直接問「你讀懂了嗎」問不出來。
...
第六種用途換的不是問題而是**輸出的語言**：讓探針把同一段中文翻成英文，判準仍是份與份之間的分歧。中文允許不決定的語法項（主詞、單複數、所有格的方向、並列的轄域、字面義還是譬喻義）英文全部強制決定，所以它抓的是前五種抓不到的一層——前五種的回答形式是摘要，而摘要允許保持模糊，翻譯不允許。順序不可顛倒：命題層還沒收斂的稿件跑它會被大量低層 finding 淹沒。判定與操作見 [#286](/report/translation-forces-disambiguation/)。
...
- [ ] 負向陳述已降到必要最小
- [ ] 每個抽象名詞都有情境化說明
- [ ] 承載推導的表格項目都選了出口（就地展開 / 拆卡 / 換連結），沒有為了格寬壓縮判準；查表型與 checklist 型段落依原則四豁免
...
- [ ] 標題與段標是承載結論的直述句、判準由推導交付（不設懸念、也不把結論抽到開頭灌輸）；檢討內容用客觀條件視角、不用第一人稱事件敘事（見 [#254](/report/write-for-readers-not-audiences/)）
...
- 核心規則摘要：
  ...
  - **Front matter**：通用層 `title` + `date` 必填；新文章必填 `description` + `tags`；卡片層加 `description` + `weight` 必填。`description` 的語意要求：寫成 recall trigger（回答「什麼情境下需要回來讀」）而非內容摘要（回答「這篇在講什麼」）；判準是「刪掉 description 後只看 title 能猜出全部內容 → 沒有增量 → 重寫」。寫法四不要：不塞操作細節（留內文）、不加假設前提（「想不起來」「卻發現」）、不用情緒修辭（「別被騙」「打架」）、不內嵌數字（「三項」「6 段」隨內文增刪過時，同 #156）——給人也給機器判斷，每 token 要有資訊量，短能到位就短。詳見 [#170](/report/description-as-recall-trigger/)。
  ...
  - **物理化錯配（人工 grep）**：抽象概念（證據 / 論證 / 結論 / 判準 / 方法 / 責任）配上承重、支撐、懸掛類動詞 — 「證據撐得住」「論證掛在某前提上」「這套說法會垮」。抽象概念沒有重量、支點與材料強度，那些動詞不帶進資訊，只把物理直覺借過來蓋住實際的關係；而且「撐得住」聽起來已經像答案，於是「支持什麼、到什麼範圍」不必被回答。判準可機械執行：**問這個動詞照字面成立需要什麼物理條件**，需要重量或支點的就換掉。替換：足以支持 / 不足以支持、依賴 / 取決於、佐證、維持不久、負責。命中是候選不是判決 — 主詞是實體或系統負載（機器撐得住多少 QPS）合規、就地展開成可逐項對應的類比合規。**下面那串清單是抽樣、不是這一類的全部**：能進清單的只有違規義項佔該詞用法多數的詞，而本義合法的直譯（滑坡）與詞頻過高的泛用動詞（拿、承載）漏在外面，要靠判準逐句掃或另設探針，判讀見 [#267](/report/keyword-list-needs-dominant-violating-sense/)。掃：`rg "撐得住|撐不住|撐得起|撐不起|能撐|撐住|撐起|撐不久|掛在|扛|垮|頂得住|咬得|咬合|啃|咬得|咬合|啃" content/`。判讀依 [#234](/report/word-choice-fits-concept-attributes/)。
...
- **段名固定為 `## Backlog`**：判準是一條 grep 要能列出全部有待辦的模組（指令見規範文章、不在此重複）。段名自創的後果是盤點時無法確定有沒有漏掉某個模組。

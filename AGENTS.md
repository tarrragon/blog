...
1. **這一篇在序列裡的位置**——說得出讀者手上已經有什麼、這一篇交付什麼、刻意不碰什麼；深度看第一項而非題材的表面複雜度。檢查：遮住 `description` 只看標題，說得出交付什麼嗎。
2. **論證要用的東西就地寫進去**——承重術語當場定義、承重機制寫進正文、每一句的指涉能從已讀文字解出來。這三項作者看不見，成因是熟悉度。
3. **往外的連結掛在本篇模型的一個變數上**——「把其中一項換掉會變成什麼」，而不是「遇到 X 去讀 Y」，也不只是列舉相鄰主題。檢查：遮住目的地，讀者能不能預測那一篇給什麼；做不到這一種代表本篇沒有建立帶變數的模型。

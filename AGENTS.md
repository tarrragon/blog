...
2. **論證用得到的材料就地寫進去**——承重術語當場定義、承重機制寫進正文、每一句的指涉能從已讀文字解出來。這三項作者看不見，成因是熟悉度。檢查：跑 `python3 scripts/loadbearing-terms.py <檔案路徑>`（可以一次給多個檔），把它列出的高頻實詞逐個回全文查有沒有一處說過它指什麼。**那支腳本每檔只回前三名**（`TOP_K = 3`），所以它是抽樣不是全集，第四名以後結構上看不到——「三個都查過」與「這篇只有三個高頻詞」在報告上長得一樣。**這支腳本只涵蓋高頻的那一半**——實測它對本站自己的一次違規（「承重」在四個檔用了十九次而全庫零定義）命中率是零，因為那個詞在單篇裡只出現五次、排在候選清單十名之後。跨檔高頻而單篇中頻的自創詞要另外掃：把候選換成「這一批反覆出現的詞」，再逐個查它在別處有沒有被定義過。

...
3. **往外的連結綁在本篇模型的一個變數上**——「把其中一項換掉會變成什麼」，而不是「遇到 X 去讀 Y」，也不只是列舉相鄰主題。這裡有兩個獨立的檢查：遮住目的地看讀者預不預測得出那一篇給什麼（測連結有沒有交代目的地，見 [#303](/report/cross-links-eat-the-article-they-live-in/)），以及那一句綁在什麼上（綁在本篇的一個可動項上才是第三種）。第二個檢查只適用於模型有兩個以上可動項的教材——單軸的判斷標準篇與術語卡結構上只有一個變數，那裡第二種就是完整的形態。

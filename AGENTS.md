...
1. **這一篇在序列裡的位置**——說得出讀者手上已經有什麼、這一篇交付什麼、刻意不碰什麼。檢查：把這三句話實際寫下來，寫不出來的那一句就是還沒定位好的地方。深度看第一項而非題材的表面複雜度，推導與它的射程限制（一次個案、無對照組）在 [#305](/report/basics-anchor-the-advanced/)。
2. **論證用得到的材料就地寫進去**——承重術語當場定義、承重機制寫進正文、每一句的指涉能從已讀文字解出來。這三項作者看不見，成因是熟悉度。檢查：跑 `scripts/loadbearing-terms.py`，把它列出的高頻實詞逐個回全文查有沒有一處說過它指什麼。
3. **往外的連結綁在本篇模型的一個可動項上**——「把其中一項換掉會變成什麼」，而不是「遇到 X 去讀 Y」，也不只是列舉相鄰主題。這裡有兩個獨立的檢查：遮住目的地看讀者預不預測得出那一篇給什麼（測連結有沒有交代目的地，見 [#303](/report/cross-links-eat-the-article-they-live-in/)），以及那一句綁在什麼上（綁在本篇的一個可動項上才是第三種）。第二個檢查只適用於模型有兩個以上可動項的教材——單軸的判斷標準篇與術語卡結構上只有一個變數，那裡第二種就是完整的形態。
...
- [ ] 稿件裡反覆出現而從未被定義的用詞、以及讀起來偏口語的譬喻，已用術語探針量過（同批含通用性控制詞與安慰劑詞、且測試詞不在探針的系統提示裡，才採用結果）；判定非通用的詞先列詞形分佈再按義項替換，有英文詞而無可靠中譯的保留英文並連回知識卡（見 §5 的第七種用途與 [#289](/report/term-probe-measures-register-not-invention/)）

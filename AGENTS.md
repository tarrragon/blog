...
**教學是定位、不是體裁。** 教學定位的意思是讀者來理解一件事，產出形式是有序的說明——有脈絡、有原理、有完整的主詞受詞。它不是教室講課：不需要建構情境讓讀者進入狀態（他從搜尋進來，自帶問題）、不需要省略主詞因為「學生就在面前」（他不在，他手上沒有作者腦中的地圖）、不需要模擬互動引導注意力（「這裡容易搞錯」「該問誰」「前面提到的四種」——操作指引直接把正確做法寫清楚，如果某個輸出容易被讀反，一開始就引導讀者看對的欄位）。LLM 訓練資料裡「教學」最高頻的範例是教室場景（線上課程講稿、教科書導入段、教學影片旁白），規範裡沒有一條把教室形式排除在外，生成端就會照那些範例寫。判讀徵兆在 [#355](/report/teaching-is-not-lecturing/)。

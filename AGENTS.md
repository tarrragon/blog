...
這一項有一個不可讓渡的設計條件：**譯者只能拿得到文字**。所以不要讓寫這段的實例自己翻，也不要餵整篇脈絡給它——拿得到意圖的譯者一樣會強制決定，只是決定得對：機制照常運作、訊號消失。推論是「在寫作當下同時產出中英文互相對照」這個做法對最該偵測的那一類全盲：**會傷到讀者的歧義，正好是作者毫無困難就能解決的那一種**，自譯的盲區與作者的盲區重合，產出的譯文不是檢驗、是一張錯誤的合格證明。寫作端要留的不是譯文而是一個問句——沒讀過前文的人拿這一句，能不能定位到唯一的對象。判讀另加兩條：譯文通順而原文的某個成分不在裡面要當成命中（翻不出來的處理有時是刪掉它），「沒有標記不確定」讀成沒有資訊、不讀成沒有問題。實測與判定見 [#290](/report/probe-independence-is-not-transferable/)。

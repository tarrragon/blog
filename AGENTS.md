...
**教學是定位，產出形式是有序的說明。** 教學定位的意思是讀者來理解一件事。產出有三個特徵：有脈絡（每段說明自帶上下文，讀者從任意段落落進來都能跟上）、有原理（每個指令旁邊寫它為什麼這樣用、背後的機制是什麼）、主詞受詞完整（每一句寫清楚是什麼工具對什麼對象做了什麼事）。開場用一句話交代適用範圍，正確的做法與讀法直接寫在指令旁邊，某個輸出有多種讀法時從一開始就引導讀者看對的欄位。判讀徵兆在 [#355](/report/teaching-is-not-lecturing/)。

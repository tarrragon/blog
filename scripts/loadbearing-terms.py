...
3. 主題詞與承重詞不可分：一篇講連結的文章裡「連結」必然高頻，而它不是
   那篇沒定義的詞。用「拿掉它之後那幾句還成不成立」濾一次。
4. 比對用 lookahead 取重疊窗，所以「凝固溫度」出現三次就數到三次。
...
的 是 在 有 和 與 就 也 都 而 一 不 了 這 那 它 們 個 之 於 為 以 或 對 到 從 被 把 讓 用
的是 的那 的話 的時 是在 是把 就是 也是 都是 而是 一個 兩個 三個 個字 個人 一句 一段 一條 一次'''.split())

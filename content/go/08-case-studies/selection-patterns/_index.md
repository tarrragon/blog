...
這類案例可以回到 [Go 的簡單哲學與認知負擔](/go/00-philosophy/simplicity/)、[composition root 與依賴組裝](/go/07-refactoring/composition-root/) 對照。
...
這類案例可以回到 [goroutine：背景工作與服務生命週期](/go/04-concurrency/goroutine/)、[channel：事件流與 backpressure ](/go/04-concurrency/channel/)、[WebSocket 服務架構](/go-advanced/02-networking-websocket/) 對照。
...
這類案例可以回到 [Go 和其他並發語言的差異](/go/00-philosophy/concurrency-language-position/)、[Runtime 與效能診斷](/go-advanced/03-runtime-profiling/) 對照。

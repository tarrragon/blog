...
大規模平台案例通常要觀察服務形狀是否一致：入口、設定、log、部署與 health check 是否能被很多團隊共用。高併發即時案例通常要觀察連線是否長時間存在，以及 server 是否需要管理大量 client 狀態。效能敏感遷移案例通常要觀察瓶頸是否集中在清楚邊界。分散式基礎設施案例則要觀察主要問題是否落在多節點協調與可靠性。

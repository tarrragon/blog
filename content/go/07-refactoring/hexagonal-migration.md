...
## 【觀察】ports/adapters 是依賴方向，不是資料夾儀式
...
這個 usecase 只依賴 domain 與 ports。HTTP handler、WebSocket router、memory repository、event log implementation 都在外面。
...
- 不要求固定目錄名稱。
- 不引入 dependency injection framework。
- 不一次遷移整個專案。
- 不設計分散式系統架構。

如果 ports/adapters 邊界要延伸到 durable queue、資料庫 transaction 或部署平台，可以接著閱讀 [Go 進階：跨節點與平台整合](../../go-advanced/07-distributed-operations/)。

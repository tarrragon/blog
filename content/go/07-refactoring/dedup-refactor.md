...
去重邏輯重構的核心觸發點是多個入口開始各自判斷「這筆事件看過了嗎」。HTTP callback、[queue](../../../backend/knowledge-cards/queue/) [consumer](../../../backend/knowledge-cards/consumer/)、background worker 或 [WebSocket](../../../backend/knowledge-cards/websocket/) action 都可能收到同一件事，若每個入口各自去重，規則很快會不一致。
...
raw payload 去重的核心問題是來源格式不是 domain 語意。不同來源可能使用不同欄位名稱、timestamp 精度、metadata 或 [request ID](../../../backend/knowledge-cards/request-id/)，但仍然描述同一件事。
...
本章先處理單一服務內的去重規則如何集中；分散式去重與 [idempotency](../../../backend/knowledge-cards/idempotency/) store，會在下列章節再往外延伸：
...
- [Go：如何新增一個即時訊息 action](../../06-practical/new-websocket-action/)

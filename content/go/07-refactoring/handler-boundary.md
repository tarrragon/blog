...
- 想新增 CLI、worker 或 [WebSocket](/backend/knowledge-cards/websocket/) action 時，只能複製 handler 內的邏輯。
...
這個 helper 可以忽略 encode error，因為 response 已經開始寫出；正式服務通常會記錄 [log](/backend/knowledge-cards/log/)。
...
本章先處理 HTTP handler 的轉換邊界；router、middleware 與 [transaction](/backend/knowledge-cards/transaction/)，會在下列章節再往外延伸：

- [Go 進階：逐步遷移到 ports/adapters 架構](/go/07-refactoring/hexagonal-migration/)

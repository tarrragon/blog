...
| event log       | 記錄實作可替換                     | `EventLog`                    |
| publisher       | WebSocket、queue、log 都可能是輸出 | `Publisher`                   |
...
- 不設計全專案通用 dependency injection 框架。
- 不建立巨型 service locator。
- 不把所有 concrete type 都改成 interface。
- 不處理 mock code generator。

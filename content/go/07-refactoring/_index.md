...
- **state 要有擁有者**：共享 map、slice、[projection](/backend/knowledge-cards/projection) 必須集中寫入並保護 copy boundary。
...
| 重構問題                             | 優先閱讀                                               |
| ------------------------------------ | ------------------------------------------------------ |
| handler 太厚                         | [把 handler 邏輯拆成可測單元](/go/07-refactoring/handler-boundary/)       |
| 外部依賴難測                         | [用 interface 隔離外部依賴](/go/07-refactoring/interface-boundary/)       |
| 事件重複或來源變多                   | [事件去重邏輯的重構策略](/go/07-refactoring/dedup-refactor/)              |
| 共享狀態外洩                         | [狀態管理的安全邊界](/go/07-refactoring/state-boundary/)                  |
| 檔案平面結構失去語意                 | [以 domain 重新整理 package](/go/07-refactoring/domain-packages/)         |
| 依賴方向需要穩定                     | [逐步遷移到 ports/adapters 架構](/go/07-refactoring/hexagonal-migration/) |
| 抽出 port 後不知道在哪裡 new adapter | [composition root 與依賴組裝](/go/07-refactoring/composition-root/)       |

...
Go 重構的核心不是做出更多抽象，而是讓邊界更清楚、測試更直接、資料競爭更少。本模組用一般 Go 服務範例說明如何在程式仍可運行的前提下，從平面檔案結構逐步走向更清楚的 package、interface、state 與 adapter 邊界。
...
- **package 代表語意邊界**：不要用 `utils`、`common` 取代清楚的 domain 名稱。
...
重構章節刻意維持「一章一條遷移路線」。每章會先說明壓力訊號，再給局部重構策略、測試保護、常見錯誤與不處理範圍。這種安排讓讀者能照著章節做小步遷移，而不是先背完整架構模板。

...
本頁整理 Go 入門篇與進階篇反覆使用的詞彙。核心目的是讓同一個概念在不同章節中保持同一種意思。
...
延伸閱讀：[如何新增一個 WebSocket action](/go/06-practical/new-websocket-action/)、[訂閱模型與訊息路由](/go-advanced/02-networking-websocket/subscription-routing/)。
...
延伸閱讀：[把 handler 邏輯拆成可測單元](/go/07-refactoring/handler-boundary/)。
...
延伸閱讀：[把 handler 邏輯拆成可測單元](/go/07-refactoring/handler-boundary/)、[逐步遷移到 ports/adapters 架構](/go/07-refactoring/hexagonal-migration/)。
...
延伸閱讀：[如何新增一種事件類型](/go/06-practical/new-event-type/)、[事件來源、處理流程與狀態邊界](/go-advanced/04-architecture-boundaries/component-boundaries/)。
...
延伸閱讀：[如何新增一種事件類型](/go/06-practical/new-event-type/)、[多來源 event 融合](/go-advanced/04-architecture-boundaries/event-fusion/)。

### Event Log

`event log` 表示記錄 domain event 的事實紀錄。它用來追蹤系統承認過哪些事件，重點是事件語意、順序、去重與後續查詢。

[event [log](/backend/knowledge-cards/log/)](/go/backend/knowledge-cards/event-log) 和 structured log 的用途不同。structured log 服務操作診斷，event log 服務業務事實追蹤；兩者可以共享 `trace_id`、`event_id`、`subject_id` 等欄位，但不應互相取代。

延伸閱讀：[如何新增結構化記錄欄位](/go/06-practical/structured-recording/)、[結構化日誌欄位設計](/go-advanced/06-production-operations/log-fields/)。
...
`event sourcing` 表示以事件歷史作為狀態真相來源。系統透過事件序列重建狀態，事件歷史本身就是真相。
...
延伸閱讀：[事件去重邏輯的重構策略](/go/07-refactoring/dedup-refactor/)、[事件去重與語義鍵設計](/go-advanced/04-architecture-boundaries/dedup-key/)。
...
延伸閱讀：[如何新增 repository port](/go/06-practical/repository-port/)、[狀態管理的安全邊界](/go/07-refactoring/state-boundary/)。
...
延伸閱讀：[如何新增 repository port](/go/06-practical/repository-port/)、[資料庫 transaction 與 schema migration](/go-advanced/07-distributed-operations/database-transactions/)。
...
`source of truth` 表示狀態轉移的寫入權責，是系統承認「誰能決定目前狀態」的邊界。
...
延伸閱讀：[如何擴展狀態資料欄位](/go/06-practical/state-fields/)、[Source of Truth：狀態邊界](/backend/knowledge-cards/source-of-truth/)。
...
延伸閱讀：[如何擴展狀態資料欄位](/go/06-practical/state-fields/)。
...
延伸閱讀：[把 handler 邏輯拆成可測單元](/go/07-refactoring/handler-boundary/)、[以 domain 重新整理 package](/go/07-refactoring/domain-packages/)。
...
延伸閱讀：[指標與資料複製邊界](/go/02-types-data/pointers-copy/)、[共享狀態與複製邊界](/go-advanced/01-concurrency-patterns/shared-state/)。
...
延伸閱讀：[用 interface 隔離外部依賴](/go/07-refactoring/interface-boundary/)、[逐步遷移到 ports/adapters 架構](/go/07-refactoring/hexagonal-migration/)。
...
延伸閱讀：[逐步遷移到 ports/adapters 架構](/go/07-refactoring/hexagonal-migration/)。
...
延伸閱讀：[把 handler 邏輯拆成可測單元](/go/07-refactoring/handler-boundary/)、[read pump / write pump 模式](/go-advanced/02-networking-websocket/read-write-pump/)。
...
延伸閱讀：[如何新增背景工作流程](/go/06-practical/new-background-worker/)、[事件來源、處理流程與狀態邊界](/go-advanced/04-architecture-boundaries/component-boundaries/)。
...
延伸閱讀：[goldmark AST 入門](/go/09-tooling-and-analysis/goldmark-ast-basics/)、[什麼是 AST](/posts/what-is-ast/)。
...
延伸閱讀：[AST 驅動的 idempotent 文字改寫](/go/09-tooling-and-analysis/ast-idempotent-rewriting/)、[Pre-commit hook 與 CI 整合](/go/09-tooling-and-analysis/pre-commit-and-ci/)。
...
延伸閱讀：[跨檔案圖分析](/go/09-tooling-and-analysis/cross-file-graph-analysis/)。
...
延伸閱讀：[工具決策的 tripwire](/go/09-tooling-and-analysis/tool-decision-tripwire/)。
...
延伸閱讀：[Pre-commit hook 與 CI 整合](/go/09-tooling-and-analysis/pre-commit-and-ci/)。

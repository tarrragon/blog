...
由此得到一條可操作的判斷標準：**每個錯誤檢查點應該是決策點**。決策的選項包括補脈絡後回傳、記錄後降級、重試、轉成協定語意（HTTP status、gRPC code）、顯式忽略並註明理由。檢視程式時的訊號很直接 — 裸 `return err` 是少數、只出現在沒有脈絡可補的薄層（單行轉呼叫）時，顯式錯誤在產出資訊；裸 `return err` 是預設姿勢時，冒泡批評對這個 codebase 成立。
...
理論選擇的判斷標準是錯誤處置的分歧度與程式的生命型態，同一個團隊在不同專案可以合法地選不同邊。

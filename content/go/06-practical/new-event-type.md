...
| 名稱                     | 類型           | 語意                                                           |
| ------------------------ | -------------- | -------------------------------------------------------------- |
| `subscribe_topic`        | command/action | client 想訂閱 [topic](../../../backend/knowledge-cards/topic/) |
...
`DomainEvent` 的核心責任是提供統一 envelope，讓不同來源的事件進入系統後使用同一種語意模型。HTTP callback、client action、timer 或 [queue](../../../backend/knowledge-cards/queue/) message 都可以被 adapter 轉成 `DomainEvent`。
...
這個函式把事件建立規則集中起來。未來若要補 schema version、[correlation ID](../../../backend/knowledge-cards/correlation-id/) 或預設時間，也不需要在每個呼叫端重複組 struct。
...
去重鍵應排除 `ReceivedAt`、raw metadata 或 [request ID](../../../backend/knowledge-cards/request-id/) 這類每次都可能不同的欄位。那些欄位適合記錄觀測資訊，不適合作為「是否同一件事」的判斷依據。
...
event processor 的核心責任是驗證、去重、更新狀態與發布結果。processor 不負責讀 HTTP request，也不負責解析 [WebSocket](../../../backend/knowledge-cards/websocket/) message。
...
processor 測試使用 fake repository、publisher 或 deduper，就能隔離事件規則。真實 message [broker](../../../backend/knowledge-cards/broker/) 或資料庫屬於 adapter integration test。
...
- [Go：如何新增一個即時訊息 action](../new-websocket-action/)
- [Go：如何新增背景工作流程](../new-background-worker/)
- [Go：用 interface 隔離外部依賴](../../07-refactoring/interface-boundary/)
- [Go：事件去重邏輯的重構策略](../../07-refactoring/dedup-refactor/)

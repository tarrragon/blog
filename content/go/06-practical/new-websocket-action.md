...
- 不實作完整 WebSocket server。
- 不介紹特定專案的 action 名稱。
- 不把訂閱狀態直接放在 handler 中修改。

完整 WebSocket server 的連線生命週期可以接著閱讀 [Go 進階：WebSocket 服務架構](../../go-advanced/02-networking-websocket/)；跨節點推送與 presence 可以閱讀 [Go 進階：跨節點 WebSocket、presence 與重連協定](../../go-advanced/07-distributed-operations/cross-node-websocket/)。

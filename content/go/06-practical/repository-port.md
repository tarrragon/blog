...
repository port 的核心語意是 usecase 需要哪些資料能力。它不是把資料庫 CRUD 方法照搬成 interface，也不是所有資料操作都塞進同一個巨大 `Repository`。
...
- 不選定特定資料庫。
- 不介紹 ORM。
- 不把 repository 變成所有資料操作的大型介面。

如果 repository port 要進一步接到真實資料庫，可以接著閱讀 [Go 進階：資料庫 transaction 與 schema migration](../../go-advanced/07-distributed-operations/database-transactions/)。

## 小結

repository port 的價值是讓 usecase 依賴資料能力，而不是依賴儲存技術。小介面、context、error wrapping、copy boundary 與 contract test 會讓 memory 實作可以自然演進到資料庫實作。不要為了架構感先建立 repository；當資料需要一致讀寫、測試替身或未來替換時，再建立這個邊界。

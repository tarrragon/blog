...
3. 用 [connection pool](../../../backend/knowledge-cards/connection-pool/)、[timeout](../../../backend/knowledge-cards/timeout/) 與批次策略控制壓力
4. 避免 [cache stampede](../../../backend/knowledge-cards/cache-stampede/) 與慢查詢連鎖
...
- 限制同時對下游發出的請求數
- 設定明確 timeout
- 避免無限 [fan-out](../../../backend/knowledge-cards/fan-out/)
- 在壓力過高時拒絕新工作
...
- [queue](../../../backend/knowledge-cards/queue/) / stream
...
- 太高的同時連線數會壓垮資料庫
- 太長的 [transaction](../../../backend/knowledge-cards/transaction/) 會卡住連線池
...
- 所有操作都帶 `context`
- 用 [worker pool](../../../backend/knowledge-cards/worker-pool/) 或 semaphore 控制同時請求數
- 對 cache miss 做去重或保護
- 對寫入高峰做批次或排隊
...
Go 服務在 Redis 與 SQL 上的關鍵是用請求節奏、timeout、pool 與 [backpressure](../../../backend/knowledge-cards/backpressure/) 保護下游。下游邊界清楚時，Go 的高併發能力才會真正變成優勢。

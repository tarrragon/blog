...
這一章不是 Redis 或 SQL 的操作手冊，而是從 Go 服務的角度整理資料存取原則。重點在於：當併發增加時，Go 端要怎麼使用下游，才不會把 Redis 或 SQL 一起壓垮。
...
高併發時，問題常常不是 Go 讀寫下游的能力不夠，而是請求節奏太快。你可以有很多 goroutine，但 Redis 與 SQL 不會因為 goroutine 多就自動變快。
...
這些不是附加優化，而是讓高併發系統能長時間穩定運行的基本做法。

## 小結

Go 服務在 Redis 與 SQL 上的關鍵不是「怎麼連上去」，而是「怎麼不要把下游打爆」。只要把請求節奏、timeout、pool 與背壓設計好，Go 的高併發能力才會真正變成優勢。

...
Go 的 goroutine 很容易開，但 Redis、SQL、HTTP API、檔案描述元與記憶體 buffer 都有容量上限。高併發設計的核心是替外部資源設邊界，讓 goroutine 數量、下游連線與排隊時間都保持可預期。
...
- [Backend：資料庫與持久化](../../backend/01-database/)：看 SQL、transaction、connection pool 與 schema 邊界如何承接服務壓力。
- [Backend：快取與 Redis](../../backend/02-cache-redis/)：看 Redis client、TTL、eviction、presence store 與 hot key 如何承接服務壓力。

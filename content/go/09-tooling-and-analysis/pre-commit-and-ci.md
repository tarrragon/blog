...
工具一旦從 CLI 進入 hook / CI，就有幾個容易出狀況的邊界：**哪些 check 該放 hook**（快、本地可執行）、**哪些該放 CI**（慢、需要乾淨環境）、**hook 改了檔怎麼 re-stage**、**--no-verify 的邊界**怎麼約定、**CI strict mode 跟 local dev 的差異**怎麼處理。本章展開這些問題，並以 `.githooks/pre-commit` + `.github/workflows/md-check.yml` 作為 concrete instance。

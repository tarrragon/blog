...
工具決策的核心責任是**用事前約定的條件決定何時升級，取代事後的模糊直覺**。Tripwire（預設觸發條件）的設計：在某個可量測訊號命中時，主動重新評估現有工具是否夠用、或該升級到下一個層級。這個約定避開兩個常見失敗 —「太早升級」（把 shell 一行解決的事過度工程化）跟「太晚升級」（regex 每週踩雷卻忍著不升，信譽破產）。

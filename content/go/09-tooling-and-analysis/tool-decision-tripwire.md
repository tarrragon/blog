...
工具決策的核心責任是**用事前約定的條件決定何時升級，取代事後的模糊直覺**。[Tripwire 決策](/go/glossary/#tripwire-決策)（預設觸發條件）的設計：在某個可量測訊號命中時，主動重新評估現有工具是否夠用、或該升級到下一個層級。這個約定避開兩個常見失敗 —「太早升級」（把 shell 一行解決的事過度工程化）跟「太晚升級」（regex 每週踩雷卻忍著不升，信譽破產）。

9.1–9.4 講了怎麼寫 Go 工具。這一章退一步，看**什麼時候該寫這個工具、什麼時候該升級既有工具**。決策錯了，寫得多好都沒用。本章介紹 tripwire 決策法、WRAP 框架在技術決策的套用、以及用 blog 工具鏈選型作為 concrete instance。
...
[9.6 pre-commit hook 與 CI 整合](/go/09-tooling-and-analysis/pre-commit-and-ci/) 回到工程落地，看工具怎麼從 binary 變成 commit 與 CI 流程裡的執行體。

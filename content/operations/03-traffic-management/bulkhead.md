...
```go
// driver name 依實際使用的 driver 而定（modernc.org/sqlite 用 "sqlite"、mattn/go-sqlite3 用 "sqlite3"）
ingestDB := sql.Open("sqlite", "events.db")
ingestDB.SetMaxOpenConns(10) // ingestion 專用

queryDB := sql.Open("sqlite", "events.db")
...
- 背壓的流量控制 → [背壓機制](/operations/03-traffic-management/backpressure/)
- 依賴失敗的快速失敗 → [熔斷器](/operations/03-traffic-management/circuit-breaker/)
- 突發流量時的綜合策略 → [模組七 突發流量](/operations/07-burst-traffic/)

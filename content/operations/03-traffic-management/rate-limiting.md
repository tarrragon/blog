...
- 被動的流量控制 → [背壓機制](/operations/03-traffic-management/backpressure/)
- 依賴失敗時的快速失敗 → [熔斷器](/operations/03-traffic-management/circuit-breaker/)
- 不同工作負載的資源隔離 → [Bulkhead 隔離](/operations/03-traffic-management/bulkhead/)
- Backend 的 rate limit 實作（middleware / Redis / 配額設計）→ [Rate Limit 實作](/backend/09-performance-capacity/rate-limit-implementation/)

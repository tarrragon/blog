...
多實例共享同一個資料庫時，寫入集中在主庫、但讀取可以擴展——這是讀寫分離。寫走主庫（primary），讀走唯讀副本（replica），讀流量超過主庫吞吐時，加副本就能擴讀路徑。副本有複製延遲要納入考量：同一可用區內的串流複製通常低於 100 毫秒，跨可用區到秒級，跨區域可能到秒至分鐘級且不保證 read-after-write——需要剛寫就讀到的查詢要強制走主庫，這正是 [Session 處理](/operations/02-horizontal-scaling/session-handling/) 那條一致性補丁。
...
- 高頻的 session 狀態為什麼要放 KV 而非 SQL → [Session 處理](/operations/02-horizontal-scaling/session-handling/)
- 什麼狀態該外置、什麼可以留本機 → [Stateless 設計原則](/operations/02-horizontal-scaling/stateless-design/)

...
證據來源是單一組織的深度重建（一個大型專案的完整反省）加上作者後續二十年的修正，不是統計。它是被後續討論反覆引用的個人經驗，而 Brooks's Law 在多大程度上被獨立實證支持，本書單沒有查證。加了人卻沒有變快的專案參與過一次，這本才有對象；沒有的話，那條定律讀起來像一句俏皮話。
...
規模化敏捷框架（SAFe、LeSS 之類）本書單未評估。它們提供的是流程模板而非結構判斷標準，適用性高度依賴組織既有的成熟度，要給出可靠的選讀建議需要在不同成熟度的組織各導入一次，這裡沒有這個基礎。

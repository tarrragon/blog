...
| 項目                                                              | 類型   | 前置條件                     | 規模 |
| ----------------------------------------------------------------- | ------ | ---------------------------- | ---- |
| 心理安全感知識卡（三篇共用前提、全站目前無承接）                  | 知識卡 | 決定建在 backend 或 record   | 1    |
| 槓桿點 / 參考組預測法 / 信奉理論 vs 使用理論三張概念卡            | 知識卡 | 心理安全感卡定調住址後跟進   | 3    |
| jelled team 與 egoless programming 兩個術語的處理（建卡或行內化） | 知識卡 | 判定它們是否夠獨立到值得建卡 | 2    |
| 讀者旅程與情境可想像性兩個 frame 的審查（本批尚未跑過）           | 跨模組 | 需異源視角、不宜自審         | 中   |

這批術語在書單裡以行內解釋處理，讀者當下讀得懂；登記的理由是它們跨多篇被當共用前提使用，而共用前提沒有住址時，每篇只給自己那一角、逐篇審查看不到缺口。已確認全站有承接的不列入：Conway's Law、Hyrum's Law、error budget 與 SLO/SLI、認知負荷、偏差正常化、後見之明偏誤。

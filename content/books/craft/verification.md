...
它是起點的理由是後面兩本都在跟它對話。分歧從一個定義開始：**測試的單元是什麼**。Beck 的用法把單元當成一組協同工作的類別，測試對外的行為；後來的另一派——書市慣稱倫敦學派——把單元縮到單一類別，把所有協作者換成 mock。兩派的所有差異都從這個定義分岔出去。

[證據來源](/books/knowledge-cards/evidence-provenance/)是單一路徑的個人經驗，形式是作者逐步演練自己的實踐而非論證——它不打算說服誰，它示範。時效上，範例是 Java 與 Python 的舊版本，測試框架也隔了兩個世代；紅綠重構的節奏與「讓測試驅動設計」這個主張不依賴那些。
...
它也是這個主題裡爭議最集中的一本。批評指向同一個地方：這種寫法讓測試綁在物件的協作結構上，於是重構內部結構時測試會大量壞掉，而重構本來不該弄壞測試。這個批評成不成立，取決於單元怎麼定義——那正是 Khorikov 那本要處理的問題。
...
證據來源是單一路徑的個人經驗加上理論建構，形式是原則加判準——這是三本裡唯一嘗試把測試設計寫成可推導體系的。範例是 C#，但判準本身與語言無關。時效上是最新的一本，尚無過時的部分。
...
這三本彼此不同意，而它們的分歧點正是這個主題最難自己判斷的那一個。Beck 給的是原始定義，Freeman 與 Pryce 把倫敦學派的主張寫到最完整，Khorikov 系統性地反對它並給出替代判準。讀完三本會發現自己原本以為的「測試最佳實踐」其實是某一派的立場，而那個發現比任何單一技巧有用。

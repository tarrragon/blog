...
版本要選：第二版（2018）把範例從 Java 換成 JavaScript，並補了不用 class 的函式式範例。這對讀者是真的取捨——寫 Java 的人第一版的範例反而貼近，而第二版的目錄本身有更新。證據來源是跨客戶的顧問經驗（作者與貢獻者群的實務歸納），形式是目錄而非論證。讀得出價值的前提是：有一段自己想改但不確定怎麼下手的程式碼——沒有那段程式碼在手上，六十幾項手法讀起來是六十幾個名詞。

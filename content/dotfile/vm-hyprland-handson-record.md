...
#### Hyprlock：渲染漂亮、但測試方式有大坑

Hyprlock config 渲染效果最好（`shot4.png`）：`path = screenshot` 截當下畫面 + `blur_passes = 2` 模糊當背景、64px 時鐘、`cmd[update:60000] date` 產生的日期 label（`Wednesday, 01 July`，locale en_US 下是英文所以無豆腐）、藍框 pill 密碼框。VM 軟體渲染下 `blur_passes` 設 2 是「看得出霧化又不卡」的折衷。
...
把實機跑出來、跟教材對得上的 gotcha 收斂在這裡，再回頭改對應教材檔案。第二步 rice 的發現分兩類：**配置層**（教材範例與實裝環境對不上、該直接修教材）與 **環境／測試層**（最小安裝缺相依、VM/自動化特有陷阱、該補成卡或教材新段）。完整待審清單與「建議轉成什麼教學內容」見 `~/step2/REPORT.md`（複製進 `vm-step2-review/`）。

| 發現                                | 類別 | 對得上的教材檔                                                  | 建議動作                                                    |
| ----------------------------------- | ---- | -------------------------------------------------------------- | ----------------------------------------------------------- |
| 字族名 MesloLGS ≠ 範例 JetBrainsMono | 配置 | `06-rice-design/desktop-shell-components.md`、`color-system-theming.md` | 範例字型與 `packages-arch.txt` 對齊                        |
| CJK 無字 → 豆腐，需 noto-fonts-cjk   | 環境 | `desktop-shell-components.md`、`03-terminal-ecosystem` 字型段   | 補「Nerd Font 不含 CJK、要另裝 fallback」                  |
| 字型可用集合在 process 啟動時決定：裝字型後已跑的 client 要重啟才生效（reload 不夠）| 機制 | 新卡 `knowledge-cards/font-availability-at-startup.md`         | 上一列的通則、跨 rice 通用，蒸餾成 atomic 卡              |
| mako 要 libnotify 才有通知可發       | 環境 | `desktop-shell-components.md` Mako 段                           | 補「daemon 只顯示、產生通知靠 libnotify」                  |
| waybar 模組在 VM 自動退化            | 配置 | `desktop-shell-components.md` Waybar 段                         | 補「缺硬體的模組自動隱藏、同份 config 通用」               |
| stow tree folding/unfolding         | 機制 | `knowledge-cards/gnu-stow.md`                                  | 補折疊/展開概念（共用目錄時的行為）                        |
| Hyprland 變數 source 範圍限制        | 配置 | `color-system-theming.md` 配色統一段                           | 已有提及、用實測確認可補一句範圍界線                       |
| hyprlock pkill ≠ 解鎖、掉失效保護    | 機制 | `color-system-theming.md` Hyprlock 段（或新卡）                | 補「鎖屏測試安全守則 + session-lock 兩層」                 |
| grim 截圖需 wl-copy --type image/png | 環境 | `desktop-shell-components.md` 截圖段（或 work-log）             | 補「無 xdg-utils 時剪貼簿型別要明指」                      |

階段零～二的 `[待驗證]` 標記（virtio-gpu 黑畫面、DHCP 帶進新系統、序列 console vs 圖形 VT）已在階段一、三第一步驗證並記錄；本節新增的是第二步 rice 挖到的九項，等使用者 review 後決定各自落點與是否建卡，再跑 multi-round-review。

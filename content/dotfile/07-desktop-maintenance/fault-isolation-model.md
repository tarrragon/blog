...
date: 2026-06-30
description: "從 Windows 轉過來想知道 Linux 桌面掛了會不會整台崩潰時讀 — kernel vs userspace 隔離、compositor 是 userspace process、TTY 永遠能救"
weight: 1
...
**GPU driver bug**。Linux 的 GPU 驅動分兩層：kernel module（可動態載入的核心擴充模組，如 `nvidia.ko`、`amdgpu.ko`）負責硬體操作，userspace 的 Mesa / NVIDIA userspace driver 負責 OpenGL/Vulkan 實作。Kernel module 出問題理論上可以 kernel panic，但實際上 Linux kernel 的 GPU driver 通常會嘗試 reset GPU 而非直接 panic。常見的表現是畫面凍結幾秒後恢復，或 compositor 被 kill 但系統存活。
...
TTY 的重要性在於它**不依賴 compositor**。Hyprland 掛了、GPU driver 出問題導致畫面凍結、桌面環境完全不回應——只要 kernel 還活著，TTY 就能用。切過去登入後可以：
...
TTY 切不過去的唯一情境是 kernel panic——但這在 Linux 桌面日常使用中極為罕見，通常只有硬體故障或 kernel module 的嚴重 bug 才會觸發。

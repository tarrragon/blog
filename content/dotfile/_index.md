...
編號模組都假設你已經有一台裝好 Linux、工具齊全、能從外部連入的機器。[Linux 安裝與機器初始化](/dotfile/linux-install/) 補的是那段被預設跳過的地基——OS 安裝選項怎麼判讀、最小系統缺哪些必要工具、怎麼連進去跑 bootstrap（含還沒有 SSH key 的情況），以及怎麼讓安裝腳本在失敗時可診斷。只要你換到全新環境（開 VM、租雲端主機、拿到空機器），就會先撞上這層。內容來自一次完整的 VM 實測，蒸餾成不綁特定發行版的判讀與決策。
...
| [模組七：桌面環境維護與故障排除](/dotfile/07-desktop-maintenance/) | 故障隔離模型、常見故障恢復、日誌判讀                 | compositor 掛了或桌面凍結時怎麼診斷和恢復 |
| [模組八：同步、Bootstrap 與環境重建](/dotfile/08-sync-bootstrap/)  | 跨機器同步、install script、secret 排除、VM 對比     | 換機器時怎麼一鍵還原、哪些東西不該進 repo |
| [模組九：從個人到團隊](/dotfile/09-team-environment/)              | devcontainer、nix、商業開發環境標準化                | 個人 dotfile 的思想怎麼延伸到團隊環境管理 |

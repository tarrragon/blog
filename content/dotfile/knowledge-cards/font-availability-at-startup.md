...
**延伸閱讀**：Nerd Font 不含 CJK、需另裝 fallback 字型的具體案例見[桌面 Shell 元件：狀態列、啟動器與通知](/dotfile/06-rice-design/desktop-shell-components/)；字型安裝方式見[終端機與編輯器配置](/dotfile/03-terminal-ecosystem/terminal-emulator-config/)的字型管理段。

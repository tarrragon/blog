...
這對應兩種風格。EFISTUB 讓韌體直接載入 kernel、不經過獨立 bootloader，最精簡，但完全依賴 NVRAM 裡的開機項。獨立 bootloader（GRUB、systemd-boot）則多一層：它有開機選單、能救援、還能裝到 ESP 的 fallback 路徑（`\EFI\BOOT\BOOT<ARCH>.EFI`，aarch64 是 `BOOTAA64.EFI`、x86_64 是 `BOOTX64.EFI`）。
...
相關概念：[initramfs](/dotfile/knowledge-cards/initramfs/)（bootloader 載入的對象之一）、[分區識別](/dotfile/knowledge-cards/partition-identification/)。bootloader 選型的判讀，見 [Linux 安裝選項判讀](/dotfile/linux-install/install-option-decisions/)。

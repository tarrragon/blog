...
K 跟 V 在 [attention](/llm/knowledge-cards/attention/) 計算中扮演不同角色、對量化的容忍度也不同。K 參與內積比較（量化容忍度通常較高）、V 是被加權平均的輸出內容（量化誤差會線性累積）、社群常見做法是 K 用較激進的量化、V 保留較高精度。
...
部分 KV cache 量化組合（特別是 V=Q4_0 / Q4_1）在 llama.cpp 上需要同時啟用 [flash attention](/llm/knowledge-cards/flash-attention/)（`-fa`）才能正常運作；沒啟用時可能載入失敗或 fallback 到 fp16。具體要求依 llama.cpp 版本變化、以實際 `llama-server --help` 跟 [llama.cpp 官方 issue / PR](https://github.com/ggml-org/llama.cpp/pulls?q=is%3Amerged+kv+cache+quant) 為準。

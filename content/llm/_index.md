...
模組零跟模組一覆蓋「裝跟用」這條最短路徑。想懂底層的讀者、模組二（[數學基礎](/llm/02-math-foundations/)）跟模組三（[LLM 理論基礎](/llm/03-theoretical-foundations/)）提供完整理論圖像、並推薦 MIT / Stanford / Karpathy 等公開課作為深入入口。
...
| 術語澄清       | [MLX](/llm/00-foundations/mlx-mtp-omlx/)、[MTP](/llm/00-foundations/mlx-mtp-omlx/)、[oMLX](/llm/00-foundations/mlx-mtp-omlx/)、speculative decoding、量化、KV cache、TTFT                                                                                                                                                                                                                                                                                                                     | post-training fine-tuning 細節                  |
...
模組二跟模組三可並讀。閱讀模組三遇到陌生數學詞時跳回模組二補完、再回模組三繼續。
...
| 裝過 Ollama 但被網路上的術語混淆          | 直接適合   | [MLX / MTP / oMLX 區分](/llm/00-foundations/mlx-mtp-omlx/) + [判讀框架](/llm/00-foundations/common-misconceptions/)              |
...
- **向量資料庫的選型**（Pinecone、Weaviate、Chroma 比較等）：交給 RAG 專門教材。
- **Kubernetes 上的 LLM 部署**：跟本地 Mac 場景方向不同。

若讀完本指南後想往這些方向走：

1. **想做 RAG 應用**：先把 Ollama + Continue.dev 跑穩、再讀 LlamaIndex / LangChain 的 RAG 教學、或 [模組三 3.7 推薦](/llm/03-theoretical-foundations/going-deeper-theory/) 的 DeepLearning.AI short courses。

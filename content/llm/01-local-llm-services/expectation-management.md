...
本地 LLM 用得順不順、九成取決於「期待對齊現實」。把本地當成「免費、永遠在線的初階 pair programmer」、它的表現會超出預期、變成日常雜事的得力幫手；把它當成 Claude Sonnet / GPT-5 替代品、跨檔案重構失敗、規劃 multi-step 任務崩潰、深度 debug 給平庸答案的場景就會接連出現、第一週體感很差。
...
| 任務                               | 切到雲端的觸發訊號       |
| ---------------------------------- | ------------------------ |
| 解釋一個 bug 的根本原因            | 本地解釋說不通 / 含糊    |
| 改寫一段較複雜的 function          | 改完後測試失敗、邏輯偏了 |
| 寫一段中等長度（< 50 行）的新 code | 跑不過或結構偏差大       |
| 翻譯 code 註解到另一種語言         | 翻得生硬或失準           |
| 寫單元測試（中等複雜度的函式）     | 測試覆蓋不到關鍵分支     |
| 回答一個技術概念性問題             | 答案模糊 / 跟你已知不符  |

這個分類的核心啟示：模糊地帶切到雲端是正常工作流、是「先用便宜的工具、不夠再升級」的合理做法、跟本地「失敗」是兩件事。把切換當成預設選項、能讓你保持輕鬆的使用心態。
...
本地 LLM 的正確心態是「免費的初階 pair programmer」：在簡單、重複、隱私敏感的任務上強、在複雜規劃、跨檔案、深度 debug 上交給雲端旗艦更划算。期待對齊現實後、「混用本地 + 雲端」是現階段最穩的工作姿勢；用一週實測決定去留、卸載也是合理結論。

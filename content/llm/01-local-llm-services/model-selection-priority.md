...
1. **看 SWE-bench Verified 分數**。如果新模型在 SWE-bench 上比現在用的高 5 分以上，值得試。
2. **看模型大小與記憶體預算**。如果新模型大小超過你 Mac 預算，不用考慮。
3. **看是否有 speculative decoding 支援**。沒 drafter 的新模型即使能力強，使用體感可能輸有 MTP 的舊模型。
4. **用自己的 5 ~ 10 個日常任務當 benchmark**。不要相信公開 benchmark，自己跑一遍。
5. **看 Ollama / LM Studio 的 release notes**。新模型需要伺服器支援；Ollama 沒收的模型用起來會麻煩。

不要每次有新模型就換。每換一次模型要重新適應它的語感、prompt 風格、體感速度，切換成本不低。一年換 2 ~ 3 次主力模型是合理節奏。
...
某些網路上熱門的模型對寫 code 場景**不推薦**：

| 模型                      | 為什麼不推薦                                           |
| ------------------------- | ------------------------------------------------------ |
| Llama 3.x base            | base model 沒有 instruction tuning，不會跟著 prompt 走 |
| 純對話模型（Vicuna 系等） | 多半是早期 fine-tune，coding 訓練資料少                |
| 多模態模型（Llava 等）    | 寫 code 用不到 vision，記憶體浪費                      |
| 中文特化模型              | 寫 code 時 prompt / 回答多半英文，中文特化是反效果     |
| 「最新最強」測試模型      | 沒 benchmark 驗證、生態支援差，不適合當主力            |

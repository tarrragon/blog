...
跑本地 LLM 的核心 invariant 跟雲端不一樣：**Mac 是 shared resource、不是 dedicated GPU**。雲端 inference server 跑進 dedicated container、結束 instance 自然回收所有資源；本地推論伺服器跑在你日常用的 Mac、忘記管理會 silently 吃光 RAM、磁碟、port、最後讓系統變慢甚至 swap。

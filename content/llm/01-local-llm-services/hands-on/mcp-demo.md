...
不需要安裝 MCP SDK——本 demo 手寫 JSON-RPC 處理、為了 inspection 透明度。Production server 應該用 [官方 SDK](https://github.com/modelcontextprotocol)（Python / TypeScript 都有）、處理 framing、capability negotiation、transport edge cases。
...
- **為什麼 description 要具體、不能說「search the blog」**：LLM 看 description 決定 call 時機。「search the blog」對 LLM 來說太模糊（搜什麼？找什麼？）；「Semantic search over blog content. Returns top-K relevant chunks with source paths.」明確描述了輸入跟輸出形狀、LLM 能判讀「使用者問技術問題時該 call 這個」。
- **為什麼 schema 用 JSON Schema、不是自訂格式**：JSON Schema 是 web 標準、所有 LLM application 都認識、跨 framework 可移植。也是 [function calling](/llm/knowledge-cards/function-calling/) 跟 [structured output](/llm/04-applications/tool-use-principles/) 的標準描述語言。
...
```json
{
  "mcpServers": {
    "blog-search": {
      ...
      "args": ["/Users/tarragon/Projects/blog/scripts/mcp-demo/blog_mcp_server.py"]
    }
  }
}
```

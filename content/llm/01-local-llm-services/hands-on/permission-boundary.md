...
設計 wrapper 時、`--auto` 該是顯式 opt-in、不該是 default。本 demo 的 mutually_exclusive 設計（`-g.add_mutually_exclusive_group()`）保證三種 mode 只能擇一、避免歧義。
...
- **Tool 暴露 `bash` tool + per-command confirm**：每個 shell 呼叫前 wrapper 暫停、問人類「該不該執行」。對 etpy 環境合理、production 用會卡死流程。
...
8. **小模型加更保守 gate**：1B 模型亂改機率高、`--auto` 不該存在；31B+ 較穩、可給 auto + audit。

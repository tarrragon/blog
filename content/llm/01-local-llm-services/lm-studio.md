...
LM Studio 跟 Ollama 一樣屬於[本地推論伺服器層](/llm/00-foundations/three-layer-architecture/)，但定位完全不同：Ollama 是 CLI-first，LM Studio 是 GUI-first。它的核心承諾是「不打開終端機也能玩本地 LLM」，特別適合對 Hugging Face model hub、量化等級、speculative decoding 還在摸索階段的使用者。
...
LM Studio 的模型瀏覽器是它最大的賣點。在 Discover 分頁搜尋模型名，例如 `gemma-3`，會列出 Hugging Face 上的對應 repo：

| 顯示資訊  | 解讀                                                       |
| --------- | ---------------------------------------------------------- |
| Repo 名稱 | 例如 `bartowski/gemma-3-27b-it-GGUF`                       |
| 量化等級  | Q4_K_M、Q5_K_M、Q8 等，列在每個檔案旁邊                    |
| 檔案大小  | 直接顯示 GB 數，方便判斷是否塞得進記憶體                   |
| 適配建議  | LM Studio 會根據你 Mac 記憶體標「Recommended / Too Large」 |

選一個合適量化等級點下載。Q4_K_M 在多數場景是甜蜜點；32GB Mac 跑 27B Q5_K_M 也順暢。下載中可以繼續操作其他功能。
...
LM Studio 內建 [speculative decoding](/llm/00-foundations/why-llm-feels-slow/) 的 UI 設定。在 model 載入頁面下方有 **Draft Model** 設定區：

1. 選 target model（主力，例如 Gemma 4 27B）
...
LM Studio 預設只開 GUI Chat，要讓 VS Code 接，要開 **Local Server** 模式：
...
1. **Server 不是常駐**。關閉 LM Studio 視窗或登出 macOS 會停止 server，IDE 連不上。要持續跑建議讓 LM Studio 一直開著。
2. **CORS 預設關**。要從瀏覽器（如 Open WebUI 跑在不同 port）連，要去 Server 設定打開 CORS。
3. **Model name 不是 tag**。LM Studio 在 API 用的 model name 是檔名（如 `gemma-3-27b-it`），跟 Ollama 的 tag 格式不同。
...
如果你習慣 Claude 的工具用法（Anthropic Console、Claude Code），LM Studio 的 GUI 體驗比較像 Anthropic Console：可以調 system prompt、看 token 計數、儲存對話。差別是：

...
LM Studio 的模型瀏覽器是它最大的賣點。在 Discover 分頁搜尋模型名，例如 `gemma-4`，會列出 Hugging Face 上的對應 repo：
...
1. **Repo 來源要看**。Hugging Face 上同一個模型有多個社群重新封裝的 repo。`google/gemma-4-...` 是官方 repo；`bartowski/...`、`TheBloke/...`（已退坑）等是社群常見的高品質 quant 提供者。挑下載量高、最近更新的 repo 較安全。

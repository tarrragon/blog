...
換更激進量化（[Q3](/llm/knowledge-cards/quantization/)）來塞更大模型也踩同樣的陷阱。Q3 70B 在 24GB Mac 上勉強載入、但 coding 任務表現常輸給同硬體的 Q5 14B 模型；衰減的判讀訊號是「同任務通過率比未量化版本低 30% 以上」「hallucination 明顯上升（編造 API、忽略 prompt 約束）」、出現這些訊號就回頭重新評估量化等級。
...
| 推論伺服器配置 | `OLLAMA_HOST` 是 [`127.0.0.1`](/llm/knowledge-cards/port-and-localhost/) 還是 `0.0.0.0`？ |
...
模型自己的產出也適用同一套警覺：LLM 的摘要與翻譯有系統性的強度升格傾向——原文的「suggests」被摘要成「證明了」、平實的標語被翻譯成口號。模型轉述過的內容要當成二手來源處理、重要 claim 回原文比對；判斷一次升格是失真還是合法的文案設計、見[轉述與翻譯要保留語意強度量級](/report/rewrite-preserves-claim-intensity/)與[誇飾的合法性由段落位置的功能決定](/report/hyperbole-legitimacy-by-position-function/)。
...
下一步：[模組一 本地 LLM 服務的安裝與應用](/llm/01-local-llm-services/)、把概念落地到實際安裝、整合 VS Code、選模型、做期望管理。

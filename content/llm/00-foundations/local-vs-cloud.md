...
本地 LLM 與雲端 LLM 的核心差異是「模型權重在哪台機器上跑、誰能看到對話內容」。把模型權重載到自己 Mac 的記憶體裡、用本機算力推論，就是本地；把 prompt 透過 HTTPS 送到 Anthropic、OpenAI、Google 的伺服器，再把結果回傳，就是雲端。
...
1. 為什麼有人願意花時間在本地跑 LLM，明明雲端更強？
2. 本地 LLM 的「免費」是不是真的免費？
3. 本地 LLM 的速度比雲端快還是慢？什麼情境下答案不同？
4. 本地 LLM 能取代 Claude / GPT-5 嗎？哪些任務可以、哪些不行？
...
| 速度 | 受本機算力與記憶體頻寬限制，首字延遲與生字速度都低於雲端旗艦模型                                   | 旗艦模型在資料中心的 H100 / TPU 上跑，首字延遲低、生字速度快  |
...
2. 本地生字速度受 Apple Silicon 的記憶體頻寬限制，而不是算力。詳見 [0.1 為什麼 LLM 生字慢](/llm/00-foundations/why-llm-feels-slow/)。
3. 長 context 的首字延遲是本地 LLM 最大的痛點。coding agent 場景塞了整個專案進 prompt 時，本地可能等 30 ~ 90 秒才開始吐字；這是為什麼後來出現 [oMLX 這種特化伺服器](/llm/00-foundations/mlx-mtp-omlx/) 來解 KV cache 問題。
...
- 跨檔案重構、設計新架構、評估技術選型、寫長篇技術文件：本地會給平庸或錯誤的答案，需要雲端旗艦。
- 規劃 multi-step plan、把模糊需求拆成可執行步驟、做 deep debugging：本地基本不行，雲端旗艦是現階段唯一選擇。
...
3. 一台中型 Mac（24GB ~ 32GB） + 雲端旗艦訂閱（Claude Code / GPT-5）的組合，現階段是大多數工程師的甜蜜點。

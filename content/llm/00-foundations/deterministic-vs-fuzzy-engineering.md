...
- **用 deterministic 的 test 思維測 LLM**：寫了一個「input X 應該得到 output Y」的單元測試、跑十次有兩次不通過。問題不是 LLM 不可靠、是 test 假設不對。Fuzzy 系統的測試是「在 N 次採樣中、output 落在期望範圍內的比例」、不是「精確等於某 string」。

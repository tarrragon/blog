...
LLM 生字慢的核心原因有兩個：**自回歸架構**（autoregressive）讓模型一次只能吐出一個 token，**記憶體頻寬瓶頸**讓 Apple Silicon 即使有大量算力也跑不快。這兩個瓶頸結合起來，才能解釋為什麼 32GB Mac 跑 31B 模型只有 30 tok/s，而資料中心的 H100 跑同樣模型能到 200 tok/s。
...
1. 為什麼 LLM 不能像翻譯軟體那樣整段一起生出來？
2. 為什麼 Apple Silicon 的「統一記憶體」對 LLM 推論是優勢？
3. 為什麼模型量化能加速，不只是省記憶體？
4. 為什麼長 prompt 的首字延遲特別痛？
...
Apple Silicon 的**統一記憶體**（Unified Memory Architecture, UMA）優勢在於 CPU、GPU、Neural Engine 共用同一塊記憶體，不需要在 PCIe 上搬資料。傳統 PC + NVIDIA GPU 的記憶體分成系統記憶體跟 VRAM；模型權重必須完全放進 VRAM，否則跨 PCIe 搬資料會慢得不能用。Mac 的 64GB 統一記憶體可以全部給模型用（扣掉系統需要的部分），而同等價位的 PC 通常只有 12GB ~ 24GB VRAM。
...
LLM 在推論時有個重要的暫存結構叫 **KV cache**（key-value cache）。每個 token 在 attention 機制中產生的中間結果會被暫存，後續 token 生成時直接讀 cache 而不重算。這讓「已經算過的 prompt」不用重複跑 forward pass。

但 KV cache 有兩個性質會放大長 prompt 的痛點：

1. **首次處理 prompt 時必須完整算過一次**，這個階段稱為 **prefill**。一個 10K token 的 prompt 在本地可能需要 30 ~ 90 秒才 prefill 完，這 30 ~ 90 秒就是 [TTFT](/llm/00-foundations/local-vs-cloud/) 的主要來源。
2. **KV cache 本身佔記憶體**。長 context 跑下來，KV cache 可能比模型權重還大，把記憶體擠爆。
...
既然瓶頸是「每生一個 token 都要讀一次完整模型權重」，那有沒有可能一次生多個 token？speculative decoding（推測解碼）就是這個想法的具體實作。

機制大致是：

1. 用一個小模型（drafter，例如 1B 參數）快速猜未來 N 個 token。
2. 把這 N 個 token 一次餵給大模型（target，例如 31B 參數），讓大模型並行驗證每個位置的機率分佈。
3. 大模型保留它認同的前綴，從第一個不認同的位置開始拒絕後續。
...
**Multi-Token Prediction**（MTP）是這個概念的一個具體實作，本質上是 speculative decoding 的工程化版本。下一章 [0.4 MLX / MTP / oMLX](/llm/00-foundations/mlx-mtp-omlx/) 會把 MTP 跟其他容易混淆的術語放在一起對照。

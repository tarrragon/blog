...
9. **3.8 reasoning models**：CoT 從 prompting 變訓練 paradigm、test-time compute 是新軸。
10. **3.9 speculative decoding 內部**：另一條推論加速軸、drafter / MTP / EAGLE 細節。
11. **3.10 constrained decoding 內部**：sampling 階段的 grammar mask、structured output 跟 function calling 的內部機制。
12. **3.11 公開課**：完整學習路線。
...
3. **多模態的理論細節**：vision encoder 內部架構、speech / audio LLM、video LLM 等理論深度交給專門課程；應用層的 vision 在 coding 工作流的設計見 [4.15 Vision in coding workflow](/llm/04-applications/vision-in-coding-workflow/)。

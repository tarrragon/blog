---
title: "3.9 Reasoning models：test-time compute paradigm"
date: 2026-05-12
...
兩條路線**不對立**、是疊加：reasoning model 本身仍跑在大 base model 上、reasoning RL 是再加一層後訓練。但它改變了能力提升的邊際成本曲線：

| 提升能力的成本 | Pretrain scaling 路線             | Test-time scaling 路線                                        |
| -------------- | --------------------------------- | ------------------------------------------------------------- |
| 一次性訓練成本 | 線性增加                          | 訓練 reasoning RL 相對小（已有 base model）                   |
| 推論成本       | 不變                              | 每次推論增加 5-50×（reasoning trace 長度動態調整）            |
| 收益最明顯任務 | 通用 benchmark、知識題、寫作      | math、debug、algorithm、long-horizon planning                 |
| 對讀者錢包影響 | 集中在訓練方（雲端 API 價變化小） | **直接傳到使用者**（reasoning model API 計費 thinking token） |
...
4. **16GB+ VRAM PC**：跑 14B distill、或 32B distill 配 [MoE CPU offload](/llm/knowledge-cards/moe-cpu-offload/) 戰術
...
下一章：[3.10 Speculative decoding 內部](/llm/03-theoretical-foundations/speculative-decoding-internals/)、看另一個推論時加速的技術細節。

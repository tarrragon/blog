...
訓練後 embedding 展現語意結構（包括「king - man + woman ≈ queen」這個著名實驗）。Word2Vec 是 word embedding 的入門教材、但已被現代 contextual embedding（如 Transformer 內的 embedding）取代。
...
| 方法       | 機制                                                          |
| ---------- | ------------------------------------------------------------- |
| Sinusoidal | 用 sin / cos 不同頻率生成固定位置向量、加進 embedding         |
| Learned    | 學一個 `(max_seq_len, hidden_dim)` 的位置矩陣、加進 embedding |
| RoPE       | Rotary Position Embedding、把位置編碼到 Q/K 的旋轉中          |
| ALiBi      | Attention with Linear Biases、在 attention scores 加位置 bias |

Llama 系列、Gemma 系列、Qwen 系列都用 RoPE、目前是主流。

詳細展開見 [3.3 Transformer 架構](/llm/03-theoretical-foundations/transformer-architecture/)。

## Tied vs Untied Embedding

「Tied embedding」指「input embedding（token → vector）跟 output projection（hidden → logits）共用同一個矩陣」。GPT-2 系列預設 tied、節省參數。

「Untied embedding」是兩者各自獨立。Llama 系列預設 untied、品質略好、但 embedding layer 跟 output layer 都要存。

實務上、untied 是現代主流、用較多參數換較高品質。

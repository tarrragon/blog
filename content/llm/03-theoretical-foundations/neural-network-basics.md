...
神經網路是「N 個 linear layer + activation function 串接」的結構。每個 layer 做矩陣乘法 + bias + activation；多層堆疊得到深度表達能力；forward pass 跑推論、backward pass 算 gradient 用於訓練。Transformer LLM 在這個基底上加入 embedding、attention、residual connection 等架構元素、構成完整模型。

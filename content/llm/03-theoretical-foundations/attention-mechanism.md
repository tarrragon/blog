...
- 長距離依賴變直接（attention 直接連到任何位置）。
- 可以並行（不同 token 的 attention 計算彼此獨立）。
- 代價是計算量隨序列長度平方增長（O(n²)）。
...
實驗發現不同 head 確實學到可解釋的功能（雖然多數 head 的功能難以直觀標籤）。Multi-head 比 single-head 表達能力強。
...
實作上 mask 是一個下三角矩陣、訓練跟推論時都套用。

「Decoder-only LLM」（GPT、Llama 系列）用 causal mask；「Encoder LLM」（BERT 等）不用 causal mask、可看雙向 context、但無法做生成。
...
代價是 KV cache 隨 [context window](/llm/knowledge-cards/context-window/) 線性增長、長 context 場景吃記憶體。Gemma 4 31B 在 32GB Mac 上實用 context 約 8 ~ 16K tokens、超過會 swap。
...
## 為什麼 attention 對寫 code 加速明顯

[Speculative decoding](/llm/knowledge-cards/speculative-decoding/) / [MTP](/llm/knowledge-cards/mtp/) 在寫 code 場景特別有效、原因落在 attention 的並行性：
...
理解這點、能解釋為什麼 MTP 對 coding 比創意寫作加速更明顯。

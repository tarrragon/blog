...
**個人 dev 場景的最低 control**：RAG exclude `.env` / secrets、tool use 加 confirm（見 [6.2 tool use 與 MCP server 的權限模型](/llm/06-security/tool-use-permission-model/)）、agent loop 設 max steps、untrusted 來源內容明確標記
...
| OWASP | 主章節                                                                                                                                                | 補充章節 / 卡片                                                                                                                                                   |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LLM01 | [6.3 IDE 場景的 prompt injection](/llm/06-security/prompt-injection-in-ide/)                                                                          | [4.4 agent loop](/llm/04-applications/agent-architecture/)、[hands-on permission-boundary](/llm/01-local-llm-services/hands-on/permission-boundary/)              |
| LLM02 | [6.4 跨雲端 / 本地的資料邊界](/llm/06-security/cross-cloud-local-data-boundary/)                                                                      | [4.16 靜態 RAG](/llm/04-applications/static-and-serverless-rag-deployment/)、[0.7 隱私 / 資安的資料流原理](/llm/00-foundations/privacy-data-flow/)                |
| LLM03 | [6.0 模型供應鏈與信任邊界](/llm/06-security/model-supply-chain-trust/)                                                                                | [4.16 client-side LLM 段](/llm/04-applications/static-and-serverless-rag-deployment/)                                                                             |
| LLM04 | [6.0 模型供應鏈與信任邊界](/llm/06-security/model-supply-chain-trust/) 部分                                                                           | [3.4 訓練流程](/llm/03-theoretical-foundations/training-pipeline/)、[hands-on fine-tune](/llm/01-local-llm-services/hands-on/local-fine-tuning/)                  |
| LLM05 | [6.2 tool use 與 MCP server 的權限模型](/llm/06-security/tool-use-permission-model/)                                                                  | [4.3 tool use 原理](/llm/04-applications/tool-use-principles/)                                                                                                    |
| LLM06 | [6.2 tool use 與 MCP server 的權限模型](/llm/06-security/tool-use-permission-model/) + [4.4 Agent 架構原理](/llm/04-applications/agent-architecture/) | [4.17 coding agent harness](/llm/04-applications/coding-agent-harness/)、[hands-on permission-boundary](/llm/01-local-llm-services/hands-on/permission-boundary/) |
...
| LLM09 | [hallucination 卡](/llm/knowledge-cards/hallucination/) + [4.21 LLM-as-Judge 評估方法](/llm/04-applications/llm-as-judge/)                            | [4.1 RAG](/llm/04-applications/rag-principles/)、[4.14 benchmarking](/llm/04-applications/benchmarking-and-evaluation/)                                           |

...
三個 registry 在實際拉模型的操作面（namespace、download 指令、本機儲存路徑）見對應安裝章節：[1.0 Ollama](/llm/01-local-llm-services/ollama/)、[1.1 LM Studio](/llm/01-local-llm-services/lm-studio/)、PC 場景的 LM Studio 見 [5.4 LM Studio 在 Windows](/llm/05-discrete-gpu/lm-studio-on-windows/)。本節聚焦三者在供應鏈管理上的相對位置：
...
2. **Continue.dev 擴充套件**：VS Code marketplace 有基本審查、但 community-published 擴充套件的供應鏈仍是個人視角。Continue.dev 安裝與 multi-provider 配置見 [1.3 VS Code + Continue.dev 整合](/llm/01-local-llm-services/vscode-continue-integration/)。

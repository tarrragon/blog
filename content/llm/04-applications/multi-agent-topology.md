...
判讀「該不該走 multi-agent」的訊號：

**該走 multi-agent**：

- **角色差異顯著**：不同 step 要不同 prompt / model / tool / memory、寫死成 step 反而失去靈活性。
- **跨產品重用**：同一個 agent 要被 marketing / product / sales 三個團隊用、每個團隊有不同 query 形式。
- **真正平行**：多個 agent 各做自己的事、最後合併、不是 single-thread 線性。
- **動態決定協作**：需要哪些 agent 參與是 query-dependent、不是事先寫死。

**不該走 multi-agent、留在 multi-call**：

- **角色其實同質**：硬拆成多 agent 只是換個名字、orchestration overhead 純增。
- **控制流可寫死**：step 順序固定、不需要 agent 自決下一步。
- **單一 user / 單一場景**：不需要跨產品重用、寫成函數比 agent 簡單。
- **debug 容易性 > 靈活性**：multi-agent 出 bug 比 multi-call 難排、團隊還不熟前先 stick to multi-call。
...
- **失敗模式**：
  - **N² 通訊複雜度**：agent 多了之後、通訊路徑爆炸、cost / latency 不可控。
...
- **典型場景**：[MCP](/llm/knowledge-cards/mcp/) 視角下、每個 MCP server 暴露一個「可呼叫的 agent」、client agent 把它當 tool 用。跨組織 agent 互通基本上走這個模式。
...
實務上組合使用：對外是 hierarchical（單一 orchestrator）、orchestrator 內部跟 specialist 通訊走 agent-as-tool 協議（透過 MCP）、specialist 之間用 flat 模式平等溝通。
...
Agent A 呼叫 B、B 呼叫 C、C 又呼叫 A、形成 cycle、無限執行。
...
每次跨 agent call 都要重新 brief 對方、context 累積、token cost 爆、latency 累積。
...
Orchestrator 必須理解所有 specialist、它的 prompt / capability 限制整個系統。換 specialist 容易、換 orchestrator 牽動所有 routing 邏輯。
...
A 給 B 一個 hallucinated input、B 沒驗證就執行、錯誤層層放大。
...
這個 framing 的價值：**agent 跨組織重用就是 MCP server 普及問題**。當業界對 MCP schema 達成共識、agent-as-tool 拓樸的工程成本會大幅下降。

...
本地 single-user 的 baseline 數字（[hands-on](/llm/01-local-llm-services/hands-on/rag-mcp-resources/) 紀錄的 RAM / latency）**在 multi-user 場景下幾乎無法 extrapolate**、根因是資源爭用會放大原本看不到的成本：
...
```text
H100 instance: ~$2/hour（以 2026 年 spot price 為例、實際隨雲廠商與當期報價變動）
H100 monthly = $2 × 24 × 30 = $1440
若 utilization > 50% 且團隊有 SRE 能力維運、自架較划算
若 utilization < 30%、或團隊無 GPU 維運經驗、API 較划算
```

...
LLM 進入工作流後、「人類什麼時候介入」變成設計重點。完全自動跑 production migration 通常是 over-trust、每個 tool call 都要 step-by-step approval 通常是 under-trust。兩個極端之間有 spectrum、選錯位置就會踩坑——不是 AI 跑偏沒人接住、就是人類疲勞變橡皮圖章。
...
Centaur 跟 cyborg 是兩種人類跟 LLM 共事的姿態、源自 HBS / UPenn / Wharton 對 BCG 顧問使用 AI 的研究。兩種姿態各有適用場景、不是哪個好哪個壞。
...
為什麼鋸齒：能力分佈跟訓練資料分佈相關、不是跟人類直覺的「難易」相關。算術靠的是符號操作、訓練資料中算術沒有特別多；翻譯靠的是大量 parallel corpus、訓練資料中有大量。

實務啟示：

- **不要用「人類直覺難易」推測 AI 能力**。試跑、看結果、不要預判。
- **「全自動」是錯題**：因為 frontier 是鋸齒、總有些子任務落在 frontier 外、需要人介入。設計時要假設「有部分子任務 AI 會失敗」、而不是「都會成功」。
...
HITL 最常見的失敗不是「沒有 HITL」、是「HITL 退化成 rubber-stamp approval」——人類點 approve 但其實沒審、跟沒 HITL 沒差別。

幾個導致橡皮圖章化的設計反模式：

### 反模式一：所有動作都 pre-act approval

不分高低代價、每個 tool call 都要 user 按 approve。結果 user 每天按 100 次 approve、按到下意識、根本沒看內容。

緩解：分級 approval。低代價（read-only、本機 sandbox 操作）full auto、中等代價（檔案寫入、本機 commit）checkpoint approval（每 N 步審一次）、高代價（push、deploy、production change）強制 step-by-step。對應 [4.3 tool use 副作用範圍](/llm/04-applications/tool-use-principles/) 的等級分類。

### 反模式二：approve UI 缺乏 diff

「approve this action?」按鈕、但 user 看不到 action 的具體內容（要做什麼、影響什麼）、只能盲簽。

緩解：approval UI 強制 show diff——準備寫的檔案內容、準備執行的 SQL、準備發的 email 草稿。沒有 diff 就沒有審查、不要假裝有審查。

### 反模式三：approval 沒有 reject 路徑

只能 approve、reject 的話 agent 不知道怎麼辦、user 怕 reject 後續流程斷、就 approve。

緩解：reject 要有明確 fallback——agent 收到 reject 後該怎麼做（換方案、停下來、escalate）。

### 反模式四：approval 訊號沒被學起來

User 一直 approve / reject、但這些訊號沒回饋進系統、agent 下次還是問一樣的問題。

緩解：approval pattern 進 trace、定期 analyze、把「總是 approve 的動作」自動降級、「總是 reject 的動作」進 prompt 改變 agent 預設行為。

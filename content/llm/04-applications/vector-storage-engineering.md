...
| [4.12 Embedding model](/llm/04-applications/embedding-model-internals/) | 處理向量怎麼生成、本章處理向量怎麼存                       |
...
想看 retrieval 品質不夠時的增強手段（query rewriting / HyDE / multi-step），回到 [4.2 RAG 檢索增強](/llm/04-applications/rag-retrieval-enhancements/)。想看 embedding 模型怎麼選、怎麼判讀 MTEB 分數，回到 [4.12 Embedding model 內部](/llm/04-applications/embedding-model-internals/)。

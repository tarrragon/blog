...
「怎麼寫好 prompt」這件事有兩個本質問題、讓它不適合寫進長期教材：

- **跨模型不可遷移**：對 GPT-4 有效的寫法、對 Claude 可能反效果。模型 SFT 分佈不同、對 prompt 結構的偏好不同。
- **跨任務不可遷移**：對 summarization 有效的格式、對 classification 沒幫助。每個任務的最佳 prompt 形狀都是 case-by-case 試出來的。

但**「有哪些 prompt 技術可選、它們各自解什麼問題、能不能組合」這層結構是穩的**——這是本章寫的內容。具體寫法（act as XYZ 怎麼設計、step 怎麼分）屬於跨模型跨任務的客製工作、不寫進指南。
...
- **失效在 reasoning model 出現後**：[reasoning model](/llm/03-theoretical-foundations/reasoning-models/) 本身就在生成隱藏 chain-of-thought、再外加 CoT prompt 通常無收益甚至反效果（模型把 explicit CoT 跟內部 reasoning 混在一起）。判讀訊號：模型卡片寫「reasoning model」、就不要再加 "think step by step"。
...
- **常見過度迷信**："you are the best in the world at this" 這類自誇式 prompt 對某些模型有微弱效果、跨模型不穩定。不值得當核心策略。
...
| --------------- | ------------------------------------------ | ------------------------------------------- |
| Accuracy        | 任務完成品質                               | —                                           |
| Latency         | 從 request 到 final response 的時間        | 多 token = TTFT 不變但 TPS 累積、CoT/reflection 顯著拉長 |
| Cost            | 每次 inference 的 token 成本               | Few-shot / CoT / reflection 都加 token、累積快 |
| Debuggability   | 失敗時能不能定位是哪一步出問題             | Single 大 prompt 失敗難 debug、chaining / decomposition 好 debug |
...
## 組合：何時 Stack、何時不要 Stack

技術可以疊、但疊不對會互相抵消。

### 何時 stack 有效

技術解決的是**不同軸的問題**時、stack 有效。
...
技術解決的是**同軸的問題**、或**底層假設衝突**時、stack 反效果。

- **CoT + reasoning model**：reasoning model 已內建 CoT、外加 explicit CoT 干擾內部 reasoning。判讀：模型卡片寫 reasoning、不要加 "think step by step"。

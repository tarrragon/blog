...
2. 畫出 [agent loop](/llm/knowledge-cards/agent-loop/) 的核心結構、看到新 agent 工具能對應到這個骨架。
3. 看到 agent 失敗時、能診斷是哪一類失敗（[context drift](/llm/knowledge-cards/context-drift/) / [目標漂移](/llm/knowledge-cards/goal-drift/) / [tool 誤判](/llm/knowledge-cards/tool-result-misread/)）。
...
### [Context drift](/llm/knowledge-cards/context-drift/)（上下文漂移）
...
### [Goal drift](/llm/knowledge-cards/goal-drift/)（目標漂移）
...
### [Tool result misread](/llm/knowledge-cards/tool-result-misread/)（工具結果誤判）
...
- 完全靠模型自己說「完成」→ 漂移風險高。
- 沒有客觀驗證 → 「成功」訊號可能是 [hallucination](/llm/knowledge-cards/hallucination/)。

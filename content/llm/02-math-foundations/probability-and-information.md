...
[詞彙表大小](/llm/knowledge-cards/vocabulary-size/)（vocabulary size）通常幾萬到十幾萬：
...
[Temperature](/llm/knowledge-cards/sampling-constraint/)（溫度）的核心定義是「softmax 之前先除以一個正數、調整輸出分佈的尖銳度」：
...
[Top-P sampling](/llm/knowledge-cards/top-p-sampling/) 的細節：先依機率排序、累加直到超過閾值 P（如 0.9）、只 sample 這些 token、其他丟掉。Token 多樣性自動依分佈尖銳度調整、比固定 K 彈性。完整 sampling 策略（含 repetition penalty、min-p、frequency penalty 等）見 [3.5 sampling 策略](/llm/03-theoretical-foundations/sampling-and-decoding/)。
...
[Perplexity](/llm/knowledge-cards/perplexity/)（困惑度）的核心定義是「e 的 cross-entropy 次方」、衡量模型預測下一個 token 的不確定性：
...
[KL divergence](/llm/knowledge-cards/kl-divergence/)（Kullback-Leibler divergence、KL 散度）的核心定義是「衡量分佈 q 偏離分佈 p 的程度」：

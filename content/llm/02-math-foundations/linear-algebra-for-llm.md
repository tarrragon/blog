...
- **Layer normalization**：每個 layer 結束後把 activation 重新正規化、避免數值爆炸或消失。
- **Embedding normalization**：embedding 模型常把向量正規化到 L2 norm = 1、讓內積等同於 cosine similarity。
- **Gradient clipping**：訓練時若 gradient 的 norm 太大、截斷到合理範圍、避免訓練不穩。

Cosine similarity（餘弦相似度）= 兩個向量的內積除以兩者 norm 的乘積、結果落在 -1 到 1 之間、是 RAG / semantic search 最常用的相似度指標。
...
模型權重數量的算法：把所有 layer 的權重矩陣大小加總、就是 31B / 70B 等參數規模。例如一個 hidden size = 4096 的 linear layer、權重矩陣大小 `4096 × 4096 = 16,777,216`、約 16.8M 參數。
...
矩陣乘法的維度規則：**左矩陣的 column 數要等於右矩陣的 row 數**。`(m × k) @ (k × n) = (m × n)`。遇到 dimension mismatch 錯誤、就是哪一步維度沒對齊。

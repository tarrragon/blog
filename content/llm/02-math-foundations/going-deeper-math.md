...
選課的原則：先從跟 LLM 連結最緊密的開始、由近至遠。3Blue1Brown 的視覺化系列適合入門複習、MIT / Stanford 的正式課程適合認真打底、Karpathy 的 YouTube 系列適合「想直接看 LLM 怎麼從零實作」。
...
| 2    | 線性代數正式課 | 想紮實打底          |
| 3    | 機率論 + 統計  | 大學以上數學        |
| 4    | 深度學習基礎   | 完成階段 1-3 或同等 |
| 5    | 從零實作 LLM   | 想直接接觸完整系統  |
...
- **課程連結**：MIT OpenCourseWare 上搜尋「18.06」
...
- **課程連結**：YouTube 上搜尋「Harvard Stat 110」
...
### Stanford CS229 Machine Learning by Andrew Ng（OCW 免費）

ML 基礎金標準、涵蓋 linear regression、logistic regression、SVM、CNN、強化學習等。雖然較舊（沒有最新 Transformer）、但基礎扎實。
...
### Stanford CS336 Language Modeling from Scratch（2024 新開）

Stanford 新開的 LLM 從零訓練課程、涵蓋資料、tokenization、模型架構、訓練、評估、部署整條鏈。
...
這幾本書都有作者放出的免費 PDF、google 「書名 + pdf free」可以找到正規來源。

...
3. **推論常見更低精度**：fp16、int8、int4 在推論時夠用、訓練時不夠。

## bf16 為什麼比 fp16 更適合 LLM
...
bf16 的 exponent 8 bits、跟 fp32 同範圍、不會 overflow / underflow。代價是 mantissa 只剩 7 bits、精度更低。但對 LLM 訓練來說、範圍比精度重要（gradient 的方向比精確值關鍵）。
...
實際每個權重的 bits 不剛好是 4、會稍高一點（例如 Q4_K_M 實際約 4.5 bits）。
...
這就是 [模型選型](/llm/01-local-llm-services/model-selection-priority/) 的核心啟示：「夠大」跟「夠好」是兩件事、別為了塞更大模型激進量化。
...
3. **Activation 維持 fp16 / bf16**：activation 不量化、避免進一步損失精度。

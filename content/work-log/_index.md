...
- **結構是「情境條件 → 判準 → 推導與驗證」**：開頭的情境是條件描述——什麼樣的程式、什麼設定、什麼操作會遇到；判準或結論在開頭就給，推導、實測與失敗輸出擺後面。案例是推導的材料，以推導順序組織，判準壓到文末的三幕劇（第一次失敗、第二次失敗、頓悟）要壓平。
- **視角是客觀條件、不是第一人稱時間線**：檢討的脈絡（做錯了什麼、為什麼錯、為什麼要糾正）用條件語言承載——「reviewer 問了 X、後來才想清楚」寫成「若對這個做法問 X 而答不出來、就該重新檢討」；人物以角色出現（reviewer、修改者）。
- **前置欄位交代事件來源**：文首 blockquote（觸發場景 / 疑問來源 / 整理目的 / 本文邊界、視內容增減）用一句話宣告「這來自一次實際事件」，正文因此不需要用個人經歷組織。

範例見 [註解防不了改壞](comment_cannot_guard_invariant/)——直述標題、結論在前、reviewer 的提問以條件形式呈現。

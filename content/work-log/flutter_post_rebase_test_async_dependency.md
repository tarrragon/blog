---
title: "Flutter rebase 後跑測試暴露的依賴洞：sync try-catch 接不到 BotToast 的 async 錯誤"
date: 2026-05-26
draft: false
description: "rebase 後 analyze 全綠不等於安全：拆解 static analysis 在 service locator、widget tree state、async error 三層的覆蓋邊界，並對應分類診斷、runZonedGuarded、fallback signature 三個機制。"
tags: ["dart", "flutter", "test", "rebase", "async", "zone", "runZonedGuarded", "service-locator"]
---

> **核心議題**：rebase 後型別檢查全綠的「安全感」會在三個面向同時失效 — service locator 的注入契約、widget tree 的 framework state、async error 的 try-catch 邊界。這三層都看不見、都要靠 test 才會炸。本文拆解三層的成因與接管機制：分類診斷、`runZonedGuarded`、fallback 訊息 signature 設計。
> **案例骨幹**：refactor 分支 rebase 後 `analyze` 全綠、跑 test 7 個失敗。「本來就有」那一批 6 個跟主線無關（自己舊 commit 把功能註解掉、test spec 漂走）；「rebase 引起」那 1 個揭露同一個 fire-and-forget API 同時持有 sync 與 async 兩條失敗路徑（缺 service 注入、BotToast 同步 assert、BotToast 從 async gap 後拋 `LateInitializationError`）。`runZonedGuarded` 把兩條路徑收斂到同一個 fallback handler。

---

## 1. Static analysis 的覆蓋邊界

`flutter analyze` 的責任是檢查宣告與名稱層的契約 — 型別一致、import 能解析、識別字能對到符號。它驗證的是「靜態可決定的事」：missing import、undefined method、type mismatch 都會在 compile 前被攔下。

它**看不到**的是 runtime 才成立的契約，這正是 rebase 後最容易踩的盲區：
...
這個邊界決定一件事：**rebase 之後 `analyze` 通過，不能當作 runtime contract 沒被破壞的證據**。主線開發者在某個函式新加一行「需要 X service」或「需要 Y widget」的程式碼，型別系統不抗議；feature branch 的 test setUp 沒準備 X 或 Y，**只有 test 跑到那行才會炸**。

對 rebase 流程的意涵：rebase 完成後，`analyze` 是必要條件、test 才是最後一道閘門。**特別是「主線 merge 期間動過的模組」相關的 test，必須跑過再 push**。

本案例的訊號：`flutter analyze` 41 個 issue 全是 `info`（既有 deprecated / lint），沒有 error 或 warning；`flutter test` 跑出 7 個失敗。下節拆解這 7 個失敗。

---

## 2. Test failure 的分類診斷

Rebase 後 test 大量失敗時，**第一個動作是分類、不是逐一修**。失敗大致有兩類，根因與處置順序完全不同：

| 類別                                                 | 訊號                                                                                  | 修法位置                                 |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------- |
| **rebase 引起**：主線新增依賴、feature branch 沒同步 | stack trace 落在「主線新加 / 新改」的 file:line；git blame 指向剛 merge 進來的 commit | test setUp，或主程式對 caller 環境的容錯 |
| **本來就有**：程式碼與 test spec 已不同步            | 同一 feature 多個 case 一起掛、其他無關；git blame 指向**自己**舊的 commit            | 對齊 test spec，或補回真正缺的實作       |

分類用兩個訊號就能定案：

1. **失敗的 pattern**：「特定 feature 全部失效、其他都對」比較像「該段邏輯被刪了 / 沒被執行」，**較不像** rebase 衝突；rebase 衝突通常會編譯失敗，或讓多個無關 case 一起壞。
2. **`git blame` 指向誰**：對 stack trace 第一行的 file:line 直接 `git blame`；rebase 引起的失敗，blame 永遠指向主線剛 merge 進來的 commit。

混在一起改是常見陷阱：把「測試環境缺依賴」當成「主程式有 bug」處理，或反過來。分開修，每一類根因都是單一的、修起來乾淨。

### 操作層：怎麼穩定撈出大量失敗

Test runner 的 log 在 terminal 滾動時，`tail` 看不到失敗清單 — 廚房印表機的 print 或測試自身的 log 會把它淹掉。穩定撈法是把 stdout/stderr 全部導到檔案再 grep：

```bash
flutter test ... --reporter expanded > /tmp/out.txt 2>&1
grep -nE "(Expected|Actual|TestFailure|\[E\])" /tmp/out.txt | head -60
```

三個關鍵：`--reporter expanded` 印完整失敗訊息、`2>&1` 把 stderr 也納入（async error 通常從這裡出）、grep regex 同時涵蓋 `expect` 斷言與 `[E]` 的 flutter test error 標記。

### 案例：兩類訊號各舉一個

**「本來就有」那一批（6 個）** 都在 `formatAmount` 千位分隔上：

```text
Expected: 'NTD1,234,567'
Actual:   'NTD1234567'
```

`git blame` 指向**自己**舊 commit `f936ea1`，看實作：

```dart
String _withThousandSeparator(String integerPart) {
  return integerPart;
...
千位分隔邏輯整段被自己註解掉、commit 訊息卻聲稱「實作格式化金額包括小數位跟千位數」 — 這條提醒：**commit message 不要相信表面字面、要看程式碼跟測試本身**。修法是讓 test spec 對齊現狀（expected 改掉千位分隔），測試名、group 名、doc 註解的「套用千位分隔」字眼一併修掉，避免名實不符。

**「rebase 引起」那 1 個** 的 stack trace 第一行：

```text
"LogService" not found.
package:unipos/data/services/auth/auth_service.dart 250:11  AuthService.afterLogin
```

`git blame` 指向剛 squash-merge 進 main 的 PR — 主線開發者在 `afterLogin` 開頭加了 toast 提示，引入 `LogService` 依賴而測試環境組裝沒同步補。這是 service locator 模式下 hidden dependency 的典型訊號（§6 會展開機制）。第一道修法是 setUp 加 `Get.put(LogService())`；解掉這層之後，同一條呼叫路徑的後續邏輯會落到 §3 的 sync 與 async error 邊界。
...
`async` 函式內的 try-catch 是常見混淆點：寫成 `try { await x; } catch (e)` 時，try-catch **能**接住 `await` 的 future rejection（`await` 把 async error rewire 成 sync throw）。但對沒 await 的 fire-and-forget 排程（直接呼叫一個會內部 schedule microtask 的 API），try-catch 仍然接不到。
...
### 案例：sync assert 與 async error 共存的訊號

`Popup.hint` 同一條呼叫路徑會打到 BotToast 的兩種失敗模式 — 一條同步、一條 async。三組訊號攤開對照：
...
| `LateInitializationError: Local 'cancelFunc' has not been initialized.` 出現在 `===== asynchronous gap =====` 之後 | async（BotToast 內部排到下一個 frame / microtask 才執行） | 不能                            |

第一條的修法是 setUp 補注入（§2 已處理）。

第二條的同步 assert 單獨看，sync try-catch 接得住。但它跟第三條 async error 是**同一個 API 的兩種失敗模式** — 包 sync try-catch 只罩到同步那條、async 那條仍漏。
...
| `onError` handler 自己拋錯                              | 上溯到 parent zone | handler 要簡短可靠；fallback 自己掛，是上層責任 |
...
## 6. Service locator 模式的 hidden dependency 風險

「rebase 後跑測試暴露隱藏依賴」這個 pattern 的根本原因是 service locator 或 DI 容器的設計：

- 主程式呼叫 `Get.find<X>()`、`GetIt.I<X>()`、`Provider.of<X>()` 時，編譯時只檢查型別、runtime 才檢查注入是否存在。
- Feature branch 的 test setUp 是「分出去當時」的依賴快照；主線新增依賴時，setUp 沒同步補。
- 型別系統看不見 → analyze 不抗議 → 只有 test 跑到那行才會炸。

緩解這個風險的著力點在流程與容錯設計（換掉 DI 容器是另一層決策、不在本節範圍）。流程面已在前文展開過：§1「rebase 後必跑 test、特別是主線 merge 期間動過的模組」、§2「failure 要快速分類」。本節集中講 §6 自己的論點：**主程式對「依賴不在」的 robust 行為**。

`Popup.hint` 對「沒有 widget tree」連環倒，不只 unit test 會炸 — isolate 內、background task 內、任何非 UI 環境都會炸。修 test 順手把主程式對缺依賴的容錯加上，是合理副產物：unit test 是觸發訊號，主程式被觸發後變得更能適應多元 caller 環境，這個改動的受益面大於原本 test 暴露的那個情境。**主程式變 robust 的價值大於「讓 test 過」**。

修主程式容錯時要分辨「容錯」與「掩蓋」的界線：log 仍要留、fallback signature 仍要可識別（§5），不能把錯誤完全靜默 — 否則 dev app 真壞掉時也看不見。
...
「rebase 暴露 hidden dependency」適用任何用 service locator / DI 容器的專案 — GetX、`get_it`、Provider 的全域 instance、自寫的 singleton 都會在主線新增依賴而 feature 分支沒同步補 setUp 時翻車。**Type system 看不見、只有 runtime 才會炸**，跑 test 才是 catch 這事的工具。

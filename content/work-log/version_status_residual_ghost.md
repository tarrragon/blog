...
**工具的檢查範圍，決定了系統的一致性邊界。** 工具只檢查當前版本，一致性就只在當前版本內維持。歷史版本的狀態漂移不會被發現——直到有人手動查看看板。

這和資料庫的外鍵約束是同一個 pattern：如果你只在寫入時驗證單筆資料的格式，而不檢查跨表一致性，orphan record 就會累積。工具的 pre-flight check 就是內部流程的「外鍵約束」——範圍太窄，殘留就會累積。

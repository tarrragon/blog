...
這裡的 opinion 是**建議而非阻擋**——工具提示預設路徑，使用者用一個參數就能覆蓋。這個區分很重要：阻擋式的 opinion（必須加 flag 才能繞過）適合風險高的操作（如 force push to main）；建議式的 opinion 適合歸屬判斷。錯誤成本不對稱決定了形式：建議錯了，使用者多打一個 `--version` 覆蓋，幾秒鐘；沉默錯了，事後遷移 ticket 加重新發布，幾小時。只要建議的正確率不是極低，建議就比沉默划算。
...
1. 列出你的 CLI 所有接受自由輸入的參數
2. 對每個問：有沒有「多數情況下正確」的預設值或建議值？
3. 有的話，加建議式 opinion（提示預設 + 允許 `--flag` 覆蓋）
4. 檢查工具的清理路徑：有沒有前一次操作應該同步但沒有同步的狀態？
5. 如果你的工具會被 AI agent 呼叫，上述每一項的優先級加倍

...
> **整理目的**：反思「工具設計者的責任邊界」——工具接受自由輸入時，預設路徑如何形塑使用者的決策慣性。如果你維護內部 CLI 工具，尤其是會被 CI/CD 或 AI agent 呼叫的工具，這篇討論的 pattern 可能正在你的工具中靜默發生。
...
這個 pattern 不限於版本管理——任何接受自由輸入的內部工具，只要輸入涉及分類或歸屬判斷，都可能有同樣的問題。我們的版本管理工具沒有表達這個語意，接下來的兩個事件是後果。
...
接下來要做根因分析和系統性防護（State Registry Pattern 重構、品質規則更新）。建立工作項目時，順手指定了 `--version 0.4.0`——v0.3.0 和 v0.3.1 都已發布，v0.4.0 是下一個功能版本，看起來是合理的選擇。
...
- **AI agent 使用者**：opinion 是最可靠的防線。規則檔和 memory 系統也能傳遞教訓，但工具在操作當下的即時引導是離決策點最近的攔截——規則是知識層，工具是執行層，兩者互補但執行層更難繞過
...
文件仍有價值——它定義「應該是什麼樣」，傳遞設計理由和架構決策。工具實現「實際是什麼樣」。兩者不一致時，優先修工具——而非加更多文件提醒使用者「請遵守操作規範」。
...
每一個「寫文件提醒使用者遵守操作規範」都是承認「工具沒做到位」的信號。看到這個信號時，優先評估能否把提醒轉化為工具的預設行為。

## Convention over Configuration

有 opinion 的工具把判斷成本從「每次使用時」前移到「設計工具時」——一次判斷，永久生效。

Rails 的「Convention over Configuration」是同一個觀念的先驅表達：框架用約定引導開發者走正確路徑，而不是提供無限彈性讓每個人自己決定。CoC 省去不必要的配置決策；opinionated CLI 在必要決策時引導方向——兩者共通的是減少使用者每次操作的判斷成本。

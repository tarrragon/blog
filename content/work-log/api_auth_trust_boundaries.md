...
date: 2026-05-18
draft: false
description: "整理 API 認證的兩層信任邊界（使用者 Bearer Token、系統 Shared Secret）與一個跨系統身分對應 workflow（Provisioning），說明各層該用什麼 secret 機制、為什麼不能混用、以及常見的設計失誤。"
...
每個問題都需要不同的 secret 機制來回答。混用會造成資安漏洞，分得不夠細又會讓系統難以維運。
...
> **前提假設**：以下所有機制都假設 transport 走 HTTPS / TLS。如果 token 與 secret 在 HTTP 明文傳輸，下面所有設計都會被中間人攔截、全部白費。HTTPS 不是 Layer 1/2/3 之外的「第 0 層」、而是**所有層的共同前提**。
>
...
**回答的問題**：「這個 request 是哪個系統發的？」

當系統 A 需要呼叫系統 B 的 API 時，Layer 1 的使用者 token 無法使用 — 它是「使用者」的身分、不是「系統」的身分。系統 B 必須能獨立驗證「這個 request 來自合法的合作系統 A」、不依賴使用者層的 credential。
...
純粹靠 Layer 1 的使用者 token 區分不出這三者 — 因為 token 是「**這位 user 的身分**」、不是「**系統 A 的身分**」。
...
[ 系統 A ] ═════════▶ [ 系統 B ]
       shared secret
       (server-to-server, 絕不能進 client)
```

**Layer 2 secret 不能放進瀏覽器 / 行動 app**。一旦進入 client 端、攻擊者就能透過反編譯、JS source map、devtools network panel 等管道取得 — 取得後即可假冒系統 A 呼叫系統 B。Mobile app 的反編譯工具（jadx、Hopper、Ghidra 等）讓這個攻擊成本極低、不應假設 obfuscation 能擋。

如果 client 端真的需要呼叫 B、正確做法是讓 client 先呼叫 A、由 A 在 server 端用 Layer 2 secret 呼叫 B（A 當 proxy / BFF）、或用 OAuth 把 short-lived token 發給 client、long-lived secret 留在 server。
...
優點：之後使用很順暢
缺點：B 上累積大量「殭屍 user」、schema migration、備份變慢
```
...
優點：B 只存活躍 user
缺點：「第一次使用」這個時機需要文件化、否則新進工程師會誤用
```

### Lazy 的「隱性 API 依賴順序」

Lazy provisioning 的最大成本不在效能、在**認知負擔**：

- 文件沒寫清楚「呼叫 B 前必須先呼叫 A 的 provision endpoint」、新進工程師會在「B 回 404 找不到 user」的訊號上花大量時間排查
- 用 SDK 包裝可以把 provision 自動處理、對外只暴露單一 API
- 不用 SDK 至少要在文件顯眼處註明這個依賴順序
...
### 錯誤一：讓使用者 token 也能驗 Layer 2

**正確責任劃分**：「使用者身分」跟「呼叫系統身分」是兩個獨立維度、各自需要獨立 credential。系統 B 對「來自 A」的信任不該綁定在任何單一使用者帳號上。

**常見誤用**：B 接受「只要 request 帶有任一合法使用者 token 就放行」。

**為什麼不足**：把系統信任降階為使用者信任 — 任一帳號被盜（釣魚、密碼洩漏、token 外流）、攻擊者就能用該使用者身分對 B 發 request、執行 B 開放給 A 的所有操作。原本「攻陷使用者」只該影響該使用者、變成「攻陷使用者 = 取得 A 的部分系統權限」。

**正確做法**：使用者層用 Layer 1 token、系統層用 Layer 2 credential、兩層都通過才放行。

### 錯誤二：把 Layer 2 secret 放進 client

**正確責任劃分**：Layer 2 secret 是「server 代表系統 A 對外的證明」、必須留在 server 端、永不離開受信任的執行環境。

**常見誤用**：把 shared secret 寫進前端 JS、行動 app 編譯時、甚至 git public repo。

**為什麼不足**：client 環境（瀏覽器、mobile app）不在受控範圍 — JS source 可在 devtools 直接看、mobile binary 用 jadx / Hopper 等工具可在短時間內反編譯出字串。Obfuscation 提高的是時間成本、不是安全等級。一旦 secret 流出、攻擊者就能不經過 A 直接以 A 的身分呼叫 B、Layer 2 形同虛設。

**正確做法**：client 需要 B 的功能時、走「client → A → B」、A 在 server 端用 Layer 2 secret 呼叫 B；或用 OAuth 把 short-lived token 發給 client、long-lived secret 留 server。

### 錯誤三：Layer 3 依賴順序沒文件化

**正確責任劃分**：跨系統依賴順序是 API 契約的一部分、屬 publisher 的責任、必須在文件、SDK 或錯誤訊號中顯式表達。

**常見誤用**：「呼叫 B 之前要先呼叫 A 的某個 endpoint」這個前置條件只存在於原始設計者的記憶中、文件沒寫、SDK 沒包、B 失敗時也只回 generic error。

**為什麼不足**：新進工程師接到「呼叫 B 失敗」的訊號、會在 B 的文件、自己的 request 格式、network 層找原因、不會想到「失敗的根因在某個我從未呼叫過的 A endpoint」。這個失敗模式的偵錯成本極高、可能花數天才定位。
...
這個三層框架的設計重點是「跨系統」— 並非所有後端架構都需要完整套用。下列情境可以簡化：
...
| 兩個系統共用同一份 user DB         | Layer 3 不需要（身分天然對應）、但 Layer 1 / 2 仍各自獨立                                             |

簡化的判準是「**該層回答的問題是否真實存在於這個架構**」 — 沒有跨系統呼叫就沒有 Layer 2 問題、沒有獨立 user DB 就沒有 Layer 3 問題。不需要為了套框架而硬塞不存在的層。

反過來、即使架構簡單、**HTTPS / TLS 與 token 儲存原則（hash + constant-time）不可省略** — 這些是任何 Layer 1 的最低要求、跟「層」的數量無關。
...
設計後端 API 時、把這三個問題分開來想、secret 機制的選擇會變清楚。混在一起想、常會誤用「為什麼這個 token 在那邊不能用」的訊號、把不同層的責任綁在同一個機制上。

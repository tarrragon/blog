...
date: 2026-05-07
draft: false
description: "Widget test 單獨跑全綠、混進全 suite 跑就隨機 ~50% fail。一開始順著 fail 訊息抓 print handler test、補了 Get.reset() 卻沒解。重新看 `+N -1` 累計位置才發現失敗的是更前面的 widget test、被 dart test runner 同 process 內前一個 file 的 GetX state 殘留污染。記錄錯誤診斷的彎路、`did not complete` 訊息為何誤導、以及用 setUp 開頭的 Get.reset() 切斷跨 file 污染。"

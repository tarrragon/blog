...
date: 2026-06-23
draft: false
description: "monitor 專案 v0.1.0 從 Proposal/Spec/UseCase 走到 Collector 實作的完整流程檢討。發現三個結構性缺口：doc→TDD 單向銜接缺少反向追溯、紅燈測試需要存根但流程沒定義、代理人超額完成讓 Ticket 追蹤失真。記錄解決方案：追溯矩陣、存根策略、邊界回補流程。"
...
測試設計的 W3-011 ANA 有涵蓋（UT-COL-02-04~08），但 UC 本身沒有更新。邊界條件「住」在測試設計文件而非 UseCase——下次有人讀 UC-01 不會知道這些邊界。

**解法**：追溯矩陣增加 `boundaries:` 區段，測試撰寫者發現新邊界時加 gap entry，PM 建 DOC Ticket 回補 UC/Spec。Phase 4d 掃描所有 gap 確認無遺漏。

### 差異 5：代理人超額完成讓 Ticket 追蹤失真

W3-002（骨架 Ticket）被指派做「main.go + Config + Storage interface」，結果代理人完成了所有模組實作——Schema Validator、Ingest Handler、Query Handler、SQLite Storage、Rule Engine、Health Handler。57 個 unit test 從紅全部變綠。

W3-003（SQLite Storage）、W3-004（Ingestion）、W3-005（Query）、W3-007（Rule Engine）的 acceptance 全被涵蓋，PM 直接 complete。Ticket 追蹤變成「4 個 Ticket 由 1 個 Ticket 超額完成」。

**解法**：Phase 3 rules 新增「超額完成處理」章節——PM 比對被涵蓋 Ticket 的 acceptance、驗證測試覆蓋、勾選 acceptance 並標註來源。
...
**防護**：派發前確保主 repo 在 main + 已 push。單一代理人和正確條件下的多代理人都驗證通過。記錄為 PC-MON-001 錯誤模式。

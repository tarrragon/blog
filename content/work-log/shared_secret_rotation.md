...
date: 2026-05-18
draft: false
description: "拆解系統間 Shared Secret 輪替的核心機制 — 雙密過渡期（dual-secret rollover）、自動化 rotation 工具比較（AWS Secrets Manager / Vault / GCP）、緊急 rotation 流程，以及多 client 環境下常見的失敗模式與緩解。"
tags: ["security", "secret-management", "rotation", "operations", "backend"]
---

## 為什麼專門寫這篇

Shared Secret 的「換新 secret」聽起來是個一行 SQL update 的事 — 直到實際做過一次就知道：
...
這些是維運層的真實痛點、不是「設計理論」 — 但市面上的安全教學多半停在「定期 rotate 你的 secret」這種口號級論述、沒展開實際怎麼做。
...
Rotation 不是「為了 rotate 而 rotate」的安全儀式 — 它降低三種具體威脅：
...
但 rotation 本身**有成本與風險** — 切錯就斷線。所以實務上的目標不是「rotate 越頻繁越好」、而是「**在切換不出錯的前提下、選一個能接受的頻率**」。
...
> **頻率不是越短越好**：NIST SP 800-63B 對多數場景認可 30-90 天足夠、過於激進的 rotation 反而提高出錯機率。7-14 天適用於合規條款明文要求或私鑰可硬體保護的場景；多數 SaaS 不需要拉到這麼短。

「永不 rotate」是有合理情境的 — 純內部 cron job、secret 沒外流管道、rotation 成本大於風險時、停在「事件觸發才換」也是工程決策。重點是有意識的選擇、不是忘記做。

---

## 核心機制：雙密過渡期（Dual-secret Rollover）

### 問題：為什麼不能直接 atomic 切換
...
問題出在 T1 到 T2 之間 — server 只認 v2、但 client 還在用 v1、所有 request 都會 401。即使這個窗口只有幾秒、production 流量在這幾秒裡會炸出大量錯誤。
...
雙密期不是「越長越好」 — 兩把同時有效、安全等級降到「兩把都沒被攻陷才安全」、且維運上要追蹤兩把 secret 的狀態。
...
優點：簡單、無狀態。
缺點：兩把 secret 都要部署到 server、env var / config 變多。
...
優點：secret 集中管理、不散落到 env var。
缺點：依賴 secret store basis、需處理 store 失效的 fallback。
...
如果規模小、不想引入完整 secret manager、自建系統的最小元件：
...
實作量不大、適合內部小規模系統。但記得「**自己寫的 secret 系統本身也需要被保護**」 — DB encryption at rest、access log、權限分離、避免做出比 vendor 工具更脆弱的方案。
...
對外開放 API 的場景、Shared Secret 散落在第三方 client 的 server、你無法強制升級。Rotation 變成「**怎麼讓第三方在你的時程內配合**」的協調問題、不只是技術問題。
...
Grace period 的關鍵是「**斷線的同時、給足夠資訊讓 client 自己修**」 — 不是無聲 401、而是有 error message 引導到下一步。
...
**症狀**：rotation 後第二週、某 client 開始 401 — 才發現他沒收到通知 / 沒升級。

**緩解**：

- 雙密期不要小於「最大已知 client 的 deploy cycle」
...
**症狀**：上次 rotate 已是 3 年前、原本的負責人離職、新人不知道有這個 secret 存在。
...
Shared Secret rotation 的本質不是「定期換」、是「**有意識管理 secret 的 lifecycle**」 — 從發放、儲存、輪替到撤銷、每個階段都有對應的工程設計。

幾個核心原則：

1. **雙密過渡期是底層機制** — 任何 rotation 方案都建立在「server 能同時接受兩把」之上
2. **自動化工具值得投資** — 規模小用 secret manager（AWS / Vault / GCP）、規模大可以自建、但別停在純手動
3. **定期跟緊急是兩套流程** — 定期重不中斷、緊急重立刻撤、別混用
4. **多 client 是協調問題** — 比技術問題難解、grace period + 強制升級工具是常用解法
5. **失敗模式要演練** — production 第一次跑 rotation 不該是真的線上場景、staging 演練過再上

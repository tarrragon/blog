...
> **根因**：`StreamController()` 預設為單訂閱，被當廣播用了一段時間沒事，加第二個 listener 立刻炸
...
幾個月後新功能加第二個訂閱者，**事故當下才知道這條 stream 的真實型別契約是「只能 1 個 listener」**。但回頭看設計，會發現原本 service 的概念上就應該是廣播：

- 副屏鏡像當前購物車 → 自然訂閱者
- 收銀畫面 sticky 高亮 → 也該訂閱
- 未來可能還有：log 服務、雲端同步、analytics、其他裝置（廚房列印、餐桌 KDS）→ 全都該訂閱

語義上**從來不應該是 single-subscription**。寫成 single 是因為當初只有一個用例，用了預設建構子沒多想。

> 為什麼這個案例算「設計瑕疵」而不是「過度設計避免」？把它放進「成本對稱性 / 可逆性 / 領域先驗」三軸框架就能釐清——本案三軸都不利於單訂閱、屬於 YAGNI 不適用的標準情境。完整推導見 [設計瑕疵還是避免過度設計？YAGNI 的真實適用條件](/record/yagni-boundary-three-axes/)。

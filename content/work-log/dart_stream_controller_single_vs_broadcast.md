...
> **根因**：在「`StreamController()` vs `StreamController.broadcast()`」這個零成本差異的選擇下、選了限制更高的單訂閱版本——當下只有一個訂閱者、限制沒曝光；後續加 listener 才撞牆。設計缺陷的本質是「**在零成本差異下不必要地縮小了未來空間**」、不是「沒預測到後來需求」。
...
第一反應通常是「我哪裡寫錯了 / 是不是哪邊忘了 cancel」。檢查程式碼會發現新訂閱者寫得沒問題，副屏的訂閱也沒問題——**問題在底層的 stream 一輩子只能被 listen 一次**。

這是 `StreamController()` 預設建構子的行為：建立的是 single-subscription stream，只允許整個生命週期內**一個** listener。原本只有副屏一個訂閱者，這個限制從來沒被觸發；新加第二個就立刻撞牆。

更深一層看：這是個**設計層面的不一致**。業務需求一直是廣播語義（多個視角同步呈現），技術選型卻是「單一管線」的工具，剛好需求初期只有一個訂閱者所以從未暴露。第二個訂閱者只是觸發條件，不是根因。
...
值得注意的不只是「不能同時兩個 listener」——單訂閱 stream 的限制是**整個 lifecycle 只能 listen 一次**。即使第一個 listener `cancel()` 了，再呼叫 `.listen()` 一樣會炸。要重新訂閱必須重建 `StreamController`。

對 POS 場景的意義：副屏服務一啟動就 listen 並且不 cancel，所以從一開始 stream 就被「鎖死」在副屏這個訂閱者身上。
...
對 POS 案例：sticky 高亮只關心未來變更，**不在意歷史事件**——broadcast 的丟棄行為剛好不傷害語義。但如果是「副屏鏡像當前購物車」這種需求，新副屏插入時若需要立即顯示當下狀態，就要在訂閱後手動 read 一次 `cart.items`。
...
POS 的副屏場景如果是無界事件源（例如背景條碼掃描器），用單訂閱又某條路徑沒 resume，**會慢慢吃光記憶體**——這是 production OOM 的常見來源之一。
...
副屏訂閱寫在 service 啟動時、是 app lifetime 訂閱、沒有 cancel / 重新訂閱情境。所以：

1. 副屏第一個訂閱 → 拿到 single-subscription 的「唯一 listener」配額
2. 後續沒有第二個訂閱方 → 限制不曝光
3. 一切看起來正常

幾個月後新功能加第二個訂閱者、**事故當下才知道這條 stream 的真實型別契約是「只能 1 個 listener」**。
...
| 當下零成本差、選了通用的、後來真的不需要                           | 沒事、額外彈性留著   |
| 當下高成本差、為「可能的未來」付了昂貴成本                         | **過度設計**         |

本 case 落在第一格——`StreamController()` vs `StreamController.broadcast()` 是 11 字元差、零認知負擔、零維護成本差異。即使當下只有副屏一個訂閱者、選 broadcast 也沒付任何代價、卻保留了未來的彈性。寫成 single 不是「對當下需求的精確匹配」、是**在零成本差異下不必要地縮小了未來空間**——這才是「設計缺陷」這個詞要描述的事。

加上 POS 系統的領域先驗強烈指向「多視角同步」（主螢幕 / 副屏 / 廚顯 / 雲端 / 列印是教科書級的 pub-sub 場景）、設計者「沒看到第二個訂閱者會出現」這個假設本身就跟領域常識矛盾。
...
| B. 第二個訂閱者透過第一個轉送            | 副屏服務變成 hub | 高，副屏不該知道 sticky 高亮 | 第二個需求是第一個的 strict subset |
| C. 新加一條平行 broadcast stream         | service 增 API   | 中                           | 兩訂閱關心不同維度                 |
| D. 改用框架的廣播原語（`Rx`、`Subject`） | service 介面變動 | 中                           | 系統性重構契機                     |

### 為什麼選 A

POS 的這條 stream 語義就是「購物車狀態變更廣播」，多訂閱者天生合理。選 B 會讓副屏服務變成轉發中樞，跟它「純顯示」的職責衝突。選 C 增加重複資料源，未來容易兩條 stream 不同步。選 D 雖然系統性更乾淨，但 scope 過大、不是事故當下適合做的決定。

A 是改一行的 minimal fix，且**修正了原本的設計缺陷**而不是繞過它。

### 容易漏的細節：mock 也要改

Service 如果有 mock 實作（測試替身），mock 端也要同步改成 broadcast。否則會出現尷尬情境：跑單元測試（注入 mock）通過、跑 production 還是炸。
...
回到事故場景：事件 payload 第二段（這次變動是哪筆）原本沒人用，所以幾個寫入路徑可能根本沒傳。副屏只看第一段（完整列表），傳不傳第二段對它沒差。**只有第二個訂閱者開始消費這段資訊時，遺漏才會暴露**。

這是廣播設計的一個系統性風險：**當 service 提供「給未來訂閱者用」的擴充欄位時，這些欄位實際上沒被任何訂閱者消費，缺漏就不會在測試中浮現**。要等到第一個真正使用該欄位的訂閱者出現，才會發現「啊原來這個 mutation 路徑沒帶」。
...
對 POS 場景：service 對外暴露的「狀態變更通知」幾乎都落在 broadcast 區——POS 的本質就是多裝置 / 多視圖共享同一份交易狀態（主螢幕、副屏、廚顯、雲端、列印機）。
...
注意：這個方法只能呼叫**一次**，第二次會炸。實務上要保留回傳值在 service 內部做 cache。
...
要嘛用 RxDart 的 `BehaviorSubject`，內建這行為。POS 副屏鏡像場景特別適合 `BehaviorSubject`：副屏進場時就能立即看到當下購物車內容，不必等下一次變更。
...
- **副屏（鏡像當前商品列表）**：只關心「列表屬性」變動，不在乎是哪一筆變動。它建立一個對列表屬性的觀察，每次變動就重畫
...
更深一層的觀察：raw stream 是「以時間軸上的事件為一等公民」的工具，適合「事件本身就是有意義的（log、命令、訊息）」場景；reactive value 是「以狀態為一等公民」的工具，適合「下游關心的是當前是什麼，不是過去發生了什麼」場景。**POS 多視角同步的本質是後者**——副屏關心的是「現在購物車裡有什麼」，不是「過去 5 分鐘掃進了哪些商品的時序」。

把這個認知一般化：當業務語義是「多個視角共享當前狀態」時，工具應該是 reactive value（Rx / ValueNotifier / BehaviorSubject）；當業務語義是「事件流的時序」時，工具才是 stream。事故的根因就是這個語義 / 工具錯配，single-subscription 只是錯配下最早暴露的症狀。
...
2. **回歸風險**：把 service 介面從 `Stream<T>` 改成 `Rx<T>`，所有訂閱方（副屏、UI、未來的 KDS / 雲端同步）都要改 listen 方式
...
下次看到 `Bad state: Stream has already been listened to.` 時，不要先想「我哪裡寫錯了」，先問三個問題：

1. **這條 stream 是 single-subscription 還是 broadcast？**
   - 從定義端確認（`StreamController()` vs `StreamController.broadcast()`），不是從 listener 端
2. **如果是 single，為什麼選 single？**
   - 看註解或設計文件。沒有 → 大概是預設值，沒人特別想過
3. **多訂閱在語義上合理嗎？**
   - 合理 → 改 broadcast，這是修正設計而不是 hack
   - 不合理 → 第二個訂閱者的需求要重新設計（透過第一個 listener 轉送、或拉新 stream）

把「這條 stream 該不該支援多訂閱」變成設計階段的明確問題，比事後修補省事得多。
...
| 副屏（顧客面）  | 商品名、單價、總價、找零      | App lifetime       |

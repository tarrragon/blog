...
寫了引用不保證一定送達：本站遇過一次完整形式寫進 message、事件卻沒有出現的情形，原因未查明。想讓引用成立的時候，用 `gh api repos/{owner}/{repo}/issues/{n}/timeline --paginate` 查對方 timeline 上有沒有這一筆，那是驗收的動作。
...
| `GH-1234`               | 裸井字號的等價寫法，指向同一個 repo 的同一號 issue，行為沒有差別     |
...
這筆事件會不會進到對方的收件匣，官方文件沒有列舉；可以確定的是它留在 issue 頁面上，後來讀這個 issue 的人都看得到。

所以這個機制的操作含意落在時機上：引用的形式在 push 之前都改得動，push 之後就只剩上面那些代價不成比例的選項。

## 判斷標準

先過一道閘門：這次的 commit message 裡有沒有關閉關鍵字。有的話，先確認號碼指向哪個 repo、以及自己對那個 repo 有沒有 write access。

閘門過了之後，判斷標準看的是這次變更與目標 issue 的因果關係。
...
| 有因果，但本 repo 先前的 commit 已經引用過同一個 issue | 拿掉井字號；用前面那條 timeline 指令查得到本 repo 引用過沒有 |

...
`tr -d '\r'` 處理的是行尾。遠端 shell 的輸出會不會帶 `\r`，取決於 adb 送這段輸出回來的時候有沒有配置 [**pty**](/linux/dotfile/knowledge-cards/pty/)（pseudo-terminal，虛擬終端機）——那是核心提供的一對裝置，一端交給遠端執行的程式、一端交給呼叫它的人，讓程式以為自己接在真實終端機上；於是核心會對經過的位元組做終端機該做的處理，把換行補成歸位加換行是其中一項。配置了的話每個換行都是 CRLF，而字串比對與拼接會在看不見的位置失敗——`"$model" = "<型號字串>"` 這種比較永遠不成立，印出來卻看不出差別。`tr -d '\r'` 在不需要的時候不改變結果，所以它在這個位置的成本是零。同一個行尾處理決定擷圖抓不抓得回完好的檔案。
...
logcat 讀的是裝置上的[環狀緩衝區](/linux/dotfile/knowledge-cards/ring-buffer-log/)，寫滿之後從頭覆蓋。可用的時間窗因此由緩衝區的兩端界定：清空劃下起點、倒出劃下終點，中間那一段是重現問題的時候裝置自己寫進去的。緩衝區本身就是收集器，不需要一個在本機持續執行的程序陪著它。
...
adb -s <serial> shell dumpsys window windows \
  | grep -E "Window #|package=|ty=|mViewVisibility="                # 當下誰正在疊著
```

兩條回答的是不同的問題，而只有第二條能下判斷。第一條列出持有權限的套件，那是能力不是行為——一台機器上常有幾個 App 有這個權限而當下什麼都沒畫。

第二條按 z-order 由上而下列出視窗，要同時看三個欄位才算數：排在安裝器之上、`ty=APPLICATION_OVERLAY`、而且 `mViewVisibility=0x0`。第三個欄位不能省——`0x4` 是存在但不可見，系統自己就常駐幾個這樣的視窗（拖放目標、螢幕裝飾），它們排在最上面卻不畫任何內容。只看前兩個欄位會把它們當成疊圖的來源，於是得到一個反向的錯判：安裝器確實被換過，卻因為上面有一個隱形的系統視窗而被判成「別人疊上來」。

三個條件都成立，成因是那個套件疊上來，安裝器本身沒有被動過；沒有任何視窗同時滿足三個條件，剩下的才是系統映像裡那個元件的內容被換過——至於是替換套件還是資源覆蓋，要看該機型的系統映像才判得出來，這一篇不涵蓋。

**截圖分不出這兩種成因。** 安裝確認對話框的視窗屬性帶 `fmt=TRANSLUCENT`，所以它底下會透出前一個畫面——截圖上看到對話框壓著 App 自己的頁面，那是 dialog 主題的正常樣子，不是「別的視窗疊上來」的證據。兩種成因在畫面上長得一樣，分得開它們的只有 z-order 那一份輸出。這一節開頭說畫面是裝置實際產生的輸出、比元件的自述可信，而這裡是那句話的邊界：畫面說得出「上面寫了什麼」，說不出「這一層是誰畫的」。

...
date: 2026-05-13
description: "LLM 寫教學文章常見盲點是內容停在「教科書級」結構、漏掉真實事故才會浮現的失敗模式跟設計取捨。本文整理一套完整流程：完整閱讀案例庫抽 findings → 基於 findings 建立內容 → Agent team 平行多輪審查 → 修正循環 → Polish pass 處理系統性殘留。用 backend/01 至 backend/06 六個模組 / 334 review issue 的實作驗證效益、含 skill 化方法論的回饋循環。"
...
實作數據：6 個模組（backend/01-06）、~38 章 / 334 個 review issue、case fidelity 落在 70-93% 區間、修正後品質升至 0 critical 編造、cross-link 全綠、規範違反 polish pass 後降到單位數低 issue。06 模組後方法論工具化為可觸發 skill、stage 1-5 流程跟 reviewer prompt template、self-scan regex 都封裝成可重用元件。
...
6 個模組（01 / 02 / 03 / 04 / 05 / 06）驗證後、每模組 reviewer 抓到的 issue 數量在 standards reviewer 抓 pattern 越來越細的趨勢下持續擴大、可作為流程預期：

| Reviewer 維度          | 01      | 02       | 03       | 04        | 05       | 06       | baseline        |
| ---------------------- | ------- | -------- | -------- | --------- | -------- | -------- | --------------- |
| Standards reviewer     | 25      | 20       | 20       | 31        | 28       | 45       | 20-45 issue     |
| Case fidelity reviewer | 9 (88%) | 20 (78%) | 15 (70%) | 6 (92.9%) | 13 (80%) | 11 (88%) | 6-20 issue      |
| Consistency reviewer   | 13      | 15       | 15       | 14        | 18       | 15       | 13-18 issue     |
| **總計**               | **47**  | **55**   | **50**   | **51**    | **59**   | **71**   | **47-71 issue** |

**模式觀察**：

- **每模組 issue 數隨 standards reviewer 抓 pattern 越來越細而擴大**：01-03 穩定在 47-55、04/05 推到 51-59、06 推到 71。趨勢來自 standards reviewer 抓的 pattern 越來越廣（從負向骨架 → 「核心責任不是」變體 → 「沒有 X 會 Y」鏈式 → 「case 引用段首」框架）。每模組 reviewer 抓出新 pattern 後、self-scan regex 跟著演進、但 standards reviewer 仍會持續發現新 pattern。
- **Case fidelity 準確率分布更廣**：04 的 92.9% 來自 skeleton case 嚴守「揭露方向、通用補充」紀律；05 的 80% 因引用 09 rich case 加入「fact vs derive 分層」新失分模式；06 的 88% 屬 medium case 紀律首次套用、揭露「實作層擴寫過頭」失分
- **Consistency reviewer 抓到的 frame 重複跟章節數成正比**：02 / 03 / 04 都有 ~13-18 個一致性 issue、05/06 跨模組 cross-link 密度高仍維持在 baseline 內
...
01 / 02 / 03 / 04 / 05 / 06 六個模組執行下來、以下陷阱在 *多數模組都重複出現*、屬於 LLM case-driven 寫作的系統性失分點。本流程下次套用前要 *主動防範*、不能依賴 stage 3 reviewer 補救（雖然 reviewer 都會抓到、但修正成本高）。
...
- 詳見 skill 內 [`principles/case-citation-three-part`](https://github.com/anthropics/...)（對應 `.claude/skills/case-first-module-workflow/references/principles/case-citation-three-part.md`）
...
本流程在 backend/01 至 backend/06 六個模組驗證後（共 ~38 章 / 334 review issue / case fidelity 70-93% 區間）、方法論已工具化為 [`case-first-module-workflow`](https://github.com/anthropics/...) skill（含 stage 1-5 流程、reviewer prompt template、self-scan regex 跟 5 個原則卡）、後續套用到：
...
06 模組是首次套用工具化 skill 的模組、驗證 skill 對 stage 1-2 加速有效、但 reviewer A 仍抓出 45 issue（高於 baseline 20-30）— 揭露 skill 改進方向（self-scan regex 需要持續演進、case 引用段首結構是 LLM 系統性傾向）。
...
- 自掃描 regex 持續演進：每個模組 reviewer 抓出新 pattern 後、回頭加進 self-scan 工具、避免在下個模組重蹈覆轍。06 模組後加 `^對應 \[` 抓段首 case 引用框架

...
- **POS-negation-lead**：正文中「不是 X、而是 Y」「不是 X — 是 Y」「不是 X，是 Y」「與其 X、不如 Y」的重點後置候選。核心概念（Y）被擠到連接詞之後、讀者要先處理一個被否定的 X 才拿到重點。這是資訊結構效率問題、跨語言成立（英文「not X but Y」、日文「X ではなく Y」）、不是中文特有句型 — 偵測可機械化、判定不可。pattern 涵蓋的連接詞枚舉不完、判斷標準是「核心概念在不在句首」而非哪個連接詞 — 漏掉的變體只是讓候選 silent 到有人讀到（規則第一版漏了「不是 X — 是 Y」、第二版漏了不帶「而」的裸逗號形式「不是 X，是 Y」，兩次都靠人發現才補；後者補上時全站多出 400 餘筆候選，而 `compositional-writing` 的 keyword bank 早就收了這個變體 — **工具落後於 bank 這件事本身不會有人回報**，要靠定期把兩邊的 pattern 並排比對）。
  - 裸逗號形式的守衛都下在「不是」的前一個字上，不在 Y 側：中段排除逗號與頓號避免跨子句誤配，而前一個字是「是」（疑問句「是不是」）或「而」（「A 而不是 B」）時跳過。**「而不是」那一條是結構性的**——規則抓的是否定起手，而帶「而」的否定在後、重點已經在前面，那正是規則要作者達到的形態。第一版曾在 Y 側排除「因」，理由是「不是 X，是因為 Z」屬於因果說明；那個判斷錯了、壓掉十筆真違規——「不是因為 attention、是因為 residual」把兩個並列的原因擺在一起而重點在第二個，那是本規則最典型的形態。刻意保留一種不精確：主詞是整個前一子句的「是」（「哪些不是自己說了算，是排序問題」）與被修正的賓語同形、要剖析才分得開，所以規則停在警告層讓人判。
...
### 5.12 縮寫術語（TERM-abbreviation、錯誤層）

`mdtools lint` 對 `content/**` 擋下工程讀者讀不出來的縮寫術語，來源是 [#289 一個詞是不是通用術語，用多份獨立定義的收斂度量](/report/term-probe-measures-register-not-invention/)：

- **TERM-abbreviation**：目前收一個詞，「判準」。它是「判斷標準」的縮寫，詞面只在科學哲學與教育評量有穩定所指，五份低階模型探針在工程語域裡全數讀不出來。寫「判斷標準」；狀態義寫「X 條件」（停止條件、完成條件）。

這一條是錯誤層而不是警告層，理由是它沒有需要語意判定的合法用法：工程句子裡沒有一句非用縮寫不可。討論這個詞本身時用 `「」` 或行內程式碼包起來即豁免，與 5.11 的豁免機制相同。它進工具鏈的原因是替換本身擋不住回流——一次全站替換清掉三千多處，一個月後又長回六百多處，中間沒有任何一道檢查在寫的當下攔它。新增詞要先跑一次術語探針確認讀不出來，再加進 `scripts/mdtools/internal/mdlint/termabbrev.go` 的清單。

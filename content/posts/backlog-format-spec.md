...
實際發生過的情況是：backend 十二個模組各自發展出不同的 backlog 段名，`大綱待辦`、`觀念網路補完方向`、`後續深化方向`、`下一輪推演大綱`、`後續推演大綱`、`規劃方向`、`後續擴充方向`、`目前缺口與後續批次` 都出現過。每個名字在它自己的模組裡都合理，合起來的後果是跨模組盤點必須逐檔閱讀、且無法確定有沒有漏掉某個段名。

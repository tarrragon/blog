...
第一輪 demo backlog 5/5 完成、方法論已小修（見上「第一輪 demo 驗證」段）。第二輪規劃方向：
...
- **同 vendor 多 sub-tool**：例如 PostgreSQL 還有 Patroni HA / Debezium CDC / pgvector RAG / vacuum tuning / partitioning — 都是同 vendor 內部的 implementation-layer 議題、跟 pgBouncer 同層

...
特徵：source / target 多數是 *同 cluster 不同 state*、不是跨 vendor；資料分佈 *sharding / partition / region / replication 拓樸* 變動、但 schema / operational / paradigm 不變。
...
Step 1: 列 6 維度
  ...
  - Application change
  - Data topology（sharding / partition / region / replication 拓樸）

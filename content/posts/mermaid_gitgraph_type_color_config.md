...
description: "Hugo + Mermaid gitGraph 的 type: HIGHLIGHT / REVERSE 顏色不生效時的根因與修復。升級 Mermaid 版本時顏色變數命名會變、要重驗。"
---
...
themeVariables: {
  ...
  // 缺：git0 / git1 / git2 等 gitGraph 變數
}
```

Mermaid 的 themeVariables 對不同圖表類型有不同的命名空間 — flowchart 用 `primaryColor`、gitGraph 用 `git0` / `git1` / `git2`。原本的配置只覆蓋到通用 / flowchart 的命名空間。
...
光靠 themeVariables 在某些 Mermaid 版本仍不穩定（命名規則隨版本改變、見下方注意事項）、加 CSS 直接針對渲染後的 SVG 元素：
...
- **顏色變數命名隨 Mermaid 版本變動**：10.6.1 用 `git0` / `git1` / `git2`、更早版本可能是 `gitInv0` / `gitInv1`。升級 Mermaid 版本時要驗證一次顏色設定還生效。
- **CSS selector 是防禦性的**：themeVariables 配對的話 CSS 不會生效、但 themeVariables 失靈時 CSS 接住。雙保險、不重複設值。
- **這篇只解決「顏色沒出來」這個視覺問題**：寫文章引用 gitGraph 的過程中還發現另一個議題 —「用 emoji 圖例區分 HIGHLIGHT / REVERSE」本身是語意混淆、不是視覺問題、修 CSS 解不了。那個議題見 [report #92 視覺手段對齊錯誤層次](/report/visual-tool-error-layer-alignment/)。

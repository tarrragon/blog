...
| 項目                                                                    | 類型   | 前置條件                                                  | 規模         |
| ----------------------------------------------------------------------- | ------ | --------------------------------------------------------- | ------------ |
| 補齊四個 permalink section 的 slug（posts / work-log / record / other） | 跨模組 | 無（量測指令與驗收基準見 markdown-writing-spec 的 L8 段） | 173 篇（大） |
| 判斷「slug 必填」要不要做成 mdtools 規則                                | 跨模組 | 上一列完成後再判（存量歸零才知道規則的誤報成本）          | 小           |

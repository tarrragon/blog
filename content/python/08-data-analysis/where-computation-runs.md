...
這一行沒有連線、沒有查詢語句、沒有資料庫。`df` 是一張已經在記憶體裡的表格，這一行做的是對一個布林陣列取值。資料的來源可以是資料庫，也可以是 CSV、日誌檔或某個 API 回傳的內容——載進來之後這一層就與來源無關了。
...
**算完之後還要做什麼。** 結果如果只是要回傳給使用者或寫回另一張表，留在資料庫端最短；如果後面還要接繪圖、統計或模型訓練，資料終究要進記憶體，那就沒有理由分兩段做。

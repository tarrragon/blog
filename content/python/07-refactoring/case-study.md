...
用 [程式碼壞味道識別](/python/07-refactoring/code-smells/) 中教的 grep 方法掃描一次：
...
這個順序的思考方式在 [DRY 原則與共用程式庫](/python/07-refactoring/dry-principle/) 中有詳細說明。
...
詳細的抽取過程在[配置分離與常數管理](/python/07-refactoring/constants-management/)中說明。
...
把散落在各 Hook 中的硬編碼錯誤訊息提取到集中的 messages 模組。這對應的是 [配置分離與常數管理](/python/07-refactoring/constants-management/) 中「禁止硬編碼字串」的原則。
...
這個事故的完整分析在[作用域迴歸案例研究](/python/07-refactoring/scope-regression/)中。
...
| ------------------------------------------------------------------ | ----------------- | -------------------------- | -------------------------------- |
| [重構的動機與策略](/python/07-refactoring/refactoring-strategy/)   | 全部              | 起點（為什麼重構）         | 認知負擔量化、階段分解           |
| [程式碼壞味道識別](/python/07-refactoring/code-smells/)            | 所有              | 起點（識別問題）           | grep 分析、5 Why                 |
| [DRY 原則與共用程式庫](/python/07-refactoring/dry-principle/)      | IMP-001           | 第一階段 W1                | 模組抽取、介面設計               |
| [配置分離與常數管理](/python/07-refactoring/constants-management/) | IMP-002, ARCH-001 | 第一階段 W2 + 第二階段 W23 | 三種硬編碼的系統性消除           |
| [大規模統一化重構](/python/07-refactoring/unified-infrastructure/) | IMP-001, ARCH-001 | 第二階段 W22-W24           | 三階段統一化、漸進式重構         |
| [重構陷阱與防護](/python/07-refactoring/refactoring-pitfalls/)     | IMP-003, IMP-005  | 第二階段 W24-W25           | 部分更新防護、AST 驗證           |
| [作用域迴歸案例研究](/python/07-refactoring/scope-regression/)     | IMP-003           | 第二階段 W24               | AST 分析、作用域規則             |
| [非程式碼的重構](/python/07-refactoring/non-code-refactoring/)     | 文件壞味道        | 第三階段 W9                | Progressive Disclosure、文件精簡 |
...
每一階段都在降低閱讀者需要同時記住的東西。這就是 [序章的認知負擔理論](/python/00-philosophy/cognitive-load/) 在實踐中的應用。

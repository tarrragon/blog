...
> **後記**：上面的數字是 v3.0.0 精簡完成時的快照。精簡後不到兩週，`plan-to-ticket.md` 因為新增「執行中額外發現」流程從 43 行長回 87 行，`decision-tree.md` 因為新增 TDD Phase 路由從 286 行長回 434 行。這不代表精簡失敗——新增的內容都是必要的新功能。但它提醒我們：**文件重構和程式碼重構一樣，是持續的紀律。**

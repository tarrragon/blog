...
date: 2026-05-01
weight: 100
description: "資安教學內容的失敗模式不是「讀者學不到」、是「讀者以為學到了並照做、實際還有破口」。讀者實作後沒警覺 = 後續驗證、修補、事件偵測都不會被觸發、破口在生產系統長期 silent 累積。識別 false sense of security 句子的判斷標準：讀者讀完後會說「我做了 X 防護所以安全」、卻無法回答「對什麼 threat 安全 / 什麼 deployment 條件 / 什麼前提失效」。"
...
| [#260 誇飾的合法性由段落位置的功能決定](../hyperbole-legitimacy-by-position-function/) | 本卡要消滅的「做了 X 就安全」句型、#260 定位成安全陳述位的誇飾（零容忍區的一種）、並給出跨位置共用的接收端判斷標準（支撐存在測試、反比操縱訊號）——audit 這類句型時多一組通用工具 |

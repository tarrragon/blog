...
**Last Updated**: 2026-09-07 — 四處回填，由併行 session 的外部實測與自陳提供。**#321**：雙向都驗過之後還有一層——方向數不等於入口數。一次外移三節之後路由表的雙向檢查全綠（零死名、零孤兒），而同一批三處跨檔指涉是失效的：檔頭的同目錄索引與正文的跨檔指涉是另外兩套指標，那個檢查一條都沒掃。雙向全綠比單向零命中更容易讓人收工，分辨的方法是先數指標系統再數方向。**#324**：判別問句的第二種用法是分判準——某條判準由「這個目標日後會不會動」（量判斷者的預測）改成「這個目標現在有沒有名字」（量對象的現況），而改寫後在撰寫它的那批修法裡當場攔下四處自己的同型缺陷；一條判準對產生它的那批文字立刻生效，比事後被套用的次數更能說明它量到了東西。**#334** 補第二個機制：有一類代理不是因為便宜才被取，是因為觀察者站的位置讓代理與本體重合（本地狀態代理遠端狀態，兩次自陳實例都查了容易查的那一側而結論依賴沒查的那一側）；它與價差型的差別在連決定都沒有發生過，所以處置是改問句的形式而不是改成本結構。**#333** 補掃描問句：這句話裡的每一個數字與每一個完成式，是同一個時點的嗎——完成式與它旁邊的狀態值分屬兩個時點時才進判別。

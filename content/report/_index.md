...
`#216 經驗談要重建分析層` → `#218 文章按分析弧拆分` → `#217 審查要有斷言支撐 frame` — 素材是從業者經驗談（訪談 / 社群貼文 / 口述）時，先用 #216 判別 source 類型（分析文自帶分析層、走 #143 拆層；經驗談只有事實 + 判讀、分析層要重建）：事實保留、判讀當 hypothesis、機制由作者從領域理論補建，承擔判準的斷言要能回答「為什麼成立、什麼條件下不成立」；規劃篇章邊界時用 #218 的分析弧判準（一篇走完情境 / 機制 / 量化取捨 / 判準 / 失效條件）、素材不夠走完弧就併入或 backlog、心態類內容依知識類型分流成篇；審查時用 #217 把斷言支撐 frame 排進第一輪（素材來源是經驗談即高風險 batch、知識類型錯位越早抓返工越小）、模組層對照分類 house style。三卡各守一端（素材轉換 / 結構拆分 / 審查覆蓋）——只修任一端、另外兩端的缺口仍會讓模組以「合規的經驗談」形態通過生產線。

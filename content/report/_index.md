...
- [#266 可核對的錨要選會下降的量](checkable-anchor-must-be-able-to-fall/) — 要為一批自己做的判定補一個作者以外可核對的指標時讀；指標的價值在它會不會在該偵測的事情發生時下降，而飽和的覆蓋率只偵測得到新增漏寫、關鍵詞計數量的是措辭不是判定

- [#267 關鍵詞清單只裝得下違規義項佔多數的詞](keyword-list-needs-dominant-violating-sense/) — 某條用詞規則已經有 grep 清單、而漏抓的實例持續從外部指正進來時讀；入選判準是那個詞的違規義項佔它全部用法多少，比例低的擴充進清單只會用噪音蓋掉訊號，要改用探針

...
**Last Updated**: 2026-09-04 — #333 補第五類（別人量的、我引用），由併行 session 的自陳實例提供：四項數據以既成事實的語氣回報給使用者，全部來自另一個 reviewer 的訊息而未自己跑過；事後補跑全部相符，而相符是運氣不是查核。它與第四類的失效模式相反——來源真實且通常是對的，所以查了也對、沒有人會再查，危害不在數字錯而在把「別人驗過」記成「我驗過」，下游因此高估了查核次數。由此推出一個處置面的結論：**外部查核偵測不到第五類**（去驗那個數字它是對的，而通過本身沒有回答這是誰驗的），所以它只能靠回報者自己標記或流程強制附上自己那一次跑的輸出，追問對它無效。第二欄的定義同步補上第二層——在不在對話紀錄裡，以及在誰的對話紀錄裡。產生它的機制是語氣一鍋端，反向指標補在 #330。

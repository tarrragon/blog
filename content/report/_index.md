...
- [#339 壓縮標籤省下的字數是一次，展開成本乘以引用點數](compressed-label-cost-multiplies-with-citations/) — 替表格列名、欄位標籤或章節標題取短名，而那個名字會被別處逐字引用時讀；**省下的字數付一次，還原限定語的成本乘以讀者數與引用點數**，所以標籤越像術語、被引用得越廣，短名的收益越小；#261 把檢索鍵位的義務定為識別充足並讓精簡優先，本卡限定那條優先序——鍵一旦被別處逐字引用它就是錨點，而錨點有兩個性質讓天平倒過來（引用的形式是逐字比對、加字即斷開，所以引用點只交付短名本身；既有標籤要改時它與全部引用點必須同一次改，修復成本同樣乘以引用點數）；**判準是兩個問句**——這個標籤裡的每個名詞讀者知道是哪一個嗎、每個動詞讀者知道主詞是誰嗎，省略的限定語補回標籤本身而不補在鄰欄（讀者先讀標籤才決定要不要讀那一列）；**既有的檢查走不到它，因為壓縮標籤讀起來是完整的詞**——#108 查的是名詞頭被砍到只剩單字的殘片，而這一類名詞頭健在、被砍的是限定語，語法上沒有痕跡，一致性掃描也無效（周圍都是對的）；偵測的入口是引用點數（`rg -F -l` 命中兩個檔以上就是錨點），例外是表頭語境下已自足的通用欄位標籤與領域內一跳查得到的正式術語
...
- [#356 先備知識與環境陷阱跟在所屬的指令旁邊，用結構承接](supplementary-knowledge-follows-its-parent-concept/) — 操作指引裡的補充說明（這個參數為什麼值得用、特定平台上同一條指令會靜默失敗）散落在正文裡像講課時的即興補充時讀；**這兩類內容工程師確實需要，問題在它們出現的位置**——講到哪補到哪是講者正在操作時的自然行為，搬到文章上變成遊離的散文段落、跟所屬的指令隔了好幾行，讀者掃指令列表時跳過它們、需要時找不到、不需要時被打斷；歸位有兩條——**參數的理由寫進參數的說明裡**（獨立成段的散文在掃描式閱讀中被跳過，實例是 `-G 16M` 的表格只寫「放大緩衝區」而理由在下面一段遊離的散文）、**環境陷阱跟在它替代的那條路徑旁邊**（`timeout` 的靜默失敗跟的是時間窗的概念，因為讀者決定要用它的時間點在讀到時間窗的時候）；判準是**讀者在讀到哪個位置時會需要這段資訊**，答案指向的位置就是它該放的地方；是 #355 其中一個形態的後果與歸位原則（判別方式不同——那裡問刪掉之後還知不知道在講什麼、這裡問需要時找不找得到）、#297 家族的位置版（那裡管材料存不存在、這裡管材料的位置，材料在而位置錯時每一段單獨讀都通順）
- [#357 口語的語氣強調寫進文章後變成讀者的解碼成本](spoken-emphasis-is-silent-in-text/) — 一句話的受詞或量詞單位在這個領域裡不存在（「一個字都沒有執行」的字、「走到解析」的走到）時讀；**口語用語氣傳遞程度，寫下來之後語氣消失而強調結構留著**，讀者拿到的是一個多了修飾成分的句子、要先拆掉再還原成事實；判準是**受詞是不是動詞在這個領域裡能作用的對象**——「執行」在這個領域裡作用於指令，「一個字都沒有執行」的「字」在領域外（沒有任何系統逐字執行東西），「一個 X 都沒有 Y」在口語裡成立的前提是 X 是 Y 的計數單位而搬到「執行」上前提不成立，讀者要做四步還原（認出慣用語→字對回指令→一個都沒有對回零個→重組），而「遠端沒有執行任何指令」直接就是終點；量詞槽位另加一條——**範圍在句內具名**（「徵兆完全一樣」在全部什麼面向上相同？句內沒說，而下一節給了分辨方法，兩者矛盾）；**判定者不代填槽位**是這張表成不成立的關鍵，一次實測判定者替「完全一樣」補了單位然後放行，所以判準要寫成是非題「單位有沒有寫在句子裡」、寫成「推不推得出來」它永遠通過；這是生成端規格（寫操作句先填四個槽位再成句），取代先前後驗的「拿掉強調結構事實變不變」；審查抓不到是因為這種句式完全合語法而且讀起來更生動，而問題從來不在讀不讀得懂、在讀者要多花一步；是 #111 的句式版（那裡管口語詞彙、這裡管口語句式，grep 策略不同——詞彙有固定關鍵詞、句式的關鍵詞是語法結構而假陽性高）、#358 的同一套四槽位規格的另一格
...
- [#363 正文裡的對象要用它是什麼來命名，序數、計數與字母代號是作者腦中的索引](ordinal-labels-are-the-authors-index/) — 文章裡出現「三支」「七步」「第三步」「第一級」「前提一」「A1 與 C」這類名字、讀者要回頭翻才知道指什麼時讀；**名字的職責跟程式變數相同：只看到名字就知道它指什麼**——三種形態（計數當集合名、序數當成員名、字母代號當實體名）常出現在同一句，作者用它們推導時最嚴重（「第一支的篇落在七步的第三、四、六步」是兩份索引的對照表）；#155 管引用句、#156 管標題裡的成員數，正文裡的實體本身叫什麼兩張都沒管；判讀用變數名稱測試與落點測試，修法是用問題／動作／判準／描述命名、段標拿掉序號、標題與網址一起改名，並刪掉跟它一起出現的編排思路句
...
**Last Updated**: 2026-09-04 — 新增 #330，由併行 session 的一次 reviewer 回報抽出。那份報告完全符合當時的契約（零命中須明寫、不可省略），而三十四條零命中裡有十六條的零來自「判準只有二元事實、造不出候選情境」，證明的是判準與 frame 不匹配、不是判準通過了。因果由該 reviewer 自己指出並且比現象值錢：那三十四條原本是一鍋端的，它是在被要求交代採信依據時才不得不去分「我試過」與「我看過覺得不用試」——分類是被問句逼出來的，報告者本來沒有那個區分。本卡是 #243 的下一層（那裡問有沒有痕跡，這裡問痕跡本身分不分得出證據力），並與 #221 / #329 / #291 構成「零命中有幾種來源」的家族。反向指標補在 #243。

**Last Updated**: 2026-09-04 — 新增 #331，由併行 session 同一批審查裡的兩次撤回抽出（兩例同一批、同一個要求者與回報者，換人是否同樣有效在證據範圍外）。觸發條件由對方指認並且比現象值錢：那兩次撤回由追問的形態帶出來——追問寫成「請把 X 寫出來／算出來，我要能自己驗證」，要的是物件而不是關於物件的判斷，而回報者自己不會走到那一步。第二例最清楚，那個數字更早一輪就由同一個回報者量過，缺的只是把它代進判準走一遍。同批把 #321 的機制一般化：每一種檢查都有入口，零命中只證明從那個入口走不到，而路由的死名與孤兒是同一機制在另一個入口上的實例，處置是雙向各驗一次並分開寫；它因此成為「零命中有幾種來源」家族的一員。#330 的家族句同步改成不自帶序數（照 #325）。反向指標補在 #330 / #243 / #315 / #321。

**Last Updated**: 2026-09-04 — 新增 #332，由併行 session 的三個連續修法批次抽出（一個 skill、一位作者、七個缺陷；三批皆出現而未觀察到隨批次遞減，三批推不出趨勢所以卡上沒有比率宣稱——這一句是該批 reviewer 自己糾正過來的）。承重的欄位是抓出者的分佈而不是缺陷清單：作者自查抓到的兩個恰好都有 pattern 可比對，獨立審查抓到的五個恰好都沒有，分界因此是「有沒有 pattern」而不是「認不認真」。同批回填 #330 的第二批數據：三格上線後「沒查」那一格立刻兌現（一個 reviewer 用它標記了未連外查證的官方引用，而它的核心論證建立在那些值為真之上），而「不適用」那一格的佔比是 frame 相依的（七個 frame 裡只有一個約四成八，成因是那個 frame 的判準混進了二元事實），這個分佈把它從回報欄位升級成診斷指標——集中在單一 frame 是判準該搬家，各 frame 平均偏高才是整體錯配。反向指標補在 #247 / #298 / #329 / #330。
...
**Last Updated**: 2026-09-04 — 新增 #334，由本階段的四個實例歸納（三個自陳、一個由併行 session 提供，四個出自同一段連續工作，所以成立的是共用結構而非頻率）。它是本批多張卡的共同上層而處置方向與它們都不同：其餘各卡給的是檢查，這一張給的是設計——改成本結構，不改注意力。同批把 #221 的兩集合判準從推導升級為正反兩側各驗過一次（規避側兩集合相等、例外側不相等，而例外的命中包含引號內的反例引用這類本來就不會失敗的位置），並記下它比自律版本強的地方在於不需要判斷動機。#330 補上兩軸被混成一維列表的成因——它們的報告措辭相同，所以分類的動作要由欄位帶進來、不會由觀察自己長出來。反向指標補在 #331 / #333 / #329 / #317。
...
**Last Updated**: 2026-09-07 — 四處回填，由併行 session 的外部實測與自陳提供。**#321**：雙向都驗過之後還有一層——方向數不等於入口數。一次外移三節之後路由表的雙向檢查全綠（零死名、零孤兒），而同一批三處跨檔指涉是失效的：檔頭的同目錄索引與正文的跨檔指涉是另外兩套指標，那個檢查一條都沒掃。雙向全綠比單向零命中更容易讓人收工，分辨的方法是先數指標系統再數方向。**#324**：判別問句的第二種用法是分判準——某條判準由「這個目標日後會不會動」（量判斷者的預測）改成「這個目標現在有沒有名字」（量對象的現況），而改寫後在撰寫它的那批修法裡當場攔下四處自己的同型缺陷；一條判準對產生它的那批文字立刻生效，比事後被套用的次數更能說明它量到了東西。**#334** 補第二個機制：有一類代理由觀察者站的位置產生——代理與本體在那個位置上重合，取它與價差無關（本地狀態代理遠端狀態，兩次自陳實例都查了容易查的那一側而結論依賴沒查的那一側）；它與價差型的差別在連決定都沒有發生過，所以處置是改問句的形式而不是改成本結構。**#333** 補掃描問句：這句話裡的每一個數字與每一個完成式，是同一個時點的嗎——完成式與它旁邊的狀態值分屬兩個時點時才進判別。

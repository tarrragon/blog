...
- [#281 理解檢查看多個低階 model 的分歧，不看單一高階 model 的判斷](comprehension-checked-by-divergence-not-judgment/) — 要驗文章的脈絡與表達會不會被誤解時讀；**判準放在多份獨立理解之間的分歧上、不放在任何一個 reviewer 的判斷上**——五個讀者對同一個指涉詞給出三種答案時，不需要任何一份察覺「這裡有歧義」，分歧本身就是證據，這繞過了同源審查的結構上限；三種檢查的分工——grep 與 regex（判準是形態匹配，抓已知形態與大量掃描，抓不到語意）、單一高階 reviewer（判準是該模型的判斷，抓結構與事實與承重論點，而**自己的理解力會把作者留下的坑填平**）、多個低階 model 讀者（判準是份與份之間的分歧，抓歧義與指涉斷裂與施事者缺失）；**低階在這個用途上是優點不是妥協**——推理能力越強越容易讀到殘缺句子時自動補完，成本與速度也決定能不能大量同步派發，而判準需要多份才成立；五個設計條件缺一不可——指令逐字相同（差異才只來自模型）、不給審查框架（給了會退化成品質較差的 reviewer）、明令標記不確定且禁止推測填平、問理解不問評價（每段在講什麼 / 動作誰做的 / 指涉詞指什麼 / 讀完會做什麼）、判準是分布不是結論；判讀——全體一致誤讀必改（作者與高階 reviewer 都看不到）、過半收斂是候選、單份是個案但構成事實錯誤時仍查句子成分、零分歧只證明表達無歧義不證明內容對；量測：一篇剛通過三輪九個高階 reviewer 的稿件仍被 5/5 一致誤讀一處（把「沒有操作能單獨刪掉這一筆」壓平成「永久記錄」、而該節列了它會消失的途徑），另 2/5 收斂四處、1/5 三處，共十處修正全是補成分與具名指涉、無新增段落，而五份對「讀完知道怎麼寫嗎」答案一致且正確——主要交付未失敗，方法分得出主線與支線；是 #279 的偵測工具、#168 的判準來源替換（那裡靠 reviewer 自陳讀不懂）、#267 空白的補位（不依賴任何清單、命中了清單外的物理借詞「做功」）、#149 分工的另一端、#114 的新軸（模型層級）；限制是單次量測、單篇、單一模型家族、最佳 N 未測

**Last Updated**: 2026-08-25（三）— 新增 #281 理解檢查看多個低階 model 的分歧。由使用者提案並實測：先前好幾輪都用 grep 與 regex 做機械式檢查，那在大量評估時有效率，但要判讀的是文章脈絡與表達能力時，機械判讀取代不了模型；而低階模型更容易曝露缺點、成本與思考速度也適合大量同步派發。實測派五個 Haiku 用逐字相同的指令讀一篇剛通過三輪九個高階 reviewer 的稿件，產出十處修正、全是補句子成分與具名指涉。最強的一項是 5/5 一致誤讀（把「沒有操作能單獨刪掉這一筆」壓平成「永久記錄」），而九個高階 reviewer 無一人讀錯它——他們讀得懂，所以那個位置對他們不構成問題。卡的核心是判準位置的轉移：不看任何一份報告的結論、看份與份之間的分歧，因此不依賴 reviewer 有沒有能力察覺自己被卡住。五個設計條件（指令逐字相同 / 不給審查框架 / 明令標記不確定 / 問理解不問評價 / 判準是分布）缺一即退化。場景路徑 51 新增。
...
`#281 理解檢查看多個低階 model 的分歧` → `#279 句子成分缺失會被讀成脈絡不足` → `#168 多輪審查要有冷讀者 frame` → `#267 關鍵字清單只收違規義項佔多數的詞` — 派多個低階 model 用逐字相同的指令讀同一篇、不給審查框架只問「你讀到什麼」，用 #281 的判讀把分歧分級；收斂的位置用 #279 分流成分問題與脈絡問題；這一輪抓到的東西 #168 的冷讀 frame 與 #267 的清單都結構性看不到。

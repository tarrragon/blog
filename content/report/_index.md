...
- [#139 新增頂層 content 資料夾要同步首頁 _index.md 入口](top-level-content-folder-needs-homepage-entry/) — Hugo 不會 auto-list 頂層資料夾、首頁清單是 content/_index.md 的手動 markdown；新增 content/<module>/ 必須同 commit 加首頁入口、否則模組對首頁讀者完全不可發現；補進 AGENTS.md §6 完稿檢查清單成為結構性保證；是 #44 SSoT 在「首頁清單」維度 + #97 metadata surface 在「上層索引」維度的具體案例
...
- [#145 外部分析改寫的交付物是可遷移框架、不是風格轉換](analysis-rewrite-must-deliver-transferable-framework/) — AI 改寫外部分析文章時、任務目標不是把語氣改成本站風格，而是抽出讀者可帶到下一個事件的判讀框架；正文要交付訊號、機制、長期影響、預警與下一步路由；是 #141/#142 之後的 deliverable 層原則
...
**Last Updated**: 2026-05-20（content/business/ 建立後漏首頁入口 retro）— 新增 #139 新增頂層 content 資料夾要同步首頁 _index.md 入口：c2c01bf 建 content/business/ 50 檔但漏更新 content/_index.md 教學系列段、business 模組對首頁讀者隱形、f665e6d 才補；本卡把這個 pattern 從一次性事故抽象成原則、同步把「新建頂層資料夾要同步首頁入口」加進 AGENTS.md §6 完稿檢查清單；是 #44 SSoT + #97 metadata surface 在「上層索引」維度的子實例。

...
**Last Updated**: 2026-08-26（二）— 新增 #289，由兩次全站術語替換抽出：「判準」3007 處 / 813 檔、站得住 97 處 / 53 檔。兩個詞都不是靠讀出來的，偵測靠五份互不通訊的低階模型各自定義同一個詞——「判準」五份全部對不到工程領域的通用說法、兩份直接寫沒見過。**非通用不等於自創**：「判準」真實存在於科學哲學（「分界判準」）與教育評量（「評分判準」即 rubric），住址不在工程寫作，處置相同而檢討的歸因不同。替換按義項走（動作修飾語縮為「X 標準」、狀態義改為「X 條件」、其餘展開為判斷標準），單一詞掃全站會把「技術判準」壓成讀作規格的技術標準。站得住依主詞分流成成立 / 可靠 / 自足 / 適用，並進物理化錯配的 grep 清單；同一次量測抓出的站穩沒進——實際用法多數合規（攻擊鏈階段名、「」內引用、就地展開的類比），這是 #267 入選判斷擋下來的。Test Oracle 改用英文卡名並連回知識卡。反向指標補在 #234 / #267 / #281 / #286 / #149。
...
### 路徑 61：技術文章寫完，而讀者照抄跑得起來、換成自己的值就卡住

`#360 示範指令的引數要說明` → `#358 每一句寫清楚誰對什麼做了什麼事` → `#342 寫出特定實例的值就取代了讀者的查證` → `#355 教學定位被讀成教室講課` — 這一條的入口是程式碼區塊，而前三張卡的偵測動作各自不同。先用 #360 對每一條指令過一次「讀者要改哪幾個位置、每個位置文章說過沒有」，掃描對象是區塊裡的字面值而不是佔位符。散文那一側交給 #358 逐句填四個槽位。兩者都做完之後用 #342 反向再問一次：有沒有哪個值根本不該寫進來——#360 要求說明的值與 #342 要求刪掉的值在版面上同形，判別靠「讀者拿它去查自己的系統，還是直接照抄」。最後用 #355 檢查說明本身的體裁，指令旁邊要給的是正確讀法而不是事後提醒。
...
- [#312 判斷標準指對了路，而它給的理由在這個個案上不成立](criterion-routes-right-while-its-reason-fails/) — 路由與理由可以獨立失效；理由失效時個案實跑一路走到底並交出正確結果，代價延後到讀者把那句沒附條件的因果帶去下一個個案
...
- [#330 要求揭露只擋得住省略，要分層得改成要求分類](disclosure-yields-one-bucket-classification-yields-layers/) — 一份契約要求執行者揭露某個彙總狀態（零命中、已檢查、無發現、全部通過、不適用）時讀；**要求揭露擋掉的是省略、擋不住一鍋端，而一鍋端比省略難察覺因為它已經照契約寫了**——省略留下一個看得見的空白，一鍋端交出一份完整的報告；分類這個動作在報告者那一側原本不存在，所以那不是隱瞞，他從頭到尾只有一堆。實例是六十六條判斷標準回報三十四條零命中、格式完全合規，追問採信依據後才拆成造過情境判定通過（17，是證據）／判斷標準只有二元事實造不出情境（16，不適用本 frame、不可當證據）／沒查（0，未知），而**分類是被問句逼出來的**。判別問句是這個欄位要求的是說出來還是分好類；修法是契約措辭從揭露改成分類（成本一句話）、類別由契約給定而非開放題、且要窮盡到「沒查」那一格，少了它沒查的會被歸進最接近的一格而最接近的是判定通過

- [#331 要交出物件，不要交出關於物件的判斷](ask-for-the-object-not-the-verdict/) — 一份回報宣稱某件事不可能、超標或不成立時讀；**一個關於物件的斷言可以在抽象層完全合理，而它的證偽只能由構造那個物件完成**，而沒有人要求構造時回報者沒有理由去做——他的主張在他自己的推理裡自洽，構造是唯一能推翻它的動作；兩個實例：主張兩條判斷標準不可同時滿足（理由是一串會累加的要求），被要求「請把那個 description 寫出來給我看，我要能自己驗證」之後構造出 206 字元、低於 250 的門檻而當場垮掉；另一例主張某檔超標，被要求算 ASCII 佔比後得七成七、內插門檻通過，而**同一個回報者更早一輪自己量過那個佔比**——資訊一直在場，缺的是把它代進判斷標準的那一步；修法是追問的動詞用「寫出來 / 算出來 / 構造一個」而非「確認 / 說明 / 檢查」、加一句「我要能自己驗證」把產物的接收者換成追問者、已量過的數字明確要求代入並印出過程；成本落在追問者這一側（他要先知道那個宣稱需要什麼物件），所以這一條寫不成回報者的自律規則

- [#332 一次修法會對同一件事寫出兩種形式，而兩種都不命中舊 pattern](one-fix-writes-two-forms-of-the-same-thing/) — 一批修法完成、準備收尾時讀；**修法會在同一批之內對同一件事寫出兩種形式，而正確的那一種通常就在旁邊**（一個檔內嵌數字、另一個檔對同一件事寫具名路由；某節套了標註習慣、另外幾列沒套），缺的不是知識——寫的人在同一批裡已經用過正確的形式，缺的是回頭比對的動作；**類別掃描對它無鑑別力而理由可指認：兩種形式都是新寫的、都不命中任何舊 pattern**，掃描問的是「舊問題還在不在」而這一類問的是「我剛寫的東西彼此一致嗎」；承重的證據是抓出者的分佈而不是缺陷本身——七個缺陷裡作者自查抓到的兩個恰好都有 pattern 可比對、獨立審查抓到的五個恰好都沒有，分界就是這條線；檢查本身是一次線性通讀（範圍限本批改動、問題限形式一致性），與類別掃描的合併規則是 AND 並各自標明負責範圍，寫成取代會讓不量測任何東西的那一個成為主導閘門

- [#333 「我以為觀測到的」躲在「引用的」那一欄裡，因為它看起來有來源](fabricated-observation-hides-among-cited-facts/) — 要為一份報告的宣稱分配查核責任時讀；**兩欄的分類法（引用的／推導的）會把虛構的觀測歸進「引用的」，而那正是它躲過查核的方式**——查核者看到來源就轉去查來源，而那個來源並不存在；四欄才窮盡（引用的／我量測的／我推導的／我以為觀測到的），而**第二欄的定義就是分界線：在不在對話紀錄裡可以驗，「我記得看過」不能**，兩者在報告文字上完全同形；這個變體比標準形態難察覺，因為填的是一份從未被呼叫的工具的結果、沒有「呼叫與結果之間」那個可疑的時間窗，而三次一致讀起來像穩定、同份報告裡其他數字經查為真又替它背書；偵測是問一個**只有握著原始輸出才答得出來的後續問題**（直接質疑走不到那裡——「你確定嗎」得到的是同一個斷言再說一次，第二次與第一次在文字上無法區分），回報者為了回答就得回去看，而一回去看就發現它不存在，且對方不知道自己正在被驗所以不會產生防衛式的補強；**第五類（別人量的、我引用）的失效模式與第四類相反**——來源真實且通常是對的，查了也對所以沒有人會再查，危害落在責任鏈上而不在數字上——它把「別人驗過」記成「我驗過」，下游因此以為過了兩道查核而實際只過一道，產生它的機制是語氣一鍋端（同一份回報裡重跑過的與沒重跑的語氣完全相同），而**外部查核偵測不到它**，所以只能靠回報者自己標記或流程強制附上自己那一次跑的輸出

- [#334 代理比本體便宜，所以成本壓力最高的階段一定伸手拿代理](proxy-is-cheaper-than-the-thing-it-stands-for/) — 收尾階段反覆出現表面不同的失效、而「下次更仔細」沒有改善時讀；**檢查一件事的時候手邊通常有兩個東西——本體，與一個站在它前面的代理**（規則的 regex 對錯誤訊息的描述、自己跑一次對別人量過的數字、成品的全部對這一輪改動過的檔、不加豁免時會失敗的位置對自己寫的豁免條款），而代理比本體便宜、兩者的產物在報告上同形（都是一個零、一次通過、一句已檢查）；**收尾階段的成本壓力最高，所以在那個結構下伸手拿代理是理性的**——「下次更仔細一點」因此無效，問題落在那個時點的成本結構上，而不在注意力的總量；四個實例的差價都小到「就用代理吧」不像一個決定，於是事後回看找不到那個決定點；修法是改成本結構而不是改注意力——降低本體的取用成本（把 regex 印進錯誤訊息、把「自己跑一次」做成一行指令、給檢查器一個不帶豁免的旗標）、讓兩者的產物長得不一樣（五欄分類與零命中兩軸做的正是這件事）、在收尾清單上明列「這一步我用的是本體還是代理」

- [#335 審查的 frame 全部在問規則的效力，沒有一個問它的代價](review-frames-ask-efficacy-never-cost/) — 一輪審查結案、finding 數量可觀時讀；**frame 集合會朝同一個方向長**——效力側問「判斷標準漏了什麼、這一類還有哪些位置、涵蓋範圍夠不夠」，代價側問「判斷標準會把哪些合規的個案判成違規、成本由誰承擔、值不值得」；成因兩端同向：找到一處漏抓是產出（多一則 finding），找到一處誤報是否定產出（少一則，且要撤回別人寫下的），而規劃 frame 的人在找「還有什麼沒被檢查」、執行的人的價值由 finding 數體現；**零 finding 與零 frame 在報告上同形**，代價側交白卷讀起來像那一側沒問題；偵測是給每則 finding 標一個方向欄（說規則漏掉了什麼／說規則多抓了什麼），一欄全滿另一欄全空時缺的是 frame 而不是缺陷；修法是 frame 清單成對排，而「找誤報會減少產出」這個成本估計本身是錯的——實測兩次撤回之後的替代 finding 都比原本那一則強
- [#336 盤點出來的缺口，處置要看補上它需要哪一種資源](gap-remedy-depends-on-the-resource-it-needs/) — 盤點出流程還有哪些維度沒被覆蓋、準備決定要不要再跑一輪時讀；**缺口清單長得像一份待辦，而待辦的預設動作是把剛才那個活動再做一次**，同一個活動再做一次就補得上的維度這個預設是對的，要時間、要另一個執行者、要另一個組織才補得上的維度再做一次補不到任何一格，而報告上「跑了第五輪」與「補上了那個缺口」同形；判別是一句話——補上這一項我要付出的是什麼，答案是「下一輪」的照原流程走、答案是時間或另一個人的處置是建票綁觸發條件；實例是七軸盤點未動的 timing 與 instance 兩軸都不是第五輪買得到的，照「未動的軸就是下一輪的價值來源」排下去會跑出一輪在七軸上補不了任何一格的審查，產出只能來自把 frame 軸再細分——正是那張卡自己列為反模式的加輪數；**加輪是「補上缺口」的代理**（便宜、會產生 finding、報告上與實際補上缺口同形），所以修法在盤點表加一欄「這一格靠什麼補上」而不在執行者的自律；停止判定要寫成兩句話，否則「停在資源用完」與「停在證據齊備」在文字上同形
- [#337 審查的射程只涵蓋被審查的對象，工具與共用產物在射程外](review-scope-never-includes-the-instrument/) — 一輪多 frame 的審查全部通過、準備判定停止時讀；**每一個 frame 都是對著審查對象設計的，所以任何不在審查對象裡的項目沒有任何一個 frame 會碰到它**——規劃 frame 的人問的是「這份內容還有什麼沒被檢查」，那個問句的值域就是內容本身；三類項目結構上落在外面：被每一輪修改而不屬於任何一輪的共用產物（入口頁、待辦清單、索引）、審查所用的工具（方法論、規範檔、腳本與它們的版本）、審查自己的產物（finding 清單、覆蓋盤點表、停止判定）；實例是一次四輪、二十餘個 reviewer 的審查全程沒有一步會發現所用的方法論落後十一個 minor 版，而那些版本差裡有一版把某條停止訊號降級並縮小了 quorum 的分母，該次的停止判定正好建立在那一條上、用新版複核不成立；**這一類靠清單補、不靠加 frame 補**（新 frame 也要先有對象，而把工具放進對象清單這個動作沒有觸發點），所以起跑前列一次工具與版本、收尾前列一次被改過而不屬於任何一輪的產物
- [#338 廢除一項共用資產時，它的前提與它的效果都在自己看不到的那一側](shared-asset-removal-reasons-across-the-boundary/) — 刪掉一個被別的專案裝過的檢查、腳本或設定之前讀；**三個判斷都跨過了自己的邊界**——沒有人在用（前提）、刪掉之後別人就沒有了（效果）、這項資產在這裡沒有用（理由），三者都有本地證據支持而本地證據對它們一個都不夠；**前提**：`grep` 自己的 settings 得到零，與沒有人在用是同一個輸出，修法是把作用域寫進那句話（「本專案的 settings 未註冊」本地驗得到，而且讓下游知道這句沒涵蓋他們）；**效果**：同步工具的刪除旗標管的是推送方向，拉取一律保留本地獨有的檔，所以刪除的射程只到「還沒裝的人」，已經裝了的人會同時持有宣告刪除的版本紀錄與那個檔、兩份互相否證而沒有訊號；**理由**：「不適用」有兩種遠看相同而處置相反——目標集合為空該廢除、作用域寫死指向別處該改成可設定，判別是「把作用域改對之後這支檢查有沒有對象可查」
- [#339 壓縮標籤省下的字數是一次，展開成本乘以引用點數](compressed-label-cost-multiplies-with-citations/) — 替表格列名、欄位標籤或章節標題取短名，而那個名字會被別處逐字引用時讀；**省下的字數付一次，還原限定語的成本乘以讀者數與引用點數**，所以標籤越像術語、被引用得越廣，短名的收益越小；#261 把檢索鍵位的義務定為識別充足並讓精簡優先，本卡限定那條優先序——鍵一旦被別處逐字引用它就是錨點，而錨點有兩個性質讓天平倒過來（引用的形式是逐字比對、加字即斷開，所以引用點只交付短名本身；既有標籤要改時它與全部引用點必須同一次改，修復成本同樣乘以引用點數）；**判斷標準是兩個問句**——這個標籤裡的每個名詞讀者知道是哪一個嗎、每個動詞讀者知道主詞是誰嗎，省略的限定語補回標籤本身而不補在鄰欄（讀者先讀標籤才決定要不要讀那一列）；**既有的檢查走不到它，因為壓縮標籤讀起來是完整的詞**——#108 查的是名詞頭被砍到只剩單字的殘片，而這一類名詞頭健在、被砍的是限定語，語法上沒有痕跡，一致性掃描也無效（周圍都是對的）；偵測的入口是引用點數（`rg -F -l` 命中兩個檔以上就是錨點），例外是表頭語境下已自足的通用欄位標籤與領域內一跳查得到的正式術語
- [#340 版號擋不住並行覆蓋，因為兩個作者會從同一基底取到同一個號](version-number-cannot-detect-divergence/) — 多個作者各自把改動推上同一份共用資產、而推送工具以版號判斷新舊時讀；**版號由作者指派而不是由內容推導，所以它證明不了誰比誰新**——兩個作者從同一基底各自取「下一個號」會取到同一個號，版號比對在這種分岔上恆真通過，推送成功那一行照樣印出來而先推的那一份被整檔蓋掉；能偵測分岔的是基底（三方比對），版號只有一個維度而分岔有兩個。**一天之內三次、跨兩個專案、角色互換**：版號相同（比對恆真）、版號倒退（工具會警告而推送端帶了旁路旗標）、版號遞增但基底是舊的（看起來完全正常）——三種形態在版號這一維上分別是相等、變小、變大，而實質都是推送方沒有對方那一份；**旁路之所以變成常態，是因為旁路的理由每次都成立**（教學用的票號示例字面命中閘門而語意正當），真違規因此混在正當豁免裡不會被單獨看見，處置順序是先消掉正當豁免再收緊閘門；第三次的成因是拿有 CDN 快取的端點當權威狀態（讀得到、格式正確、與真實狀態的落差沒有訊號），而據此做的重寫蓋掉了對方已經寫好的修正——**與它想修的錯誤同型**；另記一種消不掉的豁免：汰除某個字面時，記錄那次汰除的文字是它最後一個合法住址；**機制不限於版號這個載體**——同批另一例是一個退出碼承載兩種語意（並行衝突／守衛擋下）而讀它的邏輯只認得前一種，由此得到一條可直接用的訊號：為條件 A 設計的重試迴圈一次都沒成功時，那個信號多半代表 B
- [#341 讀者定位擋住科普化的同一句話，供應了省略解釋的理由](audience-level-licenses-omission/) — 教材裡的基本名詞用縮寫帶過、術語只提而沒有補卡，而作者的理由是讀者已經是專業人士時讀；**讀者定位是能力宣稱（有能力理解研究方法、開發原則、邏輯推導），而它被當成知識宣稱在用（已經知道這個領域的名詞、原理與縮寫）**——同一句話對兩種讀法都通順，選哪一種由當下的方便決定，而方便固定指向第二種（省略比展開便宜）；**方向由讀者的需求決定、不由他的存量決定**，專業讀者要的是每個流程的原因成本代價與用詞精準，全部是更多的文字，所以定位按它描述的需求生效時推向展開；最一般的那一層是**一條寫來防止某種失誤的規則同時供應了它反向失誤所需的理由**，而兩種違規在稿件上長得相反、掃其中一種的那一輪不會順便抓到另一種；**省略在產物上沒有痕跡**（沒補的卡不存在、沒展開的原理不佔行數），以稿件為輸入的檢查全盲，偵測要靠會產生輸出的動作——術語分級逐個詞跑出一張表（缺卡的格子是空的而不是隱形的）與理解探針；判別問句是這一處省略省下的是讀者不需要的字、還是讀者要自己補回來的脈絡
- [#342 寫出特定實例的值，就取代了讀者本來要做的查證](written-value-replaces-the-readers-check/) — 文章要寫某個機型、環境或安裝的具體值與處理方式，而讀者本來應該在自己的系統上查它時讀；**寫出來的值會佔據讀者原本要自己查的那個位置**——他要的是「我這台是什麼」而文章給了「有一台是什麼」，兩者在版面上長得一樣，而特定實例的值正好是他那台會不一樣的那些；危害因此與正確性無關（值可以完全正確、來源完全可靠，而它從來就不是關於他的系統的）。**判別問句是讀者拿這一句去用他是去查自己的系統還是直接照抄**；「標註未驗證」對這一類不夠，差別在誰承擔預設——標註要讀者的自律才生效，不寫由結構保證，而照抄的人多半搜尋直達那一行、複製、離開，路徑上不經過 caveat；與「避免專案綁定敘事」失效方向相反（那條的讀者讀不懂、當場就知道，這條的讀者讀懂了而且相信了、永遠不會知道是文章害的）；附帶一條推論：**「需要實機才寫得了」多半是把「有了會更好」講成「需要」**，而主題拆成「問題的形狀與該去哪裡查」與「某台機器上的具體答案」兩半之後，前一半不必等硬體、後一半不該寫
- [#343 拆分產生的缺陷落在檔與檔的交界，每一份單獨讀都是對的](split-defects-land-between-the-files/) — 一份文件拆成多檔之後，讀者回報跳躍、割裂、或不知道還有哪幾份檔案，而逐檔審查連續多輪零 finding 時讀；**拆分會造出一個原本不存在的位置——檔與檔之間的交界**，而每一份檔案單獨讀都是對的，所以逐檔審查與逐檔探針對這一類恆真（一份拆成八檔的文件通過三輪高階 reviewer 的九個 frame 之後，三輪二十八份低階探針產出七項判決、九個 frame 一項都沒攔下）；六種形態各附一條可執行的檢查（互指矩陣有破口、層級混疊、定義與使用分屬兩檔、指路寫在檔頭、內嵌於散文的計數、入口檔沒有起手動作），其中**層級混疊只有重排才會發現**——子檔的「步驟六七八」就是全程的「步驟三四五」而沒有一處說明，兩套編號在各自的檔案裡都連續合理，讀者走完最後一步以為結束而實際上跨了三個層級，改成依步驟切之後交棒點自然浮現，所以它同時是切法訊號；**指路的位置有乾淨的單變項因果**——八份檔頭都有一句總括式指路而三份仍判「沒說去哪查」，只把一個詞改成首次用點就地帶指路（其餘一律不動）之後降為零份，結論是讀者在正文遇到那個詞時不會回頭查檔頭；偵測要換受測單位（整組檔案而非單一檔案），三道題分別對應位置感、交棒與缺料自覺，**入口檔另跑一批只餵入口檔**（缺起手動作只在讀者拿不到其餘檔案時現形）；外移一段之後有固有的擺盪（拿掉被讀成掏空、補回被讀成堆砌），殘餘要自足回答「為什麼這一步值得慢慢做」而自足的成本是一個具體實例、不是一串後果；是 #284 的跨檔形態（那裡管篇內割裂且自陳缺探針樣本）、#325 所在位置的一般化、#321 兩套指標系統的可執行檢查、#303 的正交軸（那裡問連結交付什麼、這裡問連結放在哪裡）
- [#344 修法要落在缺陷所在的那一層，而看起來做得最多的那個修法通常不在](fix-must-land-on-the-layer-of-the-defect/) — 審查指出一處歧義、修過之後同一處又被指出來時讀；**歧義出在句子的成分層時修法也要落在成分層**——在段落層補一句警告修不掉它，因為已經按錯誤方式讀了那個句子的讀者會用同一套主語指派去讀那句警告；判別問句與理解探針共用（把主詞受詞與指涉對象補回去之後這句還讀不懂嗎），而本卡把它從診斷推到驗收——修法之後再問一次，答案該從仍然讀不懂翻轉成補回去就懂；**選中無效那一個的理由是可見度**：段落層的修法產生新文字、意圖聲明與粗體標記，diff 明顯讀起來像做了一件事，成分層的修法改詞序與具名、字數可能不變、看起來像沒做什麼，而選擇發生在動手之前、那時唯一看得到的訊號就是可見度（實例：加粗體引言而原句一字未動，複驗三份全數仍命中，其中一份指出修法動到的三個裝置全在段落層）；**第二種失敗是改對了層而陳述的範圍寫得比事實大**——改成表格之後方向性誤讀歸零，新寫的格子卻與同一節往上八行的既有陳述相反，成因是格子要求全稱陳述而事實帶條件（#262 容器形狀先驗在改寫階段的形態），所以結構層修法完成後要把新格子與同節既有陳述並排、不是重讀新的那一處；複驗的範圍要跑到修法自己也通過（第一次翻轉量到的是舊缺陷消失、量不到修法有沒有帶進新缺陷）；是 #298 的正交軸（那裡管修法的範圍、這裡管修法的層）、#332 的縱向對應（那裡橫向比對同批、這裡縱向比對同節）
- [#345 修法沒有寫進檔案，而同一輪的無關改善替它簽了名](fix-never-landed-and-the-round-signed-off-for-it/) — 一項處置判定生效結案、兩輪之後同一個位置復發時讀；**修法有沒有寫進檔案，與那一輪的量測有沒有改善，是兩件獨立的事**，而紀錄說修了、量測說好了，兩者互相背書、中間缺的那一步（修法有沒有真的落在目標檔上）流程裡沒有一處會問到；兩個各自不罕見的成因湊在一起才成立——**變更紀錄照修改計畫寫而不照 diff 寫**（批次腳本對其中一條回報零筆命中而那個回報沒進到寫紀錄那一步）與**量測項跟處置項不是一對一**（該輪的零命中來自同批另一項新增的定義表、與宣稱的處置無關）；直接推論是**一批多項時改善只能歸給整批**，要歸給某一條、那一條得是該輪唯一的變項，所以紀錄的兩種寫法效力不同（「本輪套用了這幾項、量測翻轉」可以成立，「這一項讓命中歸零」是需要單變項的歸因宣稱）；**判定生效之後復發，第一件事是回查 diff 不是重新設計修法**——復發時最順的解釋是修法設計得不好，而那個解釋會讓人重新設計一個從未被試過的修法，且那個修法會被記成「試過而無效」、下一次遇到同型缺陷時正確的做法已經在清單上被劃掉；提交前的形式是把紀錄提到的目標檔與實際暫存的檔案清單逐條對照；是 #344 的前一步（先確認落地再判層對不對）、#333 在修法紀錄上的形態、#298 同屬「產物的形式提供了它沒有量到的保證」、#291 的鏡像（工具說了實話而沒有人在聽）
- [#346 門檻要在派發之前寫死，否則「不處置」會被事後合理化](thresholds-registered-before-dispatch/) — 跑多輪量測而每一輪都要決定哪些命中該修時讀；**事後訂的門檻會被當輪的觀感蓋過去**（「這次寫得更清楚了」「三份最終都理解正確」都是判定之後才浮現的理由，而它們指向的永遠是同一個方向）；實測有一項連續五輪命中而每一輪都判不處置、理由逐輪相同，**同一輪之內另一項卻用相反的理由被推翻了不處置**——**同一條判斷標準給出兩種結論比判斷標準訂得寬鬆更糟**，寬鬆會漏東西而它一致，不一致讓「不處置」變成隨時可以事後合理化的選項，定案是把「讀者最終有沒有理解」整個移出判斷標準；門檻要寫下三件事——**處置線**（連續兩輪達到命中比例即處置，且要明寫不看讀者最終有沒有理解，否則它會從判斷標準的位置滑回理由的位置）、**換手段的條件**（同一位置用同一手段改過兩次而命中沒動，第三次不再用那個手段；單變項序列是三份全中→壓縮→兩份→再壓縮→兩份→**外移→零份**，壓縮兩次無效而外移一次即解）、**比對的對象取跨輪累計不取單輪高峰**（切換條件的目標曾被訂成當輪最累的位置而那是暫時高峰，常駐源下一輪就回來，改用累計之後規則第一次給出「不觸發」這個單輪視角給不出的正確答案）；三條都要求跨輪的資訊而每輪結束時手上只有當輪的觀感，所以寫死的作用是讓判定的依據不隨當輪觀感移動、而不是讓判定更嚴；是 #281 的跨輪對應（那裡擋用統計門檻放掉 finding、這裡擋用當輪理由放掉）、#344 的出口（層對了而命中不動就該換手段）、#324 判別問句的帶時序實例、而換手段的前提由 #345 守（先回查 diff 確認前兩次確實落地）
- [#347 審查的產出是判定還是歸因要先宣告，因為歸因在累積式產物上沒有收斂點](verdict-and-attribution-are-different-products/) — 一輪審查的待辦越修越多、關閉速度追不上新增，而每一張票單獨看都是真缺陷時讀；**判定回答「這份產物現在讀得懂嗎」、歸因回答「這一句改動有沒有效」，兩者是兩種產物而不是同一件事的粗細兩級**——歸因需要單變數實驗，而單變數實驗的定義就是一次只准動一個變數，於是 N 次改動要 N 次全量驗證，「每新增一小段就要完整重新檢驗排查」是這個目標的直接推論而不是執行不當；**累積式產物上歸因成本由既有存量決定而每次改動的價值是常數**（實測題庫從第 6 批長到第 21 批、累積成 261 個題目區塊與 118 份答案鍵，單一工作階段新建 82 張票關閉 35 張），分母由一份只增不減的存量決定時多跑幾輪只會更差、不存在可以等的收斂點；**器材會凍結在第一批的形狀**——七項器材缺陷裡五項的根因同一句，工具在只有一個批次時「當前」等於「全部」，固定檔名、固定比對最新版本、單向自查全部成立，批次累積之後這個等式失效而工具照樣輸出數字；**選錯目標的成本不會表現成任何一個錯誤的決定**（守恆式對帳、正向對照、不變量排除混淆因子每一道單獨看都對、沒有一個多餘，而它們全部為了讓單變數歸因成立才存在，約一半工作量花在讓歸因成立而非改被審查的對象，其中雙臂派發做完之後由設計者自己否定前提——兩臂測的是兩個不同的量）；處置是派發前宣告產出層級、判定取整體通過失敗才二分定位、改動批次化、題庫改拋棄式只留出題規格、停損只對會使判定翻面的發現開票；判別問句是「這一輪結束要能說出它現在通過了，還是要能說出是那一句讓它通過的」；是 #345 的出口（那裡確立歸因要單變項、這裡問這一輪買不買它）、#346 的上游（那裡寫處置門檻、這裡寫產出層級）、#337 五項工具缺陷的共同成因、#334 反方向的同一要求（成本差確實存在而從未被表述成選擇）
- [#348 規範文字的效力由它在讀者程序裡的位置決定，逐字讀過也不保證綁定](rule-binds-only-where-it-sits-in-the-readers-procedure/) — 一條寫進規範的規則反覆不生效、而讀者確實讀過那一段時讀；**送達與綁定是兩件事而逐字引用只證明送達**——實測一款被六輪全文引用、讀完仍被判定對結論無作用，文字到了讀者手上也被抄進他的依據句，而他的判定在讀到它之前就收斂了，措辭層的修法對這一種無效因為措辭沒有問題；**讀者取用的單位是程序中的一個步驟與一個有編號的款次，段落不構成取用單位**（最乾淨的對照在同一段落內：一句排除條件寫在清單引言處六輪零引用，而僅相距兩句的另一句六輪全引）；另三個形態各自推翻一個直覺——處置側的涵蓋關係陳述十五輪零引用（讀者在前一步就收斂）、判定表外的診斷句不被當作取值來源（表內取到可填的值就停止）、排除條件邏輯上成立而排除點位於讀者做出選擇之後（排除的是一個已經不存在的分支）；**射程限制是搬進款次買到的是款次的引用率而非款內每一句的**（該款 6/6 被引而末句僅 1/6），所以承載處置的句子放款次開頭、舉例與邊界留尾巴；驗收靠引用率不靠讀起來清不清楚（位置問題在稿面上沒有徵兆）；同一條套到審查意見上是旁註形式的意見與零意見在產物上同形；把 #317 的三個成員擴成四個（沒被取用改觸發／值域不夠補值域／分類軸錯位換維度／取用了值域也夠仍不綁定改程序位置），分辨動作是查那一款在讀者產出裡出現過沒有；是 #243 那份痕跡的讀法（痕跡裡有規則字面而行為沒變是位置問題不是紀律問題）、#334 在讀規範上的形態（表格是代理、表外段落是本體）、#347 同一次工作的另一層
- [#349 量測的前置步驟決定了分母，而所有檢查都落在分母之後](measurement-preprocessing-decides-the-denominator/) — 一具檢查工具報出的每個數字都驗算正確、而據以得到的結論與手工查的結果不同時讀；**過濾條件決定什麼進得來、丟棄規則決定什麼被拿掉、抽取單位的定義決定一筆算幾筆、步驟順序決定什麼在中途被吸收，四者全部在「處理」發生之前完成**，所以檢查處理結果的動作一律碰不到它們，而報告特徵是每個數字都正確、錯的是那組輸入從來沒有人看過；四個實例各漏不同的東西——過濾條件漏掉不合它預期寫法的資料（以「餵入欄含某字串」納入，沿用上一題寫法的題目被靜默丟棄而分母跟著少）、**丟棄規則漏掉的東西有方向而方向可能正好指向被測量**（「兩題輸出分支是否相同」作污染判斷標準會讓被測的失分形態每輪自動落入不可判讀，漏得越乾淨讀數看起來越健康）、抽取單位的定義讓筆數建立在沒被宣告的切法上、**順序換一下結果就不同而工具在兩種順序下都沒做錯**（先極大化再取交集會讓短字串被長字串吸收，而讀者用的正是短字串，兩步對調多找出三條廣度九以上的通路）；**抽樣比對對這一類結構上無效**——被丟棄的東西不會出現在樣本裡，要換成守恆式（把全部原始單位分類到互斥格、加總必須等於總數）；修法是被排除的每一筆都寫進輸出附理由（關掉的是「靜默」本身、根治的那一項）、前置步驟連順序一起寫進規格、丟棄規則要與被測量正交；是 #337 在單一工具內部的遞迴（清單列得出用了哪具工具、列不出它的分母怎麼算）、#291 的鄰居而分辨方式是問正對照有沒有經過同一條過濾（前置步驟沒有壞、它照設計運作而設計本身縮小了分母，所以正對照會通過）、#329 是「納入條件」的高頻實例（以本次改動的新增字面作差集基準時來源未改過的命中恆為零）、#347 的下游（這幾類前置步驟大半為了讓歸因成立才存在）
- [#350 以讀者主觀狀態為要件的條款，在最需要它的人身上不觸發](subjective-precondition-never-fires-for-the-misreader/) — 規則寫了處理不確定情形的條款、而該處理從來沒有被執行過時讀；**「無法判定時」「有疑義時」「不確定時」這類要件要讀者先承認自己處於那個狀態，而誤讀的定義就是他不認為自己誤讀**——實測三輪的自陳欄都填「是否讀出兩種以上可能判定：否」而同輪判定值全錯，條款存在、措辭清楚，而它保護的正好是走不到它的那些人；處置是換成外部可查的條件（去查文件就有答案的那種，實例是「該歸屬有沒有清單字面或界定句的直接支持」），並加一句「判定者自認顯然成立，不改變本款結果」擋回流——少了這一句讀者會用「我覺得這裡很明顯」把外部條件繞回主觀狀態；同族第二個形態是**極性詞由讀者自行附加**（某款以「成立／不成立」命名判定點而有讀者把極性讀反並反向再現，第一版處置「改成直述句」驗不過——讀者仍在直述句上自行加回極性，定案是整段廢除並明文禁用，差別在換寫法留下概念而讀者會用順手的標籤指稱它）；第三個形態是**雙重否定會連帶擴散**（單處讀起來還可解、三處並存時讀者在其中一處取到相反極性並帶到其他款次，所以要全庫一次清完，否則廢除一款的極性詞而別處還留著仍取得到舊極性）；三者共用「讀者會把規則沒給定的東西自己補上，而措辭決定他有沒有那個空間」，推論是**自陳欄的價值在它與比對欄不一致的時候**；是 #348 的正交軸（那裡問排第幾、這裡問前提成不成立，兩者都產生零效力而位置問題前移可解、要件問題前移後照樣不觸發，分辨順序是先查引用）、#243 更難察覺的版本（這裡的執行者沒有選最省力分支、他根本不知道有分支）、#281「自評欄與比對欄分開讀」最尖銳的實例
- [#351 條文可以有負效力，而禁止某條捷徑的那句話自己就是捷徑的地圖](rule-text-can-have-negative-effect/) — 一款條文量到零引用、準備刪掉或搬位置時讀；**條文的效力有正負號而零引用讀起來像中性**——「題材沒觸及」與「寫得不夠清楚」兩個最順的結論都預設它被觸及會有幫助，而第三種可能是被觸及會更差（實測某款有一支會把原本答對的讀者推向答錯：否決了誤指派但沒自陳依據假設的那一類讀者，讀到該支就從否決轉成接受），所以**評估零引用之前先問它被引用會發生什麼**，因為搬位置與刪掉是相反的動作而零引用對兩者同等相容；**最隱形的一種是結構說明本身就是旁路**——一次改文寫入「本款結論只有三種、互斥且窮盡、不得由排除其中一種逕行認定另一種」，同一輪的推導即證明它零效力（兩名判定者走的正是它禁止的路），而刪除它最強的理由是那句話**自己交代了排除法所需的全部前提**（有幾格、互斥、窮盡），禁止語與結構說明綁在同一句所以那一句同時禁止並提供，而讀者取後半更便宜，刪除後三輪皆以正面觸發條件命中、可讀性未降；**禁止語要求讀者得到答案後回頭否決自己、正面觸發條件讓他一開始就走另一條路**，要防排除法靠的是每一格各有正面觸發條件；比 #243「補痕跡別補語氣」強一階——語氣詞是零、結構說明是負的，那張的三個省力路徑問題在此多第四個（這條規則有沒有交代那條路徑需要的前提）；與 #348 要照順序用（先問被引用會發生什麼、答案有幫助才進位置判斷，顛倒會把負效力通路搬到讀者一定走到的地方）、#334 的處置形態（改成本結構不是加要求）
- [#352 表單缺一格，填表人就得把不合格式的事實改寫成合格式的](field-shape-induces-false-reporting/) — 設計自檢表、審查報告或盤點表的欄位時讀；**一組欄位決定填表人能寫下哪些事實，所以欄位不足時的失真是結構強制的、不是填表人不誠實**——設計者照「會發生什麼」列欄位，而那個問題列出來的集合會漏掉「什麼都沒發生」（最常見的結果，也最不像一個結果）；判別問句是**這一格填不下的那個事實，填表人會怎麼處理它**，兩條路都失真且方向相反（改寫成填得下的形狀讓表上多一筆假的、跳過那一列讓表上少一筆真的，所以彙總時抵銷不了也看不出來）；實測自檢表的結果欄只容得下「發現問題 → 修正」，執行者因此把「初稿即為現況」寫成「發現後已修正」——每個字都對得上某個真實，唯一假的是它隱含一次不存在的修正動作，而那正是欄位要求他提供的，**修法是補值而不是補說明**（填表的人選的是清單裡有的那一個），下游代價是讀者把它算進「這一輪修了幾處」而那個數字是後續判斷的分母；第二個形態是**結論沒標明它依不依賴涵蓋完整性**（正面命中不受涵蓋數修正影響、窮舉宣稱要跟著動，混在一起則涵蓋數被修正時只剩整份重驗或整份照用兩個極端，一次實測有分類後代價是重驗兩項而非七項）；共同機制是**欄位集合是一個關於「會有哪些結果」的宣稱而它沒被當成宣稱檢查**，驗收是逐欄列舉可能的值、問有沒有真實情形落在全部值之外；是 #333 的正交軸（那裡分宣稱從哪裡來、本卡分它依賴什麼才成立）、#243 的配套（那裡要痕跡、本卡要痕跡的值域涵蓋所有真實結果，否則產生一份合格式的假紀錄，「三級量表全部填中」同形）、#262 在欄位上的形態
- [#353 從原則卡壓出來的操作形要獨立驗一次，而本體正確正是它不被檢查的原因](derived-operational-form-needs-its-own-check/) — 把原則卡壓成 checklist、規範段落或 skill 條目之後，判斷壓出來的那一份要不要獨立量測時讀；**一張卡壓成可執行形式時掉的是限定語、定義與先行詞，而本體讀得通不構成衍生形讀得通的證據**——更尖銳的一層是**本體正確正是衍生形不被檢查的原因**（壓縮的人剛寫完那張卡、審查的人也讀過它，兩者讀衍生形時都自動把卡上的內容補回去，補完發生在讀者側而且無聲）；三種掉落各有實測——**極性疊加**（本體「不要在條文裡宣告某組結論的個數、互斥性或窮盡性」單層清楚，壓成 checklist 變成「禁止某條推理捷徑的句子沒有交代那條捷徑需要的前提」而 `禁止`+`沒有`+`交代` 疊一句，**三份讀者全數讀成「必須宣告那三項」**、照它執行方向相反；同份 checklist 前兩項也以「沒有」起手而三份全對，所以失效的是極性層數不是否定句）、**定義掉落**（四個承重術語在該節內一個都沒定義，一份明報「無法確定判斷通過或不通過的標準，因為定義沒寫」、另一份把術語擴張到不該命中的詞上——**判不出通過條件的 checklist 項等於不存在**）、**先行詞掉落**（本體「把它算進『這一輪修了幾處』，而那個數字…」壓縮後前半句不見，三份各給一個不同的模糊指涉）；既有檢查全部放過它的三個理由是機械掃描查詞表而這三種掉落都不是一個詞（實測字句層逐類掃描零命中）、衍生形在稿件清單上的身分是引用而非主張所以逐項審查跳過、以及**兩批探針只餵衍生形不餵卡才抓到**（餵了卡的讀者會答對而那份答對測不到東西）；操作是受測對象取衍生形、壓縮後三問（這一句有幾層極性／承重術語在這一節定義過嗎／指涉詞的先行詞在這一節出現過嗎，三問都不准往卡裡找）、連結不算定義、改完用逐字相同的指令複驗；是 #339 的正交軸（那裡判斷標準是引用點數、這裡是會不會被執行，所以引用一次也要自足）、#303 驗收在 checklist 上的形態（門檻由「剩得下論證」升到「剩得下通過條件」）、#281 餵入範圍要選對層、而 #348 / #350 / #351 是它的來源也是它的對象——**一張卡的操作形違反那張卡自己**是這一類最常見的形狀
- [#354 一道題通過不代表它驗到了那一款：把受測的條文遮起來，正解仍推得出來就是零鑑別力](masked-derivation-tests-necessity/) — 一組題目在某份條文上拿到高分或滿分、而要確認那個分數是不是這份條文換來的時候讀；**通過率驗的是充分性（有這一款就答得出來），必要性要另外驗（沒有這一款還答不答得出來）**，兩者的差就是這一款在這道題上的鑑別力而通過率對它完全沉默；做法是遮起受測的那一款再看正解推不推得出來，推得出來就代表這道題一直在量別的東西（題幹自帶的線索、讀者的前置知識、或另一款的內容），那一款可以整段刪掉而分數不變；**滿分的報告上看不出來**——計分只分答對與答錯，而「答對且用得到那一款」與「答對但不需要那一款」的分數相同（實測五題計分全對而四題零鑑別力），所以偵測時機是反的：**分數好的時候最不會有人去驗，而分數好正是這個缺口最可能存在的狀態**，觸發訊號是全對而依據句裡沒有出現那一款；與正對照的分界是那個問工具會不會動、本卡問題目量不量得到東西，兩者都可以全綠而量測無效，順序上正對照在前而它的綠燈不構成跳過本卡的理由；操作是遮蔽範圍事先標定（事後劃的人已知哪些字被用到，會劃出剛好保留線索的邊界）、寫進獨立檔案由第二人複核、執行者第一階段不得讀到被遮的字而**他的輸入邊界是他讀得到什麼不是交給他的文字寫了什麼**（交付一個路徑就交付了整棵目錄樹，一次列目錄即穿透）；處置分兩種——保留這道題就改題、保留這一款就換題，**兩種都不是改那一款的措辭，因為這道題從來沒有在量它**；是 #291 的鄰居（那裡守工具、這裡守題目，守不同的門）、#348 的第三個量（引用率／套用率／鑑別力三者互不蘊涵）、#349 的正交軸（那裡問量了哪些單位、這裡問題目量的是哪一個東西）、#353 的下一問（選對層之後還要問這道題對那一層有沒有鑑別力）；**證據全部來自併行專案、本站尚未套用**，限制與射程逐項記在卡上
- [#355 教學定位被讀成教室講課，產出的是經營注意力的障礙而非有序的說明](teaching-is-not-lecturing/) — 技術文章或 work-log 的開場寫成情境故事、正文省略主詞受詞、或出現模擬互動的語句時讀；**「教學」在生成端被讀成「像教室講課」，而教室的預設是講者在場、學生被排課來、注意力需要經營**，搬到文章上之後經營注意力的手段全部變成資訊障礙——教學是定位（讀者要理解一件事）不是體裁；五個形態各自獨立出現而共享同一個根因：**情境故事開場**（讀者由搜尋或路由帶來、找到這篇的前提就是他已經知道自己在排查什麼，替他建構情境做的是他不需要做的事，而它佔掉第一眼的位置）、**省略主詞與受詞**（教室裡師生共享當下的脈絡所以主詞可省，而讀者從任意位置落進來沒有那張地圖）、**模擬教室互動**（「該問誰」「這裡容易讀反」「前面提到的四種」交付的是問題與警告而不是正確讀法，某個輸出容易被讀反時正確的處理是一開始就引導讀者看對的欄位）、**判定標準包裝成自問自答**（讀者要先拆成條件再代入個案，直接給條件映射省掉前兩步）、**指標與通過標準包裝成「問得出／問不出」**（改法是寫出指標名稱與通過標準）；判別問句是**刪掉這一段之後讀者還知不知道在講什麼**（標題與 description 已經告訴他了）；成因是 LLM 訓練資料裡「教學」最高頻的範例正是教室場景（課程講稿、教科書導入段、教學影片旁白），而規範沒有一條說「教學定位不等於教室形式」；是 #254 的鄰居（那裡是刻意扣住資訊的手法、這裡是不自覺帶進來的姿態）、#191 的同根（宣導與省略是教室師生關係的兩端）、#302 在教材內部的形態、#341 的另一個省略理由（那裡是「他懂」、這裡是「他在我面前」）
- [#356 先備知識與環境陷阱跟在所屬的指令旁邊，用結構承接](supplementary-knowledge-follows-its-parent-concept/) — 操作指引裡的補充說明（這個參數為什麼值得用、特定平台上同一條指令會靜默失敗）散落在正文裡像講課時的即興補充時讀；**這兩類內容工程師確實需要，問題在它們出現的位置**——講到哪補到哪是講者正在操作時的自然行為，搬到文章上變成遊離的散文段落、跟所屬的指令隔了好幾行，讀者掃指令列表時跳過它們、需要時找不到、不需要時被打斷；歸位有兩條——**參數的理由寫進參數的說明裡**（獨立成段的散文在掃描式閱讀中被跳過，實例是 `-G 16M` 的表格只寫「放大緩衝區」而理由在下面一段遊離的散文）、**環境陷阱跟在它替代的那條路徑旁邊**（`timeout` 的靜默失敗跟的是時間窗的概念，因為讀者決定要用它的時間點在讀到時間窗的時候）；判斷標準是**讀者在讀到哪個位置時會需要這段資訊**，答案指向的位置就是它該放的地方；是 #355 其中一個形態的後果與歸位原則（判別方式不同——那裡問刪掉之後還知不知道在講什麼、這裡問需要時找不找得到）、#297 家族的位置版（那裡管材料存不存在、這裡管材料的位置，材料在而位置錯時每一段單獨讀都通順）
- [#357 口語的語氣強調寫進文章後變成讀者的解碼成本](spoken-emphasis-is-silent-in-text/) — 一句話的受詞或量詞單位在這個領域裡不存在（「一個字都沒有執行」的字、「走到解析」的走到）時讀；**口語用語氣傳遞程度，寫下來之後語氣消失而強調結構留著**，讀者拿到的是一個多了修飾成分的句子、要先拆掉再還原成事實；判斷標準是**受詞是不是動詞在這個領域裡能作用的對象**——「執行」在這個領域裡作用於指令，「一個字都沒有執行」的「字」在領域外（沒有任何系統逐字執行東西），「一個 X 都沒有 Y」在口語裡成立的前提是 X 是 Y 的計數單位而搬到「執行」上前提不成立，讀者要做四步還原（認出慣用語→字對回指令→一個都沒有對回零個→重組），而「遠端沒有執行任何指令」直接就是終點；量詞槽位另加一條——**範圍在句內具名**（「徵兆完全一樣」在全部什麼面向上相同？句內沒說，而下一節給了分辨方法，兩者矛盾）；**判定者不代填槽位**是這張表成不成立的關鍵，一次實測判定者替「完全一樣」補了單位然後放行，所以判斷標準要寫成是非題「單位有沒有寫在句子裡」、寫成「推不推得出來」它永遠通過；這是生成端規格（寫操作句先填四個槽位再成句），取代先前後驗的「拿掉強調結構事實變不變」；審查抓不到是因為這種句式完全合語法而且讀起來更生動，而問題從來不在讀不讀得懂、在讀者要多花一步；是 #111 的句式版（那裡管口語詞彙、這裡管口語句式，grep 策略不同——詞彙有固定關鍵詞、句式的關鍵詞是語法結構而假陽性高）、#358 的同一套四槽位規格的另一格
- [#358 技術文章的每一句寫清楚誰對什麼做了什麼事](command-fragment-as-subject-hides-the-actor/) — 一段技術說明讀完之後讀者說不出是哪一層在做事、對什麼資料做、做完變成什麼時讀；**每一句要讓讀者知道四件事裡的至少兩件**——誰在做（操作者、工具、系統元件）、對什麼做、做了什麼、結果是什麼；三個形態——**指令片段當主詞**（「`tr -d '\r'` 處理的是行尾」，寫腳本的人與被處理的輸出都消失了；這在指令手冊裡合理因為讀者正在看那個工具的文件，搬到跨層的技術文章就讓讀者搞不清楚是哪一層在做事）、**系統元件省略層級**（「核心會對經過的位元組做終端機處理」一句跳三層而每一層都沒指名）、**「這裡」替代條件描述**（「字串比對在這裡會失敗」——「這裡」不是領域裡的位置）；第四件連帶的是**段落順序由工具視角驅動而非讀者的問題序**（讀者的順序是看到什麼→這東西做什麼→為什麼需要它→不做的具體後果→副作用，而主詞換對之後順序通常自動修正，因為操作者視角的自然順序就是讀者的問題序）；**grep 抓不到這一類**（缺席的主詞沒有關鍵詞），實測同一篇經三輪 17 個 reviewer、理解探針與字句層 17 類全部通過，偵測靠冷讀或探針的「這個動作誰做的」那一欄；與 #355 形態二的差別在偵測難度（那裡主詞完全省略、語法不完整偶爾會被抓到，這裡語法完整所以審查者讀到時覺得沒問題）；射程到句子為止，程式碼區塊裡的引數另有一條檢查（#360）
- [#359 嚴重性評估先固定評估單位，單一實例與跨集合的模式是兩個對象](evaluation-unit-before-severity/) — 兩個評估者對同一個發現給出差一個量級的嚴重性、或逐篇審查找到的全是「一處、輕微」而懷疑背後有跨篇模式時讀；**同一句話當成一個句子評是一處校對問題、當成一種寫法評是規範問題，兩個評級都對而對的是不同的對象**，評估雙方沒有先說定對象就會把對象的差異讀成判斷的差異；**逐篇審查看不見模式是結構性的**——它的單位是一篇，每篇各自回報「一處、輕微」各自結案，組裝模式的那一步不在流程裡，看得到模式的人是讀過整個集合的人而審查者每次只拿到一篇；所以嚴重性在兩件事之後才給（評估單位固定了、普及度量過了——可 grep 的算命中數、不可的抽三到五篇冷讀），在那之前一個發現只能標「候選」；三個反模式——**從單一實例的修復成本推嚴重性**（改一句話成本趨近零，而量的是作者側、讀者側是每篇乘每處乘每個讀者）、**新發現直接歸進最接近的既有類別並繼承它的嚴重性**（歸類的速度本身是訊號，量測需要時間而借用不需要）、**評估者用自己的閱讀自動補完後回報「讀得懂」**；#355 / #357 / #358 是它的 case 來源（三張都先被評成既有類別的輕微變體，拉出來量普及度之後各是二十到三十篇的模式）；它替 WRAP 的錨點確認補第三個錨點，位置在 Step 0 之前——選項的意義取決於對象是實例還是模式，對象沒定就擴增選項，擴的是錯的問題
- [#360 示範指令的引數要說明，寫成字面值的那些最不像需要說明](literal-operand-hides-the-change-point/) — 文章的指令讀者照抄跑得起來、換成自己的路徑或套件時卻不知道要改哪裡、也看不出兩種寫法差在哪時讀；**示範指令有四個位置（工具／子命令／選項／引數）而引數最需要寫**——工具與子命令的名字讀者查得到、選項多半有 `--help` 列得出來，而引數的語意常常只有文章講得出來（路徑為什麼選在這裡、兩個引數哪個是來源、給不給這個引數會讓指令做不同的事），同時它是讀者唯一要動手改的位置；判斷標準一句話：**讀者要把這條指令用在自己的機器上，他得改哪幾個位置；每一個要改的位置，文章有沒有說過它填的是什麼**；**掃描入口是區塊裡的字面值而不是佔位符**——`<serial>` 自帶「這裡要換」的訊號，`/sdcard/screen.png` 長得像一個完整可用的答案，於是同時逃過作者的說明義務與讀者的警覺，稿件上因此看不出缺口；五種形態裡最需要結構修法的是**引數的有無改變行為**（兩種形態各給一條指令、並列在同一個程式碼區塊裡各帶一行註解，**並列是修法本身、文字說明是它的補充而不是替代**——只給一條而另一條用散文描述時，讀者要從那段文字重建出另一條指令，而他來那一節就是為了拿指令）；句子層的規則掃不到它，因為判定單位是句子而指令沒有主詞也沒有謂語；是 #358 的射程互補（那裡管散文、這裡管程式碼區塊，兩者的盲區成因相同——審查者自帶答案所以讀起來完整）、#342 的反向配對（那裡要求刪掉不該寫的值、這裡要求說明該寫的值，兩者在版面上同形，判別靠「讀者拿它去查自己的系統還是直接照抄」）、#355 第三條規則在引數這個位置的具體形態
- [#361 並排多種做法時，比較的軸要落在讀者接下來要交出的那份產出上](comparison-axis-comes-from-the-readers-decision/) — 教材並排了幾種做法、每一欄都正確而且量測過，讀者讀完仍不知道自己該怎麼辦時讀；**軸的來源有兩個**——那幾種做法彼此的差異（回的筆數、花的時間、取得到的欄位），或讀者讀完要交出的那份產出（他正在設計的資料結構、介面、型別）；取自前者的每一欄都正確、都可實測、主詞都寫清楚了，而讀者要動手改的那份產出不在任何一欄裡；**把軸寫成「這個做法假設了什麼」不算換到讀者那一側**——那是從做法的外觀移到做法的前提，主詞還是做法；換過來的問法是「我要設計這個東西，某一項要不要唯一、允不允許缺」，而那**一個**決定同時決定幾種做法各自成不成立；既有檢查全數放行——軸的數量與種類數對得上（#319 的先兆不觸發）、名字與機制也都對（不是 #257 那一種），錯在整組軸選在另一個平面上；判讀是逐欄唸標題問主詞是誰，或問**讀者讀完這一欄會去改什麼**（改這段文字的降為格內內容、改他正在設計的那份產出的留作欄並排在前面）；觸發條件不是「這篇有幾種做法」而是**讀者真正要交出的是哪一份產出**——以練習為目的的讀者交出一段程式、以工作為目的的讀者交出一份設計，同一批材料對兩種讀者要排成不同的軸；修法是先寫下那份產出再往回列軸、原本的軸降成格內材料、而**跨篇的零件要在某一處真的並排**（互相連結不產生並排，讀者一次仍然只看得到一種，而取捨只有在並排的時候才存在）；**立卡後由同批新寫的一篇當場驗證**：那一篇把三種做法並排了（修掉第三種形態）而四條軸的主詞仍然全是做法（第一種形態沒修）——並排是看得見的動作，換平面不是；是 #319 / #257 的第三種軸失效（少了、名字偏了、選在別的平面上）、#214 的上游（先有軸才有排序）、#209 的對象問題（判斷力施加在哪一個東西上）、#262 的就地展開而非換成連結
- [#362 規範指定了某個位置要交付什麼，全批長成同一個模板時沒有任何一道檢查管得到](mandated-section-hides-its-own-sameness/) — 一批文章每一篇的同一個位置都有同樣的結構，而那個位置是規範要求的時讀；**規範說的是那裡要交付什麼、不說那裡要長什麼樣**，於是生成端替它補一個形態並整批補同一個，補完之後每一篇都合規、都交付了規範要的內容，而讀者連讀十幾篇時拿到的是同一段話重複十幾次——**同骨化與合規在這個位置上無法區分**；實測一個 21 篇的分類全數以同一個段名收尾且句型逐篇相同（佔全文 7–16%、是每一篇最後留下的印象），而該分類跑過多輪審查、cadence frame 也跑了：它掃到了，而 reviewer 的下一個動作是回去對照規範、發現這是被要求的，**「這個重複是規範要求的」於是成為豁免理由**，判定停在那裡；三件事讓它更難現形——逐篇讀不顯形（那個段落每篇只出現一次，單篇冷讀與單篇合規檢查全過）、內容確實不同（各連到不同目的地、逐字比對不會相同，相同的是骨架）、沒有任何一條規則說「形態不可以整批相同」；同族的第二種形態換一個位置——**規則自己的作用單位沒寫明**時它會在規則沒打算管的那個單位上也生效（「每段首句先說核心原則」的作用單位是段落，而文章的第一段也是段落，於是同一批 21 篇的首句全數是結論句，即使另一條規範明文擋掉未經推導的開頭結論）；偵測要普查段名（指令附 `LC_ALL=C`，少了它在部分語系下不同的中文段名會被 sort 與 uniq 併成一組）再抽出可疑段落看句型；處置是審查側把豁免改成**加強掃描的訊號**、規範側在指定交付時寫明不規定形態並給兩種以上範例、規則側寫出作用單位，而**不是刪掉那個段落**；**偵測程序有兩個前置步驟，少一個就漏**——作用域要涵蓋整批而非只掃新增的部分，近似段名要先正規化再計數（實測：新寫的一篇改了三個字的段名而句型逐字同構，普查因此報成 21 比 1）；是 #308 的一般形態（那裡是 description 這個位置、歸因同樣是「因為它照規範寫」）、#302 的鄰居（那裡規則漏掉適用對象、這裡漏掉作用單位）、#254 為什麼寫對了仍守不住那個位置、#221 的特殊版（frame 有跑、規則也有，而判定被一個看起來正當的豁免收掉）、#126 的處置方向（修法不在加輪數也不在加軸，在改那一軸的豁免規則）
- [#363 正文裡的對象要用它是什麼來命名，序數、計數與字母代號是作者腦中的索引](ordinal-labels-are-the-authors-index/) — 文章裡出現「三支」「七步」「第三步」「第一級」「前提一」「A1 與 C」這類名字、讀者要回頭翻才知道指什麼時讀；**名字的職責跟程式變數相同：只看到名字就知道它指什麼**——三種形態（計數當集合名、序數當成員名、字母代號當實體名）常出現在同一句，作者用它們推導時最嚴重（「第一支的篇落在七步的第三、四、六步」是兩份索引的對照表）；#155 管引用句、#156 管標題裡的成員數，正文裡的實體本身叫什麼兩張都沒管；判讀用變數名稱測試與落點測試，修法是用問題／動作／判斷標準／描述命名、段標拿掉序號、標題與網址一起改名，並刪掉跟它一起出現的編排思路句
- [#364 摘要另一份內容時寫出成員，計數只告訴讀者清單有多長](counted-summary-delivers-only-the-length/) — 介紹另一篇、寫 description、填目錄頁交付欄時寫出「四個追問、三種寫法、四類邊界」、讀者仍要點進去才知道那些是什麼時讀；**計數交付清單的長度，摘要要交付清單的內容**——寫的人手上有那份內容所以數字對他是完整索引，讀的人手上沒有；bank 的集合指涉規則管回指已讀的集合（其他三篇），碰不到預告未讀內容的「數字＋類別名」；grep 分不出合規（成員緊鄰數字）與違規，判讀問「沒讀過那一篇的人知不知道成員是什麼」；修法是寫出成員、數字通常可拿掉，成員太多時寫共同的性質
- [#365 逐段審查量讀者學不學得會，指稱解不解得開要問沒讀過前文的讀者](reviewer-holds-the-map-readers-land-without-it/) — 教材走過逐段審查仍殘留隔好幾段的「上面那一組」「兩者」、計數對不上成員、解法只對一部分成員成立時讀；**兩個獨立成因**：量尺量的是「拿不拿得到」而「寧可少報」把中低項過濾掉，以及審查者讀過全文、用作者的地圖解開每個指稱；修法是嚴重度按學習影響分必改／建議改並刪掉少報門檻，指稱另派逐段落地的低階讀者探針，回報經對照全文的篩選、替換片段腳本驗唯一後套用
- [#366 名詞要求的「誰的、什麼的」也是槽位，論斷句與段標的省略藏在名詞裡](noun-without-its-owner-passes-the-slot-check/) — 句子主詞謂語都在、讀起來通順，而讀者說不出「誰的輸出」「什麼的來源」，或段標與總結句用了只寫在目錄頁的框架詞時讀；**四槽位規則的範圍寫成操作句，論斷句、段標、總結句、過渡句從不觸發**，缺口在本身要求補足語的名詞裡（來源、輸出、代價、差別、位置）；判斷問句是所屬能不能從這一句或前一句逐字指出來；二十句校準裡關係名詞清單與「列出缺項」探針都沒有鑑別力，「逐字引出所屬」的探針有（任務從生成換成抽取）；目錄頁框架詞外漏用 `scripts/index-leak-check.py` 掃
...
**Last Updated**: 2026-08-31 — #303 經一輪四個 frame 的多輪審查後大幅修正，四項承重宣稱被推翻。訊噪比原本寫「四個候選裡三個是真缺口」，而那四個是人從一百二十九個候選裡挑的、工具的精確度沒有量過；連結數的分母原本沒寫（受測十一篇七十二個，而分類總數一百六十）；承重術語掃描的射程限制（漢字、詞邊界、重疊窗）補進卡與腳本檔頭；承諾判斷標準從句法規則改成內容測試，因為審查找出兩個假陰性——**這一條被改過三次而三次都是把形式當判斷標準**，該形態本身比那條規則有用。核心證據的變項也分離了：同一批十一篇裡長度不預測結果（二十一到二十六行有四篇答得出來、兩篇答不出來），分開兩組的是「本篇有沒有任何一段在講自己」這個門檻，而該證據只有兩個陽性且編碼是事後做的。同批修掉兩處已發布的對外事實錯誤（quiche 的詞源年代與方言地區，CNRTL 與 Wiktionary 雙來源證實）。

**Last Updated**: 2026-08-31 — 新增 #304，由一次對 #303 判斷標準的預先登記驗證抽出。四項預測在寫稿之前寫死，其中預測一被推翻：十八條路由裡有兩條的關係方向與「組成」相反（目的地拿本篇的產物去用），五種關係歸不進去，#303 的那一格因此補上方向這一維；同批也證實五種關係不是料理批的特產——替換零條而其餘四種都用上了。本卡本身來自量測方法的一個分岔：三份探針的自評欄幾乎一致而逐條比對之後出現一條三份全錯、兩份方向答反的路由，那一條的自評欄與答對的十七條無法區分。#302 的方向也得到確認——探針抄回的手冊證據集中在實測數字與症狀對映，序列連接詞反而被明確回報為近乎不存在。反向指標補在 #281 / #303 / #289 / #267。

**Last Updated**: 2026-08-31 — 新增 #305，由一個語言層教材分類的十篇實作與使用者在過程中提出的判斷標準抽出。深度那一半有量測：最基礎的兩個題目（子句分工、別名語法）量出全批最危險的兩個缺陷，而兩者都不報錯——一個引擎接受把列的條件放進篩選分組的子句並回出錯的清單，另一個讓識別字的大小寫在一家引擎上被摺疊而另外兩家不區分。起點那一半是設計論證、明記沒有對照組。本卡限定 #219 的源頭性質（必須是基本機制、不是複雜需求），並明確擋掉 #191 的一個誤讀——「讀者不是外行人」管的是姿態不是份量。與 #303 的承重術語盲點同機制不同對象（那裡是一個詞、這裡是一整個主題）。反向指標補在 #219 / #191 / #262 / #303。
...
**Last Updated**: 2026-09-04 — 新增 #330，由併行 session 的一次 reviewer 回報抽出。那份報告完全符合當時的契約（零命中須明寫、不可省略），而三十四條零命中裡有十六條的零來自「判斷標準只有二元事實、造不出候選情境」，證明的是判斷標準與 frame 不匹配、不是判斷標準通過了。因果由該 reviewer 自己指出並且比現象值錢：那三十四條原本是一鍋端的，它是在被要求交代採信依據時才不得不去分「我試過」與「我看過覺得不用試」——分類是被問句逼出來的，報告者本來沒有那個區分。本卡是 #243 的下一層（那裡問有沒有痕跡，這裡問痕跡本身分不分得出證據力），並與 #221 / #329 / #291 構成「零命中有幾種來源」的家族。反向指標補在 #243。

**Last Updated**: 2026-09-04 — 新增 #331，由併行 session 同一批審查裡的兩次撤回抽出（兩例同一批、同一個要求者與回報者，換人是否同樣有效在證據範圍外）。觸發條件由對方指認並且比現象值錢：那兩次撤回由追問的形態帶出來——追問寫成「請把 X 寫出來／算出來，我要能自己驗證」，要的是物件而不是關於物件的判斷，而回報者自己不會走到那一步。第二例最清楚，那個數字更早一輪就由同一個回報者量過，缺的只是把它代進判斷標準走一遍。同批把 #321 的機制一般化：每一種檢查都有入口，零命中只證明從那個入口走不到，而路由的死名與孤兒是同一機制在另一個入口上的實例，處置是雙向各驗一次並分開寫；它因此成為「零命中有幾種來源」家族的一員。#330 的家族句同步改成不自帶序數（照 #325）。反向指標補在 #330 / #243 / #315 / #321。

**Last Updated**: 2026-09-04 — 新增 #332，由併行 session 的三個連續修法批次抽出（一個 skill、一位作者、七個缺陷；三批皆出現而未觀察到隨批次遞減，三批推不出趨勢所以卡上沒有比率宣稱——這一句是該批 reviewer 自己糾正過來的）。承重的欄位是抓出者的分佈而不是缺陷清單：作者自查抓到的兩個恰好都有 pattern 可比對，獨立審查抓到的五個恰好都沒有，分界因此是「有沒有 pattern」而不是「認不認真」。同批回填 #330 的第二批數據：三格上線後「沒查」那一格立刻兌現（一個 reviewer 用它標記了未連外查證的官方引用，而它的核心論證建立在那些值為真之上），而「不適用」那一格的佔比是 frame 相依的（七個 frame 裡只有一個約四成八，成因是那個 frame 的判斷標準混進了二元事實），這個分佈把它從回報欄位升級成診斷指標——集中在單一 frame 是判斷標準該搬家，各 frame 平均偏高才是整體錯配。反向指標補在 #247 / #298 / #329 / #330。

**Last Updated**: 2026-09-04 — 新增 #333，由併行 session 的一次撤回抽出。一個回報者在三則獨立訊息裡引用同一組數據並據此報了一則實測否證，被問一個方法學問題之後回去查，發現那份輸出從來沒有進過它的脈絡，四項數據全部撤回。它自己提出的四欄分類法是本卡的核心，而那個論證不依賴頻率——兩欄會把虛構的觀測歸進「引用的」，這是分類法的性質。本卡與 #330 的第三格是同一個設計原理（類別要窮盡，少一格時落在那一格的東西會被歸進最接近的一格），與 #331 同源而施力點不同（直接要求交出物件，對比問一個只有握著物件才答得出來的後續問題），與 #291 同族而成因相反（壞掉的工具，對比從未被呼叫的工具）。反向指標補在 #331 / #330 / #291。

**Last Updated**: 2026-09-04 — #333 補第五類（別人量的、我引用），由併行 session 的自陳實例提供：四項數據以既成事實的語氣回報給使用者，全部來自另一個 reviewer 的訊息而未自己跑過；事後補跑全部相符，而那個相符來自運氣、不構成查核。它與第四類的失效模式相反——來源真實且通常是對的，所以查了也對、沒有人會再查，危害落在責任鏈上而不在數字上——它把「別人驗過」記成「我驗過」，下游因此高估了查核次數。由此推出一個處置面的結論：**外部查核偵測不到第五類**（去驗那個數字它是對的，而通過本身沒有回答這是誰驗的），所以它只能靠回報者自己標記或流程強制附上自己那一次跑的輸出，追問對它無效。第二欄的定義同步補上第二層——在不在對話紀錄裡，以及在誰的對話紀錄裡。產生它的機制是語氣一鍋端，反向指標補在 #330。

**Last Updated**: 2026-09-04 — 三處補充，由併行 session 的收尾階段自我盤點提供。**#333 加放大器**：來源方越可靠，第五類（別人量的、我引用）越不會被發現，因為相符成為常態而常態不產生訊號；上游偶爾出錯反而讓它早被抓到，所以「我們的 reviewer 很可靠」正好是最不該用來放行的理由。**#330 加第二個軸**：原本三格回答「這個零可不可信」，而「這個零是什麼」是獨立的一軸——沒發現問題／零本身就是 finding（十二處全部沒有檢查點）／零是正面結果（某個詞零命中因而證明它是自造的），三者在報告上同樣被寫成「零命中」，於是「我沒找到問題」與「我找到了『沒有』這件事」同形，而後者通常是那一輪的產出。**#221 加第三種形態**：豁免條款由被檢查者撰寫時會精確覆蓋違規——一次自陳的實例是先寫規避句式、再寫跳過該句式的檢查器，而豁免字串只出現在那一列；該卡原有的驗收（擴完作用域後先確認規則對已知違規報錯）本來就攔得住它，另補一個可直接算的檢查：比對豁免命中的位置集合與不加豁免時會失敗的位置集合，兩者相等代表那是規避而非例外。skill 側 multi-round-review 1.73.0。

**Last Updated**: 2026-09-04 — 新增 #334，由本階段的四個實例歸納（三個自陳、一個由併行 session 提供，四個出自同一段連續工作，所以成立的是共用結構而非頻率）。它是本批多張卡的共同上層而處置方向與它們都不同：其餘各卡給的是檢查，這一張給的是設計——改成本結構，不改注意力。同批把 #221 的兩集合判斷標準從推導升級為正反兩側各驗過一次（規避側兩集合相等、例外側不相等，而例外的命中包含引號內的反例引用這類本來就不會失敗的位置），並記下它比自律版本強的地方在於不需要判斷動機。#330 補上兩軸被混成一維列表的成因——它們的報告措辭相同，所以分類的動作要由欄位帶進來、不會由觀察自己長出來。反向指標補在 #331 / #333 / #329 / #317。

**Last Updated**: 2026-09-04 — 新增 #335，由併行 session 三輪約一百五十則 finding 的方向分佈抽出（單一批次、單一審查對象，成因分析是推導）。同批擴充 #333 兩節：**兩個子形態**（第一個實例的來源不存在、第二個實例的來源存在而說的是別的事——後者更難防，因為查證的動作會成功而成功就是放行訊號，一句話是「有來源可查」這一欄查的是有沒有來源、不查來源說的是不是這件事），以及**分類欄位本身就是偵測程序**——第二個實例是被那張四欄表自己抓出來的，回報者在逐項填「這一項的來源在不在對話紀錄裡」時撞見自己引錯了，這一層原本沒被看出來，它讓填表的人在填的當下發現問題而不需要任何人起疑。反向指標補在 #217 / #296 / #330。

**Last Updated**: 2026-09-04 — #335 與 #334 之間補上邊界，由併行 session 指出。#335：那個偏斜是一個沒被算過的假設，而不是算過之後的成本選擇——「代價側產出少」一量就翻轉（兩次撤回的替代 finding 都更強）。#334：它的三條修法預設「代理確實比較便宜」，而那個前提要驗過才算數；打開一個檔、跑一條指令這類差價是真的，而有些看起來像成本的東西一量就消失，所以**「這是理性的成本選擇」是一個可以被驗的宣稱、不是一個解釋**，沒驗就套用會讓一個該修的偏斜看起來合理。#324 另補一條關係：它的修法（把量判斷者的訊號改寫成量對象的版本）在 #335 有一個現成實作——finding 的方向分佈，「想不出新 frame」要靠列舉的完整性而看一欄是不是全空只要讀分佈。

**Last Updated**: 2026-09-07 — 新增 #336 與 #337，由併行 session 一次四輪、二十餘個 reviewer 的實跑回饋抽出（單一批次、單一審查對象，兩張卡的一般性由結構推出而未另外量測）。#336 限定 #126 的處置那一半：那張卡把「某軸沒覆蓋」交給成本與風險的權衡，而權衡之前要先問這一格用手上的流程補不補得到——盤點表因此多一欄「靠什麼補上」，timing 填時間、instance 填另一個執行者、其餘五軸填下一輪，答案不是「下一輪」的建票綁觸發條件而不是加輪。#337 把 SKILL.md 早已記下的共用產物段一般化：共用產物只是三類射程外項目的其中一類，另外兩類是審查所用的工具與審查自己的產物，而工具那一類的實例正好發生在這次回饋的來源身上。#126 同批補三個盤點執行細節（逐 reviewer 對帳而非憑印象、不適用與未動分開記、切換強度要對齊該軸的 catch 目標）與一份含修正過程的填好範例。skill 側 multi-round-review 1.76.0。

**Last Updated**: 2026-09-07 — 新增 #338，由一次跨三個消費端的裁決往回追出來。事件是本專案刪掉共用 skill 底下的兩個 hook，兩週後另外兩個專案來問為什麼——它們都還在跑、各有四處註冊，其中一個在刪除前五天還修過那支腳本。三處判斷都在本地做過而都不完整：前提（「任何 settings 都沒有註冊它們」在本地為真而寫成了關於全體的事實）、效果（同步工具的 `--prune` 只在推送方向刪，拉取一律保留本地獨有檔，所以已安裝的消費端會同時持有刪除紀錄與那個檔，經 `skill_sync/cli.py` 確認拉取路徑沒有任何刪除呼叫）、理由（把「作用域寫死指向別處」判成了「不適用」，而後者才可以廢除）。第三項是本卡作者在回答對方提問時當場取的代理，由對方指出。同批在 `rule-must-point-at-something-openable` 回填的「廢除紀錄三選一」是本卡修法的一半。反向指標補在 #221 / #337。

**Last Updated**: 2026-09-07 — 四處回填，由併行 session 的外部實測與自陳提供。**#321**：雙向都驗過之後還有一層——方向數不等於入口數。一次外移三節之後路由表的雙向檢查全綠（零死名、零孤兒），而同一批三處跨檔指涉是失效的：檔頭的同目錄索引與正文的跨檔指涉是另外兩套指標，那個檢查一條都沒掃。雙向全綠比單向零命中更容易讓人收工，分辨的方法是先數指標系統再數方向。**#324**：判別問句的第二種用法是分判斷標準——某條判斷標準由「這個目標日後會不會動」（量判斷者的預測）改成「這個目標現在有沒有名字」（量對象的現況），而改寫後在撰寫它的那批修法裡當場攔下四處自己的同型缺陷；一條判斷標準對產生它的那批文字立刻生效，比事後被套用的次數更能說明它量到了東西。**#334** 補第二個機制：有一類代理由觀察者站的位置產生——代理與本體在那個位置上重合，取它與價差無關（本地狀態代理遠端狀態，兩次自陳實例都查了容易查的那一側而結論依賴沒查的那一側）；它與價差型的差別在連決定都沒有發生過，所以處置是改問句的形式而不是改成本結構。**#333** 補掃描問句：這句話裡的每一個數字與每一個完成式，是同一個時點的嗎——完成式與它旁邊的狀態值分屬兩個時點時才進判別。

**Last Updated**: 2026-09-07 — 兩處回填。**#336** 補「異源交換買到的是沒寫明的約定，不是 finding」：兩方各自量同一份文件的行數報出 148 與 152，兩個都對（一個扣掉 frontmatter、一個沒扣），而那份文件從未寫明門檻算的是哪一種；兩方的自檢都抓不到這個分歧，因為各自內部自洽，它只在兩邊把同一個量報給對方看時才浮出來。它與翻譯探針同構（單方的量測允許不寫明的約定，兩方報數強制它現形），推論是交換的成果要看有幾個約定被寫明、且對照要挑兩邊都做得出來的量。**#334** 補「條文自帶量測指令時，那條指令就是條文的代理」，實例是本站裝飾符號規則的三層落差：條文是全稱而指令是列舉、同一條規則在規範入口與 SSoT 兩份文件上附了長度不同的兩條指令（十個對二十二個）、兩份列舉都漏掉內容庫裡實際被當成標題前綴使用的一個符號。驗收因此有兩個——把條文的名詞與指令的輸出並排問是不是同一個集合，以及問這條規則在幾個地方附了指令。

**Last Updated**: 2026-09-07 — 一批回填，由拆分 `multi-round-review` SKILL.md（30k tokens → 入口檔 4.8k 加六份 reference）與併行 session 的對照長出來。**#291**：正對照要與待判項同粒度（聚合層的命中不能替單項的鑑別力背書）、正對照有兩個方向（會亮紅與會亮綠各驗一側）、假紅燈的鏡像（工具連錯三次會讓第四次真的紅燈先被懷疑）。**#294**：刪除清單逐項查來歷（先查磁碟 `test -e && git check-ignore`，再查歷史 `--diff-filter=D`，順序不能反——磁碟那一問是唯一跳出 tracked 樹視角的）、修法是兩件不是一件（判定分格＋措辭不宣稱成因）、零輸出的語意限定在跑它的 repo 要寫進措辭、核對放在動作之後就只是輸出不是閘門。**#321**：方向數不等於入口數（雙向全綠仍只覆蓋三套指標系統之一）、入口之外還有軸（相對連結的失效是指標搬家而目標沒動，同一種檢查對它恆真——53 條斷鏈就是這樣漏的）。**#324**：判別問句的第二種用法——判斷標準寫成預測（量判斷者）還是現況查詢（量對象）。**#333**：一個為真的動作推不出一個為真的現在（「我推過」為真而「發佈端是最新」為假，第三欄穿第二欄的衣服）、掃描問句「每個數字與每個完成式是同一個時點的嗎」。**#334**：位置重合是第二個機制（本地代理遠端、git 代理磁碟、工具措辭預先包裝好的代理）、條文自帶量測指令時那條指令就是條文的代理（本站裝飾符號規則三層落差：全稱對列舉、兩份文件兩條長度不同的指令、兩份都漏掉實際在用的符號）。**#336**：異源交換買到的是沒寫明的約定不是 finding（148 vs 152，兩個都對）、票要指定形式（兩邊各自量同一件事、各自報數）、驗收不按來源歸帳（那一類的產出沒有歸屬）。**#338**：作用域有兩個會漏掉的位置（判定與措辭）。**#221**：檢查器分不出談論與使用（用來示範路徑會斷的路徑本身被判為斷），判別是這個字串在這一句裡是被執行的還是被說的。**#317**：邊界——在路徑上、標題是關鍵字、仍未查，泛化標題無效要動作前的觸發點；成對的兩種失效（知識沒被取用改觸發時機／取用了而值域不夠改內容值域，兩者都不是紀律問題）。skill 側 `multi-round-review` 1.78.0 → 2.0.1、`compositional-writing` 1.7.2、`skill-design-guide` 1.9.0 拉入。

**Last Updated**: 2026-09-08 — 新增 #339，由併行 session 轉來的一則個案加本站自測歸納（兩例，無對照組，成立的是機制與判斷標準而非分佈）。對方的個案是一份互動規格的表格列名壓成四個字、還原要三項表外知識，而那個列名另被下游四處逐字引用；本站自測找到同型的一處——#302 定義的七種裝置有四個引用點，三處保留完整形，`AGENTS.md` 完稿檢查清單那一處把限定語砍掉十九個字（「順序關鍵加上顛倒的後果」變成「顛倒的後果」，主詞整個消失），而那正是逐項執行、每次使用都要重新解碼的位置；該處已隨本卡同一次改動補回完整形。本卡限定 #261 檢索鍵位那一列的「精簡優先」，並給識別充足一個可執行的判斷標準；它與 #155、#156 同屬錨點寫法家族（編號當錨點／名字內嵌成員數／完整概念被壓縮），三者的共同結構是把只在作者側成立的前提寫進了會被複製的標識符。與 #321 同一種入口失配的兩端——那裡名字頻率是零，這裡名字語法合格。反向指標補在 #261 / #108 / #155 / #302。skill 側 compositional-writing 1.9.0（原推為 1.8.0，與另一個並行 session 的 1.8.0 版號相撞而在發佈庫被覆蓋——兩邊改的區域不相交，覆蓋是整檔推送造成的，不是內容衝突；已合併兩側內容並讓號給先發佈的一方）。

**Last Updated**: 2026-09-08 — 新增 #340，由同一天內同一支共用 skill 在發佈庫上的三次覆蓋抽出（兩個專案三個 session，本專案兩次是被覆蓋方、一次是覆蓋方；三次都可在發佈庫 git 歷史逐個 commit 查證，敘述以那份歷史為準而不以任何一方的回報為準——其中一次的當事人回報與實際歷史不符）。單一資產、單一工具、單日，成立的是機制與修法而非頻率。它與 #339 是同一件事在兩個載體上的形態（標識符承載不了它被要求承載的區分），與 #338 同屬共用資產家族而管的是更新不是刪除，與 #291 同族而差別在工具是好的、選錯的是量測對象。第三次的成因是 #334 位置重合型代理的新實例，該卡同批回填。反向指標補在 #338 / #334 / #291 / #339 / #221。skill 側 compositional-writing 1.10.2。

**Last Updated**: 2026-09-08 — 新增 #341，由本站寫作規範自身的一次修訂抽出（單一站點、單一規範的自陳，成因由結構推出而未另外量測——量測本身受該機制阻擋，省略在產物上沒有痕跡，回頭清點需要逐篇重讀）。AGENTS.md 的讀者定位原則同批改寫：標題由〈讀者定位用內容體現〉改為〈專業讀者的意義是在乎細節，不是可以略過解釋〉，正文寫出讀者要什麼（原因、成本、代價、用詞精準）而非他已經有什麼，並加上判別問句與卡片式筆記是這個設定的直接後果這一層。知識卡片規範裡術語分級的基線定義同批補一句——基線取知識存量、不取理解能力，把「專業工程師」算進基線會讓它膨脹到涵蓋整個領域，缺卡判定因此永遠不觸發。本卡限定 #191 並把它既有的 #305 反誤讀指標推到機制層，與 #339 是同一徵狀在兩個載體（標籤省限定語／正文省解釋），與 #340 共用「理由每次都成立」的結構。反向指標補在 #191。

**Last Updated**: 2026-09-08 — 三處回填，由與併行 session 的一輪往返長出來（跨兩個專案、一天之內的四項事實型判斷失效，每一項的推翻都由對方或權威產物提供，同源審查在該批沒有產出任何一項）。**#296 適用範圍改寫**：原本寫成型別二分（register 適用、形式違規不適用），改為單一判斷標準——這個錯誤有沒有一個不繼承作者那份前提的來源可以推翻它；形式違規不適用的真正理由是工具本身就是那個來源，不是因為它「形式」。二分漏掉第三格的**事實型判斷**（把退出碼讀成唯一已知的語意、把顯示行當成閘門、用錯的鍵查檔案），它與文字讀起來如何無關而工具也抓不到，錯誤同樣來自共有的先驗——兩者都是先驗而非能力。事實型另有兩個 register 沒有的性質：可用權威產物推翻（讀原始碼、取完整歷史），以及**可以被寫進規範**——未經查證的事實寫進規則檔後每個載入者都繼承它且形態是「已查證」，懷疑它需要先懷疑規範（實例是本站曾把「同步工具印的版號比對行可以信」寫進操作規範，而該工具早已把版本字串移出同步決策）。**#337 加第四類射程外項目**：正在被寫出來的那條規則——規範是用來檢查產物的工具、本身不是被檢查的產物，與第三類的差別在時序；它的傳播係數最高，而規則不觸發的場景與後果最大的場景同向、不互相抵銷。**#317 補判別的做法與代價不對稱**：問這條規則寫下來的觸發條件與例示涵蓋了它嗎（不問該不該涵蓋，那個答案永遠是該），例示界定值域而例示讀得到；分錯的代價不對稱——把值域不足誤判成觸發落點會讓規則長出一個新的待遵守步驟，所以答不出來時預設走值域那一邊。skill 側 multi-round-review 2.3.0。

**Last Updated**: 2026-09-09 — 新增 #342，由一批硬體排錯 work-log 的審查末端抽出（單一站點、單一批次的自陳，機制由結構推出而未量測；卡上沒有照抄比例這類數字，也不需要——處置由代價的方向決定）。事件是多輪審查跑完後，待辦裡有三項是「某型號的某個指令實際輸出什麼」這一類，而作者的處置是照三層標註寫進去、把沒驗過的標成 caveat；站主否決的是整個做法。本卡限定三層標註對這一類的適用，並記下兩層理由：**誰承擔預設**（標註要讀者的自律，不寫由結構保證）與**照抄者的路徑不經過 caveat**。同批修掉本站一處實例（一台實際裝置的 vendor id 換成佔位值並寫明是佔位的），並在 AGENTS.md 立為原則十二、完稿檢查清單加一列。它與 #341 是反向的一對（該寫而沒寫 / 不該寫而寫了，兩者都在稿件上看不出來），與 #340 共用「把正確行為做進結構而不是交給記性」這個處置形態。反向指標補在 #341 / #340。

**Last Updated**: 2026-09-11 — 新增 #343 與 #344，由併行 session 一次「單檔拆成八檔」之後的三輪理解探針量測抽出（二十八份低階探針、七項判決；單一拆分事件、單一切法，「拆分造成」這個歸屬由結構推出而未拿單檔版本做對照）。該份文件在量測前已通過三輪高階 reviewer 的九個 frame，而七項判決無一被那九個 frame 攔下——成因是**逐檔審查對缺陷所在的那個單位恆真**，每一份檔案單獨讀都沒有違規。#343 記拆分自己造出來的六種形態與偵測（受測單位換成整組檔案、三道題、入口檔另跑一批），其中層級混疊與指路位置兩項就地展開，後者有單變項對照（同一個詞從檔頭總括改成首次用點帶指路，三份判「沒說去哪查」降為零份）。#344 記修法的層——同一處連續三輪被命中而中間兩次修法各失敗一次、失敗方式不同（段落層的警告修不掉成分層的歧義／改對了層而陳述範圍寫得比事實大），機制是修法的可見度與有效性各自獨立而兩者在 diff 上的排序常常相反。反向指標補在 #284 / #325 / #321 / #298。skill 側 multi-round-review 2.4.0（新增 2-B⁸ 集合探針、整合段補修法的層與單變項對照）、compositional-writing 1.13.0（`structuring-with-solid.md` 新增拆完之後的交界檢查）。

**Last Updated**: 2026-09-11 — 新增 #345 與 #346，由併行 session 一次五輪的理解探針審查抽出（單一審查對象、每輪三份，數值門檻取自這一次的分佈、換對象要重新校準）。#345 的形態需要同輪剛好有另一項無關改善才成立，所以它描述的是一種會被掩蓋的失效、不是每次修法都會發生的事；#346 的判斷標準一有單變項序列支撐，判斷標準三由一次自我矛盾推出而「寫死門檻能消除它」未經量測。同批把該次審查的其餘案例併入 skill 而未另立卡：修法會把疲勞點推到新增的內容上（連續三輪追著跑，成因是新增內容沒有經過與既有內容同等的密度檢查）、以及兩次「在補某條規則的同一批修改裡違反該規則」（犯它的是編輯動作——搬移、命名時圖簡潔，而讀規則時想的是撰寫動作，所以規則寫在文件裡擋不住這一類）。反向指標補在 #344 / #333 / #298 / #291 / #281 / #324。skill 側 multi-round-review 2.5.0、compositional-writing 1.14.0。

**Last Updated**: 2026-09-18 — 新增 #347、#348、#349，由一個併行專案交來的設計檢討抽出（該專案跑一項數日的條文驗證工作，形態是多輪審查的極端版本——低階探針讀條文作答、兩名獨立判定者判讀；單一專案、單一審查形態，成本結構的式子由該專案的器材配置推出而未在第二個專案上驗證）。對方交來十一條建議，逐條比對本庫既有原則後收三條、退三條、其餘併入 skill：**退掉的是已被涵蓋的**——「只審不改與允許改要分開、後者帶回歸檢查」已是 `sequential-fixes-compose-into-defects` 的正文（含四種實測形狀與每輪通讀的處置），「正確來源放在更省力的位置」是 #334，「未發現 X 須說明看不看得到 X」是 skill 零 finding 三格的第二格；「變更掃描與全量掃描不可互相替代」併入 #349 的關係段而不另立，因為 #329 已經涵蓋。#347 是第一節與第二節的合體而非兩張——成本結構是症狀、單變數歸因是根因，拆開會讓讀到症狀的人以為處置是優化器材。#348 把 #317 的三個成員擴成四個，它的讀者已經打開了那份文件、還逐字引用了其中一款，所以搬動住址的修法在這裡沒有著力點。#349 合 3.9 / 3.10 / 3.4 三條，共同機制是四種前置步驟全部在「處理」之前完成。同批回填：`sequential-fixes-compose-into-defects` 補量測層的合成缺陷形狀（既有四種形狀全在可讀性層，而修正動作新增的詞成為獨佔詞、讓探針靠字面比對即可選中它，判斷標準「讀起來像不像一個人一次寫完」對它恆真）、round-2-probes 補「通過也要歸因」（既有規則只寫失敗要歸因）。反向指標補在 #317 / #337 / #334 / #345 / #329。skill 側 multi-round-review 2.7.0。

**Last Updated**: 2026-09-18 — 三處回填，由 #347 來源專案的一輪回覆長出來（對方據該卡的處置清單收官，回報了處置本身的一個代價）。**#347 新增〈放棄歸因會連帶取消一些驗收條件〉**：歸因的器材同時是某些驗收條件的指涉基礎，砍掉器材會讓那些條件失去語意而不是判定為未達成——對方的舊閘門要求「全部範例同時 3/3」，而「同時」要跨輪材料才判得出來，逐輪節錄檔明文刪除之後它變成不可判定；不可判定與未達成在結案報告上同形而處置相反（前者先改條件、後者再跑一輪），混在一起會讓團隊照未達成的處置走而每輪得到同一個結果。處置是砍之前逐條列出量詞跨輪的條件（同時／連續 N 輪／相較上一輪／累計）並問它靠哪一份材料判得出來。同卡補一項限制：處置清單裡「拋棄式比累積式便宜」是結構演繹、未實際量過，照 #334 它是未驗證假設，本卡的處置自己還沒過那一關。**#298 家族的 sequential-fixes 補修法與驗收**：修法是讓那個詞不再獨佔（實測是單一字元插入），驗收比對獨佔字串總數、移除清單與新增清單三項，最後一項必須是空的——它是這一類的回歸檢查。**#349 的順序段補可執行形式**：條文側起點、每款取獨佔 n-gram、先與題幹取交集再極大化。skill 側 multi-round-review 2.8.0。

**Last Updated**: 2026-09-18 — 新增 #350、#351、#352，由同一個併行專案的第二份交付抽出（七輪探針讀規範文字作答、兩名獨立判定者判讀，十一項各有逐輪讀數；單一條文集合、單一探針配置，各項的證據強度不同並逐項記在卡上）。十一項收三張卡、四處回填，其餘併入 skill。**這一批最重要的產出是它推翻了 #348 自己的驗收方法**：該卡原寫「改完重跑探針看引用率有沒有動」，而實測引用率單獨看會把兩種相反的情形混為一談——既有段首規則 3/6 引用而 6/6 套用（照做不引、不用管），新增段首條款 6/6 引用而 4/6 套用（引了沒做完最後一步、要把末句移走），處方相反而單看一個數字兩次都判錯，已改為引用率與套用率兩個獨立的量並附對照表；同卡另補兩處——排除條件要在選支那一句裡逐字寫出錯誤讀法並否定它（實測一款三句在邏輯上已排除而排除點全部位於選支之後），以及操作段開頭的「動手之前先問這一款被引用會發生什麼」（零引用有一類的處置是刪除、而把負效力通路前移是最壞的動作，指向 #351）。另三處回填：**#155** 補〈編號已經是介面的時候：改行為用插入、不用重組〉（凍結引用讓編號成為介面，實測一次拒絕改一個字因為會讓七份引文失效、一次拒絕重編號因為位移十六份引用、一次選純插入而非合併重寫因為成本差一個數量級——三十三份對兩份；判別問句是這次改動要位移幾個既有錨點）、**#333** 補〈現行判斷標準可以改，歷史記載不可以〉（四次獨立套用全由執行者自行判斷、無人提醒；反直覺的誘惑在相反方向——看到不一致就想修正它，而那正是偽造；邊界案例是一份簡體作答保留不轉寫，因為偏離語系設定本身是一項讀數特徵）、**multi-round-review 的 `integrating-findings.md`** 新增可達性分析（「既有案例複核無翻面」是往回看的誤傷分析、對可達性缺陷零鑑別力，實測一款新增定義句使第三款觸發前提恆為空而成死條文，逐款審查看不見因為每一款單獨讀都對）。skill 側 compositional-writing 1.17.0（三張卡進 `references/principles/`、SKILL.md 兩節加路由、`reference-authoring-standards.md` 驗收清單新增五項、`designing-fields.md` 補值域涵蓋檢查）、multi-round-review 2.9.0（#348 三處修正、可達性分析、3-F 第五題）。

**Last Updated**: 2026-09-18 — 新增 #353，由把 #348 / #350 / #351 操作化進 skill 的那一次抽出（兩批理解探針、每批三份、只餵衍生形那幾行；十一項回報逐項歸因後七項進處置，而七項全部落在衍生形上、本體一項沒中）。最嚴重那一項改完後以逐字相同的指令複驗，由三份全數讀反轉為三份全數讀對。限制記在卡上：單一站點單一次操作化，「七項全部」是這一次的分佈；本體沒有被同等量測（沒派探針讀卡本身）；複驗只覆蓋最嚴重那一項。同批回填 **#317** 第二個誤判方向——把偶發現象誤判成工具缺陷，而它比值域誤判多出不可推翻這一層：一次實測把背景作業改寫工作區的中間狀態讀成「這具工具的產出不是格式化的固定點」並據此訂了「每次跑完補跑格式化」，乾淨工作區下單獨再跑一次證明該缺陷不存在；**這種多餘步驟事後不會被推翻，因為做了它之後看起來就是有效的**，所以宣稱任何工具有缺陷之前要在乾淨工作區單獨跑它一次。反向指標補在 #339 / #303 / #281。skill 側 multi-round-review 2.10.0（探針的受測對象取衍生形）、compositional-writing 1.19.0（壓縮後三問進驗收清單）。

**Last Updated**: 2026-09-22 — 新增 #361 / #362（由一個 SQL 教材分類 21 篇正文的逐篇檢視抽出；立卡當輪該分類未改，隨後同一天依這兩張卡新增了 1.0 與 1.22 兩篇，而**兩張卡都被那兩篇當場再犯一次**，實例各自回填在卡的 Case 段），並把 #355–#362 併進篇目索引。**併入時修掉一個渲染缺陷**：#355 起的六張卡先前登記在一張沒有表頭也沒有分隔列、且緊接段落無空行的表裡，goldmark 不會把它當表格——那六列在站上是黏在前一段尾巴的字串，而這張表從 #355 起累積了六個 commit 沒有被發現（判讀正是 #362：註冊的動作是「加一列」，每個人都照做了，而沒有人檢查那張表渲不渲染得出來）。同批盤點到**篇目索引有 22 張卡有檔案而沒有條目**（#21 / #22 / #135–#138 / #164 / #184–#198），分佈是連續區塊而非零星遺漏，看起來是整批未登記；未處理，逐張要補判讀敘述。skill 側 compositional-writing 1.29.0、multi-round-review 2.17.0。

**Last Updated**: 2026-09-18 — 新增 #354，並依同一份跨 repo 逐條對照修 #348 / #349 共三處（對方以我方 #348 的提問形式跑了十九組「哪一份較窄」的對照，指出三處我方較窄）。**#349 兩處是已推上共用庫的真缺陷**：守恆式原本只要求「分類到互斥格、加總等於總數」，**沒有要求分類函式的輸出就是比對函式的輸入**——對方實測到三格分類回報全部通過而比對讀的是全文，在原條文下完全合規、守恆式全綠而零約束；以及對照輸入要分兩種（驗管道通不通的那一種拿已知會命中的字串，驗分母完不完整的那一種**輸入必須落在現行過濾條件之外**、逐則寫明它不滿足哪一項、預期由哪一個分支接住），第二種不可由第一種代替，因為已知會命中的字串自帶現行規則所需的全部特徵、對過濾條件的缺口零鑑別力。**#348 一處**：把「處置側的涵蓋關係陳述十五輪零引用」這則觀測推成禁令——處置側不出題，因為題目落在讀者走不到的位置時量到的零是題目的性質而不是條文的性質，兩者在讀數上同形。對方另列十四項「我們有你們沒有」，篩三項收錄：#354（遮蔽推導驗必要性）、探針餵入範圍的邊界是它讀得到什麼而不是指令寫了什麼（本站今天正是依賴探針服從、未強制）、以及正規化的兩個相反失效方向（換行與分隔符替換成單一空白、不刪除也不當切段邊界——切段漏報、刪除生假命中，取脈絡的掃描因此寫成 `.{0,20}` 而非 `.{20}`，計數非零而脈絡零行時零的是切段不是內容）。**一處未採而回覆了理由**：對方問 #351 的移除處置路徑怎麼長出來，答案是那次刪除是他們自己的動作、逐字來自他們的交付，我方只是記錄——所以那張卡不能當成他們規格該有移除路徑的外部證據。三分路由（有幫助搬位置／沒作用刪掉／會讓結果更差優先刪掉）是本站的推導，實例不是。skill 側 multi-round-review 2.11.0、compositional-writing 1.20.0。

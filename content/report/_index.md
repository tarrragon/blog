...
**Last Updated**: 2026-09-04 — 新增 #331，由併行 session 同一批審查裡的兩次撤回抽出（兩例同一批、同一個要求者與回報者，換人是否同樣有效在證據範圍外）。觸發條件由對方指認並且比現象值錢：那兩次撤回都不是回報者自己想到要驗，而是追問寫成「請把 X 寫出來／算出來，我要能自己驗證」——要求交出物件而非關於物件的判斷。第二例最清楚，那個數字更早一輪就由同一個回報者量過，缺的只是把它代進判準走一遍。同批把 #321 的機制一般化：每一種檢查都有入口，零命中只證明從那個入口走不到，而路由的死名與孤兒是同一機制在另一個入口上的實例，處置是雙向各驗一次並分開寫；它因此成為「零命中有幾種來源」家族的一員。#330 的家族句同步改成不自帶序數（照 #325）。反向指標補在 #330 / #243 / #315 / #321。

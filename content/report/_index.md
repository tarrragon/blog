...
**Last Updated**: 2026-09-01 — 新增 #308，由使用者對 SQL 分類 description 體例的指正抽出。全站 3716 篇有 description、約九十篇是觸發條件式，而該分類 22 篇全中——這是規範直接長出來的形狀。本卡限定 #170 的射程並把適用邊界寫進 AGENTS.md 原則九（教材與查閱型分兩種體例、共用同一條驗收），順帶記下一個一般化的機制：卡片自己寫的射程限制擋不住推廣，因為限制在論述基礎段、引用的人讀的是核心原則段。反向指標補在 #170，關係段連 #306 / #302 / #303。

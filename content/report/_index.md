...
`#232 自審偵測方法要對齊規則類型` → `#221 檢查規則的作用域要顯式列舉` → `#165 register 違規判定靠異源` → `#147 規範化跟自審是兩種認知任務` — 一個檢查或自審回報 clean / 零 error 時，先問「這個通過是涵蓋內的通過、還是根本沒被涵蓋」：#232 是偵測模態軸（grep 對無關鍵詞的規則結構性失明、clean 是看不見不是沒違規）、#221 是作用域軸（規則沒被納管、零 error 是沒被檢查）、兩者同構；#165 給無關鍵詞規則的真防線（異源冷讀）、#147 給自審在規則內仍漏同義變體的限度。共同修法：通過訊號要附「涵蓋聲明」——掃了什麼、什麼沒掃、哪些要交異源，不讓假 clean 跟真 clean 長得一樣。
...
**Last Updated**: 2026-07-20 — 新增 #232 自審 sweep 的偵測方法要對齊規則類型（從本 session 多輪審查的 self-application sweep 漏網抽出：grep 對 #229 敘事開頭這類無關鍵詞規則結構性失明、回報 clean 是看不見不是沒違規、靠異源 reviewer eyeball 補回）；新增路徑 35 串 #232 / #221 / #165 / #147（通過訊號先問涵蓋不問乾淨）。前次：新增 #229-#231 分析文章結構三卡（引言修訂和 SRP 拆分抽出）、路徑 34 串 #229 / #230 / #170 / #141（讀者 surface 不放編輯 metadata）。

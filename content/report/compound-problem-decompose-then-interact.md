...
date: 2026-07-05
weight: 211
description: "問題由多個概念交互導致時（磁碟空間 = APFS 空間池 + Cryptex 大小改變 + Simulator 多版本累積），文章先各自教 A/B/C 的機制再談 A×B×C 交互。讀者理解各元件後交互作用是自然推導的。"
...
本卡從 #209 拆出。原始 case 是磁碟空間診斷系列——「空間不足」由 APFS container 空間池機制、Cryptex 改變 Preboot 大小、iOS Simulator 多版本累積、App Container UUID 命名辨識困難四個概念交互導致。四篇文章各自教一個機制，讀者理解各元件後就能自己推導整體狀況。限制：本卡的 case 是同一領域（macOS 磁碟管理）的多概念交互，跨領域的複合問題（例如效能問題涉及網路 + 資料庫 + 應用層）可能需要不同的串接方式。
...
**引用而非重複**：如果 A 的機制已經在獨立文章裡教過，B 的文章引用 A 而不重複解釋。重複解釋會產生 SSoT 違反（[#44](../ssot-single-source-of-truth/)），而且兩篇的解釋會隨維護逐漸 drift。

## 跟其他原則的關係

- [#209 知識目標決定文章結構](../teach-judgment-not-procedure/)：判斷力導向的文章在面對多概念時自然走拆分結構——每個機制獨立教，讀者各自建立判斷力。
- [#44 SSoT](../ssot-single-source-of-truth/)：拆分後各概念只在一個地方深度解釋，其他地方引用。

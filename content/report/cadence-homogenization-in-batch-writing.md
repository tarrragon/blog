...
| 維度         | 內容                                                     | 規範狀態                                                              |
| ------------ | -------------------------------------------------------- | --------------------------------------------------------------------- |
| 內容欄位模板 | 規模對照表、tripwire 條件、失敗模式、回退路徑            | 已被 AGENTS.md 原則八 enforce（情境優先於模板）                       |
...
1. **規範語言只涵蓋 *內容* 層、不涵蓋 *形式* 層**：AGENTS.md 原則八寫「不為了整齊把不同案例硬套同一模板」、配的例子是「規模對照、tripwire、失敗模式」；批量寫作時 Claude 把「保留情境差異」自動解讀成「敘事內容不同就行」、cadence / framing 不在規範定義的「模板」內、沒被擋。

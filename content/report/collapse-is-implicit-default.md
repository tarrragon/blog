---
- [#251 清單過時的代價要落在精度、不落在覆蓋](../stale-list-costs-precision-not-coverage/)：判定與偵測這個 surface 的形態。列舉式的判定規則有一個從未被決定過的預設——「沒列到就是不算」，它不是選出來的，是列舉這個寫法的自然結果。#251 要求把那個預設拿出來明寫成一條獨立於清單的退回路徑，屬於同一種把隱形預設轉成有人決定過的設計。

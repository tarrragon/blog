...
**正確概念**：組件作者已經設計好「對外介面」。客製需求出現時，先確認介面是否覆蓋這個需求。

**替代方案的不足**：直接改 theme（避開 site h1）、改組件 source（拿掉 reset class）、加 `!important` — 這些做法繞過介面、等於拒絕跟組件作者合作。短期可用、長期每次升級都要重做。

### Reset 失效是 CSS 機制、不是 bug

**正確概念**：CSS reset 的有效範圍跟 ancestor class 綁定。元素換位置 = 換 context，UA 樣式自然回來。

**替代方案的不足**：把 reset 失效當 bug 處理（加全域 reset、強制每個元素都帶 reset class），會把局部問題擴大成全站 CSS 戰。

### Specificity 線性 vs Layers 分組

**正確概念**：specificity 是個別 selector 的線性數字比較；CSS Layers 是分組比較、分組權重優先於分組內的 specificity。

**替代方案的不足**：用 `!important` 跟組件 hash class 對抗 — 短期有效，但每加一條 `!important` 把未來的 debug 成本往後推；多個 `!important` 之間沒有層級可言、最後變成相互覆寫的迷宮。

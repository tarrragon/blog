...
**核心判斷標準**：**這個客製要跟組件升級共存嗎？** 是 → 貼邊界；否 → 怎麼做都行。
...
跟 [#59 Filter × Source 合成策略](../filter-source-composition-strategies/) 同構：本卡的「四層合作」跟 #59 的「五策略」都是「離 source 公共介面越近、合作越穩」— 介面層 ≈ 推進 query (A)、邊界層 ≈ 多 index (C)、邊界 DOM ≈ 自動續抓 (B)、內部結構 ≈ 接受 D / E。同個原則套用在「客製 UI vs 客製 filter」兩個情境。

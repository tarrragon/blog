---
title: "跟外部組件合作的層次：離介面越近、合作越穩"
date: 2026-04-25
weight: 45
description: "客製外部組件的穩定性與「離組件作者保證的對外介面多遠」成反比。每往內推一層、依賴前提增加、升級風險上升、可逆性下降。本文是 #1 / #5 / #19 / #24 四篇實作的共同抽象。"
...
對應實作：[#24 CSS Layers 取代 specificity 戰](css-layers-over-specificity/)。
...
對應實作：[#5 與 framework-managed DOM 共處](coexisting-with-framework-managed-dom/)。
...
對應實作：[#19 覆寫深度的成本告知](override-depth-cost-report/)。
...
```text
組件邊界清單：
- 索引邊界：--root-selector （介面層）
- 重置邊界：.pagefind-ui--reset class （鄰接層）
- specificity 邊界：svelte hash class （邊界內 DOM 層）
...
| 抽象原則 | 跟本原則的關係 |
|---------|-------------|
| [#43 最小必要範圍](minimum-necessary-scope-is-sanity-defense/) | 跟外部組件合作時、客製範圍也該最小必要 — 兩者疊加 |
| [#44 SSoT](single-source-of-truth/) | 組件提供的 hook（CSS variable）是 fact 的一個來源、自家 token 應該對齊到組件 hook |
| [#42 2 次門檻](two-occurrence-threshold/) | 同一個覆寫戰打第 2 次失敗 = 該換維度（從同層對抗跳到 layers） |
...
| 篇 | 對應層次議題 | 焦點 |
|----|-----------|------|
| [#1 在外部組件上加客製功能：以邊界為中心](external-component-customization/) | 邊界辨識 | 索引 / 重置 / specificity 三邊界的辨識與選擇 |
| [#5 與 framework-managed DOM 共處](coexisting-with-framework-managed-dom/) | 邊界內 DOM 的隔離 | 客製 UI 留邊界外、CSS 控制位置 |
| [#19 覆寫深度的成本告知](override-depth-cost-report/) | 多層覆寫的成本管理 | 開工前報成本、讓使用者決定 |
| [#24 CSS Layers 取代 specificity 戰](css-layers-over-specificity/) | 跳出層級對抗 | 用 layers 跳出 specificity 線性比較 |

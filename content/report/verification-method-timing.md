...
> 本篇是 [#42 2 次門檻](../two-occurrence-threshold/) 抽象原則在「驗證工具切換」這個面向的應用。
...
- **機制**：堅持推理到底
- **成本特別高的原因**：推理迴圈成本累積、最後可能需要 4-5 輪才解決
- **C 才合理的情境**：實務上幾乎不存在 — 工具的價值是縮短迴圈、不是「能力不足」的標記
...
跟 [#68 驗收的時間軸](../verification-timeline-checkpoints/) 的關係：本卡是「debug 工具切換時機」、#68 是「驗收動作分散在四個時點」 — 兩者共用「動作該分配到哪個時點才有 ROI」這個結構。本卡的「第 2 次推理失敗就切工具」≈ #68 的「ship 前要設計 E2E case」 — 都是「把高 ROI 的動作放在對的時點、不要延後」。

...
**正確概念**：「X 在 Y 旁邊」是意圖、不是實作。實作前要把意圖翻譯成具體 layout 結構（grid / flex / absolute / fixed）讓使用者確認。

**替代方案的不足**：直接照位置詞挑一種實作、寫了之後等使用者試 — 多種實作對應同一句話、選錯就重做。

### 文字草圖比直接寫 CSS 便宜

**正確概念**：寫 CSS 前打 5 行 ASCII 草圖、使用者 30 秒內 yes/no — 比寫完 CSS 才被指正便宜得多。

**替代方案的不足**：跳過草圖直接寫 CSS — 看起來省一步、實際多花一輪試錯。

### 描述 layout 主角與疊層

**正確概念**：明確說明哪個元素是 layout 主角、哪些是疊層。使用者可以針對「主角是 main 還是 grid container」這類具體問題指正。

**替代方案的不足**：只描述「X 在 Y 旁邊」、不說明結構 — 使用者看到的是模糊描述、無法精確確認。

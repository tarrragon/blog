---
title: "Pattern：明示語意縮小（不承諾全集）"
date: 2026-04-26
weight: 66
description: "Pattern 卡片：當 filter 必然只能在 subset 上做、明確告訴使用者「這只在已載入範圍內找」、不假裝是全集 filter。對應 #59 策略 E 的具體實作。重點是「明示」、silent 縮小是反模式。"
tags: ["report", "Pattern", "工程方法論", "UX", "Documentation"]
---
...
對應 #59 [Filter × Source 合成策略](../filter-source-composition-strategies/) 的策略 E。
...
「Filter results」沒指明「only loaded」 — 使用者預設是全集 filter、實際是 subset → 撞回 #55 [層錯位](../view-layer-filter-vs-source-layer/) 的語意縫。
...
- E 跟 #55 silent 反模式的差別：**E 是 explicit 縮小、silent 是 implicit 縮小**
...
| 內心 OS：「反正 subset 就是 subset、寫了也沒人看」 | 停 — silent 縮小是 #55 反模式 |

**核心原則**：能接受語意縮小是可以、但必須明示。Silent 縮小（沒告知就 subset）等於 #55 層錯位、是反模式。E 的價值在「明示」這個動作、不在「subset」這個事實。

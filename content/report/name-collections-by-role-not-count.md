...
3. **緊鄰清單的行內計數**：「確認三件事：」後面直接跟三條列 — 數字跟清單同視野、改清單時順手改數字、漂移會立刻被看見。風險低、但仍是小負債、清單就在下面時「確認下列事項：」更省。
   ...
   這條豁免的前提是**同視野**，而有一種寫法看起來合規卻不在同視野裡：「五個節點裡 X 最不直觀，其餘四個不另寫」——分母在上方的表裡、被減數在這一段，兩個數字相距十幾到二十幾行，表增一列要同步改兩處而只有一處在眼前。這種形態不適用本豁免。替換句直接照抄即可，一個數字都不必留：「**其餘節點**不另寫」「**另外那幾列**的後果欄字面即完整」「**剩下的**各自在專節展開」。寫「其餘 N 個」的當下就換掉，比事後掃描便宜——這個形態在同一批稿件裡會於不同檔案獨立重現，因為它是模板句型的一部分而不是個別作者的疏忽。

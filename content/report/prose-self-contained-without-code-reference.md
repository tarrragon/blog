...
| [#360 示範指令的引數要說明](../literal-operand-hides-the-change-point/)                        | 方向相反的一對：本卡管散文賒帳給程式碼（讀者得去翻 code 才讀得懂論述），那張管程式碼賒帳給讀者（區塊裡的引數沒有說明、讀者得自己補語意）。合起來的要求是兩種載體各自自足、不互相賒帳。 |
| [#261 語句要在它的消費單位內資訊自足](../sentence-self-sufficiency-by-consumption-unit/)       | 自包含系譜的三層：本卡管段落對 code、#204 管路由條目對鄰條、#261 管句子對鄰句——#261 把三層共用的軸命名為消費單位、並給出句層的驗收（抽離重讀）。                                       |

---
title: "高 ROI 無外部觸發的工作會被結構性跳過"
date: 2026-04-26
weight: 72
description: "工作有兩個獨立維度：ROI 高低 + 是否有外部觸發。高 ROI + 無觸發 = ROI 的承諾、拖延的現實。靠紀律不可行 — 結構性偏差需要結構性對策（外部觸發 / CI / hook / 排程 / pair）。本卡是 #67 便利反相關、#68 checkpoint 跳過、#69 RED 跳過的共同上位原則。"
...
[#67 我等下會 refactor 是個謊言](../ease-of-writing-vs-intent-alignment/) 已經點到一個面向。把它推廣：
...
- **Refactor（沒功能壓力）** — [#67](../ease-of-writing-vs-intent-alignment/)
- **Test-first 的 RED 階段（修完才補測試）** — [#69](../test-first-red-before-green/)
- **Checkpoint 1（列使用者意圖完整集）** — [#68](../verification-timeline-checkpoints/)
- **Ship 前 E2E case 設計** — [#68](../verification-timeline-checkpoints/)
...
- **Setup observability / log monitor（[#68](../verification-timeline-checkpoints/) Checkpoint 4）**
- **Alert 規則 review**
- **Dashboard 維護**
...
| 原則                                                                          | 跟本卡的關係                                                                                              |
| ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| [#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/)     | #67 是本卡在「寫程式當下選哪條路」面向的展現 — 對齊 = 高 ROI 但無觸發                                     |
| [#68 驗收的時間軸](../verification-timeline-checkpoints/)                     | #68 的「Ship 前 / Checkpoint 1 結構性偏差」是本卡在驗收動作的展現                                         |
| [#69 Test-First](../test-first-red-before-green/)                             | RED 階段被跳過 = 本卡在測試協議的展現                                                                     |
| [#42 2 次門檻](../two-occurrence-threshold/)                                  | 失敗訊號需要被「外部承認」才能觸發轉折 — 跟本卡共骨                                                       |
| [#82 字面攔截 vs 行為精煉](../literal-interception-vs-behavioral-refinement/) | 本卡的 ceiling — L5 hook 只擋字面、行為錯誤需要 L4 review / multi-pass spiral、不是「再寫一條 hook 規則」 |
...
- **`make verify-red-green` script**（[#69](../test-first-red-before-green/)）— L3 工具觸發、把 retrospective 流程從文字協議升級成可執行 target

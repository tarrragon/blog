---
title: "Naming 是 iterated artifact：第一個名字幾乎不對、四輪 review 才收斂"
date: 2026-04-26
weight: 84
description: "命名（變數 / 函式 / 檔名 / slug / API endpoint）幾乎沒有「一次寫對」的可能：第一個名字基於當下狹窄的 context、會在後續 cross-call-site / grep / 重構中暴露錯位。命名的正確設計是 iterated — 寫第一版 → grep-ability 測試 → cross-call-site 一致性 → impl 洩漏 → 重命名。本卡是 #83 在「命名」場景的特化。"
tags: ["report", "事後檢討", "工程方法論", "Pattern", "Naming", "Multi-pass"]
---
...
- **SEO** — 跟 search query 的 substring match 對齊（[#73 search 匹配模式](../search-engine-matching-mode-mismatch/)）
- **kebab-case 一致**
...
| 反模式                                                  | 後果                                                                |
| ------------------------------------------------------- | ------------------------------------------------------------------- |
| 「先這樣、之後再改」                                    | [#72 結構性跳過](../external-trigger-for-high-roi-work/) — 永遠不改 |
...
| 原則                                                                          | 關係                                                                                               |
| ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| [#83 Writing 的 multi-pass review](../writing-multi-pass-review/)             | 本卡是 #83 輪 4 在 naming 場景的特化                                                               |
| [#82 字面攔截 vs 行為精煉](../literal-interception-vs-behavioral-refinement/) | 命名 lint（max length、case style）只擋字面、grep-ability / 一致性 / impl 洩漏靠 multi-pass review |
| [#67 寫作便利度](../ease-of-writing-vs-intent-alignment/)                     | 第一版命名是「容易寫」、不是「對齊意圖」、需要重命名                                               |
| [#73 search 匹配模式](../search-engine-matching-mode-mismatch/)               | URL slug 的命名要跟 search 預期匹配模式對齊                                                        |
| [#44 SSOT](../single-source-of-truth/)                                        | 同概念跨 layer 用同名 = naming SSOT、不該允許多版本同義                                            |

...
- [規則用數量當門檻時，先問那個數量在代理什麼判斷](../count-threshold-is-a-proxy-for-a-checkable-judgment/)：本卡主張「不夠」要量化，該卡限定它的適用範圍——量化在被量的東西就是決策依據時有效；當數字只是某個可直接檢查的判斷的代理時，量化會把判斷擠掉。要量化之前先確認自己在數的是證據還是代理。

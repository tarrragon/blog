---
title: "口語化修辭在判斷工具型段落會稀釋技術精度"
date: 2026-05-06
weight: 111
description: "技術文章的「判斷工具型段落」（讀者用來判斷自己 case 的論述）用「一輩子」「碰巧能用」「立刻撞牆」「沒事」這類口語修辭、會稀釋精度。修法是把口語修辭翻譯回技術屬性語言。但 hook / 引言 / narrative 段落用口語仍然合理——這是情境化的取捨、不是精度永遠優於可讀性。"
tags: ["report", "事後檢討", "工程方法論", "原則", "寫作"]
---

## 論述基礎與限制

本卡的論述基於 **1 個 case**（[dart Stream 事故的 review](../../work-log/dart_stream_controller_single_vs_broadcast/)）抽出來的觀察。具體限制：
...
「一輩子只能 listen 一次」比「整個 lifecycle 只允許被 listen 一次」少 5 個字、寫起來順、語氣強烈。但精度也差距 5 倍。寫作便利度高的版本通常離精度遠（[#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/) 的同骨——寫得順 ≠ 寫得對）。
...
修法：問「在事前、什麼條件成立時這個 X 才會出現？」、改成事前可判斷的描述。詳細處理見 [#110 設計檢討用當下三軸論證、不依賴 hindsight](../design-flaw-by-current-axes-not-hindsight/)。
...
注意 ≠「描述讀者的具體情境」：
> 「讀者讀完事後諸葛論述、能複述 case、下次遇到不同議題卻**判斷不出來**」
...
| 原則                                                                                           | 跟本卡的關係                                                                                                                                                         |
| ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/)                      | 口語修辭比精度敘事好寫——但精度才是讀者要的工具、便利 vs 對齊的同骨展現                                                                                               |
| [#110 設計檢討用當下三軸論證、不依賴 hindsight](../design-flaw-by-current-axes-not-hindsight/) | 「啊原來」「碰巧」這類詞天生帶 hindsight 視角、本卡是 #110 在「字句層級」的延伸                                                                                      |
...
| [#259 轉述與翻譯要保留語意強度量級](../rewrite-preserves-claim-intensity/)                     | 本卡的 keyword bank 跟「翻譯回技術屬性語言」的修法對自產文字有效；材料換成轉述 / 翻譯他人內容時、對照基準要從技術屬性語言換成原文——那一側的檢查見 #259               |
| [#260 誇飾的合法性由段落位置的功能決定](../hyperbole-legitimacy-by-position-function/)         | 本卡「判斷工具型段落精度優先、hook / 引言 / narrative 段落口語合理」這條界線、#260 補上它成立的機制（文體契約 × 行動耦合兩軸）——界線為何劃在那裡、從此有可推導的解釋 |
| [#261 語句要在它的消費單位內資訊自足](../sentence-self-sufficiency-by-consumption-unit/)       | 本卡的精度判斷標準（用詞能反推到機制 / 條件 / 契約）被 #261 收為資訊充足四條件之一；本卡管詞的選擇、#261 管句的成分完整度與指涉閉合——同在字句層、檢查對象不同        |
...
**核心原則**：技術文章的用詞要能反推到具體機制 / 條件 / 契約。口語修辭的便利在於寫得順、代價是讀者拿不到判斷工具。寫完每段後跑一次「這個詞對應什麼具體技術屬性？」自問、對應不出來的詞就要翻譯。

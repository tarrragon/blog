...
**正確概念**：使用者說「還原」時、執行者先 commit 當前狀態作為 checkpoint、再還原。為未來保留可能性。

**替代方案的不足**：直接刪、不留紀錄 — 未來想比較或恢復都做不到、使用者的選項被收窄。

### 確認還原到哪、不憑感覺

**正確概念**：明確問「還原到哪個 commit」 — 上個 commit、特定 commit、完全乾淨都是不同還原。

**替代方案的不足**：執行者憑感覺選一個還原點 — 跟使用者意圖不一致時要再來一次。

### Checkpoint 的 commit message 要說明來由

**正確概念**：Checkpoint 的 commit message 寫清楚「探索什麼、為什麼不採用」 — 未來看到時知道脈絡、不會誤用。

**替代方案的不足**：Checkpoint 用泛用 message（「wip」「temp」） — 未來看不出意義、可能誤刪。

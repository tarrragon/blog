---
title: "Tab Order = DOM Order = Mental Model 三者對齊"
date: 2026-04-26
weight: 71
description: "Tab 順序由 DOM 順序決定（除非用 tabindex 強制覆寫）。三者該對齊：DOM 順序、tab 順序、使用者 mental model 的互動順序。三者不一致時、優先重排 DOM 而非用 tabindex — tabindex > 0 是反模式（[#52]）。"
tags: ["report", "事後檢討", "工程方法論", "原則", "抽象層", "Accessibility", "鍵盤"]
---
...
`tabindex > 0` 反模式（同 [#52 鍵盤可達性](../keyboard-accessibility/)）：
...
<div id="search"></div>  <!-- pagefind input 在裡面 -->
```
...
```js
// PagefindUI mount 後、把 scope 移到 input 跟 drawer 之間（如果 framework 允許）
...
風險：framework 重渲染可能 reparent 回去（[#5 framework-managed DOM](../coexisting-with-framework-managed-dom/)）。要驗證穩定性。
...
| 原則                                                                      | 跟本卡的關係                                                            |
| ------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| [#52 鍵盤可達性](../keyboard-accessibility/)                              | 本卡是 #52「邏輯 tab 順序」要素的展開、含 tabindex > 0 反模式詳解       |
| [#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/) | DOM 順序便利（先寫先 render）、mental model 對齊需要刻意設計 — 反相關   |
| [#43 最小必要範圍](../minimum-necessary-scope-is-sanity-defense/)         | tabindex > 0 是「擴張範圍」反模式 — 一個 tabindex > 0 影響整頁 tab 順序 |
| [#39 native HTML > ARIA](../native-html-over-aria-role/)                  | Native HTML 元素自帶正確 tab 行為、不需要 ARIA tabindex 補              |

---

## 對應的實作篇

- 搜尋頁 scope filter 在 search input 之前的 tab 順序問題 — Checkpoint 1 retrospective 找到（[#68](../verification-timeline-checkpoints/) dogfooding）

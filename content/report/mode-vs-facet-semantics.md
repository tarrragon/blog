...
**正確概念**：影響搜尋演算法的控制（mode）與篩選結果的控制（facet）放在不同 UI 區域、視覺上明確分開、靠位置就能辨識語意層級。

**替代方案的不足**：把所有控制都塞進「filter」區 — 使用者誤以為勾「標題」是過濾標題欄位、實際上改變了搜尋演算法。

### Mode 緊貼 input、Facet 靠近結果

**正確概念**：Mode 控制「如何搜」、視覺上跟 input 綁在一起；Facet 控制「篩什麼結果」、視覺上跟結果區綁在一起。

**替代方案的不足**：所有控制集中在 sidebar — 使用者區分不了哪個影響 query、哪個影響結果。

### 語意層級反映在實作架構

**正確概念**：Mode 在 query 構造階段生效、Facet 在結果處理階段生效 — 實作上也分兩階段（query 階段套 mode、render 階段套 facet）。

**替代方案的不足**：所有控制走同一條 pipeline — 實作上看不出語意層級、未來新增控制不知道該歸哪邊。

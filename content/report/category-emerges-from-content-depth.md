...
本卡抽自 `content/macos/` 分類的建立過程。9 篇 macOS 文章原本放在 `content/other/`（兜底資料夾），寫到第 11 篇時主題群聚變得明顯——APFS 卷結構、Preboot、iOS Simulator、App Container、磁碟診斷、App 聚合佔用、新機設定——這些不是「沒地方放的雜項」，是一個有結構的 macOS 系統管理主題。建立分類是文章累積到臨界量後的自然動作，不是事先規劃的。限制：本卡的 case 是 blog 這種持續累積的內容體系，一次性交付的文件專案可能適合先規劃分類。

## 核心原則

分類是內容的組織結構，它的合理性取決於底下有沒有足夠的內容支撐。文章拆分到獨立機制的深度後（[#211](../compound-problem-decompose-then-interact/)），主題群聚會自然浮現——同一個領域的概念各自有了獨立文章，它們之間有引用關係，放在一起比散落在兜底資料夾更容易被發現和導航。

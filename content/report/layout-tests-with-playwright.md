...
**正確概念**：版型被 debug 第 2 次、表示這地方很容易壞、寫成測試把契約固定下來。第 3 次有人改時測試立刻紅。

**替代方案的不足**：每次都用截圖手動驗證 — 累積的「我記得改這個會壞」散落在工程師腦中、新人接手不知道。

### 結構斷言比視覺截圖好維護

**正確概念**：用 `expect(scopeY > inputY)` 描述「scope 在 input 之下」 — 跨字型、跨主題都對。

**替代方案的不足**：snapshot 視覺比對 — 字型微調、theme 換色都觸發 false positive、需要常 update snapshot、最後變成自動 update（測試失效）。

### 測試本身是文件

**正確概念**：寫 `test('scope UI 在三互動狀態下都在 input 與 results 之間', ...)` — 標題描述契約、內容描述驗證方式。

**替代方案的不足**：版型契約只記在 commit message 或 wiki — 寫了就忘、新人不會主動找。

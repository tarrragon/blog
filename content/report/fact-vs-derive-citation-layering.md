...
| [#249 對當下段落沒有收益的標註不會自發發生](../annotation-with-no-local-payoff-needs-a-trigger/) | 同一批標註、切入點不同 — 本卡給事實與推導的分層標準，那張卡說這個區分為什麼寫的時候不會自己發生（標「這是推導」對當下段落沒有收益），需要的是觸發時機而不只是判斷標準 |

...
**正確概念**：把版型驗證拆成多階段、每階段只測試一件事。版型對了再加內容、內容對了再加樣式。

**替代方案的不足**：一次組裝完整 UI、版型錯時面對「位置、尺寸、字型、padding、margin」全部一起出問題 — debug 從哪裡下手都可能錯。

### 色塊比真實內容更易 debug

**正確概念**：版型問題用色塊驗證 — 顏色明顯、尺寸寫死、無視覺干擾。

**替代方案的不足**：用真實內容（pagefind UI、文章標題等）debug 版型 — 內容自帶字型、padding、margin，跟版型問題混在一起。

### 標明「驗證版型」狀態

**正確概念**：色塊內含文字註明用途，避免被誤認為正式設計、避免 commit 時誤留。

**替代方案的不足**：純色色塊不加標籤 — 容易誤認為正式設計、或 commit 時忘記移除。

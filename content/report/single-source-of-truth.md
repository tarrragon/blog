---
title: "Single Source of Truth：值的住址只能有一處"
date: 2026-04-25
weight: 44
description: "同一個值（CSS token、視覺基準、runtime 量測）的權威來源只能有一個位置 — 多源時會分歧、會漏改、會讓讀者不知道哪個生效。本文是 #3 / #26 / #27 三篇實作的共同抽象。"
...
/* Fact — 設計決定 */
:root {
  --search-title-h: 64px;        /* H1 高度是設計選擇 */
  --search-form-h: 68px;         /* form 高度是設計選擇 */
  --search-gap: 20px;            /* 間距是設計選擇 */
}

/* Derivation — 從 fact 計算 */
.scope-position {
  top: calc(var(--search-title-h) + var(--search-form-h) + var(--search-gap));
...
對應實作：[#26 CSS 變數定義位置統一](../css-variable-single-location/)。
...
--search-title-h: 64px;            // 寫死
--search-form-h: 68px;             // 寫死
--search-gap: 20px;                // 寫死
--search-scope-h: ResizeObserver;  // runtime 量測寫回
```
...
對應實作：[#27 runtime 量測模式統一](../runtime-measurement-unification/)。
...
```text
問題：filter padding-top 對不準 H1 + form 下緣
...
對應實作：[#3 視覺對齊用單一真實來源](../visual-alignment-single-source-of-truth/)。
...
.search-shell .pagefind-ui__drawer {
  /* 分散引用 */
  margin-top: calc(var(--search-title-h) + var(--search-form-h));
}
```
...
```css
/* 好 — derivation 用 calc */
.scope-position { top: calc(var(--a) + var(--b)); }

/* 較差 — derivation 寫具體數字 */
.scope-position { top: 152px; }  /* 152 是 a + b 算出來的、現在固化在這裡 */
```
...
- [#263 同一個對象被兩篇各自分解一次時，不相容會長得像互補](../incompatible-decompositions-look-complementary/)：本卡在「分解方式」這個對象上的形態。本卡管一個值的住址，那張卡管一組成員的住址——而多重住址在分解方式上更難偵測：同一個值的兩個版本直接矛盾，兩套分解各自完整、還會因為互相連結而看起來像分工。

[#43 最小必要範圍](../minimum-necessary-scope-is-sanity-defense/) 處理「範圍越窄越穩定」、本原則處理「值的住址越唯一越穩定」。兩者方向不同、目的相同 — 都是讓行為可預測：
...
| 篇                                                                       | SSoT 違反類型          | 修正方向               |
| ------------------------------------------------------------------------ | ---------------------- | ---------------------- |
| [#3 視覺對齊用單一真實來源](../visual-alignment-single-source-of-truth/) | 對齊基準分量值來源不明 | 每分量都要有明確住址   |
| [#26 CSS 變數定義位置統一](../css-variable-single-location/)             | 變數定義散多 selector  | 集中在使用範圍的最高層 |
| [#27 runtime 量測模式統一](../runtime-measurement-unification/)          | 寫死跟量測混搭         | 選一邊走到底           |

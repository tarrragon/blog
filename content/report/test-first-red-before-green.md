---
title: "Test-First：先看到 RED 才相信 GREEN"
date: 2026-04-26
weight: 69
description: "一個只看過 GREEN 的測試是「未驗證的訊號」、不是「會抓回歸的測試」。必須先在「該失敗的版本」上看到 RED、再在「該通過的版本」上看到 GREEN — 兩次跑都對、才能相信測試真的 catch 到該 catch 的東西。跳過 RED 等於把驗收標準降到「跑得通」、漏掉「測試自己有沒有 bug」這層。"
tags: ["report", "事後檢討", "工程方法論", "原則", "抽象層", "TDD", "驗收"]
---
...
對應 [#42 2 次門檻](../two-occurrence-threshold/)：一次 GREEN 是低資訊量訊號、RED → GREEN 是 2 次跑（一次 fail 一次 pass）的高資訊量訊號。
...
| 原則                                                                      | 跟本卡的關係                                                                    |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| [#42 2 次門檻](../two-occurrence-threshold/)                              | 一次 GREEN 是低資訊量訊號、RED → GREEN 是 2 次跑（一次 fail 一次 pass）的真訊號 |
| [#56 視覺完成 ≠ 功能完成](../visual-completion-vs-functional-completion/) | 測試 PASS ≠ 測試 verified；同個「訊號需要驗證」結構                             |
| [#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/) | 跳過 RED 是便利（不用切 branch / 不重 build）、走 RED-GREEN 是對齊              |
| [#68 驗收的時間軸](../verification-timeline-checkpoints/)                 | 本卡是 Checkpoint 2「開發中」+ Checkpoint 3「Ship 前」內部的具體協議            |
...
- [#11 playwright-early-in-loop](../playwright-early-in-loop/) — 第 2 次推理失敗切 playwright；切過去後寫的 evaluate query 跑 RED-GREEN 才驗證
- [#15 layout-tests-with-playwright](../layout-tests-with-playwright/) — 版型 debug 兩次以上寫測試固化；測試該先在「未修版型」跑 RED 才相信
- [#23 verification-method-timing](../verification-method-timing/) — 驗證方法選對之後、實際驗證需要 RED-GREEN
...
跳過 RED 是 [#72 高 ROI 無外部觸發的工作](../external-trigger-for-high-roi-work/) 在測試協議的展現 — 修法不是「下次記得」（L1 紀律會失敗）、是 `make verify-red-green PRE_FIX=<sha>`（L3 工具觸發）+ pre-commit hook 提醒（L3 結構觸發）。

...
關鍵修法是**正向直述 B**：「X 不是 A、而是 B」→「X 是 B」、把 A 的排除留給上下文或反例段。把「不是 A」翻成「而非 A」只是換個負向詞、仍違規。
...
被合理化放行的是**特定一類**（建立概念的否定句構），有系統性偏移。同一個寬鬆預設會在每篇文章放行同一類違規 —— 跨稿件累積成 systematic miss、跟 #114 的偵測層 systematic miss 同構、只是發生在判定層。
...
[#288 關鍵詞清單換一個體裁就失準](/report/keyword-pattern-does-not-transfer-across-genres/) 管的是命中之前的涵蓋範圍，本卡管的是命中之後的語意判定。兩者的失敗方向相反而且互不覆蓋：本卡處理「抓到了而判錯」，那張卡處理「根本沒抓到」。一次實測顯示兩者可以同時發生而只有前者被看見——一位審查者逐處驗過某一類的 86 個命中、判定全部正確，而該體裁的主流違規形態一處都不在那 86 個裡面。

[#289 一個詞是不是通用術語，用多份獨立定義的收斂度量](/report/term-probe-measures-register-not-invention/) 把本卡的不對稱搬到另一種訊號上：探針的回報跟 grep 的命中一樣是候選，中間隔著一個歸因步驟。差別在那裡的歸因要離開稿件去查別的領域有沒有這個詞的穩定用法，而不是回原文找字串——把語域錯配判成自創詞，就是那一步被跳過的形態。
...
| [#291 掃描指令壞掉時不會報錯，它報一個數字](../broken-scan-reports-a-plausible-number/) | 同一條鏈的最下游。#291 問指令跑到了嗎、#288 問 pattern 涵蓋這個體裁嗎、本卡問這個命中算不算違規——三個問題依序發生，跳過前兩個會讓最仔細的語意判定落在錯的集合上 |

---

## 適用範圍與邊界

- **適用**：正向陳述優先這類「同一訊號命中、語意角色相反」的字句層規範 review；AI 輔助寫作的 self-review（最容易在判定層放水）；keyword bank 已建立、但 review 回報品質仍不穩的情境。「偵測曝光候選、判定留給語意」這條兩步驟結構、同樣適用於字句層之外的警告層偵測——lint 的位置引用掃描（REF1 / REF2）、翻譯的量級檢查（鄰詞存在測試命中後仍要判意譯 / 慣例語氣）都沿用本卡的分工。

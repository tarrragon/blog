---
title: "Pattern：預先建獨立 index（每種 mode 一份）"
date: 2026-04-26
weight: 65
description: "Pattern 卡片：build time 為每種 filter mode 各建一份 source / index、runtime 切換 mode = 切 source。對應 #59 策略 C 的具體實作。前提是能控 source 的 build pipeline、且 mode 數量有限。"
...
對應 #59 [Filter × Source 合成策略](../filter-source-composition-strategies/) 的策略 C。
...
- Source 的 query 引擎不支援該 filter（不能用 #61 推進 query）
...
```bash
# Pagefind 範例：兩份 index 各自掃不同 region
pagefind --site public --output-subdir _pagefind-all
pagefind --site public/title-only --output-subdir _pagefind-title

# 或用 --root-selector 限定 source 範圍
pagefind --site public --root-selector ".post-title" --output-subdir _pagefind-title
```
...
| 儲存空間   | 每份各自占用（pagefind 約 site 大小 2-5%）     |
...
- **C + 推進 query (A)**：在每份 index 內、再用 query filter 細分（如 `_pagefind-post.search('css', { filters: { tag: 'js' } })`）
...
for type in post page tutorial faq doc; do
  for tag in js css html ts py; do
    for date in 2020 2021 2022 2023 2024; do
      pagefind --filter "type=$type tag=$tag date=$date" --output-subdir _pagefind-$type-$tag-$date
    done
  done
done
```
...
```bash
pagefind --site public --output-subdir _pagefind-title
# build 完後沒過濾、其實 index 了 title + content 全部
```

如果只是改 output 路徑、沒改 index 範圍 → 兩份 index 內容一樣、白做。要用 `--root-selector` 或 `data-pagefind-body` 標記正確範圍。

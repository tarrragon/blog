...
**正確概念**：盤點時同時看「跑幾次」與「每次多重」、用乘積評估總成本。

**替代方案的不足**：只看單次計算（「這個 regex 應該很快吧」）— 忽略迭代次數放大效應、規模一變就爆。

### 規模放大的預估

**正確概念**：對每個迭代寫下「10x / 100x 規模時的預估時間」、用這個數字決定現在優化還是等遇到再說。

**替代方案的不足**：用「現在感覺 OK」當判準 — 上線資料量比測試大時才發現問題。

### Cache 與 IntersectionObserver 是兩種獨立優化

**正確概念**：Cache 縮「重複計算」、IntersectionObserver 縮「無謂迭代」。兩者解不同問題、可以並用。

**替代方案的不足**：把優化當單一動作、選一種就停 — 兩種風險都有時、解一種還剩一種。

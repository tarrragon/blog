...
description 給人判斷也給機器（AI）判斷，每個 token 都要有資訊量。三個不要：

1. **不要操作細節**——指令、參數、具體步驟留在內文。description 幫人決定「要不要點進去」，不是把文章壓縮塞進來。
2. **不要假設前提**——「想不起來」「卻發現」「卻不確定」「不知從何下手」這些描述讀者的困境，不是資訊。直接說用途。
3. **不要情緒修辭**——「別被騙」「打架」「空轉」「踩坑」這些不精準。用事實陳述取代。
4. **不要內嵌數字**——「三項基礎建設」「6 段結構」「兩個陷阱」把成員數烤進 description，內文增刪就要同步改。description 描述文章的功能（解什麼問題），不描述文章的結構（幾段幾項）。理想的 description 無論內文怎麼調整都不用變動。同 [#156 集合命名不內嵌數量](../name-collections-by-role-not-count/)。

對比：

| 有問題                                         | 精準                                         |
| ---------------------------------------------- | -------------------------------------------- |
| 想不起 figure shortcode 還是 Markdown 時查這篇 | Hugo 文章插圖的寫法與路徑規則                |
| 別被 fail 訊息指的 test 騙                     | fail 位置看 `+N -1` 累計而非訊息標示的 test  |
| 套模板每篇結構都打架時回來                     | migration playbook 的結構選型流程            |
| 三項基礎建設的依賴順序                         | 底層基礎建設的依賴順序（數字隨內容變會過時） |

150 字上限不是要塞滿的配額。短能到位就短。
...
- [#364 摘要另一份內容時寫出成員，計數只告訴讀者清單有多長](../counted-summary-delivers-only-the-length/)：本卡「不內嵌數字」的理由是數字會過時；#364 補上另一個理由——數字在沒過時的時候也只交付長度，讀者從「四類邊界」得不到任何一類是什麼。

---
title: "搜尋引擎的匹配模式跟使用者預期的對齊"
date: 2026-04-26
weight: 73
description: "搜尋引擎的匹配模式（prefix / substring / fuzzy / semantic）各有不同。預設多半是 prefix（為了 index size）、但使用者被 Google 訓練成預期 substring。沒對齊 = silent 失敗：搜「pre」找不到 backpressure。本卡展開五種匹配模式、跟使用者意圖的對齊協議、五個合成策略。"
tags: ["report", "事後檢討", "工程方法論", "Search", "Architecture"]
---
...
| Prefix    | `pre`、`prefix`、`prefetch`、`presence` | Pagefind / Lunr 預設 |
...
這是個典型的 [#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/) — 工具預設是「實作便利位置」、不是「使用者意圖位置」。

---

## 為什麼這個 gap 是 silent

跟 [#55 Filter × Source 層錯位](../view-layer-filter-vs-source-layer/) 共用結構：使用者打字看到結果列表、結果不空、看起來「有東西」、不會懷疑 engine 沒在做完整 search。
...
對照 [#55 silent 失敗條件](../view-layer-filter-vs-source-layer/) 三條件 — 完全一樣的結構：「有部分結果掩蓋了缺口」。
...
## 五種對策（跟 [#59 Filter × Source 五策略](../filter-source-composition-strategies/) 同構）
...
把限制告訴使用者：「搜尋為前綴匹配、想找 X 請打 Y」。對應 [#66 明示語意縮小](../pattern-explicit-semantic-narrowing/)。
...
不做任何處理、silent 接受。這是反模式（同 [#55 silent 失敗](../view-layer-filter-vs-source-layer/)）— 使用者誤以為「沒有相關內容」、放棄。
...
| 原則                                                                      | 跟本卡的關係                                                                  |
| ------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| [#55 Filter × Source 層錯位](../view-layer-filter-vs-source-layer/)       | 都是「使用者意圖跟工具實際行為的 silent gap」、本卡是 matching 維度的展現     |
| [#63 資料源的形狀](../data-source-shape-defines-feature-shape/)           | 形狀是 source 的 capability 維度、本卡是「matching mode」這個 capability 維度 |
| [#67 寫作便利度跟意圖對齊反相關](../ease-of-writing-vs-intent-alignment/) | 工具預設是實作便利、使用者預期是 mental model 對齊、反相關                    |
| [#68 驗收的時間軸](../verification-timeline-checkpoints/) Checkpoint 1    | 「source capabilities 是否對齊使用者預期」屬意圖完整集 — 容易跳過             |
| [#72 高 ROI 無外部觸發](../external-trigger-for-high-roi-work/)           | 「讀 search engine docs 確認 matching mode」沒便利路徑、容易跳過              |
...
| Pagefind / Lunr / 任何 static site search             | 預設 prefix、要主動評估是否符合需求                 |

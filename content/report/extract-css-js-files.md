...
**正確概念**：Inline 程式碼超過 30 行就拆檔、走 Hugo resources pipeline、得到 minify + fingerprint + syntax highlight 多重收益。

**替代方案的不足**：「就先 inline、之後再說」 — template 越改越長、最後 200+ 行混雜難維護、拆檔的工作只是延後、不是省掉。

### 拆檔走 resources.Get、不直接 link

**正確概念**：用 Hugo `resources.Get | minify | fingerprint` 引入、得到自動快取與整合驗證。

**替代方案的不足**：直接 `<link href="/search.css">` — 沒 minify、沒 fingerprint、改檔案後 cache 可能用舊版。

### Template 只放 HTML 結構

**正確概念**：拆完後 template 內只剩 HTML、Hugo template 邏輯、CSS / JS 引入 tag。專注於 markup。

**替代方案的不足**：拆完後 template 還塞 inline 補丁 CSS — 沒貫徹、依然混雜。

...
date: 2026-06-14
weight: 167
description: "修法（改寫違規句、補 lint 規則、改 pattern）這個動作本身會引入新違規、而且引入的常是同一類問題的變體——修「不是 X、而是 Y」就暴露「不是 X — 是 Y」、補一個 pattern 就漏下一個變體。所以 review 的 scope 要涵蓋修法後的產物、不只原始內容；停止判斷不能停在「修完這批」、因為修法本身產生新一批。同類變體靠判斷標準（重點位置）收斂、不靠窮舉 pattern；補 pattern 永遠追不上變體（打地鼠）。"
...
本卡處理的是同成因的變體，因此下一輪近 frame 抓得到。另有一類修法產生的問題由兩次**不同**成因、各自正確的修法合成，對兩個 frame 都不可見——那一類逐條看修法產物仍然看不見，要一次以整體為對象的通讀，見 [#247](../sequential-fixes-compose-into-defects/)。

---

## 為什麼修法引入的是同類變體

修法用的是跟原作者同一套文體直覺、同一個對問題的理解。改一個表面實例時，注意力放在「這一處」、而同一個底層成因（重點後置的偏好、某個高頻句型）在別處或下一個變體仍在。修「而是」連接詞、沒動到「重點後置」這個成因，於是「— 是」「不在…而在」這些同成因的近親一個個浮現。補 pattern 也一樣：每補一個連接詞、成因（重點後置）沒被判斷標準收斂、下一個連接詞的變體就漏。
...
成因層的判斷標準（[#166](../lead-with-the-point-cross-language/) 的「核心概念在不在最前」）一次涵蓋所有連接詞變體；表面層的 pattern 永遠枚舉不完（擴兩次仍冒出第四個變體）。所以工具鏈的 pattern 定位是「曝光候選」、收斂靠判斷標準——pattern 漏一個變體只是讓候選 silent、判斷標準仍能在人讀到時抓出。有些變體（「、是」頓號版）誤判率高、刻意不補進 pattern、正是「判斷標準優於窮舉」的活證明。

### 停止判斷含修法產物

「修完這批違規」不是停止訊號。修法產生新一批、停止判斷要看「frame 涵蓋 + 判斷標準到位」（[#148](../cross-round-review-stopping-signal/) 的延伸）、不看「這批修完了」。
...
- **[#166 重點優先陳述是跨語言的資訊結構原則](../lead-with-the-point-cross-language/)**：#166 講「句型 grep 枚舉不完、判斷標準是重點位置」（靜態原則），本卡是它的過程面——修法 / 補 pattern 的動作會反覆暴露同類變體，正是「枚舉不完」在 remediation 過程的動態展現。#166 self-case 記了完整的 pattern 連接詞擴展軌跡。
...
| 補 pattern / 規則後沒重掃同類風險區                    | 補完只解了已知變體、下一個變體要靠重掃或判斷標準 catch     |
| 把希望放在「補完所有 pattern 變體」                    | 打地鼠訊號 —— 表面層追不上成因、回到判斷標準（成因層）收斂 |
| 停止判斷的理由是「這批修完了」                         | 不是停止訊號 —— 修法產生新批、看 frame 涵蓋 + 判斷標準到位 |
...
- **邊界**：「修法引入同類變體」不等於「不該修」——該修、但修完要把修法產物納入下一輪 scope，且收斂靠成因層判斷標準不靠表面窮舉。
...
這條軌跡共同指向一個教訓：修法是新違規的來源、且引入的是同類變體；表面層的 pattern 窮舉追不上成因層、收斂要靠判斷標準（[#166](../lead-with-the-point-cross-language/) 的重點位置）、review scope 要把修法產物納進來。

...
**核心判斷標準**：升級成本 vs 繼續嘗試成本的比較。當每次嘗試都很貴（人類幾分鐘 / 來回溝通 / 上下文切換）、2 次就該升級；當嘗試很便宜、可以放寬到 3-5 次。

**計數會倒退**是另一種情形，它與上表那幾種不同——門檻仍然適用，只是已經數到的次數可能減少。門檻用在立卡這類判斷上時，第 2 個實例可能在後續審查中被判定不成立，於是計數退回 1。[#263 同一個對象的兩套分解](../incompatible-decompositions-look-complementary/) 的證據史記了完整過程——立卡當下數到兩個實例，其中一個後來被對抗性審查判定為粒度差而降級，門檻退回一個，直到另外找出三組才真正成立。這件事對本卡的意思是：**計數的每一項都帶著判定，而判定本身可被推翻**，所以「已達 2 次」不是一個完成後就固定的狀態。實務上的處置是記下每一次計數各是憑什麼算進去的——只記數字的話，某一項被推翻時無從知道總數該退回多少。
...
本卡的 2 次之所以站得住，是因為數的東西就是證據本身——同一個問題重複出現，重複性正是「這不是運氣」的唯一來源，沒有更直接的檢查可以取代它。規則裡的另一類數量門檻不是這樣：它們用數字頂替一個可以直接檢查的判斷，而那種門檻在使用者角度會失效。兩者的分辨方式與替換做法見 [規則用數量當門檻時，先問那個數量在代理什麼判斷](../count-threshold-is-a-proxy-for-a-checkable-judgment/)。

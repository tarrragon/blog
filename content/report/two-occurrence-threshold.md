...
本卡的 2 次之所以成立，是因為數的東西就是證據本身——同一個問題重複出現，重複性正是「這不是運氣」的唯一來源，沒有更直接的檢查可以取代它。規則裡的另一類數量門檻不是這樣：它們用數字頂替一個可以直接檢查的判斷，而那種門檻在使用者角度會失效。兩者的分辨方式與替換做法見 [規則用數量當門檻時，先問那個數量在代理什麼判斷](../count-threshold-is-a-proxy-for-a-checkable-judgment/)。

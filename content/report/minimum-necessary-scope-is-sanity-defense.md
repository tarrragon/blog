...
| 議題            | 範圍對象               | 失敗模式                                | 對應實作篇                                                               |
| --------------- | ---------------------- | --------------------------------------- | ------------------------------------------------------------------------ |
| JS 元件邊界     | 「我可以動什麼」的契約 | 越界操作 framework 管的部分、被重繪清掉 | [#13 元件邊界與 JS 操作的影響範圍](../component-boundary-and-js-impact/) |
| Selector 精準度 | 「query 命中哪些元素」 | 誤命中、未來結構變動就壞                | [#14 Selector 精準度](../dom-selector-precision/)                        |
| Observer 範圍   | 「監聽哪些變動」       | 過度觸發、layout 抖動、無限循環         | [#29 MutationObserver 範圍與觸發頻率](../mutation-observer-scope/)       |
...
[#42 2 次門檻](../two-occurrence-threshold/) 處理「失敗第 N 次該換策略」、本原則處理「第一次設計時範圍該多大」。兩者方向互補：
...
| 篇                                                                       | 議題              | 範圍對象                   |
| ------------------------------------------------------------------------ | ----------------- | -------------------------- |
| [#13 元件邊界與 JS 操作的影響範圍](../component-boundary-and-js-impact/) | JS 元件邊界       | 「我可以動什麼」契約       |
| [#14 Selector 精準度](../dom-selector-precision/)                        | DOM query 範圍    | 起點 / 範圍 / 過濾三維度   |
| [#29 MutationObserver 範圍與觸發頻率](../mutation-observer-scope/)       | Observer 監聽範圍 | root / option / 頻率三維度 |

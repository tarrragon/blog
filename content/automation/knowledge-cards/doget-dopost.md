...
一個影響靜態站的關鍵限制是：Apps Script **沒有** `doOptions`。跨網域請求的 CORS preflight 會送 `OPTIONS`，但它打不到你的程式、平台的預設回應也不帶 CORS 許可標頭，於是 preflight 失敗、真正的請求送不出去。這是「用 `fetch` 送 `application/json` 打 Apps Script 得到 CORS 錯誤」的根因，繞法是讓請求成為不觸發 preflight 的 simple request，見[前端 beacon 與 CORS 障礙](/automation/02-analytics-beacon/frontend-beacon/)。

...
時間觸發器（time-driven trigger）是讓 Apps Script 在固定時間自動執行某個函式的排程機制。它把膠水層從「被動等 HTTP 請求呼叫」（見 [doGet / doPost](/automation/knowledge-cards/doget-dopost/)）變成「主動定時執行」——不需要有人打網址，到點就自己跑。流量統計用它把 beacon 累積的原始 log 每天彙總成一張日報表，實作見[模組四](/automation/04-triggers-automation/)。

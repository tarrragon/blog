...
這個案例的核心責任是說明 stub 的結構性限制：**stub 回放的是測試作者的假設，假設錯了，測試照樣綠燈**。當 bug 的成因是「我們對後端行為的理解錯誤」時，用自己假設餵出來的 stub 永遠驗證不出來。

## 觀察

一個 APP 專案（前端 + 後端 API）。前端會為後端推送的事件建立本地追蹤記錄，記錄裡凍結了事件當下的兩個後端 id：所屬單據的 id 與明細列的 id。後續的「取消」「追加」操作用這兩個凍結 id 回寫後端。
...
| 修復         | 動作時活解析：以跨合併不變的事件 id 反查當前快照，取得現在有效的 id；凍結值只做查無時的退路 |

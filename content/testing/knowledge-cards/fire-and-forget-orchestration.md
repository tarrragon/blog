...
識別 fire-and-forget 導致 flaky 的訊號：同一條測試單獨跑通過、與其他測試檔合跑失敗，且失敗在「狀態尚未更新」或「副作用尚未發生」的斷言上。典型案例：結帳流程收尾（列印、狀態清理、資料同步）沒有被 await，測試斷言在收尾完成前執行（[T.C8](/testing/cases/fire-and-forget-test-race/)）。

...
這也讓它成為[判斷標準由實作推導](/testing/knowledge-cards/test-provenance/)這件事在站內唯一的合法形態：預期值刻意取自被測程式的實際輸出，是設計選擇而不是失誤。代價要算清楚——它因此抓不到需求被誤讀那一類，保留下來的驗證力限於崩潰、回歸與內部不一致。把它當成正確性測試用，就是把一個刻意的取捨誤當成保證。

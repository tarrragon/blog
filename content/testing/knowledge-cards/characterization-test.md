...
當依賴纏結到必須先拆解才能寫正確性測試、而拆解本身就可能改壞行為時，characterization test 補上這段真空期的回饋——這是它唯一的存在理由。這個處境在 [legacy 專案的起步順序](/testing/01-test-strategy-layers/legacy-test-bootstrap/)的「遷移安全網」段有完整的操作判準——包含三種起步路徑各自何時需要它。

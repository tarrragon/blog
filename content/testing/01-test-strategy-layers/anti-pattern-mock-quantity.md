...
一個遠端終端機 app 的 192 個 unit test 覆蓋了 `ConnectionManager`、`AnsiParser`、`TerminalBuffer` 等元件的邏輯分支。如果在 mock test 全過但實機失敗後，反應是「再寫 50 個 test」，新寫的 test 會使用同一個 `FakeWebSocketChannel`，測試更多的邏輯分支 — 更多的輸入組合、更多的邊界條件、更多的錯誤處理路徑。
...
覆蓋率 100% 意味著「在 mock 環境中，所有程式碼分支都被走過」。這不等於「在真實環境中，所有程式碼分支的行為都是正確的」。該 app 的 `sendData()` 在覆蓋率報告中是「已覆蓋」的，但覆蓋它的 test 用的是不區分 frame type 的 fake。
...
打破這個循環的起點是區分「同層覆蓋率不足」和「層級缺失」。如果問題是同層覆蓋率不足（某個分支沒被 test 走到），加 test 有效，前提是新增的測試有能區分對錯的斷言——這一條在測試可以整批產生時要[另外量](/testing/06-agent-authored-code/coverage-to-mutation-gate/)。如果問題是層級缺失（mock 結構性地遮蔽了某類行為），加同一層的 test 無效，需要引入新的 test 層級。

## 判讀訊號

數量還有另一種上游成因，跟遮蔽無關：替身數量隨測試邊界的位置變動——邊界畫在單一類別時，每個協作者各要一個替身，數量是設計選擇的下游而不是盲區的症狀。兩種成因的處方不同，判斷標準見 [Sociable vs Solitary Unit Test](/testing/knowledge-cards/unit-definition-two-schools/)。
...
區分「同層覆蓋率不足」和「層級缺失」之後，回到[三層定義與職責表](/testing/01-test-strategy-layers/three-layer-definition/)確認每層的邊界。Mock 遮蔽的結構性原因在 [Mock 遮蔽機制分析](/testing/01-test-strategy-layers/mock-masking-mechanism/)中展開。如果判斷結果是層級缺失，[判斷原則：什麼時候需要 protocol integration test](/testing/01-test-strategy-layers/when-protocol-integration-test/) 提供引入新層級的決策流程。

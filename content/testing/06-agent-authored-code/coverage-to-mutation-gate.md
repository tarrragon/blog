...
這五步自己也是機械約束——[#278](/report/mechanical-constraints-buy-the-measured-number/) 要求每個機械約束配一個它量不到的維度，配對的形式是把每一個判定問題的答案寫成一列：問了什麼、答案是什麼、依據是哪一條可指認的材料（需求文件的哪一段、想得到的那個具體錯誤寫法），而判成不適用或暫緩時這一列必填——遵循會留下測試碼，豁免什麼都不留，所以舉證的責任落在豁免的那一方。這裡只補一件它特有的：門檻採「只要求不倒退」時，最省力的達成是讓新程式落在掃描範圍的 glob 之外，所以**納入率要跟分數並列著看**，兩個數字缺一個就讀不出真相。

...
**等價突變。** 改寫之後行為完全相同。典型形態有兩種：一是等價的邊界寫法（`i < n` 換成 `i <= n - 1`），二是對結果沒有影響的短路順序（兩個都是純函式的條件互換前後）。這是工具的限制，要標記排除。為等價突變硬補斷言的代價是那條斷言必然綁在實作的寫法上，反過來傷害重構安全性，與[行為優先的 TDD](/record/behavior-first-tdd-methodology/) 要守的性質直接衝突。

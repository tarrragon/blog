...
測試**抓出需求被誤讀**的能力，取決於它的判斷標準是不是由實作推導出來的。判斷標準另有來源時，測試才有機會撞見實作對需求的誤解；判斷標準從實作推導時，[oracle](/testing/knowledge-cards/test-oracle/) 在這一項上退化成實作自身的回音。其他幾種驗證力不受影響——崩潰、回歸、內部不一致照樣抓得到，「判斷標準被污染時還抓得到什麼」那一節的表把界線畫出來。
...
獨立性可以被設計，能動的有四件事：需求的來源、產出的順序、產出的主體，以及時間間隔。前三件是本節展開的重點；**時間間隔**在開場已經出現過：寫實作與寫測試之間隔一段時間，中間發生一次重新理解需求的動作。它在人手寫的年代是免費附贈的，產出速度提高之後要刻意安排，而它也是四件裡最不可靠的一件（間隔不保證有人真的回去讀需求）。
...
一次性的探索腳本、用完就丟的資料轉換（「用完就丟」是對未來的預測，要說得出丟棄的時點或條件，說不出來的多半不會被丟）、以及[characterization test](/testing/knowledge-cards/characterization-test/) 這種本來就以現狀為判斷標準的測試，同源不構成問題——探索腳本與資料轉換的錯誤代價低到不值得建立獨立判斷標準，characterization test 的設計意圖就是複製現狀。

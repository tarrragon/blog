...
還有一種情況落在這張決策表之外：驗證對象是「對後端行為的假設」本身。mock 與 stub 回放的內容由測試作者寫入，假設與斷言出自同一人之手——假設錯了，測試照樣綠燈，結構上驗證不出來。這時需要的是持有狀態、只固化已證實行為的[語意級假後端](/testing/01-test-strategy-layers/semantic-fake-backend/)。

...
本文承接 [終端機圖形化工具總覽](/cli/cli-graphical-tools-overview/) 的訊息佇列客戶端分類。broker 端的純指令操作與 vendor 選型見 [Kafka](/backend/03-message-queue/vendors/kafka/)、[Redis Streams](/backend/03-message-queue/vendors/redis-streams/)、[RabbitMQ](/backend/03-message-queue/vendors/rabbitmq/) 服務頁。

## 跟 SQL 客戶端最大的不同：多半綁單一 broker 協議

訊息佇列 TUI 幾乎都綁定單一 broker 協議，這是選型要先認清的一點，也跟 SQL 客戶端剛好相反。[SQL 客戶端](/cli/sql-database-clients/) 一個工具靠 adapter 連 Postgres、MySQL、SQLite 多種資料庫；訊息佇列這邊，Kafka 的 TUI 說的是 Kafka protocol、不認 AMQP，RabbitMQ 的 TUI 走 management API、也不讀 Kafka topic。能同時連多種 broker 的工具是少數例外（見後文 queuepeek）。
...
- 同範式的資料庫客戶端對照：[終端機 SQL 客戶端](/cli/sql-database-clients/)。
- 把客戶端擺進可持久化的多工器 pane：[tmux 基礎](/cli/tmux-persistence-and-basics/)。
- 訊息佇列客戶端在遠端工具分類中的定位：[終端機圖形化工具總覽](/cli/cli-graphical-tools-overview/)。

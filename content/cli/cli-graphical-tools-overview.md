...
- TUI 工具：系統監控（`btop` / `htop` / `k9s`）見 [TUI 監控工具](/cli/tui-monitoring-tools/)；git 線圖（`tig` / `lazygit` / `gitui`）屬版控子題、見 [遠端 CLI 開發的 git 線圖工具選型](/cli/git-line-graph-tools-for-remote-cli/)

...
本文承接 [終端機圖形化工具總覽](/cli/cli-graphical-tools-overview/) 的檔案瀏覽分類。
...
安裝依賴是遠端的隱性分界。編譯型工具（broot、yazi、lf、nnn）搬一個 binary 就近可用；`ranger` 的 Python 依賴在不給裝套件或 Python 版本尷尬的機器上較麻煩。編譯型的單一 binary 仍要留意 glibc／musl 對得上目標系統，這點與 git 工具相同，見 [git 線圖工具選型](/cli/git-line-graph-tools-for-remote-cli/) 的單一 binary 注意事項。

預覽能力有一條邊界要先知道：程式碼與純文字檔的預覽在任何終端機都穩定，但圖片預覽需要終端機支援影像協定（sixel／kitty）。純文字遠端通道下，沒有影像協定時圖片會退回檔名與中繼資訊，這與 [總覽](/cli/cli-graphical-tools-overview/) 對影像協定的取捨一致。

## 下一步路由

- 把檔案管理器擺進可持久化的多工器 pane：[tmux 基礎](/cli/tmux-persistence-and-basics/)。
- 編譯型工具搬到遠端的單一 binary 注意事項：[git 線圖工具選型](/cli/git-line-graph-tools-for-remote-cli/)。
- 檔案管理在遠端工具分類中的定位：[終端機圖形化工具總覽](/cli/cli-graphical-tools-overview/)。

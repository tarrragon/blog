...
拿到 forecast 時，把它當參考而非命令。若 forecast 有進系統，依系統做事並用歷史數據校正差異；若沒進系統，依各料件的 LT 與市場狀況主動推前端做策略性備料。常見陷阱是把 forecast 當成準確的未來、照單全收備料，結果旺季實際暴衝或淡季庫存積壓。Forecast 的價值在於觸發思考「這個數字在什麼條件下會錯」，而不是提供一個可以照抄的答案。

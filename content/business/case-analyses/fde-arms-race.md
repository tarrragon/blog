...
OpenAI 開獨立 DeployCo、Anthropic 跟 Blackstone 與高盛合資、Google 把 FDE 納編進 Cloud—三家最大的基礎模型供應商在 2025-2026 年同時押 [FDE](/business/knowledge-cards/fde/) 模式。三家動作同步、反映的是 [SaaS](/business/knowledge-cards/saas/) 商業模式三個前提同時鬆動：邊際成本不再接近零、產品壽命被 AI 迭代壓縮、切換成本因模型 API 標準化下降。本篇用 WRAP 拆解三支柱怎麼鬆動、[Vibe Code](/business/knowledge-cards/vibe-code/) 怎麼改寫 FDE [單位經濟](/business/knowledge-cards/unit-economics/)、以及 FDE 是過渡狀態還是長期結構。
...
錨點問題聚焦在結構、而非個別公司執行：三家為什麼同步押注同一個 GTM 結構？這背後反映的是 SaaS 模式本身的哪些前提鬆動？這些前提鬆動是暫時的還是永久的？

回答這個問題能讓讀者判斷未來幾年 AI 商業化的整體輪廓、以及 SaaS 老路長期是否會復活。「FDE 模式比 PLG 更好還是更壞」這類個別執行問題本篇刻意避開、由個別公司的營收數據判讀。
...
## Widen Options：四個並陳的因果鏈

對「三家為什麼同步押 FDE」、四個都有實際擁護者的因果解釋。這些解釋不互斥、本篇判讀重點是 evidence 配重。

**解釋 (1) SaaS 三支柱鬆動下的結構性 GTM**：Prior 是 unit economics 算式倒推、Sequoia / Bain 都公開算過 AI 毛利數字。FDE 是 SaaS 三支柱鬆動後的可行 GTM、[PLG](/business/knowledge-cards/plg/) 數學在新成本結構下難以收斂。Testable prediction：PLG 收入比例下降、enterprise ARR 比例上升、所有玩家（含後進者）會走相同路。

**解釋 (2) Tacit Knowledge 不可遠端萃取**：Palantir / Karp 過去十年論述、Sierra（Bret Taylor）公開講過 LLM 應用層的 evaluation set 必須現場共建。Prior 是 AI 產品的需求探索無法靠 [PRD](/business/knowledge-cards/prd/) + [wireframe](/business/knowledge-cards/wireframe/) 遠端做、需要在客戶現場跑真實案例。Testable prediction：FDE 駐點天數高、產品迭代由 evaluation set 驅動而非 PRD 驅動。

**解釋 (3) Enterprise Procurement 結構性需求**：Box / ServiceNow 等老 enterprise SaaS 高層公開講過大企業採購流程要求「賣方派人」當合規 / 風控門檻。Prior 是高度監管產業（金融、政府、醫療）的採購框架天然偏好駐點服務模式。Testable prediction：FDE 主要服務 Fortune 500 + 高度監管產業、中小企業仍走 PLG。

**解釋 (4) Vibe Code 改寫 unit economics 後的 capacity arbitrage**：a16z「Forward Deployed AI Engineering」報告跟 Anthropic 內部數字都支持 [Vibe Code](/business/knowledge-cards/vibe-code/) 工具讓 FDE 產能變 3-5 倍。Prior 是過去 Palantir-only 模式之所以擴散、是因為工具讓中型客戶服務也算得過來。Testable prediction：中型企業合約佔比上升、FDE 工程師 utilization 高、PE-portfolio 合約密集。

四個解釋共同預測「FDE 模式擴散會發生」、差別在主導動機跟適用場景的權重。下一節用 evidence 對每個解釋配重、保留多解釋並存的可能。

## Reality Test：用 evidence 配重四個解釋

對照已知觀察、給每個解釋估計權重。先從解釋 (1)「SaaS 三支柱鬆動」這個多數分析師視為主要驅動的因果鏈展開、再對照其他三個解釋。
...
解釋 (1) SaaS 三支柱鬆動：強訊號。三家 ARR mix 都向 enterprise 偏、PLG 留存指標普遍弱化、新進玩家也選 FDE/Sales-led 而非 PLG。估佔比 ~40%。

解釋 (2) Tacit Knowledge 不可遠端萃取：強訊號。Anthropic / OpenAI 公開資料都顯示 FDE 駐點時數高、evaluation set 是核心 artifact、PRD-driven 迭代減少。估佔比 ~30%。

解釋 (3) Enterprise Procurement 結構性需求：中等訊號。FDE 客戶分布確實向 Fortune 500 + 監管產業集中、但中小企業合約也在擴散、所以是強化因素而非主導。估佔比 ~15%。

解釋 (4) Vibe Code capacity arbitrage：強訊號。Anthropic 鎖定 PE-portfolio mid-market、FDE 工程師單人服務客戶數從 1-2 增到 5-10、是模式可規模化的關鍵。估佔比 ~15%。

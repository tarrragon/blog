...
本資料夾每篇文章都用 WRAP 框架拆解一個具體案例。WRAP 是認知偏誤防護工具—它強制讀者在判斷前做 Anchor Check、列出 Widen Options、用 Reality Test 驗證、跳出短期情緒做 Attain Distance、預先 Prepare to be Wrong、設 Tripwire 監控。
...
工程背景、想系統化解構社群商業分析的人。讀者已經對自己的領域有實作經驗、但對商業分析語言相對新；他們的需求不是「告訴我結論」、是「給我可遷移的判讀骨架」、讓自己以後遇到類似事件能自己拆。
...
| ------------------- | ----------------------------------------- |
| 事件本身            | 簡述案例、為什麼值得拆                    |
| Anchor Check        | 對齊核心目標—不是在判斷什麼、是要回答什麼 |
| Step 0 資料充足度   | 已知什麼、不知道什麼、可以下到哪層結論    |
| Widen Options       | 列出至少 2-3 個本質不同的解釋路徑         |
| Reality Test        | 用案例、數據、反例驗證哪個解釋成立        |
| 結構性機制          | 確認解釋後展開具體因果鏈                  |
| Attain Distance     | 跳出短期情緒、看 5-10 年長期影響          |
| Prepare to be Wrong | 列出關鍵假設、假設崩了會怎樣              |
| Tripwire            | 設定何時要重新評估                        |
| 判讀框架            | 給讀者帶走的可遷移 framework              |

不是每篇都嚴格按這順序、但每個段落責任都應涵蓋。
...
2. 用 WRAP 結構模板逐段填寫
3. 確保每個 Widen Option 都有對應 Reality Test
4. 結尾必須給可遷移的判讀框架表
5. Tripwire 段必須具體可監控（不能寫成「再觀察」這種模糊話）

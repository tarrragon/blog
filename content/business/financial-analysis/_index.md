...
| [倖存者偏誤的幅度估計](/business/financial-analysis/survivorship-adjustment/)      | 產業基準分析的倖存者篩選展開——三條估法與估不出來時的降級用法 |
...
**從一個指標推到另一個指標時，中間的環節要寫出來或明說跳過。** 毛利率到淨利之間隔著營業費用、業外、稅與股數，營收成長到估值溢價之間隔著獲利成長；任一環節都能單獨造成同樣的形狀。缺環節時結論要降級為觀察，不能寫成判斷標準。
...
| 「成本衝擊看毛利率、需求衝擊看營收和 ROE」這條判斷標準的支撐（目前由單一案例推出，而該案例本身缺營收年增率；判斷標準被 backend 與其他產業篇引用前要補推導或降級為觀察）                                                                                                            | 主章 | 依賴上一列的營收數據                                         | 小   |
...
掃描的判斷標準是「表格含真實財務數字，而表格本身與前三行都沒有年度、季度或 FY 標記」。命中是候選不是判決：概念型表格（估值倍數的公式對照、Gross 與 Net 認列的差異、成本科目的分類、篩選條件與門檻值）本來就不繫於特定期間，不需要補。要補的是帶有具名主體實績數值的那一類——某家公司的營收、毛利率、EPS、ROE。

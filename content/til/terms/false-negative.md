...
slug: "false-negative"
date: 2026-06-18
description: "false negative（假陰性）是偵測器漏掉真實存在的東西——說「沒有」，真實卻是「有」。它和 false positive 同屬混淆矩陣的兩個錯誤格，統計叫 Type II error、訊號偵測叫 miss"
tags: ["til", "術語", "跨領域", "false-negative"]
---

> 這個詞出現在「[你的自動判斷會犯兩種錯：誤報與漏接](../detection-errors/)」這個問題裡——它是另一種錯。

false negative（假陰性）指**偵測器漏掉了真實存在的東西**——說「沒有」，真實卻是「有」。它是 [false positive](../false-positive/) 的另一半：把判斷（有/無）對上真實狀態（有/無），四種組合構成[混淆矩陣](../false-positive/)，兩者各佔其中一個錯誤格。
...
- 對偶錯誤：[false positive](../false-positive/)（誤報，說有實無）。兩者是混淆矩陣的兩個錯誤格。
- 統計裡的編號：[Type II error](../type-ii-error/)（= false negative）。
- 被它拉低的指標：[recall](../recall/)（漏接越多，recall 越低）。

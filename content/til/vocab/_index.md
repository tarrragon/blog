---
title: "單字"
slug: "vocab"
description: "學到的英文單字：字源拆解、由來故事、同字根家族、語感差異與搭配，一個單字一篇"
tags: ["til", "英文單字", "字源", "語感"]
---

這個子分類收錄**學到的單字** — 一個單字一篇。切入角度看那個字最值得記的地方：有的寫字源拆解、由來故事與同字根家族（同字根的單字互連成一張字根地圖），有的寫語感差異與搭配（同義字之間的細微分別、母語作者慣用的固定表達）。

同一個字涵蓋哪些實物、換個地區為什麼涵蓋的是別的，那是 [料理](../cooking/) 的軸；這裡的判斷落在字本身。

字源拆解切入：

- [candidate 的字源](candidate/)：穿白袍的人

語感差異切入 — 學術與新聞英文的高頻評論字，依語意族群排列。

評估可信度與可行性：

- [plausible](plausible/)：說得通但未經證明（vs possible）
- [compelling](compelling/)：強到不得不信服（vs convincing）
- [cogent](cogent/)：條理清晰而有力（vs convincing）
- [feasible](feasible/)：現實條件下辦得成（vs possible）
- [viable](viable/)：行得通、活得下去（vs feasible）
- [sustainable](sustainable/)：撐得久、不吃老本（vs viable）
- [scrutinize](scrutinize/)：帶著懷疑仔細審視（vs examine）
- [rigorous](rigorous/)：不留捷徑的嚴謹（vs strict）
- [warrant](warrant/)：足以構成…的理由（vs justify）

變動、脆弱與強韌：

- [volatile](volatile/)：劇烈震盪的變動（vs unstable）
- [precarious](precarious/)：懸在邊緣的脆弱（vs unstable）
- [tenuous](tenuous/)：細得快斷的薄弱（vs weak）
- [entrenched](entrenched/)：掘壕固守般根深蒂固（vs established）
- [robust](robust/)：禁得起考驗的強健（vs strong）
- [resilient](resilient/)：受挫後彈得回來（vs robust）

政策與論述動作：

- [curb](curb/)：把失控趨勢勒回可控
- [mitigate](mitigate/)：減輕已成的壞影響（vs reduce）
- [alleviate](alleviate/)：減輕當下的難受（vs mitigate）
- [exacerbate](exacerbate/)：讓已經糟的更糟（vs worsen）
- [bolster](bolster/)：加碼補強既有基礎（vs support）
- [undermine](undermine/)：從基礎暗中削弱（vs weaken）
- [erode](erode/)：一點一點被磨掉（vs undermine）

論述品質與重點呈現：

- [nuanced](nuanced/)：分得清細微差別的層次感（vs complex）
- [ambiguous](ambiguous/)：可作兩種解讀（vs vague）
- [salient](salient/)：最突出顯眼的那點（vs important）
- [underscore](underscore/)：事實自然凸顯重要性（vs emphasize）

---

底下自動列出本分類的所有單字、依日期排序。

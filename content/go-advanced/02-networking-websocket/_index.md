...
WebSocket 服務的核心難點不是 HTTP upgrade，而是連線建立後的長生命週期管理。每個 client 都有讀取、寫入、心跳、訂閱、推送佇列與清理流程；任何一個邊界不清楚，都可能造成 goroutine leak、concurrent write、慢 client 拖垮服務或訂閱狀態不一致。

本模組承接模組一的並發主題：read pump / write pump 對應 goroutine ownership，heartbeat 對應 select loop 與 ticker，send buffer 對應 backpressure，訂閱集合對應共享狀態與 copy boundary。
...
| [2.2](heartbeat-deadline/) | heartbeat、deadline 與連線清理 | 用 ping/pong、deadline 與統一 unregister 偵測失效連線 |
...
本模組使用虛構的即時通知服務作為範例。Client 可以訂閱 topic，server 會依 topic 推送 notification、status update 或 error message。
...
- 用 channel 作為 client send queue。
...
WebSocket 章節依照連線生命週期拆分，而不是依照程式檔案拆分。Read/write pump、heartbeat、subscription routing、slow client 是四個不同責任；它們常在同一個 hub 或 client type 中互相呼叫，但教學上應分開建立模型。
...
本模組不討論 WebSocket 壓縮、水平擴展、跨節點廣播或完整身份驗證。這些都是實務重要主題，但必須先建立單一 Go process 內的連線生命週期與容量邊界；後續可接 [跨節點 WebSocket、presence 與重連協定](../07-distributed-operations/cross-node-websocket/)。

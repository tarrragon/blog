...
本模組目前是延伸章節規劃。每章先定義問題邊界、前置章節與撰寫方向，後續可以依實戰需求逐章補成完整教材。
...
| ------------------------------ | ------------------------------------------ | -------------------------------------------- | ------------------------------------------------------------------------------------------ |
| [7.1](database-transactions/)  | 資料庫 transaction 與 schema migration     | 狀態邊界進入持久化層後如何維持一致           | [資料庫與持久化](../../backend/01-database/)                                               |
| [7.2](outbox-idempotency/)     | Durable queue、outbox 與 idempotency       | 事件跨 process 後如何避免遺失、重複與半成功  | [訊息佇列與事件傳遞](../../backend/03-message-queue/)                                      |
...
| [7.4](observability-pipeline/) | Observability pipeline、metrics 與 tracing | log、metric、trace 如何組成可操作的診斷系統  | [可觀測性平台](../../backend/04-observability/)                                            |
...
| [7.6](reliability-pipeline/)   | CI、fuzz、load test 與 chaos testing       | 測試如何從單一行為擴展到系統可靠性           | [可靠性驗證流程](../../backend/06-reliability/)                                            |

## 本模組和前面章節的關係

本模組不是入門必讀路徑。它適合在你已經理解單一 Go 服務的內部邊界後，用來補足生產環境常見的外部系統責任。

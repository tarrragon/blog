...
本模組已開始補成正文。章節先定義問題邊界與前置脈絡，再逐步補上 [transaction](../../backend/knowledge-cards/transaction/)、outbox、跨節點 WebSocket、observability、部署與可靠性驗證的實作語意；後續仍可依實戰需求繼續擴寫。
...
| ------------------------------ | ------------------------------------------ | -------------------------------------------- | ------------------------------------------------------------------------------------------ |
| [7.1](database-transactions/)  | 資料庫 transaction 與 schema [migration](../../backend/knowledge-cards/migration/)     | 狀態邊界進入持久化層後如何維持一致           | [資料庫與持久化](../../backend/01-database/)                                               |

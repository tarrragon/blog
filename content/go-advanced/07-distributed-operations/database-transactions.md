...
資料庫整合的核心責任是讓持久化行為符合 application 的狀態規則。Repository port 決定 usecase 需要哪些資料能力；[transaction boundary](../../backend/00-knowledge-cards/transaction-boundary/)、[schema migration](../../backend/00-knowledge-cards/schema-migration/) 與 [isolation level](../../backend/00-knowledge-cards/isolation-level/) 則決定這些能力在資料庫中如何保持一致。

## 本章目標

學完本章後，你將能夠：

1. 判斷 transaction boundary 應該放在 repository 還是 usecase
2. 理解 migration 為什麼要維持向前相容
3. 分辨 application validation、constraint 與 isolation level 的責任
4. 用 contract test 保護 memory repository 與 database repository 的一致行為
...
- [Backend：Source of Truth](../../backend/00-knowledge-cards/source-of-truth/)
- [Backend：Connection Pool](../../backend/00-knowledge-cards/connection-pool/)
...
## 【策略】migration 要讓舊版與新版可以共存

schema migration 的核心是讓部署期間的新舊版本能同時活著。實務上常見的是 expand/contract 流程：

...
- 只要是可以重試或可補償的外部互動，盡量不要放進 transaction。

## 【策略】migration 要讓舊版與新版可以共存

schema migration 的核心不是「一次改完」，而是讓部署期間的新舊版本能同時活著。實務上常見的是 expand/contract 流程：
...
當你同時有 memory repository 與 database repository 時，測試重點不是兩個實作的 SQL 是否一樣，而是它們對外暴露的語意是否一致。
...
這類測試可以讓 repository adapter 保持可替換，而不是每次換資料庫就重寫 usecase。

...
- [Go 進階：read pump / write pump 模式](/go-advanced/02-networking-websocket/read-write-pump/)
- [Go 進階：heartbeat、deadline 與連線清理](/go-advanced/02-networking-websocket/heartbeat-deadline/)
- [Go 進階：訂閱模型與訊息路由](/go-advanced/02-networking-websocket/subscription-routing/)
- [Go 進階：慢客戶端與 send buffer 管理](/go-advanced/02-networking-websocket/slow-client/)
...
Topic ACL 要在訂閱建立時就確認這個 client 是否有資格加入。這能減少不必要的 fan-out 與敏感資料外流。

## 本章不處理

本章不會選定特定 broker 或 presence [database](/backend/knowledge-cards/database/)。重點是先讓跨節點責任可見，再依服務需求選擇 Redis、NATS、Kafka、PostgreSQL 或其他基礎設施。

## 和 Go 教材的關係

這一章承接的是 WebSocket 連線架構與事件路由；如果你要先回看語言教材，可以讀：

- [Go 進階：read/write pump 模式](/go-advanced/02-networking-websocket/read-write-pump/)
- [Go 進階：heartbeat、deadline 與連線清理](/go-advanced/02-networking-websocket/heartbeat-deadline/)
- [Go 進階：訂閱模型與訊息路由](/go-advanced/02-networking-websocket/subscription-routing/)
- [Go 進階：慢客戶端與 send buffer 管理](/go-advanced/02-networking-websocket/slow-client/)

...
這個模型的重點不是保證「絕對只送一次」，而是讓「至少會被發現並補送」成為可能。

## 【判讀】idempotency 不是可有可無

只要事件可能重送，consumer 就要能承受重複訊息。idempotent processor 的核心不是忽略失敗，而是讓同一筆事件重複進來時，結果仍然穩定。
...
## 【策略】DLQ 是流程的一部分，不是垃圾桶

當事件重試失敗，dead-letter queue 不應只是把問題丟走，而是要變成可處理的操作流程。你要知道：
...
只要有 durable queue，就一定會有 backlog、retry 與 failure pattern。這些訊號不應只存在 broker 後台，也應進入 log 與 metric，讓工程師知道是 producer 變慢、consumer 壞掉，還是下游依賴正在抖動。

...
Table-driven test 的核心風險是「減少重複」被誤解成「所有案例都塞進一張表」。當表格開始同時控制 HTTP method、request body、repository 狀態、[WebSocket](../../backend/knowledge-cards/websocket) client、expected [log](../../backend/knowledge-cards/log)、expected event，測試就會變成難懂的迷你框架。
...
這種表格看似統一，實際上混合了 HTTP validation、repository error、client [queue](../../backend/knowledge-cards/queue) full、event emission、log assertion。讀者必須同時理解多個系統層，才能看懂單一案例。
...
這張表只測 [topic](../../backend/knowledge-cards/topic) normalization。每個案例都只有 input 和 want，失敗時也能立刻看出是哪個 normalization 規則壞了。

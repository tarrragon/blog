...
Channel 設計的核心動作是先列出誰會 send、誰會 receive、誰知道所有 sender 已經結束。這比先決定 [buffer](/backend/knowledge-cards/buffer/) 大小更重要。
...
`jobs` 關閉代表沒有更多 job。`ctx.Done()` 代表上層要求停止。這兩種退出原因不同，分開處理才能在 [log](/backend/knowledge-cards/log/)、metric 或測試中看清楚。
...
本章先聚焦單一 Go process 內的 channel close 與 goroutine lifecycle；跨 process 的 [ack](/backend/knowledge-cards/ack-nack/)、[[consumer](/backend/knowledge-cards/consumer/) group](/go-advanced/backend/knowledge-cards/consumer-group) 與分散式訊號，會在下列章節再往外延伸：

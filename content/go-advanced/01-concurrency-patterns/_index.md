...
| 章節                                                                                        | 主題                                                        | 關鍵收穫                                        |
| ------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------- |
| [1.1 channel ownership 與關閉責任](/go-advanced/01-concurrency-patterns/channel-ownership/) | channel ownership 與關閉責任                                | 用 sender lifecycle 判斷誰能 close channel      |
| [1.2 select loop 的生命週期設計](/go-advanced/01-concurrency-patterns/select-loop/)         | select loop 的生命週期設計                                  | 同時處理輸入、ticker、取消與資源釋放            |
| [1.3 非阻塞送出與事件丟棄策略](/go-advanced/01-concurrency-patterns/non-blocking-send/)     | 非阻塞送出與事件丟棄策略                                    | 把 channel 滿載轉成明確服務行為                 |
| [1.4 共享狀態與複製邊界](/go-advanced/01-concurrency-patterns/shared-state/)                | 共享狀態與複製邊界                                          | 用 lock、copy 與 owner method 保護可變資料      |
| [1.5 bounded worker pool](/go-advanced/01-concurrency-patterns/worker-pool/)                | bounded [worker pool](/backend/knowledge-cards/worker-pool) | 限制同時執行的工作量，避免 goroutine 無限制堆積 |
| [1.6 rate limiting 與 backpressure ](/go-advanced/01-concurrency-patterns/rate-limit/)      | rate limiting 與 backpressure                               | 用本地速率限制保護服務入口與下游依賴            |
...
本模組不討論分散式鎖、actor framework 或高階 [queue](/backend/knowledge-cards/queue) 系統。這些主題建立在本模組的基礎之上；本模組先把單一 Go process 內的 goroutine、worker、速率與共享資料邊界講清楚。外部 [broker](/backend/knowledge-cards/broker) 與分散式流量治理會放在 [Backend：訊息佇列與事件傳遞](/backend/03-message-queue/) 與 [Backend：部署平台與網路入口](/backend/05-deployment-platform/)。

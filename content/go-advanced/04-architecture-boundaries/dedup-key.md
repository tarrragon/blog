...
重複事件的核心困難是外觀可能不同。HTTP callback 可能每次都有新的 request ID，[queue](/backend/knowledge-cards/queue/) message 可能因 retry 改變 delivery tag，timer 可能在下一輪掃描再次產生類似事件。
...
本章先處理單一服務內的事件去重語意；跨節點一致性與 [idempotency](/backend/knowledge-cards/idempotency/) store，會在下列章節再往外延伸：

...
本系列是接在入門教學之後的延伸路線，目標不是重複語法，而是把 Go 的並發模式、WebSocket 服務架構、runtime 診斷、狀態邊界與生產環境可觀測性講到能真正用在服務上。
...
模組七保留在進階篇裡，是因為它要回答的不是「外部系統怎麼操作」，而是「Go 服務在跨出去以前，還需要先把哪些 port、訊號與測試合約準備好」。外部系統本身的選型與部署細節，則放在 Backend，讓不同語言都能共用同一套實作知識。

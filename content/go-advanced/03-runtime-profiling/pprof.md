...
1. 記錄當下流量、版本、操作、時間區間。
2. 讀 runtime metrics：heap、GC、goroutine、queue 長度。
...
本章不討論雲端商用 profiler、分散式 tracing 或完整 APM 平台。這些工具能提供更長期的觀測，但 Go 原生 pprof 已足以建立本地與單服務診斷流程。後續可接 [Observability pipeline、metrics 與 tracing](../07-distributed-operations/observability-pipeline/)。

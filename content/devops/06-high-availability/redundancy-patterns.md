...
冗餘設計模式回答「單點有了替代路徑之後，替代的那一份要怎麼擺」。三種主要模式沿兩個維度區分：備援的那一份服不服務流量（active-passive 的備援閒著等、active-active 的備援也在幹活），以及資料怎麼同步（同步副本幾乎不丟資料、非同步副本有延遲窗口）。選哪一種，決定了冗餘的成本、切換的速度、以及切換時可能丟多少資料。

...
一個危險的誤解是把「切回流量」當成恢復完成。真實的恢復往往在切回流量之後還有很長一段。有遊戲平台的一次故障中斷了 73 小時，遠超任何人預期的原因不在切換本身——是資料一致性要重建、快取要預熱、依賴服務要按正確順序啟動，這些在流量切回來之後才開始、而且各自要花時間。恢復的實際成本包含這些「切回之後」的收尾，把它們算漏，RTO 就會嚴重低估。

恢復還要分批、不能一次全開，理由跟 failover 的恢復順序同源：同時把多條路徑一起拉起來，可能在剛恢復、還脆弱的依賴上引發回源放大或連鎖過載，變成第二次故障。可靠的恢復依事故時間線與 runbook 分批推進、每批驗證 baseline 穩定再進下一批，且恢復路徑本身不能依賴任何可能還在故障的控制面——恢復工具要能在控制面掛掉時獨立運作。這條跟 [Failover 機制](/devops/06-high-availability/failover-mechanism/) 的恢復順序是同一套紀律。

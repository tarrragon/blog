...
| 1    | 02 水平擴展 | 「規模成長」路線第二站；模組一已完成、「LB 是前提」引用有落點                   | 模組一（LB 前提，已完成）      |
| 2    | 06 高可用   | 「規模成長」路線第三站                                                          | 模組四（探活是 failover 前提） |
| 3    | 08 成本管理 | 「成本控制」路線起點，但該路線明寫「模組八 → 模組五」，模組五成本模型已就位可引 | 模組五（成本模型，已完成）     |

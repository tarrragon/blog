...
| 1    | 05 容量規劃（補完） | 「規模成長」「突發應對」「成本控制」三條路線都經過；模組八的成本模型輸入          | 無（已有一章可對齊語彙）       |
| 2    | 02 水平擴展         | 「規模成長」路線第二站；模組一已完成、「LB 是前提」引用有落點                     | 模組一（LB 前提，已完成）      |
| 3    | 06 高可用           | 「規模成長」路線第三站                                                            | 模組四（探活是 failover 前提） |
| 4    | 08 成本管理         | 「成本控制」路線起點，但該路線明寫「模組八 → 模組五」，先有五的成本模型八才有得引 | 模組五（成本模型）             |

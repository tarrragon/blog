...
| 順位 | 模組                | 理由                                                                                     | 依賴                           |
| ---- | ------------------- | ---------------------------------------------------------------------------------------- | ------------------------------ |
| 1    | 04 服務探活         | 「單服務營運」路線第一站；模組六 failover 的觸發前提；解鎖 linux backlog #10；素材最齊全 | 無                             |
| 2    | 01 負載平衡         | 「單服務營運」路線第三站；模組二明寫「LB 是水平擴展的前提」                              | 無                             |
| 3    | 05 容量規劃（補完） | 「規模成長」「突發應對」「成本控制」三條路線都經過；模組八的成本模型輸入                 | 無（已有一章可對齊語彙）       |
| 4    | 02 水平擴展         | 「規模成長」路線第二站                                                                   | 模組一（LB 前提）              |
| 5    | 06 高可用           | 「規模成長」路線第三站                                                                   | 模組四（探活是 failover 前提） |
| 6    | 08 成本管理         | 「成本控制」路線起點，但該路線明寫「模組八 → 模組五」，先有五的成本模型八才有得引        | 模組五（成本模型）             |

## 各模組缺口明細

### 1. 模組四：服務探活與自動恢復 — 5 章待寫（最優先）

- **章節**（依 `04-service-health/_index.md` 大綱）：health check endpoint 設計、liveness vs readiness、systemd watchdog + 自動重啟、process supervisor 選型、graceful shutdown。

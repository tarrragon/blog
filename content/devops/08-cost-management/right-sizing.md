...
過度配置是雲端最大的浪費來源，right-sizing 就是把配置規格調回貼近實際用量。一台長期只用 30% CPU 的機器，那 70% 是純浪費——資源開著、每小時計費，卻沒在做事。right-sizing 不是一次性的動作，是持續對照實際用量調整規格的紀律，因為用量會變、機型會更新、當初配的規格很快就不再是最合適的。

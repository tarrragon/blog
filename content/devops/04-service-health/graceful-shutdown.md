...
收束的相對面是硬砍。平台給的收束時間是有上限的，超過上限服務就被 `SIGKILL` 強制結束、不走任何清理。所以 graceful shutdown 不是「有沒有清理邏輯」，是「清理邏輯能不能在 grace period 內跑完」——跑不完，等於沒有。

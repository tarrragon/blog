...
壓測要回答的問題是「這個服務在什麼負載下開始撐不住」，不是「跑一次看會不會掛」。有價值的壓測是階梯加壓、觀察系統行為在哪個點從穩定轉為劣化，把 [流量模型](/devops/05-capacity-planning/traffic-model/) 的假設拿到真實負載下驗證。工具選對，這個劣化點測得準；工具選錯，測到的是工具自己的極限、不是被測系統的。
...
這次測出的 sub-millisecond 延遲有個要老實講的前提：壓測機跟被測的 nginx 跑在同一台主機的容器網路裡，沒有真實的網路往返。這正好示範了下一節第一個反模式——壓測機跟被測機太近，延遲會被嚴重低估、p99 比 production 樂觀。
...
- 壓測用的流量模型怎麼建、怎麼驗證 → [流量模型建立](/devops/05-capacity-planning/traffic-model/)
- 用階梯加壓找飽和點、讀哪些訊號 → [規模拐點判斷](/devops/05-capacity-planning/scaling-inflection-point/)

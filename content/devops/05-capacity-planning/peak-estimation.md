...
安全係數不能對所有系統套同一個數，因為膝點的位置隨系統類型變。無狀態服務、有鎖競爭的資料庫、吃磁碟 I/O 的佇列，膝點高低不同，有狀態的關鍵系統膝點來得早、要留的 headroom 比無狀態服務厚。飽和曲線的三段區間、排隊理論的量化門檻、以及各類系統膝點的實際位置，在 [規模拐點判斷](/devops/05-capacity-planning/scaling-inflection-point/) 完整展開；這裡用它來解釋安全係數為什麼存在、為什麼不能一律套同一個數。

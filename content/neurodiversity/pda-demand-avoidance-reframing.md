...
這個層次差異有一個直接的後果：需求迴避重構跟另外兩者多數正交、可以疊加。一份輸出可以同時是「去命令化的框架」（PDA 層）和「明確結構、標好優先級」（ASD 層），也可以同時是「歸還控制權的邀請」（PDA 層）和「把狀態外化、進度可見」（ADHD 層）。但「多數正交」不等於「零衝突」：PDA 跟另兩者仍有幾處爭同一個決定——ADHD 的「結尾給一個限時的明確動作」對上 PDA 的「不施加時間壓力」（用逐項讓步吸收），ASD 的「給單一路徑」對上 PDA 的「給真實選擇」（合成成帶判斷標準的選項）。真正需要一個全域開關的衝突，是格式層內部的措辭那一組——ADHD 的動作直給 vs 去命令化——因為它爭的是每一個動作怎麼措辭；這是枚舉到目前的結果，不是結構上只可能有一組。各組怎麼逐一處理，見[方法論篇](/neurodiversity/designing-output-for-cognitive-profiles/)。

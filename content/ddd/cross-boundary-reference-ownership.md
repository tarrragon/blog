...
title: "跨邊界參照與狀態所有權"
date: 2026-07-17
description: "下游持有上游資料的 id、操作靠這個 id 回寫時：上游的哪些操作會讓 id 死亡、有沒有跨操作不變的穩定身份、身份轉移後本端的狀態搬不搬家——參照設計的判斷標準與遷移分工"
weight: 11
tags: ["ddd", "identity", "reference", "state-ownership", "snapshot"]
---

領域模型跨越邊界持有的每一個 id，有效性由上游行為決定、持有端控制不了。上游把「更新」實作成「刪除重建」是實作自由，下游用推理猜「它應該保留」擋不住——合法的出處只有契約文件或一次實測取證。這條事實推導出本章三個判斷標準的共同源頭：跨邊界參照的設計，從「對方會做什麼」開始、而不是從「我要存什麼」開始。

本章承接 [entity 與 value object 的判斷標準](/ddd/entity-vs-value-object/) 的身份語意討論，延伸到身份跨越系統邊界之後的生存問題；與 [狀態轉換與稽核軌跡](/ddd/state-transition-and-audit-trail/) 的凍結端點段落互補——凍結是時間軸上的問題（何時），本章談的是空間軸上的問題（誰的 id、誰決定它活著）。
...
上游身份轉移（合併、拆分）發生後，下游持有的多份「以上游 id 為 key」的狀態面臨遷移問題。遷移判斷標準一個問題就夠：**這份狀態消失後，能否單靠向上游重新查詢完整重建？**
...
自持狀態的每一份都是一筆負債，債主是所有會改變上游身份的操作。盤點自持狀態清單、逐項問「上游為什麼不保存這個」——若上游未來補上這份資料，自持狀態就能降級為投影，遷移程式碼隨之刪除。這個分類判斷標準與 [讀模型的升級條件](/ddd/read-model-upgrade-signals/) 同構：讀模型問「讀的形狀還是聚合根的形狀」，這裡問「這份狀態的真相在誰手上」。
...
同一筆記錄可以同時包含兩種欄位。出事的案例往往是把「顯示用的凍結值」順手拿去做「回寫用的定位」——凍結值在這個用途下跨越了它的有效期，失效的時機由上游的重建行為決定、持有端察覺不到。凍結時機的判斷標準在 [entity 與 value object 的判斷標準](/ddd/entity-vs-value-object/) 與 [狀態轉換與稽核軌跡](/ddd/state-transition-and-audit-trail/) 各自從身份語意和稽核面到達同一個結論：歷史記錄反映事件發生當下的世界。

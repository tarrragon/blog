...
| [entity 與 value object 的判準](/ddd/entity-vs-value-object/)        | 同一個概念的「同一個」由什麼定義——操作需不需要 identity-based 回寫                    | [值物件的 Dart 實作路徑](/flutter/value-object-dart-implementation/)（實作層整合）、[同一個品項、四個 model](/work-log/dart_pos_item_four_lifecycle_models/)、[Money 三段遷移](/work-log/dart_money_extension_type_migration/)、[分層 enum](/work-log/dart_payment_dual_layer_enum/)、[跨邊界參照的生命週期](/work-log/pos_cross_boundary_reference_lifecycle/) |

...
title: "DDD 領域驅動設計指南"
date: 2026-07-10
description: "領域模型的理論與判準層：什麼時候值得建 entity、不變式該落在哪一層強制。語言無關，實作限制路由到各語言模組。"
weight: 34
...
| [entity 與 value object 的判準](/ddd/entity-vs-value-object/) | 「有沒有不允許任意組合的欄位」如何決定型別的類別           | [同一個品項、四個 model](/work-log/dart_pos_item_four_lifecycle_models/)、[Money 三段遷移](/work-log/dart_money_extension_type_migration/)、[分層 enum](/work-log/dart_payment_dual_layer_enum/)                                                                                                        |
...
本模組的 case 來源是實際專案的開發記錄與 ticket：過去專案內處理掉的設計討論陸續回補成 [work-log](/work-log/) 文章，模組章節再引用這些 case。四批回補（兩個專案、三十六篇）已完成；rich 級候選已收割完畢、剩餘為 medium 級。目前的 case 基底集中在兩個 Dart / Flutter 專案，章節的判準以敘事層抽象維持語言無關；跨生態的驗證隨其他語言模組的 case 累積補強。

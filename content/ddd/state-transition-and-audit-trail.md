...
不變式讓物件出生合法（[不變式的強制層次](/ddd/invariant-enforcement-layers/)），但物件出生之後還會被變更。模組源頭句「讓違反規則的路徑走不通」在變更路徑上有一個直接的折算：狀態欄位有沒有流程約束、變更有沒有需要留痕的伴隨動作？有，變更就要收斂到領域方法，不收斂的路徑關閉。本章的作用域跟 [不變式的強制層次](/ddd/invariant-enforcement-layers/) 同在單一物件內：跨物件一致性（aggregate 邊界、事務語意）是另一個層次的主題。
...
收斂的操作化：稽核紀錄或狀態流程欄位出現在任何 public 寫入介面的參數列——從參數列移除、收進領域方法。這是 [#222](/report/design-intent-needs-enforcement-layer/) 判讀徵兆表的第五行在本章的具體展開。收斂之後、領域方法成為唯一路徑、每一條稽核紀錄都有一個業務動詞作為來源、紀錄的完整性由介面的形狀保證而不是由呼叫端的記憶保證。
...
- 同一個變更操作、有時有稽核紀錄有時沒有——追有沒有繞過領域方法的呼叫點、多半在工廠層或測試的 Arrange 段。
- 狀態轉換方法的註解宣稱轉換條件、方法內 grep 不到對應檢查——文件層約束正在靜默失效，判讀見 [不變式的強制層次](/ddd/invariant-enforcement-layers/)。
- 歷史記錄的顯示內容跟著現行資料變動——參照凍結時機漏掉、身份判準見 [entity 與 value object 的判準](/ddd/entity-vs-value-object/)。

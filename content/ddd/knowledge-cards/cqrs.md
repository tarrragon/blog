...
要不要上 CQRS 全套，不看「這個模式聽起來很適合」，看五個具體訊號（讀需求增生、形狀偏離、負載分歧、新鮮度分級、獨立演進）是否命中——本卡只回答「CQRS 是什麼、階梯怎麼分階」，訊號的完整推導與升級路徑是 [讀模型的升級條件](/ddd/read-model-upgrade-signals/) 這篇的內容，要判斷自己的專案該停在哪一階、讀那篇。

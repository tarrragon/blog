...
本模組的知識卡片把領域建模的高密度術語拆成可獨立閱讀的概念索引。教學章節（[資料袋與領域模型](/ddd/data-bag-vs-domain-model/)、[entity 與 value object 的判準](/ddd/entity-vs-value-object/)、[不變式的強制層次](/ddd/invariant-enforcement-layers/)）遇到術語時連到對應卡片；卡片先回答概念本質、再放設計責任——讓讀者先知道該概念在領域模型裡承擔什麼責任。

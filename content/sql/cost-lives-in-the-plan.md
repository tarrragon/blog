...
**這一組數字的形狀依賴引擎的存取路徑**：SQLite 逐列處理、相關子查詢編成逐列的巢狀執行，所以加一個索引能把 `EXISTS` 那一格的單價整個換掉。DuckDB 是向量化的引擎（本分類其餘各篇拿它做三家對照），它處理同一組查詢的方式與 SQLite 不同，三種寫法在它上面的差距形狀也不同。要看的是「代價由資料與可用的存取路徑決定」這個結論，不是這幾個絕對值。
...
沒有索引時最慢的是 `EXISTS`，有索引時它變成第二快——同一段查詢快了五百多倍。分組與自連接兩欄之間沒有量級上的變化：分組是 2.1 ms 對 1.1 ms，兩個值都在個位數毫秒，差的那一毫秒接近這組量測的解析度；自連接那格的方向在不同次量測之間會反轉，差距落在雜訊裡。**三段查詢的文字一個字都沒有改。**
...
資料量決定常數項會不會被放大。分布決定會不會踩到某個寫法的最壞情況——上面的自連接就是被重複程度打敗的。索引決定每一次查找的單價，而上面那組量測裡，索引的有無讓同一段查詢差了五百多倍。

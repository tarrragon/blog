...
**這一組數字的形狀依賴引擎的存取路徑**：SQLite 逐列處理、相關子查詢編成逐列的巢狀執行，所以加一個索引能把 `EXISTS` 那一格的單價整個換掉。向量化的引擎（本分類其餘各篇用來做三家對照的 DuckDB 就是）處理同一組查詢的方式不同，三者的差距形狀也不同。要看的是「代價由資料與可用的存取路徑決定」這個結論，不是這幾個絕對值。資料是這樣造的：

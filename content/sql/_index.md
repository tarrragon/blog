...
**SQL 是宣告式的：寫下的是要什麼結果，而怎麼算、名字指向誰、送出的人被允許做什麼都不在那段文字裡。** 各篇的判斷標準都折算回這一條，而它分出兩支，本分類的章節照兩支排列。
...
上面那兩支是讀完整個分類會走的順序。帶著一個具體問題進來的人不必照順序讀——多數篇的篇名是語法或語法結構，而手上的東西通常是一個業務問題，這張表把問題對到落點。
...
「Sargable」那張卡的位置在兩處既有內容之間。[1.11](/sql/cost-lives-in-the-plan/) 的結論是代價由資料與索引決定、不由寫法決定，而條件把欄位包進函式裡是那個結論有界的例外：索引存在，引擎用不上它。同一組資料上量到的計畫，兩邊欄位都包在函式裡時是兩層 `SCAN`，左邊換成裸欄位之後變成 `SEARCH` 加走索引。[索引](/sql/knowledge-cards/indexing/)那張卡替「建了而引擎不走」列了一個成因（統計資訊沒更新），條件的形狀是第二個。

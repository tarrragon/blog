...
**想把一段陌生的查詢讀準**，四篇走完就夠：[1.2](/sql/clause-evaluation-order/) 每一步手上有什麼、[1.4](/sql/join-left-operand-accumulates/) 連接鏈的左邊是什麼、[1.5](/sql/on-describes-where-filters/) 條件在哪一步生效、[1.14](/sql/declared-intent-vs-behaviour/) 文字說的話跟查詢的行為對不對得上。
...
**第一支問「結果集由什麼決定」。** 答案是一個明確的模型，而那個模型與直覺不同——子句的先後、配對關係寫在哪、連接的左右、條件擺哪一邊、分組的鍵、輸出的單位收不收起來，每一項都由[關聯代數](/sql/knowledge-cards/relational-algebra/)的組合規則定死，而每一項的規則都有一處與讀者的直覺相反。這一支要學的就是那個模型。
...
**第三支問「這段文字給人讀的那一面」。** 宣告式語言把要什麼結果寫成文字，於是同一段文字有兩個讀者：引擎讀行為，人讀宣告。[1.14](/sql/declared-intent-vs-behaviour/) 問文字說了什麼——選了 `LEFT` 就是宣告預期有配不到的列，而引擎從不查證這件事，宣告落空時查詢仍然回正確答案，代價落在下一個相信那段文字的人身上。[1.15](/sql/readable-and-fast-mostly-align/) 問文字該為誰而寫——兩個讀者的需求在 SQL 上多數時候指向同一個寫法，而分岔發生時出口在 schema 那一層。
...
| 篇                                                        | 交付                                              |
| --------------------------------------------------------- | ------------------------------------------------- |
| [1.11 識別字送進引擎之後會被改寫](/sql/identifier-rules/) | 大小寫摺疊與引號各家的規則，以及混用時失效的時機  |
| [1.12 權限的預設是什麼都不給](/sql/privilege-model/)      | 角色與 GRANT 的模型，以及最小權限在事故時省下什麼 |
| [1.13 代價由資料與索引決定](/sql/cost-lives-in-the-plan/) | 一道題三種寫法的實測，加一個索引之後排名對調      |

### 第三支：文字對讀的人說了什麼

| 篇                                                                            | 交付                                       |
| ----------------------------------------------------------------------------- | ------------------------------------------ |
| [1.14 關鍵字宣告意圖，引擎只執行行為](/sql/declared-intent-vs-behaviour/)     | 宣告落空的幾種形態，以及查證宣告的那個動作 |
| [1.15 好讀的寫法多數時候也是引擎好走的](/sql/readable-and-fast-mostly-align/) | 兩者同向與分岔的量測，以及分岔時該動哪一層 |

**編號動過的時候要掃三個地方。** 章節的編號是它在序列裡的位置，插入或搬動一篇會讓引用它的每一處失準，而失準的引用不會報錯——連結仍然解析得到，只是文字上的編號指向別人。三個地方是：本頁的三張章節表、`rg -n "1\.[0-9]+" content/sql` 掃出的篇間互引，以及分類之外的兩處——[backend 的 schema 設計](/backend/01-database/schema-design/)引 1.11、[python 的關聯代數對照](/python/08-data-analysis/same-relational-algebra/)引 1.10。這一條寫在這裡，是因為上一次重新編號就漏掉了分類之外的那一處。
...
| 兩種寫法都對，想知道哪個代價低                   | [1.13](/sql/cost-lives-in-the-plan/)：代價由資料與索引決定                                                                         |
| 查詢很慢，不知道從哪裡查起                       | [1.13](/sql/cost-lives-in-the-plan/) 先問代價由什麼決定；條件的形狀是第一個要排除的，在 [Sargable](/sql/knowledge-cards/sargable/) |
| 索引建了，計畫還是掃全表                         | [Sargable（可走索引的條件形狀）](/sql/knowledge-cards/sargable/)：欄位被包住就走不了查找                                           |
| 相關子查詢很慢，拆成 CTE 會不會比較好            | [1.15](/sql/readable-and-fast-mostly-align/) 的「方向常常相反」一節：一整段的時間隨列數超線性成長而 CTE 接近線性                   |
| 查詢很難讀，想知道改成好讀的會不會變慢           | [1.15](/sql/readable-and-fast-mostly-align/)：多數時候不用選，分岔時動 schema                                                      |
| 查詢答案是對的，而寫法讀起來像在做另一件事       | [1.14](/sql/declared-intent-vs-behaviour/)：宣告與行為分岔時怎麼查                                                                 |
| 要稽核別人寫的查詢，不知道從哪裡下手             | [1.14](/sql/declared-intent-vs-behaviour/) 的「換掉關鍵字」一節：問這段文字說的話可不可信                                          |
| 換一個資料庫之後找不到表                         | [1.11](/sql/identifier-rules/)：識別字送進引擎會被改寫                                                                             |
| 查詢回 `permission denied`                       | [1.12](/sql/privilege-model/)：權限的預設是什麼都不給                                                                              |
...
| ----------------------------------- | ------ | -------------- | ---- |
| 基數與選擇率                        | 知識卡 | 代價那篇       | 1    |
| 字串值的大小寫（LIKE 與 collation） | 主章   | 1.11 完成      | 小   |
| ORDER BY 與分頁                     | 主章   | 無             | 中   |
| SQL 引擎（實作差異的承載者）        | 知識卡 | 無             | 1    |
| 外鍵與參照完整性                    | 主章   | 1.6、1.14 完成 | 中   |
...
**「查詢跑得動而答案是錯的」目前沒有住址。** 1.1、1.2、1.5、1.6、1.8、1.9 與[約束](/sql/knowledge-cards/constraint/)、[子查詢](/sql/knowledge-cards/subquery/)、[NULL](/sql/knowledge-cards/null/) 三張卡都把它當成已知的前提在用（「不報錯」「照樣跑得動」「三個引擎都回零列」），而沒有一篇以它為題。缺的不是材料——`NOT IN` 碰到 `NULL`、輸出非分組欄、列數膨脹讓聚合變大，三種形態各自都寫得很完整。缺的是把它們並置起來說明這一類失效為什麼在關聯模型下必然安靜的那一篇。要不要獨立成章還沒決定：它可能該是一篇，也可能該是第一支的收尾段。

另外兩個題目還沒決定要不要做，所以不進上表。「引擎寬鬆度與可攜性的取捨」有五處供料（1.2、1.7、1.9、1.11、1.12）而無人並置，要先確認那個取捨值不值得獨立成章；「從問題走到工具的判準怎麼排序」要看上面那張問題表用起來夠不夠——它本身已經是那個缺口的一種修法。

...
**SQL 是宣告式的：寫的是要什麼結果，不是怎麼算出來。** 各篇的判斷標準都折算回這一條。

由它直接推出三種必須分開的順序——**書寫順序**（子句在文字上的先後）、**求值順序**（語意模型規定誰先誰後）、**執行順序**（引擎實際怎麼做）。三者不一致是宣告式這個選擇本身帶來的，而多數讀查詢時的誤判來自把其中兩種當成同一種。
...
| 窗口函數與 GROUP BY 的分工          | 主章   | 1.2 完成     | 中   |
| 基數與選擇率                        | 知識卡 | 代價那篇     | 1    |

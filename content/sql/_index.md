...
表上兩張新登記的卡各有量測支撐：「引擎」在本分類出現一百零六次而沒有一處說它指什麼（它是 DB 產品、執行元件、還是與最佳化器同層的東西，各篇讀起來不同），第二支整支的判斷標準都建在這個詞上；「外鍵」由[約束](/sql/knowledge-cards/constraint/)那張卡引進而本分類沒有承接篇，讀者拿到定義之後無處可去。

...
**段尾路由段是量最大的一項，而它的量測範圍在這一輪被校正過。** 同一個模具實測出現在 **32 篇、三個分類**（本分類 21 篇、一個料理短文分類 10 篇、一份方法論記錄 1 篇），三邊的句型逐字同構。本分類這 21 篇逐篇數過，段內各有三到六條「**把 A 換成 B**」的粗體條目，一篇都沒有換過形態。處置是逐篇改形態（有的適合寫成一段散文、有的只有一個去處不需要清單），而**不能整批套同一種新形態**——那是用新模具換舊模具，同一輪實測過一次。判讀與完成條件在 [#362](/report/mandated-section-hides-its-own-sameness/)。

...
| 手上的問題                                       | 去哪一篇                                                              |
| ------------------------------------------------ | --------------------------------------------------------------------- |
| 找出「沒有對應資料」的那些人或事                 | [1.9](/sql/in-exists-join/)：三種寫法各自描述什麼、選哪一種只問一句話 |
| 分組之後結果少了幾筆                             | [1.11](/sql/grouping-key-decides-the-unit/)：分組鍵決定一組代表誰     |
| 連接之後數量變多、總和比預期大                   | [1.7](/sql/join-changes-rows-and-nulls/)：一列配到多列就複製一次      |
| 條件寫了卻回零列，而且沒有報錯                   | [1.7](/sql/join-changes-rows-and-nulls/)：空值的比較是第三種答案      |
...
| 要拿同一批資料的兩列互相比較                     | [1.8](/sql/table-occurrence-and-alias/)：表的一次出現與別名           |
| 查詢報錯說某個名字在這裡用不了                   | [1.2](/sql/clause-evaluation-order/)：每一步手上有什麼                |
| 兩種寫法都對，想知道哪個代價低                   | [1.10](/sql/cost-lives-in-the-plan/)：代價由資料與索引決定            |
| 換一個資料庫之後找不到表                         | [1.5](/sql/identifier-rules/)：識別字送進引擎會被改寫                 |
| 查詢回 `permission denied`                       | [1.6](/sql/privilege-model/)：權限的預設是什麼都不給                  |
| 想知道引擎到底照什麼順序做事                     | [1.1](/sql/declarative-not-procedural/)：三種順序各自由誰決定         |

**選工具之前先把業務事實說成一句話。** 一句「沒消費過的顧客在訂單表裡不會有任何一列」就足以排除掉計數那一整條路——因為計數預設有東西可以數，而那些人一列都沒有。這一步在 [1.11](/sql/grouping-key-decides-the-unit/) 走了完整一遍。

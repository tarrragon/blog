...
**第一支問「結果集由什麼決定」。** 答案是一個明確的模型，而那個模型與直覺不同——子句的先後、連接的左右、條件擺哪一邊、分組的鍵，每一項都由[關聯代數](/sql/knowledge-cards/relational-algebra/)的組合規則定死，而每一項的規則都有一處與讀者的直覺相反。這一支要學的就是那個模型。

**第二支問「文字之外還有誰在決定」。** 四方各決定不同的東西：**識別字規則**決定名字指向哪個物件、**引擎的寬鬆度與設定**決定它接不接受這個寫法、**權限系統**決定送出這段 SQL 的人被允許做什麼、**最佳化器**決定要花多久——最後這一方是唯一不改變結果的，它受一個約束：輸出的列集合要與語意模型算出來的一致。

**第三支問「這段文字對讀的人說了什麼」。** 宣告式語言把要什麼結果寫成文字，於是那段文字同時是一份給人看的宣告——選了 `LEFT` 就是宣告預期有配不到的列，選了 `DISTINCT` 就是宣告這一組值本身會重複。引擎執行行為而從不查證宣告，所以宣告落空時查詢仍然回正確答案，代價落在下一個相信那段文字的人身上。
...
| [1.15 好讀的寫法多數時候也是引擎好走的](/sql/readable-and-fast-mostly-align/) | 兩者同向與分岔的量測，以及分岔時該動哪一層 |
...
| 查詢很難讀，想知道改成好讀的會不會變慢           | [1.15](/sql/readable-and-fast-mostly-align/)：多數時候不用選，分岔時動 schema                         |

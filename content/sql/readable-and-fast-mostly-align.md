...
這一篇與 [1.20](/sql/declared-intent-vs-behaviour/) 都問文字寫給誰。那一篇問文字對讀的人說了什麼，這一篇問文字該為誰而寫。

## 多數的寫法差異是免費的

同一批列的兩種寫法送進[最佳化器](/sql/knowledge-cards/query-optimizer/)之後常常收斂成同一個計畫。`CROSS JOIN` 加等值條件與 `JOIN ... ON` 寫同一個條件，計畫逐字相同（[1.20](/sql/declared-intent-vs-behaviour/) 有並排的輸出）。[1.8](/sql/in-exists-join/) 從另一條路走到同一個處置：肯定式的 `IN` 與 `EXISTS` 語意相同，所以選哪一個按可讀性挑。它的依據是語意而非計畫——這兩種寫法在 SQLite 上各自走不同的計畫（`LIST SUBQUERY` 對 `CORRELATED SCALAR SUBQUERY`），而語意相同已經足以讓選擇落在可讀性上。
...
真要在兩者之間排序的話，理由落在 [1.17](/sql/cost-lives-in-the-plan/) 已經證過的一件事上：同一組寫法的快慢排名會隨索引對調。**為當下的代價而扭曲的文字，在別人加了一個索引之後就失去理由，而那段文字會留著。** 可讀性沒有這個問題——它不隨資料庫的狀態改變。

這也對得上優化該動哪裡：[1.17](/sql/cost-lives-in-the-plan/) 列的那四項（資料量、分布、索引、統計資訊）全部在資料庫那一側，所以調整的位置在 schema 與計畫，而查詢的文字負責把意圖說清楚。
...
**把讀的人換成引擎**：同一段文字的問題從「說得清不清楚」變成「跑得快不快」，而跑得快不快答不出來的時候要向引擎要計畫。[1.17 代價由資料與索引決定](/sql/cost-lives-in-the-plan/) 拿一道重複值的題目量三種寫法，示範加一個索引之後最慢的變成第二快而查詢的文字一個字沒改。

**問這個代價該由哪一層吸收**：條件的形狀與索引的形狀是一組互補的選擇，把運算搬進索引就讓好讀的條件重新走得了。[Sargable（可走索引的條件形狀）](/sql/knowledge-cards/sargable/) 給判斷標準與三種改寫方向。

**可讀性之上還有兩層**：一層是文字說的話與查詢的行為對不對得上，[1.20 關鍵字宣告意圖，引擎只執行行為](/sql/declared-intent-vs-behaviour/) 寫宣告落空的幾種形態與查證它的動作；再上面一層是答案本身對不對，而引擎對那一層沒有回報的管道，[1.13 合不合法由引擎驗，答案對不對由提問的人負責](/sql/well-formed-is-not-correct/) 寫它為什麼不報錯。

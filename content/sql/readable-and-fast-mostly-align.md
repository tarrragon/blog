...
真要在兩者之間排序的話，理由落在 [1.16](/sql/cost-lives-in-the-plan/) 已經證過的一件事上：同一組寫法的快慢排名會隨索引對調。**為當下的代價而扭曲的文字，在別人加了一個索引之後就失去理由，而那段文字會留著。** 可讀性沒有這個問題——它不隨資料庫的狀態改變。

這也對得上優化該動哪裡：[1.16](/sql/cost-lives-in-the-plan/) 列的那四項（資料量、分布、索引、統計資訊）全部在資料庫那一側，所以調整的位置在 schema 與計畫，而查詢的文字負責把意圖說清楚。
...
**把讀的人換成引擎**：同一段文字的問題從「說得清不清楚」變成「跑得快不快」，而跑得快不快答不出來的時候要向引擎要計畫。[1.16 代價由資料與索引決定](/sql/cost-lives-in-the-plan/) 拿一道重複值的題目量三種寫法，示範加一個索引之後最慢的變成第二快而查詢的文字一個字沒改。

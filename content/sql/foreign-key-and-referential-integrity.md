...
本篇的表在[共用資料庫](/sql/#各篇共用的範例資料庫)上重建，差別是這一次把約束寫進結構裡；顧客表的三個人是佳穎、宗翰、雅文，而只有佳穎下過單。談補約束的那一節另外用到一張出貨表。指不到父列的那一列，下文叫**孤兒列**。

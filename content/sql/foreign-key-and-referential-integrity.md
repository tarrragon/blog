...
業務規則那一項接上 [1.13 合不合法由引擎驗，答案對不對由提問的人負責](/sql/well-formed-is-not-correct/) 的一個結論：「答案對不對」這一層有一部分判準交得出去，而交得出去的界線落在**判斷需要什麼**——在寫入的那一刻用該筆資料本身、或它指向的那一列就判得完的，寫得成約束。外鍵是這條界線上最靠外的那一種：它要讀另一張表的內容才判得出來，而讀的範圍到那一列為止。

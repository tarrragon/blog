...
**把「引擎每次寫入都查證的宣告」換成「引擎從不查證的宣告」**：`LEFT JOIN` 的 `LEFT` 也宣告了一件事——預期有配不到的列——而引擎照著算完就結束，宣告落空時查詢仍然回正確答案。[1.19 關鍵字宣告意圖，引擎只執行行為](/sql/declared-intent-vs-behaviour/) 把這一類收在一起，並給一個查證宣告的動作。兩篇合起來是同一個問題的兩邊：一段文字說的話，什麼時候會被執行。

...
這些推論的代價落在寫入那一側：每插進一列，資料庫要去父表確認那個值在不在，所以外鍵是拿寫入的工作換讀取的保證。這筆交換要用量測來判，而讀代價的工具在 [1.17 代價由資料與索引決定](/sql/cost-lives-in-the-plan/)。要不要加這條約束，則是設計的決定：它在強制完整性與演進自由度之間取捨，正式的核心資料與跨系統整合的答案不同——[Schema Design](/backend/01-database/schema-design/) 的「Table 與 Relation」一節寫這個取捨，並把宣告與執法分開，列成設計時要先確認的一項。
...
**同一個資料庫之外的指向不在射程裡。** 外鍵認的是另一張表，所以一個指向別的服務的識別碼、或指向物件儲存的一個鍵，資料庫無從檢查。這一類的完整性要在別的層落地，而本分類不涵蓋那些層。應用程式碼內部的落點分幾層、各層違反規則時發生什麼，在 [不變式的強制層次](/ddd/invariant-enforcement-layers/)——那一篇的作用域是單一物件的規則，跨資料庫的參照要往哪裡放本站還沒有落點。

...
這幾個位置的共同點是**外鍵只在寫入的那一刻現身**：查詢的文字裡沒有一個字提到約束，讀的時候也沒有差別。所以「這條保證在不在」要另外查，而查法在系統目錄那一側——PostgreSQL 用 `information_schema.table_constraints`、MySQL 用 `information_schema.REFERENTIAL_CONSTRAINTS`、SQLite 用 `PRAGMA foreign_key_list(表名)`。**建表語句裡看得到宣告，系統目錄裡看得到的才是生效的那些。**
...
本篇底下有四樣可以各自換掉：這條宣告生不生效、它保證的是哪一件事、被指向的那一側消失時怎麼辦、父表與子表是不是同一張表。

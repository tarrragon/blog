...
最難發現的是 `NOT IN`：子查詢的結果裡只要有一個 NULL，整段查詢回零列而三個引擎都不報錯。推導、實測與 `NOT EXISTS` 的安全範圍在 [1.6 連接產出的是新的關係，列數與空缺都變了](/sql/join-changes-rows-and-nulls/)。

## 設計責任

**否定式的成員判斷一律用 `NOT EXISTS`**，除非能保證子查詢那一欄不會有 NULL——而那個保證要來自欄位的[約束](/sql/knowledge-cards/constraint/)，不是來自習慣。

**那條約束是建表時的一個決定，而它有代價。** 允許為空讀起來就是事實的形狀（沒有值就是沒有值），不允許則要替「沒有」發明一個實際的值；兩條在設計當下都合理，而選了前者之後每一段碰到這一欄的查詢都要處理三值邏輯。[backend 1.16 設計時下的每一個決定，替往後每一次查詢定價](/backend/01-database/design-decisions-price-every-query/) 的第一節走這個取捨，並給改回來要付的遷移成本。

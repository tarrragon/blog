...
**比較：拿 NULL 去比對任何值，答案是第三種——未知。** `NULL = NULL` 的結果既不是真也不是假。而 `WHERE` 只留下判斷為真的列，所以「未知」與「假」在那一步的下場相同：列被丟掉。判斷有沒有值要用 `IS NULL` 與 `IS NOT NULL`，它們問的是位置有沒有值、答案只有真假兩種。
...
**否定式的成員判斷一律用 `NOT EXISTS`**，除非能保證子查詢那一欄不會有 NULL——而那個保證要來自欄位的約束，不是來自習慣。

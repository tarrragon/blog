...
**要看的是條件把不把兩邊綁起來，而不是用了哪個關鍵字。** `CROSS JOIN` 加一個等值條件，與 `JOIN ... ON` 寫同一個條件，對引擎是同一段查詢（[1.20 關鍵字宣告意圖，引擎只執行行為](/sql/declared-intent-vs-behaviour/)）；反過來，`JOIN` 寫了而 `ON` 的條件只提到單邊的欄位，展開照樣發生。

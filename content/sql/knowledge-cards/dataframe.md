...
DataFrame 是一張表在程式記憶體裡的表示：有具名的欄、有列，整份資料握在執行這段程式的行程手上。Python 的 pandas 是最常見的實作，R 與 Julia 各有自己的一套。它與 [relation](/sql/knowledge-cards/relation/) 裝的東西幾乎一樣，而兩者在三個地方分開。
...
**寫下的是動作還是描述。** DataFrame 的程式碼是一串照順序發生的動作：載入、篩選、合併。SQL 寫下的是要什麼結果，怎麼算由引擎決定。這一條是本分類每一篇的起點。

## 概念位置

DataFrame 在本分類裡的角色是對照組。宣告式這個性質單獨看不出形狀，跟一段逐步執行的程式並排才清楚——同一件事，一邊每一行指定下一步做什麼，一邊只描述要什麼。

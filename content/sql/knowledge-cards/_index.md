...
| [Cardinality 與 Selectivity（基數與選擇率）](/sql/knowledge-cards/cardinality-and-selectivity/) | 集合有幾個成員、條件留下多少比例；最佳化器走不走索引的依據  |
| [Semi-join 與 Anti-join（半連接與反連接）](/sql/knowledge-cards/semi-join-and-anti-join/)       | 配起來只帶走左邊那一份；`EXISTS` 與 `NOT EXISTS` 底下的運算 |

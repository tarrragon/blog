...
這與命名慣例是兩件事。取什麼名字是設計問題（見下方路由），這裡談的是取好的名字送進引擎會發生什麼。代價落在換一個引擎的時候：一段建表語句在原本那個引擎上建好了表，同樣的名字搬到另一個引擎去查卻找不到——名字在送進引擎的路上會先被改寫一次，而兩個引擎改寫的規則不一樣。
...
**這一篇為什麼屬於同一個主題**：查詢的文字不足以決定結果，識別字規則是補上差額的其中一方。四方並置在[模組頁的推導源頭](/sql/)那一段，而最佳化器那一方的完整推導在 [1.1](/sql/declarative-not-procedural/)。

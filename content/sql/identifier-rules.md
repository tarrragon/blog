...
**把「名字指到誰」換成「誰被允許動它」**：同一張表，一個問它叫什麼、一個問誰能讀寫。[1.13 權限的預設是什麼都不給](/sql/privilege-model/) 寫角色與 `GRANT` 的模型，以及新建的角色在拿到授權之前連讀都讀不了。

**把識別字規則放回四方並置的位置**：查詢的文字不足以決定結果，識別字規則是補上差額的其中一方。[模組頁的推導源頭](/sql/)那一段把四方並置——識別字規則、引擎的寬鬆度與設定、權限系統、最佳化器——而最佳化器那一方的完整推導在 [1.1](/sql/declarative-not-procedural/)。

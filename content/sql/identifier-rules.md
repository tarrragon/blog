...
**把名字換成值**：同一種大小寫問題在資料那一層由另一套規則管，而兩套彼此獨立——PostgreSQL 把沒加引號的表名摺成小寫，卻不會把 `'Anna'` 這個值摺成小寫。[1.14 字串的相等與大小由 collation 決定](/sql/string-comparison-and-collation/) 寫比較規則住在哪一層，以及索引與條件的規則要對得上。

**把「名字指到誰」換成「誰被允許動它」**：同一張表，一個問它叫什麼、一個問誰能讀寫。[1.15 權限的預設是什麼都不給](/sql/privilege-model/) 寫角色與 `GRANT` 的模型，以及新建的角色在拿到授權之前連讀都讀不了。

**把識別字規則放回四方並置的位置**：查詢的文字不足以決定結果，識別字規則是補上差額的其中一方。[模組頁的推導源頭](/sql/)那一段把各方並置——識別字規則、引擎的寬鬆度與設定、權限系統、最佳化器、約束與 collation——而最佳化器那一方的完整推導在 [1.1](/sql/declarative-not-procedural/)。

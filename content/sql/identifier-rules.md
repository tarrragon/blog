...
**把名字換成值**：同一種大小寫問題在資料那一層由另一套規則管，而兩套彼此獨立——PostgreSQL 把沒加引號的表名摺成小寫，卻不會把 `'Anna'` 這個值摺成小寫。[1.15 字串的相等、大小與索引可用性都由 collation 決定](/sql/string-comparison-and-collation/) 寫比較規則住在哪一層，以及索引與條件的規則要對得上。

**把「名字指到誰」換成「誰被允許動它」**：同一張表，一個問它叫什麼、一個問誰能讀寫。[1.16 權限的預設是什麼都不給](/sql/privilege-model/) 寫角色與 `GRANT` 的模型，以及新建的角色在拿到授權之前連讀都讀不了。

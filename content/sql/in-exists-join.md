...
**把「`NOT IN` 什麼時候失效」換成「這一類失效有什麼共同點」**：子查詢裡的一個 `NULL` 讓整段回零列，而零列與「真的沒有」在結果上分不開。[1.11 引擎驗的是合不合法，不是問對了沒有](/sql/well-formed-is-not-correct/) 拿同一個問題的三種錯法並排，說明形狀正確為什麼是這一類失效安靜的原因。

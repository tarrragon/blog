...
**把「`NOT IN` 什麼時候失效」換成「這一類失效有什麼共同點」**：子查詢裡的一個 `NULL` 讓整段回零列，而零列與「確實沒有」在結果上分不開。[1.11 合不合法由引擎驗，答案對不對由提問的人負責](/sql/well-formed-is-not-correct/) 拿同一個問題的三種錯法並排，區分「問了另一個問題」與「對模型的規則預期錯了」兩種來源。

...
**要不要用到被比對的那張表的欄位。** 要用到就得 `JOIN`，因為只有它把那張表帶進 `FROM`。這時候要順帶檢查列數是不是預期內的——後面如果接聚合，`sum` 與 `count` 算的是配對後的列；這個膨脹怎麼發生、可加與不可加的聚合各自怎麼修，在 [1.6 連接產出的是新的關係](/sql/join-changes-rows-and-nulls/)。

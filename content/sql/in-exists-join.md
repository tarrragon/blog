...
它一次只交得出一個值，所以要多欄或多列時仍然回到 `JOIN`；代價那一側它是逐列執行的，見 [1.14](/sql/cost-lives-in-the-plan/)。
...
只是判斷有沒有，就用 `IN` 或 `EXISTS`。它們是條件，對每一列只做留或丟，寫出來的意圖比 `JOIN` 加 `DISTINCT` 清楚——而哪一種比較快是另一個問題，答案取決於資料與索引（[1.14](/sql/cost-lives-in-the-plan/)）。
...
**把「按可讀性挑」推到整個語言**：本篇對這一組寫法下的結論是語意相同就按可讀性挑，而那個處置適用的範圍比這一組大。[1.16 好讀的寫法多數時候也是引擎好走的](/sql/readable-and-fast-mostly-align/) 量了哪幾類寫法差異是免費的、哪一類會分岔，以及分岔時第一個該問的問題。

**把「`NOT IN` 什麼時候失效」換成「這一類失效有什麼共同點」**：子查詢裡的一個 `NULL` 讓整段回零列，而零列與「確實沒有」在結果上分不開。[1.11 合不合法由引擎驗，答案對不對由提問的人負責](/sql/well-formed-is-not-correct/) 把這一種歸成「對模型的一條規則預期錯了」，而那一類的性質是修得掉——改對之後永遠對，另外兩類沒有這個性質。

**把「哪一種對」換成「哪一種快」**：三者都對的時候，快慢由資料與索引決定而不由寫法決定。[1.14 代價由資料與索引決定](/sql/cost-lives-in-the-plan/) 用一道重複值的題目量三種寫法，並示範加一個索引之後三者的排名重排。

...
只是判斷有沒有，就用 `IN` 或 `EXISTS`。它們是條件，對每一列只做留或丟，寫出來的意圖比 `JOIN` 加 `DISTINCT` 清楚——而哪一種比較快是另一個問題，答案取決於資料與索引。[1.17 代價由資料與索引決定](/sql/cost-lives-in-the-plan/) 用一道重複值的題目量這三種寫法，並示範加一個索引之後三者的排名重排。

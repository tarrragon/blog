...
本篇交代順序從哪裡來、`ORDER BY` 看得到什麼、以及比不出大小的值落在哪一端。**規則沒有規定的那一半在什麼時候變成錯誤，在 [1.12 分頁要一個全序](/sql/pagination-needs-a-total-order/)**——整批看的時候同分的列換位置沒有後果，每一頁各查一次的時候它變成重複與遺漏；那一篇寫排序鍵要補到兩兩可分的判斷標準，以及把游標從位置換成值的寫法。

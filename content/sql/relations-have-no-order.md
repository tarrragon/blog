...
範圍之外的兩件事各有專篇。哪些列會進到這份結果由條件與連接決定，順序管不到那一層：條件擺哪一邊決定留下哪些列在 [1.5](/sql/on-describes-where-filters/)，連接怎麼改變列數在 [1.6](/sql/join-changes-rows-and-nulls/)。視窗函數的 `OVER (ORDER BY ...)` 是另一個順序，它決定計算時誰算相鄰、不決定輸出怎麼排，兩者可以不同，[1.10](/sql/window-keeps-rows-grouping-collapses/) 的「相鄰的是什麼」一節寫那一種。

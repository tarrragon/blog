...
| 穩態元素 | 最小可用標準                                                                       | 判讀價值                   |
| -------- | ---------------------------------------------------------------------------------- | -------------------------- |
| 服務成功 | success rate / [error budget](/backend/knowledge-cards/error-budget/) 在可接受範圍 | 判斷是否需要升級事故       |
...
穩態 breach 的判斷標準對應兩個升級門檻：tick rate 低於感知門檻時，遊戲體驗開始劣化，需要啟動 load shedding 或關閉新 match 入口；session drop rate 超過門檻時，代表大量玩家掉線，需要升級事故等級並啟動 rollback。
...
Partial outage 適合處理 [blast radius](/backend/knowledge-cards/blast-radius/) 已被限制的事故。穩態定義應說明受影響 region、tenant、功能與預期恢復路徑，避免把局部可控誤讀成全域恢復。

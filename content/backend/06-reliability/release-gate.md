...
資料庫 migration 的 gate 要把 evidence 放回 rollout 階段判讀。Expand、backfill、cutover 與 contract 需要不同 checks：compatibility result、validation query、mismatch rate、replication lag、[rollback window](/backend/knowledge-cards/rollback-window/) 與 owner。完整欄位形狀可接到 [1.7 Schema Migration Rollout 證據](/backend/01-database/schema-migration-rollout-evidence/)。
...
Release gate 對所有變更套同一套標準會兩頭不討好：對小變更過度治理、對高風險變更治理不足。可重複套用的做法是先做變更分層、再對應分層 gate 政策。

對應 [MS1 Microsoft 變更治理與可靠性門檻](/backend/06-reliability/cases/microsoft/change-management-and-reliability-governance/)：揭露「變更分層 + 漸進發布 + 復盤回寫」三個機制、適用大型 SaaS 高頻變更累積回歸的場景。對應 [G2 Google Postmortem AI Closure](/backend/06-reliability/cases/google/postmortem-action-item-closure-governance/)：揭露 P0/P1 action item 必須綁定 release gate、未完成不得放行關聯變更（這層綁定讓 gate 從 release 工具升級為事故治理工具）。
...
分層政策的核心價值是「讓高風險變更承受高 gate 成本、讓低風險變更不被高成本拖累」。沒有分層的話，團隊容易在所有變更上採同樣門檻、結果是高風險變更走得太快、低風險變更走得太慢。

...
[Release gate](/backend/knowledge-cards/release-gate/) 是把放行決策從「看起來可以」變成「條件已經達成」的控制面。它的責任不是擋住所有變更，而是把哪些變更可以進、哪些變更要等、哪些變更必須先補證據說清楚。當 gate 被寫成政策，團隊就能用同一套條件判斷 CI、SLO、migration、相容性與高風險時段。

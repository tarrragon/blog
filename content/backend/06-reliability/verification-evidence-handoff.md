...
SLO evidence 適合支援變更節奏。當 [burn rate](/backend/knowledge-cards/burn-rate/) 上升或 [error budget](/backend/knowledge-cards/error-budget/) 緊張，release gate 需要知道哪些 user journey 受影響、資料品質是否可信、freeze 是否觸發。

Load test evidence 適合支援容量與成本決策。它要保留 workload model、traffic shape、data volume、dependency saturation、cost threshold 與觀測限制。

Chaos evidence 適合支援 incident drill。它要保留 injected failure、steady state、stop condition、[blast radius](/backend/knowledge-cards/blast-radius/)、[decision log](/backend/knowledge-cards/incident-decision-log/) 與 action item。

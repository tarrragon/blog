...
這裡的 replay 指的是系統自己重送或重播事件，來源是正常的可靠性機制。攻擊者攔截合法請求後刻意重送是另一個問題，收斂點在驗證素材的新鮮度而非冪等鍵，見 [Replay Attack](/backend/knowledge-cards/replay-attack/)。兩者的處置分工是：冪等鍵讓重複輸入不產生第二次副作用，新鮮度檢查讓重複輸入根本不被受理。

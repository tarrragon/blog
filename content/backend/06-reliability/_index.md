...
本輪規劃採問題驅動方法、用 SRE 領域 first-class 詞彙（[SLI](/backend/04-observability/sli-slo-signal/) / [SLO](/backend/06-reliability/slo-error-budget/) / [Error Budget](/backend/knowledge-cards/error-budget/) / Failure Mode / Chaos Hypothesis），把驗證議題拆成問題節點，蒐集公開 SRE 實踐作為服務級案例庫，再把控制面交接到可觀測性、部署平台與事故處理模組落地。
...
| SLO / Error Budget | 可靠性是否已經被消耗、變更是否還能繼續推進         | [burn rate](/backend/knowledge-cards/burn-rate/)、[error budget](/backend/knowledge-cards/error-budget/) remaining |
...
第一批缺口回填建議先做三條驗證題目：FinTech 補變更凍結與風險分層（回寫 6.6、6.8）；Gaming 補高峰流量下的穩態定義與回退條件（回寫 6.22、6.24）；Healthcare 補 DR rehearsal 與合規留存對 recovery 排序的影響（回寫 6.7、6.19）。

| 產業案例類型 | 驗證回寫重點                                        | 章節路由                                                                                                             |
| ------------ | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| FinTech      | error budget 觸發凍結、交易關鍵路徑的 release gate  | [6.6](/backend/06-reliability/slo-error-budget/)、[6.8](/backend/06-reliability/release-gate/)                       |
| Gaming       | 高峰事件前穩態定義、規則推送回退與停止條件          | [6.22](/backend/06-reliability/steady-state-definition/)、[6.24](/backend/06-reliability/rule-rollout-safety-gate/)  |
| Healthcare   | DR rehearsal 節奏、合規約束下的恢復驗證與 readiness | [6.7](/backend/06-reliability/dr-rollback-rehearsal/)、[6.19](/backend/06-reliability/reliability-readiness-review/) |
...
| [6.17 Feature Flag Governance](/backend/06-reliability/feature-flag-governance/)                   | Feature Flag Governance                   | 把 feature flag 從上線工具升級為有 lifecycle / debt 治理的 artifact                                                          |
...
| [6.19 Reliability Readiness Review](/backend/06-reliability/reliability-readiness-review/)         | Reliability Readiness Review              | 把上線前、重大變更前與高風險操作前的可靠性準備度變成可檢查門檻                                                               |
| [6.20 Experiment Safety Boundary](/backend/06-reliability/experiment-safety-boundary/)             | Experiment Safety Boundary                | 定義 chaos、load test、DR drill 的 blast radius、停止條件與權限約束                                                          |
| [6.21 Reliability Debt Backlog](/backend/06-reliability/reliability-debt-backlog/)                 | Reliability Debt Backlog                  | 把反覆事故、演練缺口與手動修復累積成可排序、可關閉的 reliability debt                                                        |
| [6.22 Steady State Definition](/backend/06-reliability/steady-state-definition/)                   | Steady State Definition                   | 在 chaos 與 failover 前先定義系統應維持的穩定狀態與可接受退化                                                                |
| [6.23 Verification Evidence Handoff](/backend/06-reliability/verification-evidence-handoff/)       | Verification Evidence Handoff             | 把 SLO、load、chaos、DR 與 readiness 結果包成 release / incident 可用證據                                                    |
| [6.24 規則推送安全閘門](/backend/06-reliability/rule-rollout-safety-gate/)                         | Rule Rollout Safety Gate                  | 把規則、策略與控制面配置推送變更納入高擴散風險 gate                                                                          |
| [6.25 Provider Dependency Release Gate](/backend/06-reliability/provider-dependency-release-gate/) | Provider Dependency Release Gate 實作示範 | 以 payment provider 變更示範 gate、stop condition 與 [rollback window](/backend/knowledge-cards/rollback-window/) 的實作交接 |

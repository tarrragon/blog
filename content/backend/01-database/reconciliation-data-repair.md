...
對應 [9.C20 Zomato](/backend/09-performance-capacity/cases/zomato-tidb-to-dynamodb-migration/) — migration 期間 shadow read 持續對帳、抓 mapping 規則漂移。
...
備份策略決定 RTO（Recovery Time Objective、多久能恢復）跟 RPO（Recovery Point Objective、最多丟多少資料）、但這兩個技術指標要 *對照業務可接受中斷時間*、才能判斷夠不夠。常見錯誤是把 RTO/RPO 訂在「技術上能做到的最佳值」、忽略業務側真正的容忍上限。
...
判讀重點：對照表不是寫一次就完、要每年 review。業務模式變了（例如從 B2C 變 B2B 客服 SaaS）、容忍時間會大幅縮短、RTO 必須跟著降。
...
- 每個角色有 *primary* + *backup*、單人不可用時不卡
- 角色責任寫進 runbook、不是事故當下查職位
- 定期 tabletop 演練、角色之間的交接點明確
- 跨時區團隊有 follow-the-sun 輪值、不能只靠單一時區

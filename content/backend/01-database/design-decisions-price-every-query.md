...
**改回來要付什麼**。開啟執法之前要先清掉存量的孤兒列，而「清掉」是個業務決定——那些訂單是要補一個顧客、還是要刪掉、還是要標記。跟第一節一樣，這個決定當初沒有人做，所以現在要回頭做。補約束的完整順序在 [1.6 資料庫轉換實作：雙寫、回填、切流與回滾](/backend/01-database/database-migration-playbook/) 的加約束段。

...
- **UUID v4**：隨機分布、適合分散式系統、寫入分散；缺點：16 byte 比 bigint 大、index 體積大、隨機 insert 拖累 B-tree
- **UUID v7 / ULID**：時間序、保留 UUID 隨機性 + 加時間前綴、insert 性能接近 sequential
- **Bigint sequence**：最快寫入（sequential）、index 小；缺點：跨 shard 容易衝突、洩漏業務資訊（用戶數可猜）
- **Snowflake-like**：64-bit、含 timestamp + machine_id + sequence、適合分散式 + 時間序需求

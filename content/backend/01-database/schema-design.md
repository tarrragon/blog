...
主鍵策略要先回答「如何穩定識別」與「如何支援查詢」。自然鍵可讀性高但變動風險高；代理鍵穩定且易擴展、常搭配業務唯一鍵一起使用。外鍵策略則要平衡完整性與演進自由度：正式核心域可強約束、跨域整合可由應用層保護並保留遷移彈性。另外要分清宣告與執法：schema 宣告了 FK 不等於執法開啟（連線層開關、約束的 NOT VALID 狀態、storage engine 都可能讓宣告形同註解）、給存量表補約束的順序見 [1.6 的 Type I 加約束](/backend/01-database/database-migration-playbook/)。
...
取好的名字送進引擎之後會被改寫（大小寫摺疊、引號、保留字），那一層在 [SQL 1.12 識別字送進引擎之後會被改寫](/sql/identifier-rules/)。本節談的是取什麼名字。

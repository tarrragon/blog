...
3. 與 04 的交接：交易失敗需要對齊 [Observability Evidence Package](/backend/04-observability/observability-evidence-package/) 的查詢與證據欄位。
4. 與 06 的交接：高風險交易變更納入 [Release Gate](/backend/06-reliability/release-gate/) 與 [Migration Safety](/backend/06-reliability/migration-safety/)。
5. 與 08 的交接：交易層回退或 fail-forward 判斷記錄到 [Incident Decision Log](/backend/08-incident-response/incident-decision-log/)。

## 下一步路由

要把交易與資料演進放在同一路徑看，接著讀 [1.6 資料庫轉換實作](/backend/01-database/database-migration-playbook/) 與 [6.11 Migration Safety](/backend/06-reliability/migration-safety/)。要看交易邊界如何落成 production migration 的 validation query、gate 與 decision log，接著讀 [1.7 Schema Migration Rollout 證據](/backend/01-database/schema-migration-rollout-evidence/)。要把交易外副作用接到非同步流程，接著讀 [3.3 outbox pattern](/backend/03-message-queue/outbox-pattern/)。

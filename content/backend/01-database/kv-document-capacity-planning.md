...
### 彈性不是魔法、是 partition 設計結果

讀「DynamoDB IOPS 從 20 衝到 135,000」這種數字、不該理解成「DynamoDB 神奇」、是 *partition key 設計均勻的結果*。partition key 不均、IOPS 上限就是「最熱 partition 上限」（DynamoDB 每 partition 3000 RCU / 1000 WCU）、不是「總和」。

對應 [9.C15 Tixcraft](/backend/09-performance-capacity/cases/tixcraft-ticketing-flash-sale-spike/) — 售票 IOPS 6,750 倍彈性是 partition key 設計（按 user_id 分散、不是按 event_id）的結果。如果用 event_id 當 partition key、同一場演唱會所有訂單擠進同一個 partition、實際 IOPS 上限是 1000 WCU、不是 135K。

判讀重點：看到「Amazon Ads 9000 萬 reads/sec」、「DynamoDB 1.51 億 RPS」這種數字、要追問「partition 設計是什麼」、不是反射認為自己的服務也能達到。換 DynamoDB *不會* 自動拿到這個彈性、必須先解決 partition key 設計、再選 DB。順序顛倒會踩到「換了 DB 但 hot partition 依舊」的坑。
...
- **新服務 / 中小規模**：選計費粒度小的選項（Cosmos DB serverless、Spanner granular sizing、DynamoDB on-demand）、避免一開始就過配
- **穩定大規模**：計費粒度可大（DynamoDB provisioned with reserved capacity、Spanner full-node provisioning）、單價較低
- **POC / 測試**：永遠選 on-demand 或 serverless、不要為了測試付 reserved 成本

判讀重點：計費粒度是 *vendor 商業策略* 也是 *工程顆粒*、選 vendor 時不只看 max throughput、要看 *min sizing* 跟 *增量 granularity*。
...
- 新增 cache：讀次數從 100 降到 5（cache hit rate 95%）
- 新增 anti-fraud 檢測：每寫入觸發 N 次 read 驗證

判讀重點：容量規劃 review cadence 不只看流量、要 review *讀寫比* 是否漂移。比例跳量級是設計需要重做的訊號、不是 capacity 增加的訊號。
...
flash-sale 或極端負載場景、限流不一定是顯眼的元件。看架構圖找不到「rate limiter」、不代表沒限流。

對應 [9.C15 Tixcraft](/backend/09-performance-capacity/cases/tixcraft-ticketing-flash-sale-spike/) — 售票架構圖上看不到明確「rate limiter」元件、但實際上限流發生在多層：
...
判讀重點：選哪種限流模式取決於業務願意接受什麼用戶體驗、不是工程偏好。隱性限流犧牲透明度換流量吸收能力、明確限流犧牲流量吸收能力換體驗可見度。兩者並存、沒有「best practice」。

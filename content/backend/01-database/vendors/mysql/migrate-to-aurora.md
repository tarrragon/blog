...
date: 2026-05-19
description: "自管 MySQL → Aurora MySQL 是 Type C operational hybrid migration — wire protocol 一致、ops 責任轉到 AWS。本文走 6 維 audit（Operational High）、Aurora storage architecture 衝擊、4-phase migration、5 production 踩雷、何時不要遷。"
weight: 26
...
### 3. Aurora-only feature 寫進 application、不能 rollback
...
- *Backtrack*：72 小時內 in-place rollback 整個 DB（不是 PITR）
...
- 把 Aurora-only feature 跟 *Aurora 特定 cluster* 綁定、不要把 application 邏輯依賴 Aurora-only
...
- Application connection pool 強制 *DNS re-resolve*（不要 cache）
...
自管 cross-region replication 是 *chained replication*（primary → region2 replica → region2 cascading replica）。Aurora Global Database 是 *storage-level replication*（不是 binlog、是 storage page 直接 ship）、跨 region < 1 秒 lag、failover < 1 分鐘。

但 Aurora Global Database 不是 *active-active*、是 *active-passive*（primary region 可寫、secondary region 只讀）。如果原本自管已經 cross-region active-active write（用 multi-master 或應用層 sharding）、Aurora Global Database 滿足不了。
...
| OS-level access        | 完整 SSH access        | 無（managed、不能 SSH）              |
| MySQL 8.0 latest patch | 你決定                 | 跟 Aurora major version 對應、有滯後 |
| InnoDB log_file_size   | 自己改                 | 不能改（Aurora 不用 redo log）       |

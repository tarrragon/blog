...
選 Vitess 的核心 driver：*保留 MySQL wire protocol + 應用層幾乎不必改 + 透明分片*。代價是 4 個 component 的 operational complexity — Vitess 不是「裝個 proxy」、是「裝個分散式系統」。
...
VReplication 不是 *application 用的 feature*、是 *Vitess operator 用的工具*。但跟 application 行為直接相關（resharding 期間有 write split 行為）。
...
- 大規模需要 atomic 的場景應該換 distributed SQL（CockroachDB / Spanner）、不是繼續 Vitess
...
- Resharding 期間 *暫停非關鍵 VStream*（analytics ETL 可暫停、real-time recommendation 不能）
- 確認 binlog disk capacity > resharding 期間預估 binlog 量 × 2（buffer）
- Resharding 完成後 *手動驗證* VStream offset 已 catch up（不是自動驗證）
...
- 真的失敗用 `Reshard cancel` 回 old state、不要硬幹

### 5. VReplication workflow 卡住 — 不能 cancel
...
兩者不是直接替代。超過 Aurora single-region 上限的場景才考慮 Vitess。詳見 [Aurora vendor page](/backend/01-database/vendors/aurora/)。

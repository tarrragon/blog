...
PlanetScale（managed Vitess）走 *branch-based schema migration* — 建 schema branch、跑 schema change、deploy 時 atomic merge。schema change 由 PlanetScale 內建流程承擔。詳見 [PlanetScale migration playbook](/backend/01-database/vendors/mysql/migrate-to-planetscale/)。

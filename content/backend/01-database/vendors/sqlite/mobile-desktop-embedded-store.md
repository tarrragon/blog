...
Embedded state model 的核心責任是把 local database file 放回 application lifecycle。SQLite 是典型的 [embedded database](/backend/knowledge-cards/embedded-database/)：database file 通常跟著 app sandbox、user profile、CLI config directory 或 device storage 存在，它的 owner 是 application，而非獨立 DBA。

...
[Interleaved Table](/backend/knowledge-cards/interleaved-table/) 把 parent table（如 `Customer`）跟 child table（如 `Order`）的 row 在 storage 層 *物理上交錯儲存* — child row 跟對應 parent row 在同一個 split。不是純 foreign key、是 storage layout：
...
Interleaved 的效果：parent + child JOIN 在同一個 [Range Sharding](/backend/knowledge-cards/range-sharding/) split 完成、不跨 split = 不跨 Paxos group = 低延遲 transaction。這條設計把「FK 是 logical constraint」翻成「parent-child access pattern 是 physical co-location」、對 access pattern 固定的 workload（customer → orders、user → posts、tenant → records）是巨大 latency benefit。

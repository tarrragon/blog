...
**Fact vs derive 分層警告**：本段「coordinator bottleneck → TrueTime + Paxos」frame 是跨 Spanner 2012 OSDI 論文 + 公開文件（2024-2026）+ 9.C10 case 合成的工程 frame、不是 9.C10 case 直接展開實作層細節。9.C10 案例直接揭露的 fact 是線性擴展數字跟 dogfood 邊界；本文 derive 的 frame 是「為什麼傳統 OLTP coordinator 是 bottleneck」。寫稿時這條分層在每段引用具體數字時都會重申。
...
**Fact source 分層警告**：1-7ms 是 Google 2012 OSDI 論文 + Spanner 公開文件（2024-2026）引用的範圍、9.C10 dogfood case 未直接揭露 production ε 分布。寫稿時這組數字明標「來自 Spanner vendor docs / 2012 論文、不是 9.C10 case 直接揭露」、避免讀者把兩種來源混為一談。
...
commit wait ≈ 2ε 的數學保證了「下一個 transaction 拿到的 timestamp 一定 > s」、external consistency 的全序性質就由這個 wait 撐住。**Fact source 分層**：commit wait ≈ 2ε 的推導來自 Spanner 2012 OSDI 論文 + 官方文件、不是 9.C10 case 直接展開實作層數學。寫稿時引用這條數學要附「來源 vendor docs / paper」、避免讀者誤以為這是 case 揭露。
...
- [ ] TrueTime 是手段、line-rate scaling 才是目的的商業邏輯先行段在「問題情境」之前
- [ ] 9.C10 數字（45K / 90K reads、10 億 req/sec）每處引用都明示「Google internal dogfood、不是 customer-facing capacity」
- [ ] TrueTime ε 1-7ms 範圍跟 commit wait ≈ 2ε 數學引用都明標「vendor docs / paper 來源、非 case」
- [ ] external-consistency / linearizability / quorum 三張卡互引
- [ ] sibling 對比 PostgreSQL serializable（SSI）、Aurora DSQL TrueTime 替代方案（跨 sibling 文章展開）

...
- 從 PostgreSQL 升級 Spanner 的判斷標準：流量明確跨 region，且跨 region 一致性是 product requirement
...
這條路由的判斷標準是交易順序是否跨 region 影響產品正確性。Spanner 的價值在 external consistency、schema 與 SQL 能力、全球 deployment 與 Google Cloud operation model 的組合；若產品只需要 eventual / session consistency，較輕的 NoSQL 或 managed SQL 常有更低成本。

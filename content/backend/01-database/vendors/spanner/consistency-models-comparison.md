...
跨洲 quorum 在這個模型裡是 *dominant term*、不是 commit wait — 寫稿時要明示「commit wait 跟跨 region quorum 是兩個獨立的物理 cost、不能混用一個 latency 數字解釋兩者」。讀者常見的誤解是把 100-200ms 寫成「Spanner commit wait」、實際 commit wait 只是其中 2-14ms、剩下 100ms+ 是物理光速限定的 quorum RTT。
...
Strong consistency + multi-region 互斥議題（包含 Cosmos DB 5 levels 的 Strong + multi-region 限制）的 SSoT 是 [Cosmos DB consistency-levels-engineering](/backend/01-database/vendors/cosmosdb/consistency-levels-engineering/)。本篇 cross-link 不展開、避免重複展開同議題。
...
Strong consistency + multi-region 互斥議題的 SSoT 在 [Cosmos DB consistency-levels-engineering](/backend/01-database/vendors/cosmosdb/consistency-levels-engineering/)、本篇不重複展開。

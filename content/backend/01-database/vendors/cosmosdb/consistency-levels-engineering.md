...
- 限制：*single-write region 限制*；multi-region write 不可同時用 Strong（時間敏感 claim、查 [最新文件](https://learn.microsoft.com/azure/cosmos-db/consistency-levels)）
- 適合：金融交易、庫存扣減、status 機器寫後 read confirm
- 為什麼互斥：詳見 [multi-region-write-conflict](../multi-region-write-conflict/) 的 AP 取捨段、本篇不展開

### Bounded staleness

- 機制：read 落後 *不超過 K 個 version 或 T 秒*（取較嚴格者）；單 region 內 linearizable、跨 region 有 bounded lag
...
- Strong / Bounded read ≈ 2x Session / Eventual 的 RU
...
互動式產品其實 Session 就夠、用 Strong 浪費 2x RU + 限制 multi-region write、cost 暴漲且 multi-region 配置受限。徵兆是「RU consumption 明顯偏高、且 multi-region write 開不起來」 — 才發現預設選 Strong。

...
| --------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Access control / RLS  | table、row、function、extension 與 service account 權限如何切                    | [7.4 Data Protection](/backend/07-security-data-protection/data-protection-and-masking-governance/)、[Audit Log](/backend/knowledge-cards/audit-log/)                                                     |
...
| Cross-region DR       | region 失效時要 async replica、PITR、Aurora Global Database 還是 distributed SQL | [RPO](/backend/knowledge-cards/rpo/)、[RTO](/backend/knowledge-cards/rto/)、[Failover](/backend/knowledge-cards/failover/)、[PITR + WAL Archiving](pitr-wal-archiving/)、[Patroni HA](patroni-ha/)        |
| Developer / DBA split | application schema、migration、query、index 與 rollback 誰負責                   | [1.2 Schema Design](/backend/01-database/schema-design/)、[1.6 Migration Playbook](/backend/01-database/database-migration-playbook/)、[6.11 Migration Safety](/backend/06-reliability/migration-safety/) |
...
| Cloud SQL / AlloyDB            | GCP 內 managed PostgreSQL 與 Google operation model         | extension / version matrix、IAM / backup / cost model | 後續 managed PG comparison deep article                                                                                                                               |
...
上述候選已分流到既有引用路徑。Logical decoding 先接 [Logical Replication + Debezium](logical-replication-debezium/) 與 [Replication Slot Management](replication-slot-management/)；pg_partman advanced 先接 [Declarative Partitioning](declarative-partitioning/)；pooler comparison 先接 [Connection Scaling](connection-scaling/) 與 [pgBouncer Config](pgbouncer-config/)；Aurora cost 先接 [→ Aurora](migrate-to-aurora/)；AlloyDB / Cloud SQL 先停在本頁 managed PG 變體路由，等實際 migration 或 managed selection case 出現再升 deep article。
...
- **待升級維度**：Security / RLS / audit logging、cross-region DR、application developer vs DBA 視角分工已補 overview 路由；YugabyteDB / TiDB migration playbook、pgvectorscale / Cosmos DB for PG / AlloyDB managed 變體仍等實際案例或選型需求再升 deep article / playbook。

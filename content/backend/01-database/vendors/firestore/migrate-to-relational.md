...
主導維度是 **paradigm 與 application change**：六維裡五維落在 High。這定義了結構 — 不是 schema 翻譯（Type A）、不是 drop-in（Type B），而是 **Type E paradigm shift**：存取模型反轉、部分能力重建、可能長期混合（資料層自建、認證仍留平台）。

...
| [1.7](/backend/01-database/schema-migration-rollout-evidence/) | Schema Migration Rollout Evidence 實作示範 | 以訂單付款狀態欄位演進示範 evidence、gate 與 decision log |
...
資料庫的第一條實作路徑已完成： [1.7 Schema Migration Rollout Evidence 實作示範](/backend/01-database/schema-migration-rollout-evidence/)。這篇以訂單資料表付款狀態欄位演進為例，說明 migration plan、validation query、rollback condition 與 incident decision route 如何一起成立。

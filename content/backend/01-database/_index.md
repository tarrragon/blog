...
**查詢語言本身另有一個模組。** 子句怎麼切、連接的左右決定誰被保護、條件放 `ON` 還是 `WHERE`、代價為什麼不在查詢的文字裡——那些在 [SQL：這個語言為什麼長這樣](/sql/)。本模組接手的有兩段，而它們在時間軸上分別落在查詢的兩側。**查詢寫下來之後**的是交易邊界、遷移、每請求的查詢次數預算。**查詢還不存在的時候**的是設計——建表那一刻下的每一個決定都在替往後每一次查詢定價，而那個代價在設計當下看不出來（[1.16 設計時下的每一個決定，替往後每一次查詢定價](/backend/01-database/design-decisions-price-every-query/) 走六個這樣的決定並逐條實測）。兩段之間的那一層——查詢怎麼寫對——才是 SQL 分類的責任。
...
| 章節                                                                                                                                      | 主題                                                     | 關鍵收穫                                                                                                                                   |
| ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| [1.1 高併發下的 SQL 讀寫邊界](/backend/01-database/high-concurrency-access/)                                                              | 高併發下的 SQL 讀寫邊界                                  | 共用 `sql.DB`、控制連線池、縮小 transaction 範圍                                                                                           |
| [1.2 Schema Design 與資料建模](/backend/01-database/schema-design/)                                                                       | schema design 與資料建模                                 | 規劃 table、index、key 與命名規則                                                                                                          |
| [1.3 Transaction 與一致性邊界](/backend/01-database/transaction-boundary/)                                                                | transaction 與一致性邊界                                 | 判斷何時使用 transaction、retry 與 isolation                                                                                               |
| [1.4 Repository Adapter 實作](/backend/01-database/repository-adapter/)                                                                   | repository adapter 實作                                  | 把 SQL row mapping 與錯誤轉換封裝成 adapter                                                                                                |
| [1.5 攻擊者視角（紅隊）：資料層弱點判讀](/backend/01-database/red-team-data-layer/)                                                       | 攻擊者視角（紅隊）：資料層弱點判讀                       | 用越權查詢、資料外洩路徑與恢復成本檢查資料層設計                                                                                           |
| [1.6 資料庫轉換實作：雙寫、回填、切流與回滾](/backend/01-database/database-migration-playbook/)                                           | 資料庫轉換實作                                           | 把雙寫、回填、切流與回滾做成可分段驗證流程                                                                                                 |
| [1.7 Schema Migration Rollout 證據（Schema Migration Rollout Evidence）實作示範](/backend/01-database/schema-migration-rollout-evidence/) | Schema Migration Rollout 證據實作示範                    | 以訂單付款狀態欄位演進示範 evidence、gate 與 decision log                                                                                  |
| [1.8 State Ownership 與 Query Boundary](/backend/01-database/state-ownership-query-boundary/)                                             | State Ownership 與 Query Boundary                        | 分辨正式狀態、派生狀態與不同查詢責任                                                                                                       |
| [1.9 Reconciliation 與 Data Repair](/backend/01-database/reconciliation-data-repair/)                                                     | Reconciliation 與 Data Repair                            | 把資料錯誤轉成可驗證、可修復、可稽核流程                                                                                                   |
| [1.10 KV / Document DB 容量規劃](/backend/01-database/kv-document-capacity-planning/)                                                     | KV / Document DB 容量規劃                                | partition key 設計、capacity mode、multi-model 取捨                                                                                        |
| [1.11 全球分散式 OLTP](/backend/01-database/global-distributed-oltp/)                                                                     | 全球分散式 OLTP                                          | Spanner / Aurora DSQL / Cosmos DB multi-region 跟 [CAP](/backend/knowledge-cards/cap/) 取捨                                                |
| [1.12 大規模 DB 遷移實戰](/backend/01-database/large-scale-db-migration/)                                                                 | 大規模 DB 遷移實戰                                       | dual-write / [shadow read](/backend/knowledge-cards/shadow-read/) / cutover / [rollback window](/backend/knowledge-cards/rollback-window/) |
| [1.13 應用層查詢反模式與 Query 預算](/backend/01-database/query-anti-patterns/)                                                           | 應用層查詢反模式與 Query 預算                            | N+1、select *、缺索引、ORM lazy load、long transaction 與每請求 query 預算                                                                 |
...
| [1.16 設計時下的每一個決定，替往後每一次查詢定價](/backend/01-database/design-decisions-price-every-query/)                               | 設計時下的每一個決定，替往後每一次查詢定價               | 六個 schema 決定各自讓查詢多付什麼、代價在什麼條件下才浮現、改回來要動什麼                                                                 |
...
| 補完方向         | 需要回答的問題                                       | 主要路由                                                                                                                                                                                                        |
| ---------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| State ownership  | 哪些資料是正式狀態，哪些只是 cache、index 或事件副本 | [source of truth](/backend/knowledge-cards/source-of-truth/)、[0.2 狀態與資料儲存選型](/backend/00-service-selection/state-storage-selection/)                                                                  |
| Query boundary   | 交易查詢、列表查詢、報表查詢與對帳查詢是否混在一起   | [4.20 Observability Evidence Package](/backend/04-observability/observability-evidence-package/)、[4.17 Telemetry Data Quality](/backend/04-observability/telemetry-data-quality/)                              |
| Migration safety | schema 變更是否能分批、驗證、暫停與回退              | [6.11 Migration Safety 與 DB Rollout](/backend/06-reliability/migration-safety/)、[6.8 Release Gate 與變更節奏](/backend/06-reliability/release-gate/)                                                          |
| Reconciliation   | 資料錯誤發生後如何驗證、修復、對帳與留下證據         | [8.19 Incident Decision Log](/backend/08-incident-response/incident-decision-log/)、[8.22 Incident Evidence Write-back](/backend/08-incident-response/incident-evidence-write-back/)                            |
| Data protection  | 正式資料在查詢、匯出、修復與刪除時如何保留責任邊界   | [7.4 資料保護與遮罩治理](/backend/07-security-data-protection/data-protection-and-masking-governance/)、[7.7 稽核追蹤與責任邊界](/backend/07-security-data-protection/audit-trail-and-accountability-boundary/) |
...
資料庫 vendor 文章的下一輪重點是把 PostgreSQL / MySQL batch 經驗變成可重複使用的撰寫規格。後續寫 SQLite、MongoDB、DynamoDB、Aurora、Spanner、Cosmos DB 與 CockroachDB 前，先讀 [資料庫 Vendor 文章撰寫規格](/backend/01-database/vendor-article-spec/)；該文分清 vendor overview、deep article 與 migration playbook 的責任，並列出 PG / MySQL 回收出的橫向調整項。
...
- → [infra 模組五：資料庫上 IaC](/infra/05-core-services/deployment-order-database/)：RDS 的 IaC 描述（subnet group、parameter group、連線管理、read replica）與部署順序
- → [infra 模組五：Stateful 資源保護](/infra/05-core-services/stateful-protection-dependency/)：multi-AZ、backup retention、deletion protection、PITR 的 IaC 設定

...
該用同步 RPC 的少數場景：使用者請求路徑需要立即回應（「使用者按下查詢、顯示結果」）、且兩個服務都在同一個 latency budget 內。其他都優先事件。（選了同步 RPC 之後、內部框架與協議的選型見 [gRPC 內部 RPC 選型](/backend/11-api-design/styles/grpc/grpc-internal-rpc-selection/)。）

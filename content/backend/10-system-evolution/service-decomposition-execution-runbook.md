---
title: "10.2 服務拆分執行 Runbook（Strangler Fig / 雙寫期 / 切流 / 回退）"
date: 2026-05-27
description: "10.1 決定該拆之後、實際怎麼動手拆 — Strangler Fig pattern、雙寫期管理、切流策略、回退條件設計"
weight: 2
...
1. 與 [10.1 服務拆分判讀](/backend/10-system-evolution/service-decomposition-boundaries/) 的交接：10.1 給「該拆」的判讀、本章給「怎麼拆」的執行。
...
要看拆分判讀（該不該拆）、回 [10.1 服務拆分與邊界判讀](/backend/10-system-evolution/service-decomposition-boundaries/)。要看拆分後跨服務通訊設計、進 [03 模組訊息佇列](/backend/03-message-queue/)。要看部署層的切流技術細節、進 [5.8 Deployment Rollout](/backend/05-deployment-platform/deployment-rollout-drain-rollback/)。

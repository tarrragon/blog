...
2. **回源行為控制**：edge 沒命中時用 [Cache Stampede](/backend/knowledge-cards/cache-stampede/) 保護機制（origin shield、coalescing、request collapsing）把同時打進來的 N 個請求合併成一次 origin 呼叫，避免擊穿。
3. **failure fallback**：origin 不健康時，edge 可以回傳 stale-while-revalidate 或 stale-if-error 的舊版本，避免使用者直接看到 5xx。代價是 [Stale Data](/backend/knowledge-cards/stale-data/) 風險暫時提高，需要在 freshness budget 內。
...
- [9.C13 Hotstar：1800 萬同時觀眾的 IPL 直播](/backend/09-performance-capacity/cases/hotstar-ipl-eighteen-million-concurrent/) — 看 CDN 如何配合 origin shield、多 CDN 策略支撐極端峰值；對照本章「origin protection」段判讀命中率提升如何反映在 origin 壓力。
- [9.C18 Zoom：COVID 30 倍突發](/backend/09-performance-capacity/cases/zoom-covid-surge-dynamodb/) — 看靜態資源（會議連結頁、登入頁）如何被邊緣層吸收，讓 API 集群只面對真實會議流量。
- [2.C7 Cloudflare Cache Reserve 與 Tiered Storage](/backend/02-cache-redis/cases/cloudflare-cache-reserve-tiered-storage/) — 看 CDN 自身如何用 tiered storage 把 hot / warm / cold 內容分層；對照本章「三層快取」段。

引用時的判讀順序：先看案例的 cache hit ratio 跟 origin 流量比例，再對照本章「origin protection」三大策略；接著看 purge 操作的時序，對照「purge 操作模型」段判讀全球同步延遲如何被處理；最後看 stale-if-error 與 fallback 的使用方式，對照「failure fallback」段。
...
要把邊緣失效跟應用層失效串成一條 invalidation pipeline，接著讀 [2.2 cache aside 與失效策略](/backend/02-cache-redis/cache-aside/)。要看高峰活動如何把 CDN 與排隊機制組合成保護網，接著讀 [9.11 高峰事件準備](/backend/09-performance-capacity/peak-event-readiness/)。要看 origin 端的入口流量合約怎麼寫，接著讀 [5.3 load balancer 合約](/backend/05-deployment-platform/load-balancer-contract/)。

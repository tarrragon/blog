...
2. **Multi-stage build**：把建置環境（compiler、dev dependencies）和執行環境分開。最終 image 只包含 runtime 必要檔案，體積小且攻擊面收窄。指令、layer 與 multi-stage 的實作見 [Dockerfile 設計](/backend/05-deployment-platform/vendors/docker/dockerfile-design/)。
...
**多進程容器**：一個容器跑多個進程時，PID 1 要負責信號轉發與子進程回收（zombie reaping）。如果 PID 1 不做 wait()，結束的子進程會變成 zombie。解法是用 tini 或 dumb-init 作為輕量 init，或在 Kubernetes 設 `shareProcessNamespace: true` 讓 kubelet 處理。這也是「一容器一職責」原則的 runtime 依據之一——把不同服務塞進一個容器會讓 PID 1 語意變糊，該不該拆見 [一容器一職責](/backend/knowledge-cards/container-per-service/)。
...
memory limit 是硬邊界——超過就 OOM kill，不走 graceful shutdown（被 SIGKILL 砍、退出碼是 137，怎麼從退出碼認出是 OOM 見 [OOM killer 與退出碼 137](/linux/dotfile/knowledge-cards/oom-exit-code-137/)）。OOM kill 的判讀分兩種情境：
...
把 `latest` tag 當成版本標記，會讓 rollback 指向無法預測的 image。image tag 在 registry 上是 mutable——同一個 tag 可以被覆蓋指向新 image。用 immutable tag（commit SHA、content digest）才能保證 rollback 的確定性（image 為何本身不可變、tag 與 digest 的差別見 [不可變 Image](/backend/knowledge-cards/immutable-image/)）。

...
**生命週期訊號**的紅隊風險不是「服務掛了」、而是「服務在不該接流量時接流量」。readiness 過早通過、shutdown 階段仍在處理 in-flight request、drain 視窗內接收新請求，都會把短暫的脆弱窗口拉長給攻擊者利用。
...
平台遷移（self-managed → managed、單 cluster → 多 cluster、舊版本 → 新版本）會短期擴大攻擊面、然後逐步收斂。沒有顯式管理這個變化、雙軌期變成攻擊面雙倍期。
...
遷移期的攻擊面變動不會自動觸發 release gate、需要在遷移計畫中明確把資安 review 列為 gate 之一。沒有這道 gate、遷移期容易被當成「技術項目」處理、漏掉攻擊面的隱性擴大。

...
平台與入口威脅建模的核心責任是把部署平台的弱點維持在可操作的概念層。列舉框架本身（資產、邊界、攻擊者能力假設三者並排）見[威脅模型](/backend/knowledge-cards/threat-model/)，本章只做平台這一面的展開。本章的輸出是平台問題地圖、案例對照與交接條件，讓實作前決策可先對齊，避免進入 YAML / unit file / LB rule 前就已經漏掉攻擊面。

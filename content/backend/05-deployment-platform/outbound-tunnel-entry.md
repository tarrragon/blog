...
Outbound tunnel 入口的核心責任是定義「本機服務在不開公網 port 的前提下、怎麼安全地被外部觸及」。它跟 load balancer 入口是兩種不同的入口形態：LB 假設 instance 有對外可達位址、流量從外網路由進來;tunnel 由本機進程主動外連到邊緣、把流量沿反向隧道帶回來、路由器零開 port、對公網零入站面。家用服務、個人自架工具、無固定 IP 的環境常用這種入口。
...
| 隧道頻繁重連、外部間歇中斷      | 本機外連不穩或邊緣側抖動              | 查網路品質、確認重連 backoff 策略        |

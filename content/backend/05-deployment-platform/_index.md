...
部署平台模組的核心目標是說明服務如何和外部調度、網路入口與資源限制對齊。語言教材會處理 [graceful shutdown](../00-knowledge-cards/graceful-shutdown/)、health / [readiness](../00-knowledge-cards/readiness/) endpoint 與 signal handling；本模組負責平台設定與操作語意。
...
Container 解決服務包裝與 runtime 依賴；Kubernetes 解決多 instance 調度、probe、rolling update 與資源限制；systemd 適合單機或 VM 上的 service lifecycle；load balancer 解決流量入口、draining、idle timeout 與 [health check / liveness](../00-knowledge-cards/health-check-liveness/)；service discovery 解決服務彼此如何找到 endpoint；runtime config 解決環境差異、[secret](../00-knowledge-cards/secret-management/) 與 feature gate。

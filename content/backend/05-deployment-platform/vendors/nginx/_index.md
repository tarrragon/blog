...
```bash
# 1. 啟動 nginx（docker / 系統包）
# TODO: docker run -d -p 80:80 -v ./nginx.conf:/etc/nginx/nginx.conf:ro nginx

# 2. 寫 reverse proxy config
# TODO: location / { proxy_pass http://upstream; }

# 3. reload + 驗證
# TODO: nginx -t（test config）
# TODO: nginx -s reload（reload without restart）
```
...
- 對應 attacker view: [2.attacker view cache risks](/backend/02-cache-redis/attacker-view-cache-risks/)
...
| 案例                                                                                                        | 對 nginx 的對應                     |
| ----------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| [5.C9 cutover without drain](/backend/05-deployment-platform/cases/failure-platform-cutover-without-drain/) | nginx upstream drain 配置           |
| [5.C10 規模對照](/backend/05-deployment-platform/cases/contrast-platform-migration-by-scale/)               | 小型直接 nginx / 中型 ingress-nginx |

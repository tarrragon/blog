...
這篇講的是**自用 / 小規模的版本升級**——建新的、並跑對照、切換。它刻意不碰「線上服務不中斷的帶電升級」：那涉及在流量中把舊版逐步排空、新版逐步接手、出錯即時回退，是另一個量級的工程，見 [Infra 環境與系統升級](/infra/upgrade/)。判斷標準很簡單：自用能接受「切換時停一下」就用這篇的做法；線上不能停就走 infra 的 zero-downtime 遷移。

...
四件事任一缺失、就是 [跨 warehouse data policy](/backend/07-security-data-protection/) 邊界的待補項目。
...
**PAIG 的 prompt / response governance**：LLM application 的 data security 問題不是模型本身、而是 *prompt 內帶 PII 進 LLM context*（資料外洩到第三方）跟 *response 含 sensitive 內容流回 user*（policy bypass）。PAIG 在這兩個邊界做 redact / block / log、把資料治理規則套到 LLM I/O。實作關鍵是 *latency 預算*（每個 prompt 過一次 scan）、*false positive 容忍度*（redact 太多 LLM 回答品質掉）、*audit log retention*（哪些 prompt 該保留多久）。
...
- 上游：[7 資料安全與保護](/backend/07-security-data-protection/)（cross-warehouse policy 章節）

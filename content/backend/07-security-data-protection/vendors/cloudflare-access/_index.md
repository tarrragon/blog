...
- **Device Posture 把合法 user 鎖在外**：EDR agent 暫時離線或 OS 升級導致 posture check fail — fallback 設 graceful（降級到只讀 / 加 [step-up](/backend/knowledge-cards/step-up-authentication/) MFA）、不是直接 deny；EDR fleet coverage 沒到位前不要 hard enforcement

...
2. 列高風險項：檢查 [Diagnostic Endpoint](/backend/knowledge-cards/diagnostic-endpoint/)、[Admin Endpoint](/backend/knowledge-cards/admin-endpoint/)、credential 與網路入口。
3. 看交付閘門：檢查 [Release Gate](/backend/knowledge-cards/release-gate/) 是否包含高風險設定驗證。
4. 看漂移：持續偵測環境偏移與權限擴張。
...
1. 已完成高風險設定分類後，交接到 [模組五：部署平台與網路入口](/backend/05-deployment-platform/)。
2. 已定義設定漂移事件節奏後，交接到 [8.5 復盤與改進追蹤](/backend/knowledge-cards/post-incident-review/)。

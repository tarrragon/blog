...
3. 看保護面：檢查 [rate limit](/backend/knowledge-cards/rate-limit/)、[backpressure](/backend/knowledge-cards/backpressure/)、[timeout](/backend/knowledge-cards/timeout/) 與 [load shedding](/backend/knowledge-cards/load-shedding/)。
...
1. 已完成高成本路徑盤點後，交接到 [模組六：可靠性驗證流程](/backend/06-reliability/)。
2. 已定義降級與回復事件節奏後，交接到 [8.3 止血、降級與回復策略](/backend/08-incident-response/containment-recovery-strategy/)。

...
最右欄是唯一在選型當下不產生成本、而在一年後才產生成本的一欄，開場那句說的就是它。共享密鑰與 API key 的分界也在這一欄而非密碼學上：兩者都是持有即通過的憑證、都做等值比對，差別只在存不存在一張把憑證對應到呼叫方的登記表。有那張表，撤一把只影響一個；沒有，撤一把影響全部。所以「改用 API key」這個決定的實質內容是「建那張表」。
...
它是取得憑證的流程，不是第五種機制，而流程裡用哪一種方式向授權伺服器證明自己，才決定秘密送不送出去。用 client secret 時秘密要送、而且是每次換 token 都送一次：RFC 6749 §4.4.2 要求每次 token 請求都做 client 認證，§4.4.3 又說不該發 refresh token，所以 token 過期只能再送一次 secret 去換——暴露頻率因此等於呼叫頻率除以 token 有效期，token 一小時的整合一天要走二十幾趟傳輸路徑。
...
基礎建設沒有跟上的後果在這張表裡最不直觀，因為選型當下沒有任何東西會提醒它存在；其餘三個節點的風險後果欄字面就是完整的後果（憑證的可讀範圍等於紀錄的保存範圍、撤銷單一對象要中斷全部、所有呼叫方同時失去存取），讀完就想像得出場面。合規要求對外整合走 mTLS，團隊建了一個內部 CA、為當時的兩個對接方各簽了一張一年期憑證，上線順利。一年後兩張憑證在同一週到期，兩條整合同時斷，而那一週沒有任何人在等這件事發生。

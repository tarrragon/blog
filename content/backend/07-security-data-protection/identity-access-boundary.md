...
防護的著力點在密碼之外，因為攻擊者手上的密碼是對的。三條互補的路徑：**註冊與變更密碼時比對已知外洩的組合**，把重疊的那一批擋在門外；**按來源與行為特徵限速而非按帳號**，因為這一類攻擊對每個帳號只試一兩次、按帳號計數的門檻永遠碰不到；**對成功登入做異常判讀**（新裝置、陌生地區、與平常不同的時段），因為前兩條擋不住的那些會以一次完全合法的登入通過。第二因子讓密碼正確也不足以登入，是成本最高也最有效的一層，觸發條件見 [step-up 驗證](/backend/knowledge-cards/step-up-authentication/)——它與上方 MFA fatigue 那一節共用同一組訊號，差別在那裡的主體是員工。

暴力破解與這一類共用登入端點而防護不同，兩者容易被歸成一格：暴力破解對單一帳號嘗試大量密碼，速率限制與帳號鎖定攔得住；憑證填充對大量帳號各試一兩組，同一套機制結構上看不見它。完整的機制對照見 [credential stuffing](/backend/knowledge-cards/credential-stuffing/)。

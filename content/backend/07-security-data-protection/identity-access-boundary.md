...
**授權範圍擴張過快**出現在長出代理操作能力的內部工具：客服要能替使用者查訂單、營運要能替商家改設定、管理後台要能切換身分重現問題。這些能力各自都有正當理由，而權限模型多半用角色承接，於是角色隨功能一路累積。識別特徵是系統裡有一個叫「管理員」的角色，而沒有人列得出它現在能做什麼——那份清單只存在於程式碼的判斷式裡。

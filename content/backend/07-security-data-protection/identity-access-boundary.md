...
授權範圍擴張過快的收斂點不在定期清理權限，在把代理操作與直接操作分成兩種授權。客服「能查訂單」與客服「能以某位使用者的身分查訂單」是不同的能力：前者的資源邊界由操作類型決定（訂單這一類資料），後者的資源邊界由被代理的對象決定（那位使用者看得到的一切）。用角色承接時兩者長得一樣——都是角色上的一個項目——所以角色沿著功能一路長大，而每一次擴張在當下都有正當理由。
...
這是 [step-up 驗證](/backend/knowledge-cards/step-up-authentication/) 在授權層的形態：身分層用它把「登入成功」與「執行高風險操作」拆開，授權層用同一個機制把「持有這個角色」與「現在要代理某人」拆開。兩層獨立成立——身分層做了 step-up 而授權層沒拆時，攻擊者取得的是一個已經通過強認證、且能代理任何人的會話。
...
判讀訊號裡的「代理操作鏈過長」指的是多跳代理——某個員工以 B 的身分操作，而 B 本身也是一個代理身分。每一跳讓歸屬多一層轉換，而稽核紀錄多半只留最後一跳。設計階段的收斂條件因此是代理不可遞移：代理身分不能再發起代理。

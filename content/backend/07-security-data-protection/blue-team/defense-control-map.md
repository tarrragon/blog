...
控制面地圖的責任是把每個風險節點放進固定欄位，確保跨團隊溝通一致。欄位沿用 [7.8 的交接模板](/backend/07-security-data-protection/security-routing-from-case-to-service/)、本篇不另立一套，每筆映射至少要填其中四項：
...
本篇獨有的那一軸是控制面歸屬（這個風險節點由身分、入口、資料、供應鏈、偵測或治理哪一面承接），它決定上面第 2 項的主責是誰。

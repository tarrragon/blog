...
控制面地圖的責任是把每個風險節點放進固定欄位，確保跨團隊溝通一致。欄位沿用 [7.8 的交接模板](/backend/07-security-data-protection/security-routing-from-case-to-service/)、本篇不另立一套。八項裡有四項在地圖上由列本身承載（問題摘要與風險邊界寫在列標題與判讀欄、控制面優先序由控制面分組表達、這一輪的完成條件在地圖這種常態維護的產物上由驗證與回退承接，因為地圖沒有「這一輪」），剩下四項每筆映射都要填：

1. 判讀訊號：觸發判讀的第一個訊號。
2. 主責與協作角色：這個控制面的主責角色，以及需要協作的角色。
3. 驗證與回退：判斷控制面生效的證據，以及沒生效時的退路。
4. 回寫規則：回寫到哪個章節或卡片。

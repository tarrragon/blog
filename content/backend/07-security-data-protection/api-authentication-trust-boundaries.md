...
委任型憑證的機制選型（token exchange 的交換流程、actor 與 subject 各自的撤銷路徑、驗證方要檢查哪些欄位）本站尚無專章，已列入本模組 backlog。在那之前的最小判準是：兩張分開的憑證能表達同一件事，而且它不需要任何新機制——先用兩張走完一輪，等到「驗證方要自己拼湊關係」這件事真的造成成本（多個驗證方各自拼、拼法不一致）再換委任型。這個順序的理由是委任型的成本落在驗證端與撤銷端，而那兩端的複雜度要有多個驗證方才顯現。
...
這一類的失敗長這樣：串接初期兩邊的帳號都是同一批人手動開的，映射就順手在第一次呼叫時建起來，沒有人覺得需要另外設計開通動作。等到客戶把自己的人事系統接進來，新進員工第一次登入就失敗，而系統回的是認證失敗——值班的人照認證的排查手冊查憑證、查時鐘、查有效期，全部正常，隔了幾輪才有人想到去看映射表。真正缺的是一筆對應資料，而那條排查路徑上沒有任何訊號指向資料。補起來要加的是明確的開通動作，那牽涉兩邊的流程協調，不是自己這端改程式碼就能收尾。開通動作本身要定義什麼（誰觸發、映射在哪個時間點寫入、既有帳號怎麼回填、找不到映射時回什麼錯誤）見 [API 認證的三層信任邊界](/work-log/api_auth_trust_boundaries/) 的「Layer 3：跨系統 Provisioning」一節。

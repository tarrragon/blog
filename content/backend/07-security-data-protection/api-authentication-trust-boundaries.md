...
委任型憑證的機制選型（token exchange 的交換流程、actor 與 subject 各自的撤銷路徑、驗證方要檢查哪些欄位）本站尚無專章，已列入本模組 backlog。在那之前的最小判準是：兩張分開的憑證能表達同一件事，而且它不需要任何新機制——先用兩張走完一輪，等到「驗證方要自己拼湊關係」這件事確實造成成本（多個驗證方各自拼、拼法不一致）再換委任型。這個順序的理由是委任型的成本落在驗證端與撤銷端，而那兩端的複雜度要有多個驗證方才顯現。本章只談憑證形態——代理這件事在授權層要配什麼（發起的理由、有效時窗、紀錄裡的雙重歸屬），以及多跳代理什麼時候可以開放，見 [7.2 代理操作的授權邊界](../identity-access-boundary/#代理操作的授權邊界)。兩處的預設方向不同是因為問的不是同一件事：那裡問授權可不可遞移，這裡問憑證要不要合併成一張。

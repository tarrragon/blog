...
交接模板的責任是讓不同章節用同一種輸入輸出格式合作。這七項是本模組交接欄位的定義處，其他章節沿用這組名稱：
...
[7.B1 防守控制面地圖](../blue-team/defense-control-map/) 的每筆映射用的是同一組欄位（它的軸是「哪個控制面主責」、欄位不另立一套），[7.21 資安如何成為服務設計輸入](../security-as-service-design-input/) 的交接路由欄則是這份模板在設計評審那個時點的填法。

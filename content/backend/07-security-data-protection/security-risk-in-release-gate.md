...
這一軸要問三件事，而後兩件在只問可逆性的判定裡不會浮現。**機制上收得回來嗎**——這次變更若判斷錯了，回退能不能把風險收回來。**誰有權執行**——回退權在甲方、監理機關或平台方那一側時，本地的 rollback condition 觸發了也沒有人能按下去。**要多久**——強制前置期（合約規定的書面申請、外部核准排隊、供應商維護窗口）會讓回退的實際延遲遠大於技術上的回退時間。後兩件任一項答不出來時，這個變更要當成不可逆處理。

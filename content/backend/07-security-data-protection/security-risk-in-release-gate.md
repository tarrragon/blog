...
這張表是代理、不是機制本身。真正決定風險的是這次變更改了哪一條信任假設，而資產類別是它在發版前唯一可機械判定的代理——代價是它會過度觸發：在既有認證授權後面加一個不碰新資料的端點命中對外暴露面，而信任假設沒有變。命中而信任假設未變時允許降級成標準流程，條件是把「哪一條信任假設沒變」寫進放行紀錄；信任假設怎麼列舉見 [7.21 的信任邊界欄](/backend/07-security-data-protection/security-as-service-design-input/)。
...
這一軸要問三件事，而多數團隊只問了第一件。**機制上收得回來嗎**——這次變更若判斷錯了，回退能不能把風險收回來。**誰有權執行**——回退權在甲方、監理機關或平台方那一側時，本地的 rollback condition 觸發了也沒有人能按下去。**要多久**——強制前置期（合約規定的書面申請、外部核准排隊、供應商維護窗口）會讓回退的實際延遲遠大於技術上的回退時間。後兩件任一項答不出來時，這個變更要當成不可逆處理。

...
| 宣稱層與生效層的共用語彙（7.21 的控制意圖對控制實作、7.22 的證據對應機制對證據對應欄位、7.25 的活動指標對能力指標，三篇各用一組本地名稱重述同一個區分；另有詞義碰撞——模組 index 與 7.8 的「實作層」指下游模組承接，7.22 的「流程層對實作層」指 gate 對生產環境的控制。判定為缺卡、三篇沿用同一張卡的語彙，建卡前先跑全站反證）                                                                                                                                                                                                                                         | 知識卡 | 無（三篇的角已盤點）                                   | 小          |
| 組織形態的適用邊界（7.21 / 7.22 / 7.25 三篇共用一個沒寫出來的前提：一個自我治理的單一組織，擁有整條 pipeline、所有關係人都在內部且可被指派。四個個案實跑顯示這個維度有四個取值——參與者太少、證據受眾在外部、執行權在外部、沒有參與者——而三篇的驗收條件都只容納第一種。已在三篇各補一句邊界，仍缺一處並置說明四個取值各自怎麼調整判準）                                                                                                                                                                                                                                 | 主章   | 無（四個個案的停頓點已記在 Round 2 審查）              | 中          |
...
| 例外協議的三份分解收斂（7.14 分五個責任：決策 / 期限 / 補償 / 觸發 / 關閉；7.17 分六個欄位：風險範圍 / 到期日 / 補償控制 / owner / 關閉條件 / 回寫目標；`security-exception` 卡的設計責任段持有與 7.17 字面相同的第三份，而 7.14 沒有連那張卡。雙向對映兩側各有單側成員——tripwire 不在 7.17 的六欄位裡、關閉條件不在 7.14 的五責任裡（在該章判讀流程第二步）。依 [#263](/report/incompatible-decompositions-look-complementary/) 收斂：卡層是載體候選，兩章沿用卡的欄位、各自只留自己的判讀）                                                                          | 主章   | 無（判準與載體選法已定）                               | 小          |
| 拆卡試點的兩項驗證（異源冷讀：兩輪的冷讀對照都由改寫者本人執行，跳轉成本的方向未驗；第二人分診：兩輪都由同一個人分診，對同一批列的執行者一致性未測。兩項各自獨立、不必等全分類盤點，判準與登記見 [#262 的試點結果段](/report/content-pressure-resolves-by-expansion-not-compression/#試點結果逐列判定與處置分診)）                                                                                                                                                                                                                                                     | 主章   | 無（對象與協議都已確定）                               | 小          |
| 樣板叢集的容器改寫（「本篇的責任是 + 核心論點 + 完稿判準」這組樣板剩下的主章 9 篇與藍隊 12 篇（量測指令 `rg -l "本篇的責任是" content/backend/07-security-data-protection/`，數出來要扣掉記錄本列的 `_index.md` 自己）：每節一句、表格格是既有卡的壓縮 gloss、路由欄是裸模組編號。7.21 / 7.22 / 7.25 已作為拆卡試點改寫完成，逐列判定與處置分診見 [#262](/report/content-pressure-resolves-by-expansion-not-compression/)）                                                                                                                                            | 主章   | 無（試點已完成、改寫標準已確立）                       | 21 篇（大） |

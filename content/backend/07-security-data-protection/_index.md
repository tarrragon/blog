...
| 7.28 兩則微案例候選（原語與威脅不匹配、驗證素材定義不一致——後果同樣不直觀，而現有專節寫的是成本結構與修法、是分析不是場面）                                                                                                                          | 案例   | 無（7.28 形態段已標明是候選）                          | 小          |
| 把 7.28 的「驗證素材的對齊成本」與「重放窗口的收斂條件」抽成獨立一章（連同問題節點表那兩列）——兩節的前提是已選定共享密鑰簽章且在接 webhook，與「金鑰放哪一格」不同軸，而接第三方 API 的讀者路線整條落在那兩節                                        | 主章   | 無（7.28 判讀流程第 4 步已自承那幾格不由金鑰位置決定） | 中          |
| 機器憑證的機制選型（API key / 共享密鑰簽章 / mTLS / OAuth client credentials 的判準，目前唯一落點是 work-log 層的設計筆記）                                                                                                                          | 主章   | 無（7.28 / 7.29 / 7.32 三處寫作邊界都指過來）          | 中          |

...
| 樣板叢集的容器改寫（「本篇的責任是 + 核心論點 + 完稿判準」這組樣板的主章 10 篇與藍隊 12 篇：每節一句、表格格是既有卡的壓縮 gloss、路由欄是裸模組編號。7.21 / 7.22 已作為拆卡試點改寫完成，逐格判定與出口分布見 [#262](/report/content-pressure-resolves-by-expansion-not-compression/)） | 主章   | 無（試點已完成、改寫標準已確立）                       | 22 篇（大） |

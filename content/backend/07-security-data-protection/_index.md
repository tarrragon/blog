...
| 委派身分的資料歸屬（使用者離開來源之後資料歸誰）                                                                                                | 主章   | 無（7.31 委派段指過來、已給最小做法）         | 小          |
...
| 四段 meta 開場的 register 整理（threat scope 樣板句中英混雜且對讀者祈使、「從本章到實作」向後引用尚未出現的表欄、交接路由欄的裸模組編號無對照） | 主章   | 無（本模組多輪審查指出，18 章共用同一組）     | 中          |

...
4. 短效憑證：把憑證有效時窗縮短，降低竊取後可利用時間。這個詞蓋住兩種形態，成本落點不同——**動態換發**是每次執行向平台換一把新的，長期憑證因此不存在，成本落在換發路徑的可用性；**壓短 TTL** 是既有憑證的有效期調小，長期憑證仍在，成本落在換發頻率與對發行方的依賴。判讀時要先確認講的是哪一種。
...
以下基於通用工程知識補充：scope 收斂的工程瓶頸通常在第三方平台的權限粒度 — 廠商提供的 scope 選項可能比實際需求粗、組織要在「接受粗 scope」「自建中間層收斂」「換廠商」之間取捨。中間層收斂是常見折衷、把第三方 token 在內部 proxy 後降權再傳遞給下游 service。中間層存在時、第三方 scope 跟內部 [blast radius](/backend/knowledge-cards/blast-radius/) 解耦；中間層缺位時、兩者直接綁定。
...
- 平台身分在某條整合上用不上時（該章列出的五種情形，涵蓋外部條件與自己的可靠性與合規需求）：[7.32 機器憑證的配發](../machine-credential-issuance/)

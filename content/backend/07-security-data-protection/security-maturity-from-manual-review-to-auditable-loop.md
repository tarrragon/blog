...
這張階梯與下方的評估維度是本模組成熟度語彙的唯一定義處。判出階段之後，把它排成固定日程（回顧間隔、改進項目的承接者、指標的分母）走 [7.25 資安成熟度的組織節奏](/backend/07-security-data-protection/security-maturity-organization-cadence/)——該章沿用這裡的階段名，不另立一套切法。

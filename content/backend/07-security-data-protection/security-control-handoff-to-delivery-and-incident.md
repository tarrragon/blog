...
交接契約的責任是把「要做什麼」轉成「誰用什麼證據在什麼時機完成」。欄位沿用 [7.8 的交接模板](/backend/07-security-data-protection/security-routing-from-case-to-service/)、本篇不另立一套。八項裡有三項屬於風險判讀階段的產物、在交接發生前就已經定案（判讀訊號、風險邊界、控制面優先序，它們的落點是主控制面那幾章），模組之間的交接要填剩下五項：

1. 問題摘要：一行描述風險與影響範圍。
2. 主責與協作角色：第一承接角色，以及跨模組的協作角色。
3. 驗證與回退：判斷控制面生效的訊號，以及沒生效時的退路。
4. 這一輪的完成條件：該輪任務完成、責任可以交回的條件。
5. 回寫規則：回寫章節與問題卡位置。

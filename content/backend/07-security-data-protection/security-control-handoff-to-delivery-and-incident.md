...
date: 2026-04-30
description: "同一條資安風險在主控制面、補控制面與事故流程之間交接三輪時，每一輪各要交出什麼"
weight: 88
---
...
資安控制面交接的核心概念是「每個風險判讀都要落到一個承接流程」。這裡的「[控制面](/backend/knowledge-cards/security-control-domain/)」指防守責任的歸屬類別（身分、入口、資料、供應鏈、偵測、治理），與基礎設施的 control plane 同名不同義；下文的主控制面與補控制面，指的是同一條風險在不同輪次的承接者。7.x 負責判斷風險落點，05/06/08 模組負責實作、驗證、回復與復盤。
...
交接契約的責任是把「要做什麼」轉成「誰用什麼證據在什麼時機完成」。欄位沿用 [7.8 的交接模板](/backend/07-security-data-protection/security-routing-from-case-to-service/)、本篇不另立一套。九項裡有三項屬於風險判讀階段的產物、在交接發生前就已經定案（判讀訊號、風險邊界、控制面優先序，它們的落點是主控制面那幾章），模組之間的交接要填剩下六項：

1. 問題摘要：一行描述風險與影響範圍。
2. 承接範圍：這一輪由哪個模組接（上方交接模型那張表給的就是這一欄的預設值）。
3. 主責與協作角色：範圍裡具名的第一承接角色，以及跨模組的協作角色。
4. 驗證與回退：判斷控制面生效的訊號，以及沒生效時的退路。
5. 這一輪的完成條件：該輪任務完成、責任可以交回的條件。
6. 回寫規則：回寫章節與問題卡位置。

同一個風險會被交接不只一次，而這是本篇跟其他沿用這組欄位的章節最大的差別。主控制面接第一輪收斂、補控制面接擴散管理、incident 流程接最終回寫與追蹤，三輪各有自己的承接者與完成條件。所以「主責與協作角色」與「這一輪的完成條件」在一次風險的生命週期裡要填三次、每輪一次，而只填一次的交接單通常記的是第一輪，後兩輪的責任沒有落點。

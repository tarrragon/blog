...
本篇適合銜接 [7.R7 事故案例庫](/backend/07-security-data-protection/red-team/cases/) 與 [案例引用地圖](/backend/07-security-data-protection/red-team/cases/case-reference-map/)。讀者讀完後、應該知道如何把案例引用成工程訊號、避免讓案例停在背景故事層。
...
交接的責任是把抽象失效變成具體檢查點。欄位沿用 [7.8 的交接模板](/backend/07-security-data-protection/security-routing-from-case-to-service/)、本篇不另立一套，workflow 任務要填其中四項（問題摘要與承接範圍由 problem card 與上一步的控制面映射承載、風險邊界與控制面優先序在判讀階段已定案、回寫規則由下一節的回寫閉環承接）：

1. 判讀訊號：何時進入這條流程。
2. 主責與協作角色：誰負責做判讀與執行。
3. 驗證與回退：要收集哪些證據，以及證據不足時的退路。
4. 這一輪的完成條件：什麼條件代表本階段完成、責任可以交回。
...
| 任務清單沒有判讀訊號、主責角色或完成條件 | 流程執行責任不清、完成定義模糊 | 依 7.8 的交接模板補 workflow 任務欄位 |

...
金鑰交換在絕大多數服務裡不是獨立的選型決定，它包在傳輸層協定裡：用 TLS 就等於用了該版本協商好的交換方式，判讀點落在協定版本與憑證信任鏈，見 [7.5 傳輸信任與憑證生命週期](../transport-trust-and-certificate-lifecycle/)。需要自己設計交換協議的情境（點對點加密、離線裝置配對）本站尚無專章。

隨機數產生的判讀只有一條：用作業系統或語言標準庫提供的密碼學安全來源，不用一般用途的亂數產生器，也不自己組合。這一條沒有取捨空間，因此不需要獨立章節；判別方式是看函式名稱屬於哪一類（多數語言把兩者分在不同模組，密碼學那一組的名稱通常帶 secure 或 crypto）。

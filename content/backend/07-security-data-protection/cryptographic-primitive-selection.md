...
它的失敗長這樣：需求寫「這個欄位要加密後儲存」，實作的人用了手邊那個函式庫裡名字看起來對的函式，寫進去的是一段誰都看不懂的字串，測試通過、審查通過、上線。那個函式算的是驗證值——它證明內容沒被改過，而內容本身仍然由原值決定、也還原得回去。某次資料庫的備份檔外流之後才發現這件事。

沒有被發現，是因為每一道檢查問的都是同一個問題：有沒有加密。程式碼裡有那個函式的呼叫、文件寫著已加密、稽核回覆也照抄了那句話，三處互相佐證。會當場戳破它的是另一個問題——拿一筆存下來的值，看得懂嗎——而那個問題不在任何一張檢查表上，因為「已經加密」在被問到之前就已經是共識了。

止血要補的是加密而非調整既有機制，三個決定與既存紀錄的重寫成本見下方風險邊界那一條。真正的修法在需求階段：把「要加密」換成「要讓誰看不到」，抽象詞一旦換成對象，選型就不會走錯。

...
本章聚焦依賴金鑰的原語，判讀主軸是金鑰位置。金鑰交換與隨機數產生的判讀軸不是金鑰位置（前者看協商過程能否被中間人介入、後者看熵來源），因此不在本章範圍，各自的落點見下方 threat scope 段之後。金鑰的產生、保存與輪替節奏屬於治理層，見 [7.6 秘密管理與機器憑證治理](../secrets-and-machine-credential-governance/)。案例在問題觸發時作為證據參考。
...
五個節點裡重放窗口的後果最不直觀，它的微案例在下方「重放窗口的收斂條件」一節。其餘四個不另寫：金鑰隨程式發佈與簽章金鑰無隔離的後果欄字面即完整，原語與威脅不匹配的代價寫在風險邊界那條（團隊用「已加密」描述只做了驗證的路徑），驗證素材不一致的成本結構在它自己的專節展開。

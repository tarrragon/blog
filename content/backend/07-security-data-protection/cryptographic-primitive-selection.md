...
**重放窗口未收斂**出現在接收外部推送的端點，尤其是那些照著對方文件實作驗簽就上線的整合。這一格與前四格的成因不同：它不是選錯，是選對之後少做一步，因此團隊的能力水準與它的出現率無關。識別特徵是 payload 裡帶著時間戳與識別值，而接收端的程式碼裡找不到比對它們的地方。
...
- 團隊用「已加密」描述只做了訊息驗證的路徑時，代表機密性需求沒有被實際滿足，修法是補上加密而非調整既有機制——訊息驗證與加密解的是不同問題，把驗證值加長、演算法換強都不會讓內容變成不可讀。補加密要接著決定三件事：加在哪一層（傳輸中用 [TLS](/backend/knowledge-cards/tls-mtls/)、落地後用 [at-rest encryption](/backend/knowledge-cards/at-rest-encryption/)，兩者防的對象不同、常常都要）、金鑰放哪一格（回到「金鑰位置決定對抗對象」那一節，機密性的保護範圍同樣由這一格決定）、以及既有資料怎麼補加密（既存紀錄要重寫一輪，這一輪的成本由資料量與可停機窗口決定，通常比改程式碼大一個量級）。同一句話用在金鑰位於客戶端的可逆編碼上則是另一種誤稱，那裡要移動的是金鑰位置。

...
代理操作的紀錄要能同時回答「是誰的資料被動了」與「哪個系統代表他做的」，而後者只有在憑證本身帶著代理方時才會自動落進紀錄。憑證形態的選擇（委任型帶雙重歸屬、冒用型只留被代理者）見 [7.33 委任型憑證](../delegated-credential-selection/)；落在冒用型的整合要注意雙重歸屬只剩發行端的交換紀錄一份，那份的保存期限要對齊稽核要求而非對齊一般的存取日誌。授權層要一併記下的發起理由與有效時窗見 [7.2 代理操作的授權邊界](../identity-access-boundary/#代理操作的授權邊界)。

...
於是這裡有兩個生命週期——外部身分的（由對方的目錄決定：建立、停用、刪除）與本地紀錄的（由這邊決定）。委派同步的是「登入」這一個時點，其餘的時點各走各的。本章的三個問題都是這兩條線在某個時點分岔的結果。
...
選哪一種由對方支援什麼決定，所以這是導入當下要問的問題。三個都沒有時，唯一不依賴對方支援的做法是縮短本地會話的有效期、強制使用者定期重新走一次對方的登入——這把「不知道對方停用了」的窗口從無限長縮到一個會話的長度。代價是登入頻率上升，而這個代價要在導入時就算進去，因為它是替代方案而不是最佳做法。

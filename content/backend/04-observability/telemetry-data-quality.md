...
本段聚焦 sampling 對資料品質的失真風險；sampling 策略（Head / Tail / Adaptive / Exemplar）的 SSoT 在 [4.7 Sampling 策略](/backend/04-observability/cardinality-cost-governance/#sampling-策略)。
...
4. 在 [incident decision log](/backend/08-incident-response/incident-decision-log/) 記錄資料限制。
...
1. **固定一組 baseline query**：選定關鍵服務的核心 SLI query（error rate、p99 latency、throughput），新舊管線各跑一份、定期比對。
2. **設定偏差閾值**：每個 SLI 設可接受偏差（例如 ±5%）。超過閾值的時段標記為待調查，不能無視。
3. **追蹤 missing signal 比例**：missing span、missing metric、missing log 的比例是漂移的早期指標。比例持續上升時，停止下一批服務切換。
4. **退出條件顯式化**：「對照偏差連續 N 天 < X%」作為關閉舊管線的退出條件，把雙軌期變成有界的、不是無限延長。
...
雙軌期的成本是顯而易見的：兩份採集、兩份儲存、兩份查詢。但放棄對照的代價更大 — 沒有對照證據，事故時無法分辨是「服務問題」還是「遷移問題」，回退也失去依據。詳細的回退判讀流程由 [4.11 telemetry pipeline](/backend/04-observability/telemetry-pipeline/) 處理，本章關注的是品質指標的對照設計。

## 與 SLO 和事故的關係

Telemetry data quality 是 SLO 與事故 evidence 的可信度前提。SLI 若建立在失真資料上，[error budget](/backend/knowledge-cards/error-budget/)、burn rate alert 與 release freeze 都會被錯誤資料牽動。
...
- [4.1 log schema](/backend/04-observability/log-schema/)：治理欄位漂移
- [4.7 cardinality / cost](/backend/04-observability/cardinality-cost-governance/)：sampling 策略矩陣、高維度截斷與成本取捨
- [4.11 telemetry pipeline](/backend/04-observability/telemetry-pipeline/)：追查 drop、delay 與 ingest 問題
- [4.14 anomaly detection](/backend/04-observability/anomaly-detection/)：避免模型學到偏誤資料

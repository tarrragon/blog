...
演練前，readiness review 的責任是確認驗證行為能被觀測。chaos experiment、load test 或 DR drill 需要同時產生故障與判讀證據，讓團隊能確認 [steady state](/backend/knowledge-cards/steady-state/)、[blast radius](/backend/knowledge-cards/blast-radius/) 與回復狀態。
...
- [4.5 威脅建模](/backend/04-observability/attacker-view-observability-risks/)：觀測盲區跟資料暴露的上線前檢查
- [4.25 觀測共命運失效](/backend/04-observability/observability-shared-fate/)：上線前檢查「觀測退化時的備援」有沒有（dead man's switch、獨立 status page、演練驗證）

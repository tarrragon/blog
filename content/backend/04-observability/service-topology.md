...
這些維度進入拓撲訊號後，配合 [6.14 dependency budget](/backend/06-reliability/dependency-reliability-budget/) 才能把依賴可靠性變成可量化決策。
...
事故時把 blast radius 從拓撲推導出來、再對照實際看到的 5xx 跟 SLO [burn rate](/backend/knowledge-cards/burn-rate/)、能驗證影響面是否符合預期。當實際影響超出推導 blast radius、通常意味著存在未紀錄依賴。
...
- [6.14 dependency budget](/backend/06-reliability/dependency-reliability-budget/)：拓撲是依賴可靠性評估的資料來源

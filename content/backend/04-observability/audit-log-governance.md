...
把 audit log 視為 operational log 的子集是常見的誤判。兩者的事件對象、保留要求、存取權限、不變性保證都不同。混在同一 stream 治理時，事故證據鏈會在第一次合規稽核或法規請求時被打斷。
...
Audit log 治理的常見失敗不是技術不到位，而是責任鏈斷在團隊邊界。應用團隊只記應用層事件、基礎設施團隊只記 infra 層存取、IAM 團隊只記授權變更；當需要還原一次跨團隊的敏感操作時，三段證據各自殘缺。
...
對應 [4.C1 FinTech retention policy drift](/backend/04-observability/cases/fintech-audit-evidence-observability/) 跟 [4.C3 Healthcare retention boundary violations](/backend/04-observability/cases/healthcare-access-traceability-and-retention/) 都揭露這個方向：retention 不是設定一次就好、要把偏離視為治理事件。

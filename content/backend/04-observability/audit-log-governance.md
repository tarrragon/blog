...
表格裡的「事件不可否認」這一格值得單獨判讀：它成不成立取決於記錄是誰簽的。系統自己寫入、自己保管的記錄，在爭議中無法排除「記錄被管理者竄改」這個可能，因此它提供的是內部追蹤而非對外舉證能力。真正撐得起對第三方舉證的是非對稱簽章 —— 判準見 [Non-repudiation](/backend/knowledge-cards/non-repudiation/)，原語的選型層見 [7.28 密碼學原語選型](/backend/07-security-data-protection/cryptographic-primitive-selection/#原語選型模型)。

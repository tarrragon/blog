...
本章列的 API / async / dependency / error 診斷欄位，跟 [11.11 Status 與錯誤的雙向契約](/backend/11-api-design/error-bidirectional-contract/) 是同一組欄位的兩種讀法：本章的動機是觀測（留欄位才能事後定位），11.11 的動機是契約（這些欄位是 provider 對 consumer 的義務、不給就是把成本外部化給對方）。兩種動機指向同一批欄位並非巧合——可診斷性本身就是一種對外承諾。
...
- [11.11 Status 與錯誤的雙向契約](/backend/11-api-design/error-bidirectional-contract/)：同一批診斷欄位從 provider↔consumer 契約義務角度的讀法（接收方怎麼判讀、重試、回報）

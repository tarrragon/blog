...
| [Time range](/backend/knowledge-cards/time-range/)     | 標示查詢時間窗                | 對齊 incident timeline            |
| [Query link](/backend/knowledge-cards/query-link/)     | 保留可重跑查詢                | 支援 handoff 與復盤               |
| Owner                                                  | 指定可解釋資料的人            | 避免 evidence 失去語意            |
| [Data quality](/backend/knowledge-cards/data-quality/) | 標示 completeness / freshness | 防止資料限制被誤讀                |
| [Confidence](/backend/knowledge-cards/confidence/)     | 標示 confirmed / suspected    | 支援分級與決策                    |
| [Known gap](/backend/knowledge-cards/known-gap/)       | 標示 missing signal 或 drift  | 回寫 04 readiness 與 data quality |
| Retention                                              | 標示保存期限                  | 支援 audit、PIR 與長事故          |

Source 欄位讓讀者知道 evidence 的能力邊界。Metric 適合看趨勢，log 適合看事件細節，trace 適合看路徑，audit log 適合看責任鏈。

[Time range](/backend/knowledge-cards/time-range/) 是 evidence package 的基本欄位。事故前後 30 分鐘、部署期間、DR drill 時窗、[burn rate](/backend/knowledge-cards/burn-rate/) 短窗與長窗都需要明確，否則同一張圖可能被不同人解讀成不同結論。

[Query link](/backend/knowledge-cards/query-link/) 比截圖更重要。截圖適合溝通當下狀態，query link 才能讓下一班 on-call、可靠性 owner 或 PIR reviewer 重跑同一個判讀。

[Data quality](/backend/knowledge-cards/data-quality/) 欄位讓 evidence 保留限制。sampling ratio、ingest delay、schema drift、log drop、cardinality truncation 與 timestamp skew 都應直接出現在證據包中。
...
資料庫 migration 用的 evidence package 要回答「資料語意是否能進入下一階段」。它重視 [validation query](/backend/knowledge-cards/validation-query/)、row count、mismatch sample、replication lag、slow query 與資料限制；完整服務路徑可接到 [1.7 Schema Migration Rollout 證據](/backend/01-database/schema-migration-rollout-evidence/)。
...
兩個案例共同指向同一個判讀原則：證據包不是蒐集越多越好，而是要保留「能支持當下決策」的最小閉環，至少包含事件時間窗、跨訊號對位、資料限制與決策責任人。

## 誤判風險與修正路徑

事故中的誤判多半不是演算法錯，而是證據包缺少判讀上下文。當 evidence 只有結論沒有限制，下游就會把暫時訊號當成穩定事實。

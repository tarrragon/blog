...
可觀測性威脅建模的判讀目標是「觀測系統本身有哪些弱點會讓事故更難處理、更慢收斂、或擴大成資安事件」。威脅建模這個框架本身（把系統攤成資產、邊界與攻擊者能力再逐項列舉）見[威脅模型](/backend/knowledge-cards/threat-model/)。觀測系統是事故處理的核心工具 — 工具失靈時，事故的 MTTD（偵測時間）跟 MTTR（修復時間）都會被拉長。

...
Operating model 不是一套通用的角色矩陣。組織規模決定能投入的治理人力、能承受的協調成本、能維持的審核頻率。把大組織的治理模型套到小團隊會造成過度治理；把小團隊的鬆散模型套到大組織會造成責任懸空。

對應 [4.C10 規模差異下觀測遷移](/backend/04-observability/cases/contrast-observability-rollout-by-scale/)：揭露三種規模團隊的治理失敗方向，以下基於通用工程知識展開。

小型組織的 operating model 重點是「角色合一、節奏明確」。一個 SRE 同時承擔 platform、service、on-call、cost owner 多重身份。治理重點不是角色拆分，而是顯式記錄當前 ownership 跟 review cadence，避免「大家都管 = 沒人管」。Dashboard review、alert review、cost review 可以合併在同一個月會中，但要有具體的決議紀錄。
...
三類組織的共同教訓是：先把 ownership 視為可演進的，再決定當前該採哪種配置。組織成長過程中 ownership 矩陣會反覆調整，每次調整都要把新配置寫進文件、進入 release / runbook 流程，而不是默契傳遞。

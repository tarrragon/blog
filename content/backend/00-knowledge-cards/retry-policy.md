...
Retry policy 的核心概念是「定義失敗後何時再試、試幾次、用什麼間隔、何時停止」。重試可以吸收暫時性故障，但也可能放大下游壓力或重複造成副作用。

## 概念位置

Retry 適合網路抖動、暫時 timeout、rate limit 後恢復、broker 短暫中斷等情境。永久性錯誤，例如 payload 格式錯、權限拒絕、業務狀態不允許，應分類處理或送進 dead-letter。

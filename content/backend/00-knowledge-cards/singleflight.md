...
Singleflight 的核心概念是「相同 key 的並發工作只讓一個執行者真正打下游，其餘等待同一份結果」。它用來降低 cache miss、重建資料或初始化時的重複下游壓力。

## 概念位置

Singleflight 是 cache stampede 與 thundering herd 的防護工具。它適合相同 key、相同結果、短時間內大量重複請求的場景。

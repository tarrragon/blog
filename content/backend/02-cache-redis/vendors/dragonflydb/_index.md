...
### 直接相關案例（待補 DragonflyDB-specific case）

DragonflyDB 2022 年才開源、production case 累積中。可追蹤方向：DragonflyDB customer adoption blog、benchmark 比較文章。

### 跨 vendor 對照

| 案例                                                                                            | 對 DragonflyDB 的對應                          |
| ----------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| [2.C10 規模對照](/backend/02-cache-redis/cases/contrast-cache-strategy-by-scale/)               | DragonflyDB 適合中大型 single instance 場景    |
| [2.C9 Cache Stampede](/backend/02-cache-redis/cases/failure-cache-stampede-rollout-regression/) | TTL jitter 通用、DragonflyDB 行為跟 Redis 一致 |

**待補 DragonflyDB 案例**：早期採用者的 benchmark 報告、從 Redis Cluster 收回 single instance 的遷移案例、BSL 授權實際商業使用評估。

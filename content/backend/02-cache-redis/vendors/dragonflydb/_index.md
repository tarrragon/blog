...
```bash
# 1. 啟動 DragonflyDB（thread 數預設 = CPU 核數、自動多核）
docker run -d --name dragonfly -p 6379:6379 \
  docker.dragonflydb.io/dragonflydb/dragonfly

# 2. 用 redis-cli 驗證（wire-protocol 相容、直接用 redis-cli）
redis-cli SET foo bar    # → OK
redis-cli GET foo        # → bar

# 3. 確認版本與多核：DragonflyDB 回報相容的 redis_version + 自身版本 + thread 數
redis-cli INFO server | grep -E "redis_version|dragonfly_version|thread_count|multiplexing_api"
# redis_version:7.4.0          ← client library 以此判斷相容性
# dragonfly_version:df-v1.39.0 ← DragonflyDB 自身版本
# thread_count:8               ← 自動對齊 CPU 核數（shared-nothing 多核）
# multiplexing_api:epoll
```

第三步是 DragonflyDB 跟 Redis 的核心差異證據：`thread_count` 自動對齊 CPU 核數、每個 thread 管自己的 partition（shared-nothing），這是它高吞吐的來源；`redis_version:7.4.0` 讓既有 Redis client 直接相容、無需改 code。實機驗證於 dragonfly df-v1.39.0、最後檢查日 2026-06-16。實際遷移評估見 [Redis 相容邊界](#redis-相容邊界)。
...
```bash
redis-cli INFO server | grep -E "dragonfly_version|thread_count"
# dragonfly_version:df-v1.39.0
# thread_count:8                ← 對齊 CPU 核數才能發揮多核

...
[TTL](/backend/knowledge-cards/ttl/) 是資料類型的新鮮度預算，用單一時間常數理解它會漏掉關鍵差異。商品描述、推薦列表、活動文案可容忍較長 TTL；價格、庫存、配額、權限則需要更短 TTL 或事件失效。
...
| [stale read](/backend/knowledge-cards/stale-read/) 事件上升 | TTL 過長或失效機制不足        | 縮短關鍵欄位 TTL、補事件失效        |

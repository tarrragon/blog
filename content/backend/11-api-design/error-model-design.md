...
錯誤模型是契約的一級公民：消費者的重試邏輯、監控的告警規則、前端的使用者訊息、全部建立在錯誤回應的形狀上。錯誤格式一旦被依賴、變更成本跟正常回應完全相同（承諾成本結構見 [11.1](/backend/11-api-design/api-boundary-responsibility/)）、但多數 API 把設計精力全放在成功路徑、錯誤格式在第一個 handler 裡即興決定 — 之後每個新錯誤都在複製那次即興。

## 第一刀：可重試與終態

錯誤分類的第一個維度是對消費者行為的指示：這個錯誤重試有沒有用。可重試（服務暫時失效、限流、鎖衝突）指示消費者退避後重送；終態（參數錯誤、權限拒絕、業務規則拒絕）指示消費者停止重試、走修正或人工路徑。這一刀切錯的代價是雙向的：終態錯誤被標成可重試、消費者的 retry 迴圈空轉打爆服務；可重試被標成終態、暫時性故障變成使用者眼中的永久失敗。

HTTP status 承擔這一刀的粗分類（4xx 終態、5xx 與 429 可重試、見 [11.3](/backend/11-api-design/resource-modeling-operation-semantics/) 的 status 承諾段）、錯誤 body 承擔細分類。兩層要一致 — body 說可重試、status 給 400、中介層跟 SDK 只看 status、消費者的兩層邏輯就打架。
...
錯誤模型的最後一段責任是「錯誤發生時、系統還敢做什麼」。Twilio 2013 年的計費事故提供了反例時序：餘額資料遺失歸零且無法寫回、自動扣款在「餘額為零、扣款成功、餘額寫不回去」的循環裡對客戶重複扣款（見 [11.C45](/backend/11-api-design/cases/idempotency-twilio-billing-postmortem/)、反例）。post-mortem 的修復方向是 fail-safe：狀態寫不進去時、禁止產生金流 side effect。落到錯誤模型的通用判準：關鍵狀態讀寫失敗的錯誤處理、預設要往「拒絕服務」收斂、而非「帶著壞狀態繼續」— 錯誤分類表裡要有一類「狀態不可信、停止副作用」、它的處理路徑跟一般 5xx 不同。

## 常見設計錯誤

- **業務失敗包 200**：觀測與重試鏈失真、修法見 11.3 的 status 承諾段。
...
- **錯誤格式沒有演化條款**：第一版沒宣告「未知欄位忽略」、之後加欄位都提心吊膽 — 條款要在 day one 寫進文件。

## 爭論地圖與下一步

錯誤格式的跨風格交鋒（RFC 9457、envelope 包裝、GraphQL 的 200-with-errors 慣例）是掛在本章的爭論深度文章 backlog（見 [模組頁](/backend/11-api-design/) 章節規劃）— GraphQL 把 transport 層 status 跟業務錯誤解耦的做法、在該文攤開、本章不展開。

- 可重試錯誤的消費端行為設計：[11.8 API 層冪等設計](/backend/11-api-design/api-idempotency-design/)、[06 冪等與重放](/backend/06-reliability/idempotency-replay/)

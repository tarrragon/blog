...
紀律需要兩個邊界條款才能長期運作。**到期行為**：宣告過的 breaking 變更到期執行時、回明確錯誤而非靜默改語意 — Facebook Graph v1.0 的靜默切換讓未遷移 app 默默壞掉（見 [11.C17](/backend/11-api-design/cases/versioning-facebook-graph-v1-forced-upgrade/)、反例）、與之對照、GitHub 用 brownout 讓消費者在預告時窗先撞到明確故障（見 [11.C13](/backend/11-api-design/cases/versioning-github-password-auth-brownout/)）。**豁免宣告**：每次變更公告要明列「誰不受影響」— GitHub 的密碼認證廢止同時明列 2FA 使用者、GHES、GitHub App 不受影響、讓多數消費者第一段就能停止閱讀、注意力留給真正要動的人。
...
- 消費端驗證（consumer-driven contract test 把「誰依賴什麼」顯性化）：[06 契約測試](/backend/06-reliability/contract-testing/)

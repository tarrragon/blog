...
date: 2026-07-04
description: "錯誤跨服務傳遞時誰該轉譯、收到的錯誤能信多少、對外暴露多少細節 — 服務鏈上每一跳同時是 consumer 與 provider 的責任判斷標準"
weight: 23
tags: ["backend", "api-design", "error-contract"]
---

服務鏈上的錯誤處理有一個單服務視角看不到的結構：A 呼叫 B、B 呼叫 C、C 掛了 —— B 對 C 是 consumer、對 A 是 provider、**同一個服務在同一次失敗裡承擔兩份契約責任**。C 的錯誤怎麼變成 B 回給 A 的錯誤、是每個中間服務都要回答的設計題；答錯的形態是把上游錯誤原樣倒給下游（洩漏 + 語意錯位）、或把一切包成不可判讀的 500（資訊消滅）。三個判斷標準依序展開：錯誤的哪一層能傳多遠、收到的錯誤能信多少、對外暴露多少。本文掛在 [11.11 雙向契約](/backend/11-api-design/error-bidirectional-contract/)；跨服務轉譯沒有單一規範明文、本文的責任推導從各規範的單跳條款出發、逐處標明。

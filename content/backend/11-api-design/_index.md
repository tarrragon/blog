...
主章（判準層）與 styles/rest 流派批已完成；其餘流派層與爭論深度文章是 backlog、完成後逐篇補站內連結。案例支撐欄的 C 編號對應 [案例庫](/backend/11-api-design/cases/)；標「合成」的章節沒有專屬 case、內容從全庫推導、寫作時依 fact vs derive 紀律標明。案例編號慣例：章節內文首次引用寫 `[11.C<n>]` 連結、同章後續與索引表可用 `C<n>` 裸編號。

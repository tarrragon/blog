...
主章（判準層、11.1-11.11）與六個 styles/ 流派批（rest、graphql、grpc、rpc-revival、standards、realtime）全數完成、11.11 的四篇深度文章已交付；剩四篇爭論深度文章是 backlog、完成後逐篇補站內連結。案例支撐欄的 C 編號對應 [案例庫](/backend/11-api-design/cases/)；標「合成」的章節沒有專屬 case、內容從全庫推導、寫作時依 fact vs derive 紀律標明。案例編號慣例：章節內文首次引用寫 `[11.C<n>]` 連結、同章後續與索引表可用 `C<n>` 裸編號。
...
| 11.7 | 分頁之爭               | offset、[cursor](/backend/knowledge-cards/pagination-cursor/)、keyset；cursor 不透明性算承諾還是逃生門 | C37                |
...
| 項目                                              | 類型   | 前置條件                                     | 規模 |
| ------------------------------------------------- | ------ | -------------------------------------------- | ---- |
| 爭論深度文章：版本策略之爭                        | 主章   | 無（案例庫已就緒）                           | 中   |
| 爭論深度文章：錯誤格式之爭                        | 主章   | 無（案例庫已就緒）                           | 中   |
| 爭論深度文章：分頁之爭                            | 主章   | 無（案例庫已就緒）                           | 中   |
| 爭論深度文章：idempotency key 標準化              | 主章   | 無（案例庫已就緒）                           | 中   |
| 05 的 gateway 限流實作章（11.9 的執行面交接落點） | 跨模組 | 需 05 部署平台模組承接，本模組只提供路由需求 | 中   |

### 目前缺口與後續批次

主章（11.1-11.11、含 11.11 四篇深度文章）與六個流派批全數完成、`styles/` 流派層收尾。剩餘工作只有爭論深度文章：

| 缺口         | 內容                                                                                   | 前置條件                                   |
| ------------ | -------------------------------------------------------------------------------------- | ------------------------------------------ |
| 爭論深度文章 | 版本策略流派之爭、錯誤格式之爭、分頁之爭、idempotency key 標準化（掛對應主章、見上表） | 無（案例就緒、各主章的路由落點等它們補實） |

模組外的關聯缺口、記錄備查：知識卡四張候選（見下方知識卡候選段）；05 部署平台模組缺 gateway 限流實作章、[11.9](/backend/11-api-design/external-traffic-semantics/) 的執行面交接目前落在該模組首頁（已在責任分工表標 backlog）。

...
主章（判準層）與六個 styles/ 流派批（rest、graphql、grpc、rpc-revival、standards、realtime）全數完成；剩爭論深度文章是 backlog、完成後逐篇補站內連結。案例支撐欄的 C 編號對應 [案例庫](/backend/11-api-design/cases/)；標「合成」的章節沒有專屬 case、內容從全庫推導、寫作時依 fact vs derive 紀律標明。案例編號慣例：章節內文首次引用寫 `[11.C<n>]` 連結、同章後續與索引表可用 `C<n>` 裸編號。
...
主章與六個流派批（rest、graphql、grpc、rpc-revival、standards、realtime）全數完成、`styles/` 流派層收尾。剩餘工作只有爭論深度文章：

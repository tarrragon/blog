...
本模組處於規劃階段、以下章節表是 backlog、完成後逐篇補站內連結。案例支撐欄的 C 編號對應 [案例庫](/backend/11-api-design/cases/)；標「合成」的章節沒有專屬 case、內容從全庫推導、寫作時依 fact vs derive 紀律標明。

### 主章（判準層）

| 章節  | 主題                   | 核心問題                                                                                                         | 案例支撐               |
| ----- | ---------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------- |
| 11.1  | API 作為服務邊界的責任 | 承諾的成本結構：改內部便宜、改對外語意昂貴；consumer 視角建模                                                    | 合成（全庫）           |
| 11.2  | 風格選型總覽           | 消費者形狀、同步 / 非同步、演進成本的判準軸；各風格深度收在 `styles/`                                            | 合成（C18-C34 為主）   |
| 11.3  | 資源建模與操作語意     | 資源導向與動作導向的取捨、HTTP method / status 的承諾意義、跨資源操作                                            | C1、C5（偏論證型）     |

...
**採 cursor 而沒查過跳頁需求**。遷移完才發現後台的頁碼列跟總數回不來，此時的選項只剩下再開一套 offset 端點並行。檢查問法在遷移之前：現有介面的頁碼與總數，有哪個角色正在用它做事。消費者在組織外而答不出這一問時，退路是看 `page` 參數的深度分布——沒有人翻超過第三頁，跟每天有人翻到第兩百頁，是兩個不同的決定。

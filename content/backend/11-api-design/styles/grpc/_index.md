...
gRPC 的選型判讀分三層、對應三篇：契約怎麼安全演進（編碼層紀律 + CI gate）、上線前要知道哪些部署約束（trailers、瀏覽器、debug 可及性）、值得選的組織前提是什麼（框架層集中而非序列化效能）。本目錄不追 gRPC 的技術史、只回答「什麼情境該選、選了怎麼用、什麼時候別選」。中性選型標準見 [11.2 風格選型總覽](/backend/11-api-design/api-style-selection/)。
...
| [streaming 與部署邊界](/backend/11-api-design/styles/grpc/grpc-streaming-deployment-boundary/) | trailers 與瀏覽器、翻譯 proxy、debug 可及性判斷標準 | C30、C32 |

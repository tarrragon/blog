...
冪等鍵的標準化現況是業界實作先行、正式標準停滯：IETF httpapi 工作組的 `Idempotency-Key` header draft 推進到版本 07 後過期，狀態為 expired（見 [11.C40](/backend/11-api-design/cases/idempotency-ietf-key-header-draft/)）。draft 期滿是 IETF 的流程事實，本文不推測它為什麼沒有續推 —— 工作組人力、優先序、爭議未收斂都可能是原因，而公開紀錄沒有給答案。本文分析的是另一件事：即使 draft 當年推進到 RFC，統一得了的仍只有一層。各家對「用哪個 header 傳這個 key」的分歧是命名之爭，標準一句話就能收；而對「同一個 key 重送時服務端回什麼」的分歧是語意之爭，它跟業務形態耦合，transport 層的標準統一不了。本文攤開各家條款的實質差異與它們各自的成立情境。自建一套冪等機制要承諾哪些條款，見 [API 層冪等設計](/backend/11-api-design/api-idempotency-design/)。
...
各家實際會不一樣的條款有五項，本文其餘各節逐項展開，整合任何一家時它們就是檢查表：**header 名**、**replay 回什麼**、**保存期**、**同 key 並發怎麼處理**、**同 key 不同參數怎麼處理**。這五項是[11.8 的冪等契約條款清單](/backend/11-api-design/api-idempotency-design/)在跨家比較下的切面 —— 該章寫的是自建時要承諾什麼（包含 key 由誰生成、作用範圍限 POST 這類各家沒有分歧的項目），本文只收各家答案會不同的那些。
...
**拿 Stripe 的語意假設去打別家**。Stripe 的條款是目前公開文件裡最明確的一份，因此常被當成冪等鍵的預設語意。整合另一家時，前述五項條款逐一會不同，而錯誤的形態是靜默的：重試拿到的回應跟預期不同，程式繼續往下走。檢查問法：五項在整合文件裡都查得到嗎。查不到的逐項套保守預設 —— header 名假設每個 API 都要各查一次、replay 假設回最新狀態（因此不把回應當終局）、保存期假設只有幾分鐘（超過就先查詢再決定重送）、並發假設第二個請求會失敗（因此重試要帶退避）、同 key 不同參數假設服務端不檢查（因此自己保證同一個 key 只配一份參數）。
...
**假設消費者不會誤用 key**。同 key 不同參數直接報錯這條的作用是防止 key 被當 session id 濫用（C39）。它在五項條款裡的位置特殊 —— 其餘四項的答案隨業務形態變動、各家不同是正常的，而「同 key 綁定同一份請求語意」在任何業務形態下都成立，因此這一條可以給通則。缺了它，消費者無意間重用 key 時拿到的是前一次操作的結果，而它看起來像成功。檢查問法：用同一個 key 送兩個金額不同的請求，第二個拿到的是錯誤還是第一筆的結果。

無標準的現況把兩端推向對稱：同一份五項條款，自建時是要寫進文件的承諾，整合時是要逐家讀的檢查表。自建方寫明條款，消費者才設計得出重試；整合方讀不到條款，就在自己這端補上對應的保守處理。這份對稱是雙方在沒有共同規範時可依賴的東西 —— 它不需要任何一方等 IETF，只需要兩邊都認得同一組問題。

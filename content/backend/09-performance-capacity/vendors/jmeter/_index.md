...
- **Thread group 計算錯 / 真實流量對不上**：把 thread 當「user」直接設、忽略 think time + ramp-up、結果壓出來的不是業務流量而是 thread 全速跑 — 改用 Concurrency Thread Group 或 Throughput Shaping Timer 直接以 RPS 為目標、配 Constant Timer 模擬 think time

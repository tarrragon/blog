...
Vegeta 適合回寫單 endpoint hot path 與修補驗證案例。它可接 [Coinbase ultra-low latency](/backend/09-performance-capacity/cases/coinbase-ultra-low-latency-exchange-2023/) 的 latency distribution 判讀、[Tubi feature store](/backend/09-performance-capacity/cases/tubi-elasticache-ml-feature-store/) 的低延遲 lookup 驗證，以及 [Lemino connection limit](/backend/09-performance-capacity/cases/ntt-docomo-lemino-japanese-streaming/) 的 RDB bottleneck 探測。

這些案例的重點是快速定位與比較。Vegeta 頁引用案例時，要把 case 轉成 endpoint、rate、duration、latency budget、target saturation metric 與 runner limit。

...
- Storage：profile sample 大、retention 7-30 天的 storage cost 不小（每 service 每天可能 1-10 GB）
...
對 *已經有 Datadog APM 帳單* 的 org、profiling 是 add-on（不獨立計費）、跟 APM host 一起進預算、不需要額外 ops headcount。這條 TCO 拉力對 50-500 人 eng 規模最強 — 小於 50 人 self-host 也撐得住、大於 500 人 self-host 的 economy of scale 開始 favored Pyroscope。
...
- **Language coverage**：當前 Pyroscope 用哪些 SDK？Datadog Profiler 支援的語言列表跟 Pyroscope 不完全一樣（Go / Java / Python / Node / Ruby / .NET 都有、但 Rust / PHP / Erlang 等小眾語言 Datadog 可能沒有原生）
...
- **Retention requirement**：Pyroscope retention 可自管（30-90 天甚至更長）、Datadog Profiler default 7 天、長 retention 加價且仍有上限（30-90 天 enterprise）
...
- **Cost forecast**：每個 APM host 加 profiling add-on 的月費（Datadog 公開 pricing 約 $5-10 / host / mo additional）、預估規模後跟 Pyroscope self-host TCO 比
...
### 5. Retention 政策變動造成歷史 query 斷層

Pyroscope 自管 retention 可以設 90 天甚至更長、Datadog Profiler 預設 7 天（custom retention 加價）。如果原 Pyroscope user 習慣查 30-60 天前 flame graph（找 long-term regression / seasonal pattern）、cutover 後突然只能查 7 天、用戶痛感強。修法是 Phase 1 確認 retention requirement、必要時加價買 Datadog Profiler extended retention（typical 30 天）、超出 30 天的長期 trend 改用 Datadog metrics-derived signal（cumulative CPU% / memory growth rate）而非直接 flame graph。

## Capability 對照

| 能力                                    | Pyroscope（self-host）                               | Datadog Continuous Profiler                                      |
| --------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------- |
| Language SDK 覆蓋                       | Go / Java / Python / Node / Ruby / .NET / Rust / PHP | Go / Java / Python / Node / Ruby / .NET（無 Rust / PHP 原生）    |
...
| Retention                               | 自管（無上限、cost 自負）                            | 7 天 default / 30-90 天 enterprise tier                          |
| 資料主權                                | 完全自管                                             | SaaS（profile 出境）                                             |
| Ops ownership                           | 自管（storage / scaling / on-call）                  | Vendor                                                           |
| Cost model                              | self-host TCO                                        | $5-10 / host / mo（APM add-on）                                  |
...
- **Long retention 強需求（> 90 天）**：Datadog Profiler retention 上限不夠、保留 Pyroscope
- **Rust / PHP / Erlang 等 Datadog 沒原生 SDK 的語言**：保留 Pyroscope 為對應 service profiling

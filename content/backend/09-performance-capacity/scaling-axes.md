...
擴展軸的核心責任是讓系統面對流量成長時有預定的因應路徑。垂直擴展（scale-up）跟水平擴展（scale-out）對應不同壓力來源、各自承擔不同代價：垂直擴展用「換更大的機器」換取簡單，水平擴展用「加更多機器」換取彈性。讀者在規劃容量時先判讀自己的壓力屬於哪種，再選對應的擴展軸；選錯軸的代價會在事故時放大。
...
**協調成本**：多臺機器要處理「誰是 leader、誰來執行排程、誰來處理同一筆訂單」這類問題。leader election、distributed lock、consensus protocol 都會引入新的故障模式。
...
引用判讀順序：先看案例的擴展軸選擇（垂直 / 水平 / 混合），對照本章「兩個軸的責任差異」段；接著看 stateful 部分如何處理，對照「stateless 是水平擴展的前提」段；最後看 auto scaling 或 scheduled scaling 的應用，對照「Auto Scaling 的操作模型」段。對於非直接對應的案例（Capcom、Netflix Aurora），用反向追問把案例的決策放回擴展軸框架，而非把案例本身當成擴展軸示範。

## 跨模組路由

1. 與 [9.1 壓測理論與系統行為](/backend/09-performance-capacity/performance-theory/) 的交接：USL 跟 Little's Law 在理論上推導水平擴展的曲線、本章解釋實務上怎麼撞到這道牆。
...
要看具體容量計算與 headroom 模型，接著讀 [9.6 容量規劃模型](/backend/09-performance-capacity/capacity-planning/)。要看擴展前的瓶頸定位，接著讀 [9.5 瓶頸定位流程](/backend/09-performance-capacity/bottleneck-localization/)。要看服務拆分如何配合水平擴展，接著讀 [0.18 服務拆分與邊界判讀](/backend/00-service-selection/service-decomposition-boundaries/)。

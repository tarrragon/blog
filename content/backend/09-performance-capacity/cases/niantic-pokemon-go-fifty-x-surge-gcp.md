...
這個案例的核心責任是說明「surge load」（突發遠超預期）跟 event-peak（事件型可預測峰值）的差異。Pokémon GO 在 2016-07 上線時、實際流量達到原始容量規劃目標的 50 倍 — 這不是峰值規劃的失敗、而是 *根本沒人能預測這個產品會這麼紅*。這類負載對容量設計的要求跟其他案例本質不同。

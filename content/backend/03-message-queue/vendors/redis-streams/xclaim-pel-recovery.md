...
選型回路：單 stream 撞到單 shard 天花板、或 failover 一致性要求超出 at-least-once 時，需要的是跨節點的 partition 與 replication，改評估 [Kafka](/backend/03-message-queue/vendors/kafka/)；需要的是複雜 routing 時看 [RabbitMQ](/backend/03-message-queue/vendors/rabbitmq/)，想交給託管服務時看 [SQS](/backend/03-message-queue/vendors/aws-sqs/) 或 [Pub/Sub](/backend/03-message-queue/vendors/google-pubsub/)。Cluster 層的 slot / topology 行為見 [Redis Cluster Re-sharding](/backend/02-cache-redis/vendors/redis/cluster-resharding/)。

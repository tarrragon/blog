...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [Kafka](kafka/)                  | [rebalance/lag](kafka/consumer-rebalance-lag-diagnosis/) / [replication/ISR](kafka/replication-isr-exactly-once/) / [retention/tiered](kafka/retention-tiered-storage/) / [schema registry](kafka/schema-registry-evolution/) / [multi-tenant](kafka/multi-tenant-quota-acl/) | [↔ NATS](kafka/migrate-from-to-nats/) / [→ MSK](kafka/migrate-to-msk/)            |
| [RabbitMQ](rabbitmq/)            | [queue 模型選型](rabbitmq/queue-types-classic-quorum-stream/) / [network partition](rabbitmq/network-partition-clustering/) / [DLQ retry escalation](rabbitmq/dlq-retry-escalation/) / [quorum vs mirrored](rabbitmq/quorum-vs-mirrored-queue/)                               | [→ Kafka](rabbitmq/migrate-to-kafka/) / [→ AWS SQS](rabbitmq/migrate-to-aws-sqs/) |
| [NATS](nats/)                    | [JetStream/supercluster](nats/jetstream-supercluster-design/) / [JetStream durability/consumer](nats/jetstream-durability-consumer/)                                                                                                                                          | ↔ Kafka（見 Kafka 列）                                                            |
| [Redis Streams](redis-streams/)  | [XCLAIM/PEL](redis-streams/xclaim-pel-recovery/) / [consumer-group PEL recovery](redis-streams/consumer-group-pel-recovery/)                                                                                                                                                  | [→ Kafka](redis-streams/migrate-to-kafka/)                                        |
| [AWS SQS](aws-sqs/)              | [visibility/polling/Lambda](aws-sqs/visibility-polling-lambda-cost/) / [visibility timeout/redrive](aws-sqs/visibility-timeout-redrive/)                                                                                                                                      | [→ Google Pub/Sub](aws-sqs/migrate-to-google-pubsub/)                             |
| [Google Pub/Sub](google-pubsub/) | [ordering/DLT/schema](google-pubsub/ordering-dlt-schema/) / [push/pull/ack flow control](google-pubsub/push-pull-ack-flow-control/)                                                                                                                                           | —                                                                                 |

deep article 走 [6-section 模板](/posts/vendor-deep-article-methodology/)、指令均經 Docker / emulator 實機驗證（驗不了的標 caveat）；migration playbook 走 [6-type 結構](/posts/migration-playbook-methodology/)。

本表合併了 main 與 feat/backend_03 兩批平行撰寫的 03 deep article、部分主題重疊、尚待人工去重（de-dup pending）：RabbitMQ quorum（`queue-types-classic-quorum-stream` 與 `quorum-vs-mirrored-queue`）、NATS JetStream（`jetstream-supercluster-design` 與 `jetstream-durability-consumer`）、Redis Streams PEL（`xclaim-pel-recovery` 與 `consumer-group-pel-recovery`）、AWS SQS visibility（`visibility-polling-lambda-cost` 與 `visibility-timeout-redrive`）。每組待決定合併、保留不同角度或取捨。後續候選見上方「T1 服務頁大綱」段、各 vendor `_index.md` 進階主題段與下方「後續候選」表。

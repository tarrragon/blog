...
| [RabbitMQ](rabbitmq/)            | [queue 模型選型](rabbitmq/queue-types-classic-quorum-stream/) / [network partition](rabbitmq/network-partition-clustering/) / [DLQ retry escalation](rabbitmq/dlq-retry-escalation/)                                                                                          | [→ Kafka](rabbitmq/migrate-to-kafka/) / [→ AWS SQS](rabbitmq/migrate-to-aws-sqs/) |
...
| [Redis Streams](redis-streams/)  | [XCLAIM/PEL](redis-streams/xclaim-pel-recovery/)                                                                                                                                                                                                                              | [→ Kafka](redis-streams/migrate-to-kafka/)                                        |
| [AWS SQS](aws-sqs/)              | [visibility/polling/Lambda](aws-sqs/visibility-polling-lambda-cost/)                                                                                                                                                                                                          | [→ Google Pub/Sub](aws-sqs/migrate-to-google-pubsub/)                             |
| [Google Pub/Sub](google-pubsub/) | [ordering/DLT/schema](google-pubsub/ordering-dlt-schema/) / [push/pull/ack flow control](google-pubsub/push-pull-ack-flow-control/)                                                                                                                                           | [← Kafka](google-pubsub/migrate-from-kafka/)                                      |
...
main 與 feat/backend_03 兩批平行撰寫過 03 deep article、重疊主題已去重：RabbitMQ quorum、Redis Streams PEL、AWS SQS visibility 三組各保留涵蓋較完整、經實機驗證的一篇（主題框架已併入）。NATS 保留兩篇互補定位——[core 到 JetStream 邊界](nats/jetstream-durability-consumer/) 是採用決策入口、[JetStream 設計與 supercluster/leaf](nats/jetstream-supercluster-design/) 是完整實作。後續候選見上方「T1 服務頁大綱」段、各 vendor `_index.md` 進階主題段與下方「後續候選」表。

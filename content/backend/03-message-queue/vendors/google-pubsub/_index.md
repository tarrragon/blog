...
# 2. 建 subscription（pull 模式、綁定 topic）
gcloud pubsub subscriptions create demo-sub --topic=demo-topic

# 3. publish + pull 驗證
gcloud pubsub topics publish demo-topic --message="hello"
gcloud pubsub subscriptions pull demo-sub --auto-ack
```

最短路徑驗證「topic / subscription 建得起來、能發能收」。實際應用見[日常操作](#日常操作與決策形狀)。指令對真實 GCP 需設定 project 與認證；本機要先驗證可啟動 Pub/Sub emulator、用 `gcloud config set api_endpoint_overrides/pubsub` 把同一組 CLI 指向 emulator 跑通。
...
ordering key、dead-letter topic 與 schema enforcement 已展開為 deep article：[ordering key / DLT / schema enforcement](ordering-dlt-schema/)、[push / pull / ack flow control](push-pull-ack-flow-control/)。下列子議題段保留選題判讀入口。
...
# 看 ackDeadlineSeconds（預設 10s）與 messageRetentionDuration（預設 604800s / 7 天）是否符合處理時間與 replay 需求
```

...
<!-- TODO(merge): feat/backend_03 worktree 同時在深化 03 vendor overview。本檔是 main 上新增的 deep article、未動 google-pubsub/_index.md。合併後須檢查：(1) 與對方主題重複 (2) google-pubsub/_index.md 是否加 deep-article 指標 (3) vendors/_index.md 覆蓋表合併。 -->
...
**ack deadline 是 Pub/Sub 版的可見性逾時**。consumer 收到訊息後，有一段 ack deadline 來處理並 `ack`。在 deadline 內沒 ack，Pub/Sub 重新投遞（at-least-once）。跟 [SQS visibility timeout](/backend/03-message-queue/vendors/aws-sqs/visibility-timeout-redrive/) 同樣是雙邊風險：太短→處理中就重投、太長→失敗後恢復慢。處理中可用 `modifyAckDeadline`（client library 通常自動 lease extension）延長。
...
4. 對照 [SQS redrive](/backend/03-message-queue/vendors/aws-sqs/visibility-timeout-redrive/)：兩者都是 managed 原生 DLQ/DLT、比自建省事
...
- **跟 [SQS visibility timeout](/backend/03-message-queue/vendors/aws-sqs/visibility-timeout-redrive/)**：ack deadline 對應 visibility timeout、DLT 對應 redrive，兩個 managed queue 的可靠消費模型高度對位、可對照閱讀。
...
- 對照 vendor：[AWS SQS visibility timeout](/backend/03-message-queue/vendors/aws-sqs/visibility-timeout-redrive/)、[RabbitMQ DLQ](/backend/03-message-queue/vendors/rabbitmq/dlq-retry-escalation/)

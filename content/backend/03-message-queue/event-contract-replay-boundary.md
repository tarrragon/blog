...
event id 支撐 incident timeline 與重複投遞判讀。schema version 支撐新舊 consumer 共存。occurred time 支撐 replay window 與對帳查詢。dedup key 支撐 idempotency。PII scope 支撐 audit 與資料保護。這些欄位先成立，broker [retention](/backend/knowledge-cards/retention/) 或 partition 設計才有可依附的語意。
...
實體 broker 文章要承接本篇的 event contract 與 replay boundary。Kafka 的長期 retention、RabbitMQ 的 routing 與 DLQ、SQS 的 [visibility timeout](/backend/knowledge-cards/visibility-timeout/)、NATS JetStream 的 stream/consumer 模型，都要放回事件契約與重播邊界下判讀。

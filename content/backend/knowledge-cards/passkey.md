...
它擋掉的是兩類與共享秘密綁在一起的攻擊。[Credential stuffing](/backend/knowledge-cards/credential-stuffing/) 失效的原因是沒有可重用的秘密——別處外洩的那些值在這裡對不上任何東西。釣魚失效的原因是簽章由瀏覽器綁在當下的網域上，釣魚站的網域不同就產不出可用的簽章，而這一項與使用者有沒有警覺無關。

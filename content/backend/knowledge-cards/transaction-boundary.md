...
交易適合保護同一個 database 內的正式狀態變更。建立訂單與扣庫存可能需要同一個交易；寄信、呼叫外部 API 或發布 [broker](/backend/knowledge-cards/broker/) 訊息則常需要 [outbox pattern](/backend/knowledge-cards/outbox-pattern/) 或補償流程，因為它們超出 database transaction 能力。

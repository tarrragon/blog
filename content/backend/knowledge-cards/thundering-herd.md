...
Thundering herd 的核心概念是「大量等待中的工作同時醒來並競爭同一個資源」。這種同步行為會製造瞬間尖峰，即使平均流量看起來可承受。

## 概念位置

Thundering herd 常出現在 cache 過期、鎖釋放、服務恢復、排程任務、client reconnect 與廣播通知。它和 cache stampede 相近，但範圍更廣，包含各種同時醒來與同時競爭。

...
Slack 從 offset 分頁遷移到 opaque cursor 時明列了付出的代價：失去 total count 與跳頁能力，這是明示的產品決策而非技術妥協——選 cursor 前要先跟產品端確認「第 N 頁」跟「共幾筆」是不是真需求，而不是遷移完才發現這兩個功能回不來了。判斷標準是資料量小、寫入頻率低、產品要跳頁時 offset 分頁合理且便宜；資料量大或寫入頻繁時該用 cursor。

## 判讀方式

不透明性另有一個常被漏掉的維度：cursor 綁不綁呼叫者。cursor 不綁身分時，A 拿到的 cursor 交給 B 就能從 A 的位置繼續翻——對套用了列級權限的集合，opaque cursor 因此成為一條繞過起點檢查的枚舉面。它在多數情境是選配，而兩個條件會讓它變成必要：集合的不同呼叫者看得到的子集不同，或 cursor 會流經轉售通路與第三方整合方之手。這一維跟不透明性的強度是獨立的——加密過的 cursor 一樣可以被轉交。

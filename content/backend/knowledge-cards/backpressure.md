...
Backpressure 出現在 [in-process channel](/backend/knowledge-cards/in-process-channel/)、[queue](/backend/knowledge-cards/queue/)、[worker pool](/backend/knowledge-cards/worker-pool/)、[HTTP client](/backend/knowledge-cards/http-client/)、[connection pool](/backend/knowledge-cards/connection-pool/)、[broker](/backend/knowledge-cards/broker/) 的 [consumer](/backend/knowledge-cards/consumer/) 與 [stream pipeline](/backend/knowledge-cards/stream-pipeline/)。它處理的是速度不匹配：進入速度高於處理速度。

Backpressure 與 [rate limit](/backend/knowledge-cards/rate-limit/) 的差別在於資訊流向：rate limit 由上游主動設閘門（「每秒最多 N 個」），屬於容量規劃；backpressure 由下游回饋壓力（「我現在只能吃 M 個」），屬於動態調速。兩者常搭配使用：rate limit 處理已知的規劃容量，backpressure 處理無法預先預測的即時變化。

## 可觀察訊號

需要 backpressure 的訊號包含 [queue depth](/backend/knowledge-cards/queue-depth/) 上升、記憶體持續增加、[timeout](/backend/knowledge-cards/timeout/) 比例擴大、[consumer lag](/backend/knowledge-cards/consumer-lag/) 加深、下游 error rate 上升。當這些指標同時出現而上游流量維持穩定時，代表處理鏈某一段已成為瓶頸，壓力需要向上傳遞，而不是繼續往 buffer 堆積。

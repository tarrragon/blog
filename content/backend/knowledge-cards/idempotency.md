...
Idempotency 是副作用邊界的穩定性設計。查詢通常天然接近 idempotent；建立訂單、扣款、出貨、寄送通知與發放點數都需要額外設計 idempotency key 或狀態檢查——key 作為對外契約的語意（誰生成、存多久、衝突怎麼回）獨立成 [Idempotency Key](/backend/knowledge-cards/idempotency-key/) 卡。 可先對照 [Idle Timeout](/backend/knowledge-cards/idle-timeout/)。

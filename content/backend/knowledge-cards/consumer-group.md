...
Consumer group 的核心概念是「一組 consumer 共同承擔某個 stream 或 topic 的處理進度」。同一 group 內的 consumer 分攤工作；不同 group 可以各自獨立處理同一批事件。

## 概念位置

Consumer group 是事件流與多服務訂閱的協調模型。分析服務、搜尋索引服務、通知服務可以用不同 group 讀同一 topic；每個 group 有自己的進度與 lag。

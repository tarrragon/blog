...
Worker pool 的核心概念是「用固定或可控數量的 worker 處理工作」。它讓系統限制同時執行量，避免每個 request 或 message 都直接建立無界工作。 可先對照 [In-Process Channel](/backend/knowledge-cards/in-process-channel/)。

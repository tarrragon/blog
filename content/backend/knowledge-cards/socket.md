...
Socket 相關設計要定義連線數上限、read / write timeout、idle timeout、[connection pool](../connection-pool/)、[buffer](../buffer/) 大小與關閉流程。操作上要觀察連線數、timeout、reset、重連次數與下游 latency，避免網路等待耗盡 application 的 worker 或檔案描述符。

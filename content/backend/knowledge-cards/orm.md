...
date: 2026-09-01
description: "看到程式用類別與方法描述查詢、想確認那段程式碼實際送出什麼到資料庫時"
weight: 442
...
連線這一層同樣被它蓋住：一段程式碼實際佔用哪一條連線、佔多久，由框架與連線池的設定決定，而不由這幾行決定。

每一段查詢佔用的仍然是 [connection pool](/backend/knowledge-cards/connection-pool/) 裡的一條連線，而那條連線佔多久由框架的作用域決定、不由這幾行決定。

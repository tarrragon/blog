...
決定要不要導入 dedicated HSM，先確認合規要求是否明文寫死物理隔離，或信任模型上是否不接受供應商持有金鑰。若答案都是否定，multi-tenant 的 managed key service 通常已經滿足同等的 FIPS 等級，且運維成本低得多。已經導入的部署要檢查跨可用區的叢集拓樸、管理身份與操作身份是否分離、備份是否定期演練還原——任一項缺失都是待補項目。

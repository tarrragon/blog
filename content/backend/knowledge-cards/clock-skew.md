...
設計時要把容忍值的來源寫下來，讓它可以被重新計算：量測實際的偏移分布取上界，再決定要不要蓋過尖峰事件。這是一個二選一的決定，不是憑感覺加一個餘裕 —— 要涵蓋快照還原、容器被暫停這類分鐘級偏移，容忍值就得放寬到那個量級；不涵蓋，就把容忍值壓在 NTP 正常收斂的範圍內，改由監控在偏移超標時把該節點移出輪替。前者容忍值大、攻擊窗口跟著大，後者反過來，取捨點是該端點的操作後果。容忍值寫成常數而不記錄依據時，後續沒有人敢動它。

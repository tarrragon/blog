...
這條定律把「承諾」與「實際契約」拆開：你承諾的是文件寫的、你實際負責的是被依賴的。常見形態：consumer parse 錯誤 message 的文字做分支、provider 改個錯字就是 breaking change；consumer 依賴回應欄位的順序、序列化庫升級就壞；consumer 依賴未文件化的 timing 行為、優化反而炸鏈。

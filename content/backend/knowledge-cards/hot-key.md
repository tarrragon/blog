...
Hot key 的核心概念是「少數 key 承受遠高於平均值的流量」。快取、資料庫、broker partition、rate limit counter 與排行榜都可能因 hot key 形成單點瓶頸。

## 概念位置

Hot key 是流量分布問題。總 QPS 看起來可承受時，單一商品、直播間、明星帳號、熱門活動或全域 counter 仍可能把某個 shard、partition 或 Redis node 打滿。

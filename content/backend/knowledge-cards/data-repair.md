...
Data repair 的出場時機是 [rollback window](/backend/knowledge-cards/rollback-window/) 關閉之後、[fail-forward](/backend/knowledge-cards/fail-forward/) 之前的資料層動作：回退回不去舊狀態時，修復是把資料改到正確狀態的具體手段。修復對象分三種，處理路徑各不相同——正式欄位錯誤要修 [source of truth](/backend/knowledge-cards/source-of-truth/) 本身，衍生狀態（cache、index、read model）錯誤要重建，外部副作用（退款、通知、webhook）漏做要走補償流程。判讀重點是先確認問題落在哪一層，再決定用哪條路徑。

...
判斷一個訊號是否夠格當 out-of-band signal，先問它的失效域跟主系統重疊多少——共用同一個雲帳號、同一個網路、同一組憑證都算重疊。這類訊號平時難以自我辯護：沒事故時看不出價值，review 時最容易被當成過度準備砍掉，但它的 ROI 全押在觀測退化的那一小時。完整的機制拆解與客訴升級路徑見 [4.25 觀測共命運失效](/backend/04-observability/observability-shared-fate/)。

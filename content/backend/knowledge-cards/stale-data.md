...
Stale data 的核心概念是「資料已落後於正式來源，但仍被讀取或展示」。它可能來自 cache、replica、search index、read model、CDN 或前端本地狀態。

## 概念位置

Stale data 是一致性與使用者體驗的交界。某些資料過期可接受，例如推薦列表；某些資料過期會造成交易或安全問題，例如價格、庫存、權限。

...
Migration 是 database 與 release 流程的交界。小型服務可能只需要簡單版本檔；正式服務通常需要 [Expand / Contract](/backend/knowledge-cards/expand-contract/) 策略，先新增可相容欄位，再部署 application，最後移除舊欄位。
...
Migration 設計要包含相容性、[backfill](/backend/knowledge-cards/backfill/)、索引建立成本、鎖表風險、回滾策略與 [Release Gate](/backend/knowledge-cards/release-gate/)。高風險 migration 應先在接近正式資料量的環境驗證。

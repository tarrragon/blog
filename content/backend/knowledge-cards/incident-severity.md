...
Incident severity 的核心概念是「用一致標準把事故影響分級」。分級不是描述技術細節，而是描述產品影響範圍、持續時間、資料風險與回復緊急程度。

## 概念位置

Incident severity 連接 [alert](../alert/)、[runbook](../runbook/) 與 [escalation policy](../escalation-policy/)。同一類技術錯誤在不同業務場景可能有不同等級，因此分級要以產品後果為主。

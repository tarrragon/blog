...
Leader election 的核心責任是「從一群對等節點選出單一主來執行某類獨佔工作（執行排程、接受寫入、coordinate cluster state）」。Leader 失效時、剩餘節點要在有限時間內選出新 leader。底層通常依賴 [consensus protocol](/backend/knowledge-cards/consensus-protocol/)（Raft / ZAB）保證選舉一致、避免兩個 leader 同時存在（split-brain）。是 [distributed lock](/backend/knowledge-cards/distributed-lock/) 在「服務角色互斥」這個情境的應用。
...
Election timeout 太短會誤判（網路抖動觸發不必要的切換）、太長會延遲 failover。Fencing token 是必備設計 — 舊 leader 跟新 leader 並存的 split-brain 期間、用單調遞增 token 讓資源側拒絕舊 leader 的寫入。應用層接 leader election 要明確「我不是 leader 時做什麼」— 通常是 stand-by + 監控 election event、不該盲目假設自己持有 leader 角色。

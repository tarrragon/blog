...
判讀的起手是問使用者在來到這個服務之前已經有哪個帳號，答案決定委派可不可行。這類需求的陷阱是把它當成純技術偏好（「用不用密碼」），而實際的判準是可離線猜測的材料最後落在哪裡。完整的取捨、常見組合與判讀流程見 [7.31 認證方式選型](/backend/07-security-data-protection/authentication-approach-selection/)。

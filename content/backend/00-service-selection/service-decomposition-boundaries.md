...
服務拆分的核心責任是讓系統的演進速度跟組織能力與業務獨立性對齊。Monolith 與 microservice 是兩種耦合策略，各自承擔代價：monolith 用單一程式碼庫換低協作成本，microservice 用獨立邊界換團隊與部署彈性。決策起點是辨識當下壓力來源，再選擇拆分軸；流行度與堅持習慣都是次要訊號。
...
- [5.C2 Condé Nast：EKS 平台整併與標準化](/backend/05-deployment-platform/cases/conde-nast-platform-modernization-eks/) — 看多服務整併到單一控制面的決策；對照本章「拆分代價 / 運維複雜度」段。
- [9.C12 Riot Games：246 個 EKS cluster 的多遊戲多地區治理](/backend/09-performance-capacity/cases/riot-games-eks-multi-cluster/) — 看極端規模下服務 / cluster 邊界如何維持；對照本章「拆分軸 / 部署邊界」段。

引用時的判讀順序：先看案例的拆分軸（是沿團隊、資料、流量還是部署？），再對照本章「拆分軸的判讀」段；接著看案例的代價分布（哪一類代價最重？），對照「拆分代價與回退路徑」段；最後看案例是否做過收回動作，對照「拆分過度的收回」段。
...
要看具體服務如何獨立部署，接著讀 [5.2 Kubernetes 部署策略](/backend/05-deployment-platform/kubernetes-deployment/)。要看拆分後的跨服務事件設計，接著讀 [03 模組訊息佇列](/backend/03-message-queue/)。要看拆分如何影響擴展軸的選擇，接著讀 [9.13 擴展軸與 stateless 前提](/backend/09-performance-capacity/scaling-axes/)。

...
| 事務一致性 | 本地 transaction 就解決      | 跨服務需要 saga（一系列補償型 transaction）、[outbox](/backend/knowledge-cards/outbox-pattern/) 或最終一致性 |
...
三個案例可分別對應本章的不同維度：Netflix Aurora consolidation 對應「拆分過度的收回」操作面、Condé Nast 對應「拆分代價 / 運維碎片化」的成本面、Riot Games 對應「拆分軸 / 多維交集」的設計面。把三個案例放在「拆分時機判讀」對照表的不同節點上、比逐案順讀更能看出拆分決策的多面性。

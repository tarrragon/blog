---
title: "0.21 交付形態選型：託管平台、BaaS 與自建的邊界"
...
邊界訊號：第一個撞牆的通常是報表 — 老闆要一張跨集合的月報、Firestore 查不出來、工程師開始把資料複製第二份、複製管線本身變成要維護的系統。第二面牆是帳單：讀寫計費隨流量線性成長、某個月的發票讓人重新打開計算機比對自建。第三面牆最安靜：client 直連資料庫的模型把授權邏輯全部塞進 security rules、規則檔長到沒人敢改時、[0.8](/backend/00-service-selection/security-data-protection-requirements/) 的整個控制面已經壓在一個難以測試的規則語言（DSL）裡。

遷出代價：中高 — 資料可匯出、但資料模型沿平台特性設計（為遷就查詢限制、同一份資料複製存放多處的反正規化結構、加上平台專屬的即時同步語意）、遷到關聯式資料庫等於重做資料層；認證體系可攜性視平台而定（Firebase Auth 可匯出密碼雜湊、是少數友善案例）。
...
遠端操控自己的主機、家庭自動化、個人備份同步這類工具、使用者就是所有者（單人或家人）、沒有對外客戶。它在光譜上是自建的一個特例：差異化不在「賣給誰」、而在「只給自己用」 — 所以自建成立、但本模組其餘章節的多數問題不適用。沒有租戶、沒有使用者資料庫、沒有對外服務、常駐在自有主機（launchd / systemd）或私有網路上。本模組裡真正展開的只剩三條：入口怎麼安全進來（部署平台）、誰能存取（資安）、密鑰怎麼保管（secret）;資料庫、快取、queue、多租戶隔離多半 N/A。
...
邊界訊號：使用者從「自己」變成「自己 + 幾個朋友」。第一個多出來的使用者就打破整個模型 — 共享密鑰無法分辨是誰、生物辨識綁在單一裝置、沒有帳號就無法個別撤銷。這時回到完整自建訪談、把認證升級成帳號系統。

遷出代價：方向跟託管形態相反 — 不是「遷出平台」、而是「長成服務」。工具沒有累積對外使用者資料、遷移成本低;真正要重做的是認證與授權層（從單人共享密鑰換成多使用者帳號系統）、以及入口（從個人 tunnel 換成能承載多人的公開入口）。
...
| 平台月費 + 抽成超過一名工程師成本的明確比例                     | 成本曲線交叉、重算自建的總帳；起始門檻可設「平台年費用超過半個工程師年薪」、依團隊薪資水準調整 |

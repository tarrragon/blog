...
1. 區分本地背景工作、broker queue、stream、pub/sub 與 outbox
2. 用投遞保證、重試需求與 fan-out 需求判斷服務類型
...
這類設計的主要風險是生命週期。worker 要能停止、記錄錯誤、控制 queue full，並在 shutdown 時有明確策略。語言教材通常會處理這一層，例如 Go 的 `Run(ctx)`、channel 與 worker pool。
...
這類設計的主要風險是投遞語意。服務要決定 ack/nack、retry、dead-letter queue、poison message 與 idempotency。RabbitMQ、NATS JetStream、Redis Streams 都可以承擔部分 durable delivery，但模型不同。
...
pub/sub 的核心責任是把訊息即時傳給目前訂閱者。當訊息偏向即時通知，且訂閱者離線後可以透過其他方式補狀態，pub/sub 通常是好候選。
...
這類設計的主要風險是可靠性邊界。pub/sub 適合即時 fan-out；若訊息需要離線補送、audit 或強可靠，通常還需要 durable queue、event log 或資料庫狀態搭配。

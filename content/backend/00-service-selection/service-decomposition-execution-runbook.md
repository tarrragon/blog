...
[0.18 服務拆分與邊界判讀](/backend/00-service-selection/service-decomposition-boundaries/) 處理「該不該拆」、本章處理「決定拆之後實際怎麼動手」。拆服務不是一次性大爆炸（big bang）動作、而是漸進演進。Strangler Fig pattern 是這層的工程基底 — 用「新功能在新服務、舊功能慢慢搬」的方式、把整個 monolith 包圍、逐步替換。

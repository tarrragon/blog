...
| 層               | 主要回讀章節                                                                                                                                                                                                                                                                                                                                                                                         | 回讀時機                             |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| 01 DB            | [1.1 高併發下的 SQL 讀寫邊界](/backend/01-database/high-concurrency-access/)、[1.3 Transaction 與一致性邊界](/backend/01-database/transaction-boundary/)、[1.6 資料庫轉換實作：雙寫、回填、切流與回滾](/backend/01-database/database-migration-playbook/)、[1.7 Schema Migration Rollout 證據（Schema Migration Rollout Evidence）實作示範](/backend/01-database/schema-migration-rollout-evidence/) | transaction 或 schema 問題           |
| 02 Cache         | [2.1 高併發下的 Redis 讀寫邊界](/backend/02-cache-redis/high-concurrency-access/)、[2.2 cache aside 與失效策略](/backend/02-cache-redis/cache-aside/)、[2.7 Cache Copy Boundary 與 Freshness](/backend/02-cache-redis/cache-copy-freshness-boundary/)、[2.9 Cache Migration 與 Stampede Rollback（實作示範）](/backend/02-cache-redis/cache-migration-stampede-rollback/)                            | invalidation 或 stampede 問題        |
| 03 Event         | [3.3 outbox pattern 與發佈一致性](/backend/03-message-queue/outbox-pattern/)、[3.4 consumer 設計與去重](/backend/03-message-queue/consumer-design/)、[3.6 Processing Semantics 與 Recovery Semantics](/backend/03-message-queue/processing-recovery-semantics/)、[3.7 Event Contract 與 Replay Boundary](/backend/03-message-queue/event-contract-replay-boundary/)                                  | delivery、idempotency 或 replay 問題 |
| 04 Observability | [4.3 tracing 與 context link](/backend/04-observability/tracing-context/)、[4.4 dashboard 與 alert 設計](/backend/04-observability/dashboard-alert/)、[4.17 Telemetry Data Quality](/backend/04-observability/telemetry-data-quality/)、[4.22 Checkout API Evidence Package 實作示範](/backend/04-observability/checkout-api-evidence-package/)                                                      | 訊號斷鏈、盲區或 evidence 問題       |
| 操作閉環         | [0.13 操作控制 vertical slice 實作入口](/backend/00-service-selection/operations-control-vertical-slice/)                                                                                                                                                                                                                                                                                            | 從訊號進入驗證、事故與回寫流程       |

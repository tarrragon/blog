...
- [runbook](/backend/knowledge-cards/runbook/)：定義觸發條件、決策邊界與停止條件。
- [incident timeline](/backend/knowledge-cards/incident-timeline/)：建立跨團隊共用時間軸。
- [post-incident review](/backend/knowledge-cards/post-incident-review/)：保留可追蹤 action items。
- 量測指標：例如 [MTTR](/backend/knowledge-cards/mttr/)、告警到升級時間、回復耗時。

## 從案例到 workflow

案例入口在 [7.R7 事故案例庫（可引用）](/backend/07-security-data-protection/red-team/cases/)。
...
workflow 設計完成後要反向驗證案例覆蓋是否充足。引用地圖在 [案例引用地圖](/backend/07-security-data-protection/red-team/cases/case-reference-map/)。

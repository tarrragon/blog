...
| 分類                                                                  | 內容方向                                                                 |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| [Incident severity](/backend/knowledge-cards/incident-severity)       | 事故分級、影響判斷、啟動條件                                             |
| Command model                                                         | incident commander、角色分工、決策邊界                                   |
| Containment and recovery                                              | 止血、隔離、[降級](/backend/knowledge-cards/degradation)、回復、rollback |
| Incident communication                                                | 內部通報、外部溝通、狀態更新節奏                                         |
| Incident [runbook](/backend/knowledge-cards/runbook)                  | 場景化 [playbook](/backend/knowledge-cards/playbook)、查詢入口、停止條件 |
| [Post-incident review](/backend/knowledge-cards/post-incident-review) | [RCA](/backend/knowledge-cards/rca)、行動項、驗證與關閉流程              |
| [Readiness](/backend/knowledge-cards/readiness) and drills            | 值班訓練、演練設計、[game day](/backend/knowledge-cards/game-day)        |

## 選型入口

事故處理設計的核心判斷是先界定產品影響，再安排處置節奏。分級回答影響多大，指揮模型回答誰決策，止血回復回答先保護什麼結果，通訊流程回答誰需要知道什麼資訊，復盤流程回答下一次如何更快更準。

接近真實網路服務的例子包括付款成功率下降、[broker](/backend/knowledge-cards/broker) [consumer lag](/backend/knowledge-cards/consumer-lag/) 持續擴大、憑證過期導致 HTTPS 失效、資料回填錯誤造成查詢結果偏差。這些場景的共同問題是跨角色協作與時間壓力，因此需要明確分工與標準化流程。

## 與既有模組關係

1. [可觀測性平台](/backend/04-observability/) 提供事故訊號與判讀資料。
2. [部署平台與網路入口](/backend/05-deployment-platform/) 提供切換、回滾與流量控制能力。
3. [可靠性驗證流程](/backend/06-reliability/) 提供事故前演練與風險驗證。
4. [資安與資料保護](/backend/07-security-data-protection/) 提供權限、稽核與高風險操作約束。
5. [紅隊案例庫（7.R7）](/backend/07-security-data-protection/red-team/cases/) 提供可引用事故案例與 workflow 檢查點來源。
...
| 8.2  | 事故指揮與角色分工             | 定義 commander、owner、scribe、[on-call](/backend/knowledge-cards/on-call) 協作 |
...
1. [incident severity](/backend/knowledge-cards/incident-severity/)
2. [incident command system](/backend/knowledge-cards/incident-command-system/)
3. [escalation policy](/backend/knowledge-cards/escalation-policy/)
4. [incident timeline](/backend/knowledge-cards/incident-timeline/)
5. [blast radius](/backend/knowledge-cards/blast-radius/)
6. [rollback strategy](/backend/knowledge-cards/rollback-strategy/)
7. [post-incident review](/backend/knowledge-cards/post-incident-review/)
8. [RCA](/backend/knowledge-cards/rca/)
9. [RTO](/backend/knowledge-cards/rto/)
10. [RPO](/backend/knowledge-cards/rpo/)
11. [MTTR](/backend/knowledge-cards/mttr/)

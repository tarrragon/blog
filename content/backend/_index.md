...
| 路線                       | 適合讀者                              | 建議順序                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 讀完能做什麼                                                                                                                 |
| -------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| 系統心智模型               | 想理解後端服務分工                    | 00 → knowledge cards → 01 / 02 / 03 的共同觀念 → [0.18 服務邊界](/backend/00-service-selection/service-decomposition-boundaries/) / [0.19 雲端對照](/backend/00-service-selection/cloud-vendor-capability-mapping/) / [9.13 擴展軸](/backend/09-performance-capacity/scaling-axes/)                                                                                                                                                                                                                                                        | 能把需求分類成正式狀態、暫存副本、非同步交接、觀測、部署或可靠性問題、並用服務邊界與擴展軸描述系統走向                       |
...
| Vendor / Migration         | 已懂分類、要比較工具或遷移            | 對應模組主章 → [0.19 雲端對照地圖](/backend/00-service-selection/cloud-vendor-capability-mapping/) → `vendors/` → migration playbook（含 [0.20 拆分執行 runbook](/backend/00-service-selection/service-decomposition-execution-runbook/)）→ cases（含 [1.14 slow log 閉環](/backend/01-database/production-slow-log-loop/) / [9.14 連線池放大](/backend/09-performance-capacity/connection-pool-amplification/)）                                                                                                                          | 能先判斷分類責任，再對照雲端能力、比較具體服務、操作成本與遷移風險                                                           |

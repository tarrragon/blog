...
逐篇檢查的**產出有三種**，不是每篇都要補內容。判定適用時先問這篇的讀者站在哪個時刻：讀者若是「已經有系統在跑、正在考慮要不要動」，內容給訊號就夠了，補形態反而冗餘；設計階段的讀者才需要形態。判定不需補形態時，接著查卡連結覆蓋——缺形態與缺術語入口是同一個問題的兩種形式，讀者都得自己補一塊才能用。同分類內的卡連結數落差是比絕對數量更可靠的偵測訊號。

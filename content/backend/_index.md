...
這項工作**逐篇獨立、可隨時中斷**，不需要規劃批次。目前 204 篇主章中，形態代理掃描有 140 篇兩項指標都未命中，但抽樣顯示其中有偽陰性（用其他措辭給了情境的也會落在這裡），實際缺口小於這個數字，逐篇判定時才算數。已完成的篇章與判定結果（形態｜微案例分開記，避免把三種補法壓回兩種）：7.28 密碼學原語選型（形態已補、微案例待補，「重放窗口未收斂」是全批後果最不直觀的節點）、7.29 API 認證信任邊界（形態已補、微案例挑了後果最不直觀的兩類：使用者憑證代理系統層、跨系統對應順序未定義）、11.6 向後相容的變更紀律（形態已補、微案例未判定）；10-system-evolution 整個分類（3 篇）跑完後判定兩篇不需補形態（讀者已有系統在跑、訊號是對的形式）、一篇不適用（程序型 runbook），真缺口出現在卡連結覆蓋上並已補齊。

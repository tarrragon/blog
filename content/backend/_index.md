...
| 服務路徑 | 用同一條業務流程演練資料、快取、事件、觀測、部署、驗證、資安與事故交接 | 各模組主章（01-09）+ 本頁下方「貫穿式案例：Checkout 服務演進」                                                                              |
| 真實服務 | 把分類語言落到 vendor / platform / tool 的能力、成本與遷移             | 各模組 `vendors/` 子目錄、案例庫見 [07 紅隊案例](/backend/07-security-data-protection/) 與 [09 效能案例](/backend/09-performance-capacity/) |
...
| 路線                       | 適合讀者                                            | 建議順序                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 讀完能做什麼                                                                                                                 |
| -------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 沒部署過服務               | 會寫 code、但從沒把服務上線                         | [服務上線的業界常識地基](/going-live/)（部署是什麼 → 主機形態 → 域名 HTTPS → 自架 vs 託管 → 備份）→ 回本模組做選型                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 能把一個服務從本機推上線、並知道接著補哪塊地基                                                                               |
| 系統心智模型               | 想理解後端服務分工                                  | [0.21 交付形態](/backend/00-service-selection/delivery-mode-selection/) → 00 → knowledge cards → 01 / 02 / 03 的共同觀念 → [10.1 服務邊界](/backend/10-system-evolution/service-decomposition-boundaries/) / [0.19 雲端對照](/backend/00-service-selection/cloud-vendor-capability-mapping/) / [9.13 擴展軸](/backend/09-performance-capacity/scaling-axes/)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 能把需求分類成正式狀態、暫存副本、非同步交接、觀測、部署或可靠性問題、並用服務邊界與擴展軸描述系統走向                       |
| API 到資料流               | 想設計 API 的對外契約與其背後的資料、快取與事件流程 | [11 API 設計](/backend/11-api-design/) → 01 Database → [1.13 query 反模式](/backend/01-database/query-anti-patterns/) → 02 Cache → 03 Queue → 04 Evidence → [1.14 slow log 閉環](/backend/01-database/production-slow-log-loop/)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 能說明一次 checkout 如何跨 DB、cache、queue 與 evidence package、並在進 production 前用 query 預算與 slow log 攔下查詢反模式 |
| Production 操作            | 想學上線、觀測、驗證與事故閉環                      | 04 Observability → 05 Deployment（含 [5.9 邊緣分發](/backend/05-deployment-platform/edge-cdn-static-distribution/)）→ 06 Reliability → 08 Incident                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 能把 release、alert、gate、邊緣層 origin protection、incident decision log 與 write-back 串成操作閉環                        |
...
| Vendor / Migration         | 已懂分類、要比較工具或遷移                          | 對應模組主章 → [0.19 雲端對照地圖](/backend/00-service-selection/cloud-vendor-capability-mapping/) → `vendors/` → migration playbook（含 [10.2 拆分執行 runbook](/backend/10-system-evolution/service-decomposition-execution-runbook/) / [10.3 託管遷出](/backend/10-system-evolution/managed-platform-exit/)）→ cases（含 [1.14 slow log 閉環](/backend/01-database/production-slow-log-loop/) / [9.14 連線池放大](/backend/09-performance-capacity/connection-pool-amplification/)）                                                                                                                                                                                                                                                                                                                                                                                                                              | 能先判斷分類責任，再對照雲端能力、比較具體服務、操作成本與遷移風險                                                           |
| 規模成長                   | 已能做出「能跑」的 SaaS、要學「能撐」               | [0.21 升級 tripwire](/backend/00-service-selection/delivery-mode-selection/) → [0.0 後端需求分類地圖](/backend/00-service-selection/backend-demand-taxonomy/) → [10.3 託管遷出](/backend/10-system-evolution/managed-platform-exit/)（託管出身者適用）→ [10.1 服務拆分與邊界判讀](/backend/10-system-evolution/service-decomposition-boundaries/) → [9.13 擴展軸與 Stateless 前提](/backend/09-performance-capacity/scaling-axes/) → [1.13 應用層查詢反模式與 Query 預算](/backend/01-database/query-anti-patterns/) → [1.1 高併發下的 SQL 讀寫邊界](/backend/01-database/high-concurrency-access/) → [2.2 cache aside 與失效策略](/backend/02-cache-redis/cache-aside/) → [5.9 邊緣分發與靜態資源（CDN / Origin Protection）](/backend/05-deployment-platform/edge-cdn-static-distribution/) → [03 Queue](/backend/03-message-queue/) → [9.11 高峰事件準備](/backend/09-performance-capacity/peak-event-readiness/) | 能描述一個服務從一臺機器演進到多區域多服務的時序、各階段該撞哪面牆、要先補哪一塊能力                                         |
...
這條路線跟「API 到資料流」路線的差別：API 路線教讀者「怎麼把功能做出來」，規模路線教讀者「做出來之後怎麼撐住」。兩條路線可以分開讀、也可以順序讀，建議寫完第一個 MVP 後就回到規模路線盤點。MVP 是用託管平台或 BaaS 做的話、先用 [0.21 交付形態選型](/backend/00-service-selection/delivery-mode-selection/) 的升級自建 tripwire 判斷該不該啟動自建評估；評估成立、遷出執行見 [10.3 託管形態遷出](/backend/10-system-evolution/managed-platform-exit/)、遷入後再走規模成長階段。
...
| 微服務 / 拆分丁丁茶業      | [10.1 服務拆分與邊界判讀](/backend/10-system-evolution/service-decomposition-boundaries/)                                                                              |
...
| F    | 業務分化、團隊發版互相阻擋                  | 沿真實邊界拆服務（資料 / 團隊 / 部署 / 流量四選一） | [10.1 服務拆分](/backend/10-system-evolution/service-decomposition-boundaries/)                                          |
...
### [模組十：系統演進與遷移](/backend/10-system-evolution/)

整理服務拆分、跨服務重構、大型遷移與雲端切換的執行紀律。跟模組零是 sibling 設計／執行紀律：00 看「該選哪個服務」、10 看「決定要動之後、怎麼安全動手」。當前收三章：服務拆分的 [10.1 邊界判讀](/backend/10-system-evolution/service-decomposition-boundaries/) 與 [10.2 執行 Runbook](/backend/10-system-evolution/service-decomposition-execution-runbook/)、加上 [10.3 託管形態遷出](/backend/10-system-evolution/managed-platform-exit/)；未來擴充跨服務 schema 演進、大型雲端遷移、基礎設施替換與容量重平衡。
...
| [07 資安與資料保護](/backend/07-security-data-protection/) | 18   | vendor deep article 早期階段、既有整合的盤點形態與發行方自助發放無落點      |
...
| 橫向：簡報式段落拆卡試點（見下段）                         | 3    | 兩輪已完成（07 的三章）；異源冷讀與第二人分診比對未做、全分類盤點的範圍待決 |
...
各模組的判讀類章節普遍只給機制屬性（描述某個機制具備什麼特性），沒給「什麼樣的系統會遇到」「什麼事件會逼出這個動作」與「出事會長什麼樣」。缺這些時讀者得自己補上情境才能使用內容，而補這一塊要的正是這個判斷的輸入之一——內容的可用程度因此隨既有經驗遞減，與 AGENTS.md 原則六（預設讀者工程經驗有限、用現實服務案例補足經驗差距）不符。

作法與四組正反例見 [判讀內容要給情境與後果](/skills/compositional-writing/judgment-content-needs-scenarios/)。

逐篇檢查的**產出不只補內容一種**。判定適用時先問這篇的讀者站在哪個時刻：讀者若是「已經有系統在跑、正在考慮要不要動」，內容給訊號就夠了，補形態反而冗餘；設計階段的讀者才需要形態。判定不需補形態時，接著查卡連結覆蓋——缺形態與缺術語入口是同一個問題的兩種形式，讀者都得自己補一塊才能用。同分類內的卡連結數落差是比絕對數量更可靠的偵測訊號。第四種產出來自逐節讀的副產物：撞到不屬於這篇的內容時標路由目的地並登記待辦，不當場搬——搬遷要驗證來源與目的地兩端，尺度大於補情境。第五種與第四種方向相反：補形態要回答「讀者對號入座之後去哪」，而判讀表不問這個問題，於是缺解法落點的那幾列會在補形態時現形。辨識訊號是同一張表的列與深化段不對稱（實測遇過五列的表只有四列有深化段），而表格欄位齊全正是遮住它的原因。

這項工作**逐篇獨立、可隨時中斷**，不需要規劃批次。目前 204 篇主章中，形態代理掃描有 140 篇兩項指標都未命中，但抽樣顯示其中有偽陰性（用其他措辭給了情境的也會落在這裡），實際缺口小於這個數字，逐篇判定時才算數。已完成的篇章與判定結果（形態｜微案例分開記，避免把三種補法壓回兩種）：7.28 密碼學原語選型（形態已補、微案例已補「重放窗口未收斂」——第三拍的來源是文章自己已寫出的「這一格在功能測試裡表現正常」，機制上必然；觸發事件判定不適用，本篇是設計階段內容、讀者不是在事故當下讀它）、7.29 API 認證信任邊界（形態已補、微案例挑了後果最不直觀的兩類：使用者憑證代理系統層、跨系統對應順序未定義）、11.6 向後相容的變更紀律（形態已補、微案例未判定）；7.3 入口治理（四個節點的形態軸刻意各異：系統演進 / 架構長相 / 信任模型錯位 / 組織分工；微案例挑「修補與驗證節奏分離」，第三拍機制上必然——「修補完成」有明確訊號而「異常是否消失」沒有；觸發事件判定不需補，本篇的判讀流程已承接）；7.4 資料保護（四節點零深化段，形態與出口全缺；微案例挑「備份資產權限混層」，後果最不直觀且第三拍必然——備份存取本來就有大量讀取、異常混在裡面，且備份多半不在應用層監控範圍）；7.5 傳輸信任（兩個深化段都是機制而非形態；微案例挑「憑證輪替覆蓋不足」，第三拍是自動化讓監控只盯得到自動化管的那批）；7.6 秘密管理與機器憑證治理（形態已補四個節點，本篇無「不小心寫對」的參照樣本、取 7.29 / 7.2 的顆粒度對齊；微案例挑「CI secrets 集中」，它的後果最反直覺——集中化的動機是降低日常管理成本，而它推高的是事件當天的協調成本，第三拍來源是案例庫已記的「客戶端難以快速判斷最小必要輪替範圍」）；7.2 身分與授權邊界（形態已補四個多人 SaaS 節點——第五個節點本來就有完整形態段，正是「同篇有一兩節不小心寫對了」的形態；微案例挑「會話失效節奏落後」，它的後果最不直觀且第三拍機制上必然：撤銷分散在多個發行方、各自宣告完成而無人交叉核對）；10-system-evolution 整個分類（3 篇）跑完後判定兩篇不需補形態（讀者已有系統在跑、訊號是對的形式）、一篇不適用（程序型 runbook），真缺口出現在卡連結覆蓋上並已補齊。

微案例的覆蓋率不追求填滿。每一拍要有來源（親歷、案例庫已記的形態、或機制上必然），第三拍尤其推不出來，寫不出真實來源時留白比杜撰好——因此各篇的微案例會停在「有素材的幾類」，其餘登記缺口等案例庫累積。
...
新增教學模組時要在本表補一列，新增知識卡時要在該分類的卡片目錄補一列，這與新增頂層 `content/<module>/` 要更新 [content/\_index.md](/) 是同一類的登記責任。卡片那一條由 `mdtools cards` 的 L6 檢查守著（規則說明見 [L2 與 L6 問的是兩個不同問題](/posts/markdown-writing-spec/#l2-與-l6-問的是兩個不同問題)）——本表那一條目前仍靠人記得。

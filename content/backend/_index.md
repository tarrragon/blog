...
各模組的判讀類章節普遍只給機制屬性（描述某個機制具備什麼特性），沒給「什麼樣的系統會遇到」「什麼事件會逼出這個動作」與「出事會長什麼樣」。缺這兩者時，讀者要能使用內容就得自己補上情境，而能補的人本來就會判斷了——實際受眾因此收斂成已經有經驗的人，與 AGENTS.md 原則六（預設讀者工程經驗有限、用現實服務案例補足經驗差距）不符。

作法與四組正反例見 `compositional-writing` skill 的 `references/judgment-content-needs-scenarios.md`：判定適用 → 掃出同篇已寫對的段落當參照 → 逐節列出機制陳述 → 補形態、觸發事件或微案例 → 決定獨立成節或行內補 → 掃第二人稱與封閉計數兩個副作用。
...
這項工作**逐篇獨立、可隨時中斷**，不需要規劃批次。目前 204 篇主章中，形態代理掃描有 140 篇兩項指標都未命中，但抽樣顯示其中有偽陰性（用其他措辭給了情境的也會落在這裡），實際缺口小於這個數字，逐篇判定時才算數。已完成的篇章與判定結果（形態｜微案例分開記，避免把三種補法壓回兩種）：7.28 密碼學原語選型（形態 ✓、微案例待補——「重放窗口未收斂」是全批後果最不直觀的節點）、7.29 API 認證信任邊界（形態 ✓、微案例 2/4 類）、11.6 向後相容的變更紀律（形態 ✓、微案例未判定）；10-system-evolution 整個分類（3 篇）跑完後判定兩篇不需補形態（讀者已有系統在跑、訊號是對的形式）、一篇不適用（程序型 runbook），真缺口出現在卡連結覆蓋上並已補齊。

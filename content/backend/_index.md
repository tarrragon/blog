...
| [07 資安與資料保護](/backend/07-security-data-protection/) | 19   | vendor deep article 早期階段、既有整合的盤點形態與發行方自助發放無落點 |
...
| 橫向：知識卡未列入卡片目錄（見下段）                       | 1    | 78 / 422 張只能靠章節連結或搜尋抵達，瀏覽卡片目錄的讀者看不到          |
...
[前置知識卡片](/backend/knowledge-cards/) 目錄頁列的卡片少於實際存在的卡片，目前的差是 78 張（422 張裡）。這 78 張從章節內文連得到、也搜尋得到，但瀏覽卡片目錄的讀者看不到它們——目錄頁沒有宣告自己是選錄，所以讀者會把它當完整清單讀。

缺口的成因是登記動作沒有觸發器。寫章節時建一張新卡的動機明確（該篇要連它），而回卡片目錄補一列不影響那篇的完成度，於是每個批次都留下幾張。未登記卡片的建立時間橫跨五月到七月、weight 分布在每個主題段，證實它是逐批累積而非某一次遺漏。

`mdtools cards` 攔不到這一類，因為 L2 問的是另一個問題：「這張卡有沒有被非卡片內容連到」，而卡片目錄本身也在 cards root 之下、它的連結不算 inbound edge。要讓缺口持續可見，補的是一條新規則（卡片檔存在而該 cards root 的 `_index.md` 沒有連它），偵測完全機械、無語意判定，適合放 `mdcards` 的警告層。

新增教學模組時要在本表補一列，新增知識卡時要在卡片目錄補一列，這與新增頂層 `content/<module>/` 要更新 [content/\_index.md](/) 是同一類的登記責任。

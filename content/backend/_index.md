...
作法與四組正反例見 `compositional-writing` skill 的 `references/judgment-content-needs-scenarios.md`：判定適用 → 掃出同篇已寫對的段落當參照 → 逐節列出機制陳述 → 補形態或觸發事件 → 決定獨立成節或行內補 → 掃第二人稱與封閉計數兩個副作用。

逐篇檢查的**產出有四種**，不是每篇都要補內容。判定適用時先問這篇的讀者站在哪個時刻：讀者若是「已經有系統在跑、正在考慮要不要動」，內容給訊號就夠了，補形態反而冗餘；設計階段的讀者才需要形態。判定不需補形態時，接著查卡連結覆蓋——缺形態與缺術語入口是同一個問題的兩種形式，讀者都得自己補一塊才能用。同分類內的卡連結數落差是比絕對數量更可靠的偵測訊號。第四種產出來自逐節讀的副產物：撞到不屬於這篇的內容時標路由目的地並登記待辦，不當場搬——搬遷要驗證來源與目的地兩端，尺度大於補情境。

這項工作**逐篇獨立、可隨時中斷**，不需要規劃批次。目前 204 篇主章中，形態代理掃描有 140 篇兩項指標都未命中，但抽樣顯示其中有偽陰性（用其他措辭給了情境的也會落在這裡），實際缺口小於這個數字，逐篇判定時才算數。已完成的篇章與判定結果：7.28 密碼學原語選型、7.29 API 認證信任邊界、11.6 向後相容的變更紀律三篇補了形態；10-system-evolution 整個分類（3 篇）跑完後判定兩篇不需補形態（讀者已有系統在跑、訊號是對的形式）、一篇不適用（程序型 runbook），真缺口出現在卡連結覆蓋上並已補齊。

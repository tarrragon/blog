---
title: "環境分離與模組化 — 目錄結構、module 參數化與 retrofit 路徑"
date: 2026-06-26
description: "用目錄結構在第一天就隔開 dev 與 prod 的 state，用 module 讓環境共用同一套邏輯只差參數，以及已經單環境跑起來後怎麼安全拆分"
weight: 1
tags: ["infra", "iac", "environment", "module"]
---

[環境分離](/infra/knowledge-cards/environment-separation/)的核心責任是讓 dev 的實驗、staging 的驗證、prod 的真實流量彼此不可見也不可達。從目錄結構就定好環境邊界的專案，dev 跟 prod 是兩棵獨立的 state 樹、改錯一邊不會波及另一邊；等資源都長出來、流量都上線了才回頭切的專案，每一次 retrofit 都在帶電作業，動到的是正在服務客戶的網路與身分。同樣一套 module、同樣的工程師，差別只在「環境邊界是設計出來的、還是事後補的」，而這個差別在第一天幾乎零成本、在第一百天可能是一個季度的遷移專案。
...
光譜越靠粗的一端，隔離越強、跨環境共用越少、初始與維運成本越高；越靠細的一端，重複越少、邊界越隱性。多數早期團隊在目錄分離這一格落腳，因為它在顯式邊界與維運成本之間平衡得宜。當隔離需求升高（例如 prod 要法規等級的帳務與權限隔離），再沿光譜往帳號級或獨立 repo 移。帳號級隔離的權限邊界設計見[模組二：身分與憑證地基](/infra/02-identity-credentials/)。
...
```hcl
# modules/database/main.tf — module 用參數建資源，不含環境判斷
resource "aws_db_instance" "primary" {
  identifier              = "${var.service_name}-${var.env}"
  engine                  = "postgres"
...
module "database" {
  ...
  subnet_group_name     = module.network.private_subnet_group
  security_group_ids    = [module.network.db_security_group_id]
}
```

```hcl
# environments/dev/main.tf — dev 傳縮小版的值
module "database" {
  source                = "../../modules/database"
  service_name          = "payments"
  env                   = "dev"
...
這樣 dev 與 prod 跑的是位元層級相同的 module code，差異全部收斂在 `main.tf` 的呼叫參數裡、一眼可審。Review 時只要 diff 各環境的參數區塊就能看完所有環境差異。如果發現有人為了某環境的特例去改 module 內部，那是漂移正在發生的徵兆——該把特例改寫成新的參數，而非在 module 裡加 `if env == "prod"` 分支。核心服務怎麼用 module 跨環境重用，見[模組五：核心服務上 IaC](/infra/05-core-services/)。
...
仍然走參數化、不分叉 code — 分叉 code 是漂移的起點。跨可用區與冗餘的網路面怎麼鋪，見[模組三：網路地基](/infra/03-network-foundation/)。
...
```bash
# retrofit 前備份 state
aws s3 cp s3://acme-tf-state/prod/terraform.tfstate \
  ./state-backup-$(date +%Y%m%d).tfstate
```

第三，`moved` 區塊優先用宣告式（可 review、可回滾），手動 `state mv` 留給 `moved` 表達不了的跨 module 搬遷。整個 retrofit 走 PR 流程、讓 plan 輸出在 review 時可見，見[模組七：infra 走 PR 流程](/infra/07-infra-as-pr/)。
...
- → [模組二：身分與憑證地基](/infra/02-identity-credentials/)：帳號級隔離的權限邊界
- → [模組三：網路地基](/infra/03-network-foundation/)：跨可用區冗餘的網路面
- → [模組五：核心服務上 IaC](/infra/05-core-services/)：核心服務怎麼用 module 跨環境重用
- → [模組七：infra 走 PR 流程](/infra/07-infra-as-pr/)：retrofit 的 plan 輸出怎麼進 review
- → [跨帳號策略](/infra/02-identity-credentials/multi-account-strategy/)：帳號級隔離是環境分離光譜最粗的一端

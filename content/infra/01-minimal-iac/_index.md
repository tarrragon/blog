...
讓鐵律落地靠的是權限、不是自律。光靠約定「別在 Console 改」維持不了多久，救火當下手最快的永遠是 Console。真正讓鐵律有效的，是把人的日常身分收斂成唯讀、把寫入權限留給跑 apply 的自動化身分，讓「在 Console 改不動」變成預設狀態 — 這道權限地基屬於模組二：身分與憑證地基的範圍，本階先確立紀律方向。

...
date: 2026-06-26
description: "共享主機沒辦法裝 agent、沒辦法串 log pipeline，用外部 HTTP check、錯誤追蹤服務與效能基線建立最低成本的監控能力"
weight: 13
tags: ["infra", "takeover", "monitoring", "uptime", "php"]
---

共享主機通常不允許安裝監控 agent（Datadog agent、New Relic APM daemon 都需要 daemon 常駐或 root 權限），伺服器的內部指標（CPU、記憶體、磁碟）只能從主機商的控制面板看到靜態數值，沒有告警機制。這種環境的監控策略是從外部觀測——用 HTTP check 確認服務存活、用不需要 agent 的錯誤追蹤服務捕捉例外、用定期量測建立效能基線。每一層都不依賴 server 端安裝任何東西。
...
UptimeRobot 的免費方案對多數共享主機站台足夠——50 個 monitor 可以覆蓋一個站台的主要入口。
...
PHP 的錯誤追蹤在共享主機上有兩條路徑：server 端用 PHP 內建的 error_log、client 端用不需要安裝的 SaaS 服務。
...
這段程式碼會在 PHP 拋出未捕捉的例外或觸發 error 時，把錯誤資訊（stack trace、request context、使用者資訊）透過 HTTP 送到 Sentry 的 SaaS 平台。免費方案每月 5,000 個事件，對流量不大的共享主機站台通常足夠。
...
共享主機通常按流量或磁碟空間計費，異常流量（bot 掃描、DDoS、爬蟲）會讓帳單飆高或觸發主機商的流量限制。
...
設定只需要把 domain 的 nameserver 改成 Cloudflare 提供的 NS、原始 DNS record 在 Cloudflare 重建。對共享主機站台來說這是投資報酬率最高的單一改善動作——不動 server、不改 code、但同時拿到流量可見性和基本防護。
...
Tier 1 在接手當天就能建好（30 分鐘設定 UptimeRobot + Sentry + GA），零成本提供基本的「服務掛了會知道、程式碼出錯會收到、流量異常看得到」的覆蓋。Tier 2 適合站台有營收或合約 SLA 要求時。Tier 3 是離開共享主機後的正規化路徑，監控從外部觀測升級為 server 端全面可觀測性，見[模組六：可觀測性與 log](/infra/06-observability-logging/)。

## 跨分類引用

- → [共享主機與 FTP 環境的接管](/infra/takeover/legacy-ftp-shared-hosting/)：本篇的母篇，監控建立在盤點與本地環境之後

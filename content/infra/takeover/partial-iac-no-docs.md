...
# state 裡有什麼（結構化摘要：type + name + provider）
terraform show -json | jq '.values.root_module.resources[] | {type, name, provider}' > managed-summary.json
```
...
```bash
# 用 jq 從 state JSON 撈敏感值候選
terraform show -json | jq -r '
  [.. | objects | to_entries[] |
   select(.key | test("password|secret|key|token"; "i"))] |
  unique_by(.key) | .[] | "\(.key): \(.value)"
' 2>/dev/null
```
...
```bash
# 手動備份 state（不論 bucket 有沒有 versioning 都先拉一份）

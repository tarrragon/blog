...
技術正確的論述說服不了商業決策者。「我們需要 Infrastructure as Code 來確保環境可重現」這句話在工程會議裡有重量，在預算會議裡沒有 — 決策者聽到的是一個他不懂的技術詞、一個他看不到的好處、以及一筆他得批的時間預算。infra 推不動，多數時候不是因為它不重要，而是因為提案的語言跟決策者的語言對不上。
...
這個反對意見的翻譯是「我看不到壞掉的風險」。回應的方式不是講技術風險，而是問一個具體問題：「如果我們的主要服務現在掛了，我們能在多久內重建起來？」如果答案超過一小時、或者答案是「不確定」，這本身就是論證 — 決策者通常能理解「不知道能不能救回來」的商業代價。

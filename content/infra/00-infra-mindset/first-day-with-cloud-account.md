...
如果有正在跑的 EC2 instance、有 Lambda function、有 RDS 資料庫、有 S3 bucket 存著資料——這是一個已經有資源在跑的帳號，可能是前人建的、可能是其他團隊在用的。
...
如果帳號裡有既有資源但不確定是不是還在用、能不能動，先盤點。以下指令需要 AWS CLI 並完成認證——安裝和 `aws configure` 設定見模組一的[前提段](/infra/01-minimal-iac/iac-tool-state-backend/)（macOS 快速安裝：`brew install awscli && aws configure`）：
...
這些指令只做讀取，不會改動任何資源。如果輸出很多資源，去讀[接手維運](/infra/takeover/)再決定下一步。如果幾乎是空的，走「從零建置」路線。
...
| 帳號有既有資源、要接手維運         | 接手前人專案 | [接手維運](/infra/takeover/)                                                               |
...
| 拿到一台主機、要從 OS 層連入初始化 | 機器初始化   | [Linux 安裝與機器初始化](/linux/install/)                                                  |

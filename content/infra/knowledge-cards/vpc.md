...
VPC 是[模組三：網路地基](/infra/03-network-foundation/vpc-subnet-security-group/)的最外層邊界。Infra 系列的網路設計從 VPC 開始：先圈定地址空間，再往內切 subnet、掛 route table、設 security group。環境之間的 VPC 怎麼分（每個環境一個 VPC），屬於[模組四：環境分離](/infra/04-environment-separation/directory-module-parameterization/)的設計決策。

...
### Step 4：建立 `_index.md`（section 索引）
...
### Step 5：補 Hugo frontmatter 到 SKILL.md 與每份 reference
...
lint 通常會在 Step 5 漏掉某些檔案時報 `front-matter-required` 或 `MD025`；hugo 則會在連結還指向 `references/` 時報 "REF_NOT_FOUND"。兩個訊號都指向明確的修補位置。

### Step 7：更新本 `content/skills/_index.md` 的「目前收錄」表

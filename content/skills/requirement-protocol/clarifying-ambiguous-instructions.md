...
date: 2026-04-26
description: "requirement-protocol reference：空間 / 相對位置 / 隔離 / 決定權四類模糊指令的澄清模板 + visible 三問判準。"
tags: ["skills", "requirement-protocol", "對話協議", "需求澄清"]
---

收到含模糊形容詞或缺數字的指令時 — 把指令翻譯成可驗證的具體輸入、給選項讓使用者點頭、再開始實作。

適用：空間 / 尺寸（「padding 加大」「對齊」）、相對位置（「在 X 旁邊」「靠近 Y」）、隔離（「不要動 X」「跟 Y 分開」）、決定權（「breakpoint 設多少」）。
不適用：內部技術選擇（grid / flex、observer 種類）— 那些自決即可、見「可決定 vs 該確認」段落。

> **自包含聲明**：閱讀本文件不需要先讀其他 reference。本文件涵蓋四類模糊指令的澄清模板與「自決 vs 確認」的判準。

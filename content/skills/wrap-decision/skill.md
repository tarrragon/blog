...
date: 2026-05-04
description: "WRAP 決策框架的 SKILL 入口：錨點確認、資料充足度閘門、擴增選項、實境檢驗、機會成本、行前預想與絆腳索。"
...
> 本 SKILL 為通用 WRAP 規則，獨立於任何專案框架。專案若需要 Hook、CLI 或任務系統整合，請在該專案內另建落地層文件。
...
| **根因深度反思**                            | 用戶質疑「分析太表層」「不夠深」、或需要把失敗歸因寫成可重用規則            | **完整 WRAP 強制**：先列 5+ 候選假設 + Reality Test + 2 層深因，再用 W/A/P 檢驗 |
...
| **CLI / 規則 autopilot**（決策路徑層 3.1）  | CLI 撞錯後立即重試或猜變體（非查 `--help` / 規則文件）                      | 快速（強制查文件後再試）                                                        |
| **Anchor — 既有結論優先**（決策路徑層 3.2） | WRAP W 階段選項能一句話概括 / 全指向同一根因                                | 完整（強制 Consider the Opposite + 重新定義問題）                               |
| **規則失敗草率改規則**（決策路徑層 3.3）    | 失敗第一反應「改規則」，未先重試 2 次                                       | 快速（先挖根因再決定改規則或改行為）                                            |
| **多步驟 perplexity 盲**（決策路徑層 3.4）  | 多步驟計畫中所有中間步驟都預測成功                                          | 快速（R 階段基本率 + 每步獨立驗證）                                             |

> 上述 4 項決策路徑層因子可由各專案映射到自己的 Hook、CLI 或任務系統；本 skill 只保留通用判斷語意。
...
| 執行 Reality Test 時發現「數據來源是群體統計」       | 這屬於 Step 0 應提前檢出的問題                 |
...
| **方案性質三類涵蓋**  | 候選方案是否涵蓋 **新增工具 / 改造既有 / 零工具或純文件** 三類？ | 全屬「新增工具」→ LLM toolification 偏見，強制補「改造既有」與「零工具」選項 |
| **方案關聯性檢查**    | 任兩方案是否可退化為同一方案或既有機制？                         | 可退化 → 實質選項數量要重算（假多樣性）                                      |
| Consider the Opposite | 如果我相信的正好相反，會怎樣？真正該問的問題是什麼？             | 能提出有力相反論述 → 重新定義問題後從 W 重跑                                 |

> Consider the Opposite 是最容易被跳過但最重要的檢查 — 也是防止「在錯誤問題框架下擴增選項」的最後防線。詳細操作與範例見 [detailed-techniques](/skills/wrap-decision/detailed-techniques/)。

> **方案性質三類涵蓋**與**方案關聯性檢查**用來抵抗 LLM 的 toolification 偏見：訓練資料常把「新增工具」寫成答案，卻忽略「改造既有機制」與「零工具」選項。即使補上多個方案，若方案間強關聯也可能退化為 1 個實質選項。
...
> 分析類任務（根因調查、設計決策）建議完整執行三層質疑、Reality Test 閘門與警告信號檢查。
...
| **Tool selection check** | 物化 / 步驟數 / 目的地 / 白名單 4 問（見下）                            | 任一問暴露繞路 → 回 W 換 1 步工具            |
...
> 完整 4 問操作細節見 [detailed-techniques](/skills/wrap-decision/detailed-techniques/) 「Tool selection check 詳細技巧」章節。
...
詳見 [anti-paternalism](/skills/wrap-decision/anti-paternalism/)（含規則設計中的 paternalism 悖論案例）。
...
| [anti-paternalism](/skills/wrap-decision/anti-paternalism/)       | 悖論識別檢查清單 + 自我暴露偏好實踐（4 條件 benevolent paternalism 測試、自我參照悖論識別、Recommended 標記是 dark pattern） |

---

**Last Updated**: 2026-04-28
**Version**: 2.3.0 — 觸發條件新增 4 項決策路徑層干擾（CLI autopilot / Anchor / 草率改規則 / 多步驟 perplexity 盲）；既有觸發條件不變動（向後相容）。

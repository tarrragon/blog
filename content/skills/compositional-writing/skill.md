...
**論斷句、總結句、段標與過渡句同樣要填槽位，而它們的缺口多半在名詞裡。** 這一節原先只寫操作句，而同一種省略在不描述操作的句子裡更常見，也更難看出來：「錯的來源不只一種，而輸出都不帶訊號」主詞與謂語都在、語法完整，四個槽位查下來全部通過，缺的是名詞自己要求的補足語——來源是什麼的來源、輸出是誰的輸出、訊號是什麼的訊號；「錯」是形容詞被當成名詞用，連什麼錯了都沒說。所以槽位多一格：**本身要求「誰的、什麼的、哪兩者之間的」的名詞**（來源、輸出、結果、原因、代價、差別、位置、範圍、邊界、訊號、層、線、情形）要把它的所屬寫在這一句或緊接的前一句裡；形容詞與動詞不直接當名詞用（「看不出錯」寫成「看不出答案錯了」）。判斷問句是**這個名詞是誰的、什麼的，能不能從這一句或前一句逐字指出來**，判定者同樣不代填。改寫：「錯的來源不只一種，而輸出都不帶訊號」→「答案錯掉的成因不只一種，而引擎一種都不會報錯」。

這一類集中在作者做抽象與收束的位置——段標、段落的第一句與最後一句、總結段、過渡句——寫到這些位置時逐句問一次。**只寫在目錄頁、讀者看不到的框架詞也是同一個成因**：「結果集由什麼決定，到這一篇有了完整的答案」的問題本身只存在於作者的編排底稿裡。審查端的偵測與它的量測在 [noun-without-its-owner-passes-the-slot-check](/report/noun-without-its-owner-passes-the-slot-check/)。

段落層的順序跟著槽位走：讀者會看到什麼 → 這個工具做什麼 → 為什麼需要它（機制，系統元件指名）→ 不做的具體後果 → 副作用。動作者槽位填對之後段落順序通常自動修正——操作者視角的順序就是讀者的問題序。詳見 [command-fragment-as-subject-hides-the-actor](/report/command-fragment-as-subject-hides-the-actor/) 與 [spoken-emphasis-is-silent-in-text](/report/spoken-emphasis-is-silent-in-text/)。
...
**引用錨點用語意標題、不用位置編號**：引用另一個章節 / 階段 / 條列項時寫「見核心問題」、不寫「見 Stage 3」— 編號是結構排列的 derivation、結構重排時引用句字面完好、語意 silent 指向錯的內容（比 broken link 難偵測：連結斷掉會報錯、編號錯位會成功解析到錯的東西）。對應要求是每個結構單位的標題要承載核心意義（「Stage 3：核心問題」、編號只作排序前綴）、引用取語意半邊；發布方凍結的編號（RFC 段號 / 法條）是 fact、可引用。**相對位置（末段 / 下段 / 上節 / 下方）是同構的另一種 derivation 而判斷標準寬一階**——同檔緊鄰的合法且常比具名好讀，失效條件有兩個、各自獨立：跨檔即失效（讀者手上的「上節」是他正在讀的那一份的）、目標可能移動即失效（不報錯，成功解析到現在住在那個位置的東西）。由第二個推出**位置指涉的主要來源是修法自己**（寫下的當刻位置一定是對的），所以掃描的觸發是改動、不是完稿；定址型產物（地址表 / 索引 / 路由表）一律具名、不看距離。詳見 [reference-by-semantic-title-not-number](/report/reference-by-semantic-title-not-number/)。
...
**讀者定位：缺經驗的專業人士、不是外行人**：技術教材的讀者是在特定領域缺乏經驗的專業人士，不是完全不懂的外行人。寫法是補足經驗缺口（直接描述情境與操作需求），不是從零科普（故事線導入、比喻堆疊、宣導語氣）。宣導式語氣（「你可能沒注意到」「把 X 想成 Y」「跑得好好的」）預設讀者無能、降低教材可信度。**這條定位是能力宣稱、不是知識宣稱**——它說讀者有能力理解（研究方法、開發原則、邏輯推導），不說他已經知道這個領域的名詞、原理與縮寫；兩種讀法對同一句話都通順，而只有後一種會授權省略。方向由需求決定：專業讀者要的是每個流程的原因、成本、代價與用詞精準，全部是更多的文字，所以這條原則推向展開。判別問句是**這一處省略，省下的是讀者不需要的字，還是讀者要自己補回來的脈絡**；省略在產物上沒有痕跡（沒補的卡不存在、沒展開的原理不佔行數），所以偵測靠術語分級表與理解取樣、不靠讀稿。詳見 [audience-is-professional-not-layperson](/report/audience-is-professional-not-layperson/)。

**不寫會取代讀者查證的東西（上一條的反向配對）**：特定實例的值與處理方式（某台機器、某個環境、某次安裝）寫進文字之後，就佔據了讀者原本要自己查的那個位置——他要的是「我這一台是什麼」而文字給了「有一台是什麼」，而**特定實例的值正好是他那一台會不一樣的那些**。**危害與正確性無關**：值可以完全正確、來源完全可靠，而它從來就不是關於他的系統的。判別問句是**讀者拿這一句去用，他是去查自己的系統，還是直接照抄**；修法是換成該去哪裡查，或在機制需要例子才講得清時用明顯的佔位值並寫明它是佔位的。不受約束的三類是機制、規範定義、公開介面的行為（在每個實例上相同，查證與照抄得到同一個答案）。**「標註未驗證」對這一類不夠**，差別在誰承擔預設——標註要讀者的自律才生效，不寫由結構保證；而照抄的人多半搜尋直達那一行、複製、離開，路徑上不經過那個標記。伴生判斷「需要實機才寫得了」多半是把「有了會更好」講成「需要」：主題拆成「問題的形狀與該去哪裡查」與「某個實例上的具體答案」兩半之後，前一半不必等硬體、後一半不該寫。與上一條是反向的一對——那條管該寫而沒寫，這條管不該寫而寫了，兩者都在稿件上看不出來。詳見 [written-value-replaces-the-readers-check](/report/written-value-replaces-the-readers-check/)。
...
**深度的分配不看主題的表面複雜度**：判斷一段要寫多詳細時用「讀者讀到這裡手上有什麼」，而不是這個主題看起來簡不簡單。基本處讀者手上的既有結構最少、作者的熟悉度最高，兩邊的需求剛好反向，所以用熟悉度當訊號必然分配錯——而陷阱聚集在基本處有結構上的原因：基本語法用得最頻繁，同一個誤解在每次使用裡重複，且失效多半是靜默的（實作為相容性接受寬鬆寫法，寬鬆的那一版還能算出看起來合理的值）。**「難」與「危險」是兩個不同的軸。** 同一條也管進階議題的起點——推導從底下的基本機制開始、複雜需求留成案例，驗收用一句話測試（這個起點能不能在不預設本模組任何內容的前提下說完）。詳見 [basics-anchor-the-advanced](/report/basics-anchor-the-advanced/)。
...
**句子層文法標準（全部書面產出適用）**：技術文章描述操作的每一句由四個槽位組成——動作者、動詞、受詞、結果——每個槽位填的是這個領域裡實際存在的東西，量詞計數的是領域裡的單位。判斷標準是**受詞是不是動詞在這個領域裡能作用的對象**：「執行」作用於指令，「一個字都沒有執行」的受詞「字」在領域外，句子語法完整而讀者要做四步還原；「處理的是行尾」的受詞「行尾」不是被處理的東西，被處理的是輸出裡的位元組。這條標準獨立於文章的定位、獨立於消費位（鍵位或段落位都適用），與行自足是兩個獨立的檢查——行自足管的是句子在脈絡外能不能還原命題，本條管的是每個槽位填的名詞在不在領域裡。它是生成端規格（見〈每一句先填槽位再成句〉），寫的當下逐句填槽位；**不只操作句**——論斷句、總結句、段標與過渡句另有一格：本身要求「誰的、什麼的」的名詞（來源、輸出、代價、差別、位置）要寫出所屬，形容詞與動詞不直接當名詞用。審查端 grep 抓不到（領域外的名詞與缺席的所屬都沒有固定關鍵詞），操作句靠冷讀或理解探針的「這個動作是誰做的」與「對什麼做的」兩欄，名詞所屬靠逐字引出所屬的探針（見 [noun-without-its-owner-passes-the-slot-check](/report/noun-without-its-owner-passes-the-slot-check/)）。詳見 [command-fragment-as-subject-hides-the-actor](/report/command-fragment-as-subject-hides-the-actor/) 與 [spoken-emphasis-is-silent-in-text](/report/spoken-emphasis-is-silent-in-text/)。
...
**欄位集合是一個關於「會有哪些結果」的宣稱，而欄位不足時的失真是結構強制的**：一份自檢表的結果欄只容得下「發現問題 → 修正」時，「本來就對」的項目沒有格子，於是被虛構成有問題（實測有人把「初稿即為現況」寫成「發現後已修正」）或被整列跳過——**兩種失真方向相反，彙總時抵銷不了也看不出來**。驗收是逐欄列舉它可能的值，問有沒有一種真實情形落在全部的值之外；修法是補值而不是補說明，因為填表的人選的是清單裡有的那一個。同一張卡另記報告結論要分兩類（正面命中不依賴涵蓋完整性、窮舉宣稱依賴），不分類則涵蓋數被修正時無從判斷哪幾項要重看。詳見 [field-shape-induces-false-reporting](/report/field-shape-induces-false-reporting/)。
...
**驗證手段要對齊錯誤的層次**：字面攔截（掃得到的固定字串）與行為精煉（要看行為才判得出的）是兩種錯誤層次，用錯層的手段驗不出東西而報告看起來正常。詳見 [literal-interception-vs-behavioral-refinement](/report/literal-interception-vs-behavioral-refinement/)，那裡給兩層的分界與各自該用的驗證手段。
...
| 要把一個題目的幾種做法並排比較（比較表有幾欄、每一欄比什麼、跨篇的零件在哪裡並排）                                                                                                                   | `/report/comparison-axis-comes-from-the-readers-decision/`                                                         |
...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **泛用詞濫用 grep**：`rg "坑|東西|搞|弄|處理一下|情況"` — 同一個泛用詞蓋過不同具體情境時、依情境換精確詞（意外 / 陷阱 / 出問題 / 發生狀況）；命中密集且各指不同事才算違規、真泛指 / 引號引用 / 輕度 hook 合規；「坑」另有地區偏移面（某些地區高頻、某些少用）。**合規邊界**：「視情況」「預設情況下」為固定搭配，不計入「情況」命中。見 [avoid-overused-generic-words](/report/avoid-overused-generic-words/)
  ...
  - **計數式摘要（無穩定 pattern，按位置逐句讀）**：在介紹另一份內容的位置——文章結尾指向下一篇的句子、`description`、目錄頁的交付欄、閱讀路線的說明欄——逐一找「數字＋類別名」的片語（「四個追問」「三種寫法」「四類邊界」「上線前的三件事」），問沒讀過那一篇的人知不知道成員是什麼。成員緊鄰數字寫出來的合規（「自連接、視窗函數與相關子查詢三種寫法」），量測事實合規（「差三個量級」），只有數字加類別名的違規。**grep 分不出兩者**，`rg "^description:.*[一二三四五六七八九十兩]+ ?(個|種|類|項|條)"` 只用來縮小要讀的範圍。「位置與集合指涉」那條管回指已讀的集合（其他三篇），碰不到這一類。見 [counted-summary-delivers-only-the-length](/report/counted-summary-delivers-only-the-length/)
  - **名詞缺所屬（無穩定 pattern，按位置逐句讀）**：在段標、段落的第一句與最後一句、總結段、過渡句，逐一找本身要求「誰的、什麼的、哪兩者之間的」的名詞（來源、輸出、結果、原因、代價、差別、位置、範圍、邊界、訊號、層、線、情形）與被當成名詞用的形容詞或動詞（「看不出錯」的「錯」），問它的所屬能不能從這一句或前一句**逐字指出來**；指不出來就違規（「錯的來源不只一種，而輸出都不帶訊號」）。**名詞清單只能用來知道該看哪裡，不能當判決**：一次二十句的校準裡，清單命中已知違規的一半、也命中已知合規的七成，因為合規句的所屬寫在前一句（「原因是配對是逐列做的」）。判定某個譬喻要換之後，先數它在整個集合裡出現幾次，整批換成同一個領域用詞——只改被標出的那一句，同一個概念就有了兩個名字。只寫在目錄頁而讀者看不到的框架詞是同一個成因，那一半可以用腳本掃。見 [noun-without-its-owner-passes-the-slot-check](/report/noun-without-its-owner-passes-the-slot-check/)
  - **縮寫術語 grep**：`rg "判準"` — 「判準」是「判斷標準」的縮寫，詞面只在科學哲學與教育評量有穩定所指，工程讀者讀不出來；一律寫「判斷標準」，狀態義寫「X 條件」（停止條件、完成條件）。全站替換過兩次仍回流，所以 `content/` 由 lint 規則 `TERM-abbreviation` 以 error 擋下；**skill 目錄不跑 lint，這一類要手動掃**。討論這個詞本身時加「」或行內程式碼。見 [term-probe-measures-register-not-invention](/report/term-probe-measures-register-not-invention/)
  - **序數、計數與字母代號當名字 grep**：`rg "[一二三四五六七八九十兩]支|[一二三四五六七八九十]步|第[一二三四五六七八九十][步支節級層章階種類條項]|前提[一二三四五]|兩個軸|另一個軸|這個軸"` 與 `rg "\b[A-E][0-9]?\b 那|\b[A-E][0-9]? 與 [A-E][0-9]?\b"` — 正文裡的實體用作者的索引當名字（「第一支的篇落在七步的第三步」「D 與 E 在這裡」），讀者要回頭翻才知道指什麼；判讀用變數名稱測試（當成程式變數名稱過不過得了審查）與落點測試（從中段落進來知不知道它指什麼），修法是用問題／動作／判斷標準／描述命名、段標拿掉序號。**詞綴隨內容變、這條 pattern 非窮舉**：換一個分類先盤點那裡用的是支、步、級、節、層、階段還是別的。命中是候選——緊鄰清單且隨即逐一具名展開的計數、量測事實的數字、外部凍結編號、確實在描述位置而前一句剛列完成員的序數都合規；**「集合不會再變」不構成放行理由**。命中密集的段落順手問它是不是在寫作者的編排思路（各篇怎麼分組、兩篇怎麼分工、改哪一篇），那一類整段移出正文。見 [ordinal-labels-are-the-authors-index](/report/ordinal-labels-are-the-authors-index/)
  ...
  - **教室形式三形態**（教學定位被讀成教室講課時的殘留，三個形態各有檢查方式）：
    ...
    - 根因與判讀徵兆見 [teaching-is-not-lecturing](/report/teaching-is-not-lecturing/)
  - **補充資訊歸位**（先備知識與環境陷阱有沒有跟在所屬的指令旁邊）——grep 抓不到，判別問句是「讀者在讀到哪個位置時會需要這段資訊」。遊離的形態：參數的理由獨立成段而讀者掃表格時跳過它、環境陷阱跟在無關的段落後面。修法是搬到所屬的指令或概念旁邊——表格欄位裡、或緊跟指令的說明段。改前：`-G 16M` 的放大理由寫在表格下方一段散文裡；改後：搬進表格的用途欄。見 [supplementary-knowledge-follows-its-parent-concept](/report/supplementary-knowledge-follows-its-parent-concept/)

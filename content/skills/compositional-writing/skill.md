...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **名詞缺所屬（無穩定 pattern，按位置逐句讀）**：在段標、段落的第一句與最後一句、總結段、過渡句，逐一找本身要求「誰的、什麼的、哪兩者之間的」的名詞（來源、輸出、結果、原因、代價、差別、位置、範圍、邊界、訊號、層、線、情形）與被當成名詞用的形容詞或動詞（「看不出錯」的「錯」），問它的所屬能不能從這一句或前一句**逐字指出來**；指不出來就違規（「錯的來源不只一種，而輸出都不帶訊號」）。**名詞清單只能用來知道該看哪裡，不能當判決**：一次二十句的校準裡，清單命中已知違規的一半、也命中已知合規的七成，因為合規句的所屬寫在前一句（「原因是配對是逐列做的」）。只寫在目錄頁而讀者看不到的框架詞是同一個成因，那一半可以用腳本掃。見 [noun-without-its-owner-passes-the-slot-check](/report/noun-without-its-owner-passes-the-slot-check/)

...
date: 2026-04-24
description: "Zettelkasten 寫作方法論的 SKILL 入口：三大支柱、六大原則速查、觸發路由對應表。"
...
**機會成本語氣優先**：程式設計大多是多目標取捨、討論的是「在什麼情境下哪個選項較划算」。把絕對二元語氣（「正確概念是 X / 替代方案不足 / 應該這樣做」）翻成情境化敘述：「比較好的做法是 A、因為 [情境] / B 在 [其他情境] 合理 / D 的成本特別高、只在 [極端情境] 才划算」。機會成本教讀者「思考方式」（能套用到新情境）、絕對主義教讀者「規則」（壓力下會忘）— 所以前者是預設語氣。例外保留給物理 / 法律 / 數學事實（安全性、數據完整性、合規、雜湊必有碰撞）。絕對二元語氣有兩種形式：**命令式**（「應該做 X」）讀者聽得出是主張、會審；**必然式**（「X 天生就是 Y / 本質就是 / 必然」）偽裝成事實陳述、更隱形 — 把設計選擇講成自然法則時尤其要 catch、還原成「在選了某前提後 X 才以此形式成立」。詳見 [#152 設計選擇講成選擇不講成必然](/report/design-choices-framed-as-choices-not-necessity/)。
...
**Case 引用段落的三段式結構**：三段式是案例引用段落的順序紀律 — 把「概念 → 案例 → 操作」三層分開承擔（段首給概念定義、case 引用居中、通用工程知識展開）、讓段落結構跟讀者學習新概念的認知順序對齊。LLM 從 case 反推內容容易把 case 揭露當概念出發點、實證觀察 11/12 段都犯這個錯。詳見 skill 內部支撐卡 `case-citation-three-part-structure`（mirror 版本見 references/principles）。
...
**Production 教學文章追加輪 8-10（字句層 catch、跑 N 輪仍漏時觸發）**：跑了 5 輪基本 frame 仍系統性漏 catch 字句層問題（口語修辭 / 廢話前綴 / 地區漂移 / 依賴 code / **裝飾符號 emoji** / 對讀者喊話 / 自評誇飾 / 必然性框架）時、追加三個換軸機制——輪 8 keyword bank（換工具、含 emoji / 裝飾 unicode 掃描）、輪 9 reader simulation（換視角、兩 lens：自包含性 + register/stance）、輪 10 self-criticism（換層次、審視 framework 本身覆蓋度）。短文 / 即時 note 不需要、production 教學文章在跑 5 輪後仍漏同類問題時 opt-in。**keyword bank 命中是候選、不是判決**——grep 命中後仍要語意判定（建立概念的違規 vs 合規的反例對照 / hook），偵測跟判定是兩個認知步驟；**register/stance 類（喊話 / 誇飾 / 必然）無穩定關鍵詞、reader-sim 是主 keyword bank 是輔**。詳見 [#114 frame 顆粒度盲點](/report/multi-pass-review-frame-granularity-blindspot/)、[#149 命中是候選不是判決](/report/keyword-bank-hit-is-candidate-not-verdict/)、[#150 不對讀者喊話](/report/teaching-register-states-not-addresses-reader/) 跟 `references/writing-articles.md` 輪 8-10 段。裝飾符號禁用規範見 [Blog Markdown 寫作規範 §5.9](/posts/markdown-writing-spec/)。

**Instance 軸：跨 reviewer instance 隔離**：Instance 軸是 multi-pass review 的另一條擴展軸 — N 個獨立 reviewer instance 各自獨立 context、各自跑 background、解「單一 reviewer 同時看多維度容易維度盲點 + context 污染」的問題。Instance 指獨立 reviewer 程式實體（如 agent tool spawn 出的 subagent）、跟同一 reviewer 換輪次 frame（frame 軸）正交可疊加。適用 production 教學文章 / 高 stakes 內容 / 跨章節教學模組這類維度複雜度高的審查場景。詳見 skill 內部支撐卡 `agent-team-context-isolation`。

詳見 #83 Writing 的 multi-pass review、#85 Methodology 的 multi-pass 該 embed 在 pillar、Metadata surface 要納入寫作 review 範圍、False sense of security 是高 stakes 寫作的主要失敗模式、Risk-asymmetric audit standard、[#111 口語修辭 keyword bank](/report/colloquial-rhetoric-erodes-technical-precision/)、[#113 self-contained 論述](/report/prose-self-contained-without-code-reference/)、[#112 地區用語對齊](/report/regional-terminology-alignment/)、[#114 frame 顆粒度盲點](/report/multi-pass-review-frame-granularity-blindspot/)、[#110 design-flaw 三軸論證](/report/design-flaw-by-current-axes-not-hindsight/)。
...
兩類流程互補疊加 — 教學模組的每章內部寫作仍套本 skill 6 原則、case 引用段落用三段式結構（[case-first + agent team review 標準流程](/posts/case-first-agent-team-review-workflow/)）、agent team review 用跨 instance 隔離設計。當下游專案沒有跨章節教學模組需求、本 skill 即可獨立運作；當有需求、教學模組生產流程是本 skill 的擴展層、不取代本 skill。
...
- **本 skill（compositional-writing）** 提供每輪 frame 的字句層 keyword bank — Round 1-A 寫作規範 reviewer 必須跑：
  ...
  - **裝飾符號 grep**：`rg "✅|❌|⚠️|🚨|🟡|🟢|⭐|📌|✓|✗"`
  - **對讀者喊話 grep**：`rg "很多人|大家|不少人|你天天|你會|你可能|先讀懂|先釐清|別搞混|別被"` — 教材中性陳述、不安撫 / 不第二人稱 / 不祈使（hook / narrative 輕度第二人稱可留）
  - **自評誇飾 grep**：`rg "教科書級|堪稱|可謂|完美|經典|範本級|大師級|漂亮地|優雅地|最佳實踐|best practice"` — 品質 verdict 頂替技術理由
  - **必然性框架 grep**：`rg "天生|與生俱來|本質就是|本來就是|必然|唯一|註定|理所當然"` — 把設計選擇講成自然法則、還原成條件性（物理 / 法律 / 數學事實除外）

詳細各維度的判讀規則跟修法、見對應 reference（writing-articles / writing-documents 等）跟 [#111 口語修辭 keyword bank](/report/colloquial-rhetoric-erodes-technical-precision/) / [#112 地區用語對齊](/report/regional-terminology-alignment/) / [#114 frame 顆粒度盲點](/report/multi-pass-review-frame-granularity-blindspot/) 等卡。
...
**Version**: 0.10.0 — 從 13 張 knowledge cards 批量改寫負向表述的經驗回流：(1) 新增「跟 multi-round-review 的協同」段、列出 Round 1-A 寫作規範 reviewer 必須跑的 5 個 grep pattern（正向陳述 / 口語修辭 / 地區用語 / 廢話前綴 / 裝飾符號）、明示兩 skill 垂直協同關係；(2) 修正 multi-round-review 漏抓字句層的盲區、跟 multi-round-review v1.1 同步 cross-trigger 設計
**Version**: 0.9.1 — 同步 source v0.9.1 mirror 修正：兩補強段段首改概念定義句、原則 6「詳見」list 補新 mirror、Directory Index 補；五大 / 六大原則 drift 對齊
**Version**: 0.9.0 — 從跨章節教學模組生產經驗回流：原則 3 補「Case 引用段落三段式結構」段；原則 6 補「Instance 軸：跨 reviewer instance 隔離」段；新增「跟特化寫作流程的分工」段（明示本 skill 是單篇基礎方法、跨章節教學模組生產流程是擴展層）
**Version**: 0.9.2 — 從 business case-analyses 演變回流：新增 `source-to-teaching-analysis.md` 路由，處理外部分析文章 / 產業評論 / 投資人備忘錄到教學型分析文章的轉換；新增三張 principle（external-analysis-source-layering / cross-domain-reader-level-alignment / analysis-rewrite-delivers-transferable-framework），把 source 分層、跨領域讀者降層、可遷移框架交付抽成 portable 規則。
**Version**: 0.8.1 — 第 6 原則同步 writing-articles v0.8.1：補「Production 教學文章追加輪 8-10」段（換工具 / 換視角 / 換層次三機制處理「跑 N 輪仍漏」字句層問題）；「詳見」連結加 5 張新 report 卡（[#110 design-flaw](/report/design-flaw-by-current-axes-not-hindsight/) / [#111 口語修辭](/report/colloquial-rhetoric-erodes-technical-precision/) / [#112 地區用語](/report/regional-terminology-alignment/) / [#113 self-contained](/report/prose-self-contained-without-code-reference/) / [#114 frame 顆粒度](/report/multi-pass-review-frame-granularity-blindspot/)）

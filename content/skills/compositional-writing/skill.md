...
**Production 教學文章追加輪 8-10（字句層 catch、跑 N 輪仍漏時觸發）**：跑了 5 輪基本 frame 仍系統性漏 catch 字句層問題（口語修辭 / 廢話前綴 / 地區漂移 / 依賴 code / **裝飾符號 emoji**）時、追加三個換軸機制——輪 8 keyword bank（換工具、含 emoji / 裝飾 unicode 掃描）、輪 9 reader simulation（換視角）、輪 10 self-criticism（換層次、審視 framework 本身覆蓋度）。短文 / 即時 note 不需要、production 教學文章在跑 5 輪後仍漏同類問題時 opt-in。詳見 [#114 frame 顆粒度盲點](/report/multi-pass-review-frame-granularity-blindspot/) 跟 `references/writing-articles.md` 輪 8-10 段。裝飾符號禁用規範見 [Blog Markdown 寫作規範 §5.9](/posts/markdown-writing-spec/)。

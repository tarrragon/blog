---
title: "Managing Article Collections — 跨多篇相關文章的結構設計"
date: 2026-04-25
description: "compositional-writing reference：跨多篇相關文章的結構設計（三層、素材庫比例、MOC、Pattern 卡片、跨篇引用 idiom）。"
...
分析導向的教學模組（判斷標準密集、讀者要帶走判斷力的內容），模組級結構要是**推導體系**、不是主題集合：一個源頭機制（一個成本結構、一個約束、一個生命週期——各篇判斷標準能折算回去的基準），每篇承擔一條從源頭展開的推導線，模組入口能一句話說出「推導的起點是什麼」。

源頭機制買到四件事：各篇判斷標準折算到同一把尺（讀者學一把尺加 N 個用法、不是 N 套規則）、跨篇判斷標準矛盾才會現形（兩篇判斷標準互斥要有共同基準才「是矛盾」）、新篇有明確的掛載點（擴篇時先問「新篇折回源頭的路徑」）、推導式的閱讀路線成立。主題集合式的模組被批評「淺」時無從補起——沒有源頭就沒有「往哪裡深」的方向。

邊界：目錄型模組（工具清單、vendor overview）與異質 case 記錄不適用，硬找源頭是強加敘事；源頭是判斷標準的折算基準、不是每篇的開場模板——寫成固定開場會複製 cadence 同質化。詳見 [teaching-module-needs-derivation-anchor](/report/teaching-module-needs-derivation-anchor/)。

---

## 按功能組織時，另給選擇一層路由

模組按功能分章（子句、API、元件各一節）時，每個功能都有住址，而「我這個問題該用哪一個功能」沒有。這是組織方式的直接結果——章節照被解釋的對象切，選擇發生在對象之間。讀者手上是問題不是功能，要從問題走到功能他得先知道答案。

**修法是在模組入口加一張「問題 → 落點」表**，章節照原樣留著。這一層很便宜，不必重寫任何一章。

兩個判準：

- **問題用讀者的語言寫，不用功能的語言。** 「分組之後結果少了幾筆」是讀者說得出來的；「分組鍵的選擇」不是——後者已經預設他知道問題出在哪。
- **承擔選擇的章節從業務事實開場，不從功能的比對開場。** 一章如果做的是「這幾種選哪一個」，第一段要是那個讓選擇成立的業務事實。實作中同一章寫過兩版：從結果反推（「答案已經寫在每一列上了」）與從業務事實正推（「沒消費的人不會出現在訂單表」），後者不需要讀者先產出一個結果才看得到結論。功能型章節（「這個東西做什麼」）維持原樣，套上去只會製造多餘的鋪陳。

辨識訊號：章節的名字全部是被解釋的對象、沒有一個是問題；使用者的提問是「我這樣寫犯了什麼錯」而不是「這個功能怎麼用」。

新增的這一層路由同樣要驗目的地承接——問題對到的那一篇要真的處理它，而不只是主題相關。
...
**核心判斷標準**：抽出後是否多篇受惠？只有一篇用到、抽出純粹是 fragmentation。
...
**Version**: 0.3.0 — 從 writing-articles.md 過載反思整合內容：「拆分標準（focus 是議題完整度）」完整搬入（含寫作 / review 自問三題、議題切一半的辨識訊號、反例 vs 正例、完成標準）+「三層 structure 詳細對照」展開為完整模板（情境 / 抽象 / Pattern 各一份）+「structure 分類錯誤的徵兆」清單。現在這份 reference 完整涵蓋「跨多篇相關文章的結構設計」、不需要回 writing-articles.md 找

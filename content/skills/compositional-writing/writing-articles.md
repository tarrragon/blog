...
- **拆分判準（focus 是議題完整度）**：見 [managing-article-collections.md → 拆分判準](/skills/compositional-writing/managing-article-collections/#拆分判準focus-是議題完整度)
...
| 8   | Keyword bank（換工具）                                                                        | 跑 grep 比對固定 keyword list（口語修辭 / 廢話前綴 / 地區漂移 / 依賴 code / 裝飾符號 / 對讀者喊話 / 自評誇飾 / 必然性框架）、不靠 reviewer 記憶；**命中是候選不是判決**、命中後要語意判定（建立概念的違規 vs 合規的 hook / 反例 / 真必然）——詳見 [colloquial-rhetoric](/report/colloquial-rhetoric-erodes-technical-precision/) + [regional-terminology](/report/regional-terminology-alignment/) + [prose-self-contained](/report/prose-self-contained-without-code-reference/) + `decorative-symbols`（skill 內部支撐卡） + [teaching-prose-neutral-register](/report/teaching-register-states-not-addresses-reader/) |
| 9   | Reader simulation（換視角）                                                                   | 兩個 lens：(a) **自包含性**——拿掉所有 code block 重讀論述是否仍能 parse？跳段直接讀能拿到關鍵資訊？(b) **register/stance**——這段在「陳述概念」、還是在「管理 / 評價 / 絕對化讀者」（喊話 / 誇飾 / 必然）？register 類無穩定關鍵詞、keyword bank（輪 8）抓不到、**reader-sim 是主、keyword bank 是輔**；且這類最依賴 external cold-read、同 reviewer 模擬有限——production 教材建議刻意換視角或外部讀者。詳見 [teaching-prose-neutral-register](/report/teaching-register-states-not-addresses-reader/)                                                                                                                   |

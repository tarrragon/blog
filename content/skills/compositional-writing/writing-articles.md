...
事後檢視類文章（post-mortem / 設計缺陷分析 / retrospective）有一個常見的失敗模式：論述依賴「結局已發生」的視角、產出的判斷工具只在 case 已觸發意外事件後才能用。

例：「[工具] 預設為 [限制版本]、被當 [通用版本] 用了一段時間沒事、後來出問題 → 設計缺陷」——這個論述需要讀者知道結局、且歸因落在個人預見性。換成當下三軸論述「在零成本差異 + 強領域先驗下選了限制更高的選項 → 設計缺陷」、判斷不依賴結局、歸因轉到工具預設與制度。
...
詳細展開、修法、跨情境多面向應用見 [#110 設計檢討用當下三軸論證、不依賴 hindsight](/report/design-flaw-by-current-axes-not-hindsight/)。
...
| 8   | Keyword bank（換工具）                   | 跑 grep 比對固定 keyword list（口語修辭 / 廢話前綴 / 地區漂移 / 依賴 code / 裝飾符號 / 對讀者喊話 / 自評誇飾 / 必然性框架）、不靠 reviewer 記憶；**命中是候選不是判決**、命中後要語意判定（建立概念的違規 vs 合規的 hook / 反例 / 真必然）——詳見 [#111 口語修辭](/report/colloquial-rhetoric-erodes-technical-precision/) + [#112 地區用語](/report/regional-terminology-alignment/) + [#113 self-contained](/report/prose-self-contained-without-code-reference/) + [#149 命中是候選不是判決](/report/keyword-bank-hit-is-candidate-not-verdict/) + [#150 不對讀者喊話](/report/teaching-register-states-not-addresses-reader/) + [裝飾符號禁用 §5.9](/posts/markdown-writing-spec/) |
| 9   | Reader simulation（換視角）              | 兩 lens：(a) 自包含性——拿掉所有 code block 重讀論述是否仍能 parse？跳段直接讀能拿到關鍵資訊？(b) register/stance——這段在「陳述概念」還是在「管理 / 評價 / 絕對化讀者」（喊話 / 誇飾 / 必然）？register 類無穩定關鍵詞、reader-sim 是主 keyword bank 是輔、且最依賴 external cold-read。詳見 [#150 不對讀者喊話](/report/teaching-register-states-not-addresses-reader/)                                                                                                                                                                                                                                                                                                               |
| 10  | Self-criticism（換層次）                 | 我跑的 N 輪 catch 哪些問題類型？同個規則下還有哪些違反句型沒掃到？framework 是否有 known blind spot？——詳見 [#114 frame 顆粒度盲點](/report/multi-pass-review-frame-granularity-blindspot/)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

跳輪規則同 #83 Writing 的 multi-pass review — 短文 / 即時 note 跳 4-7、production 卡片 / 教學文章全跑；輪 8-10 是 production 教學文章專用、catch 字句層問題、跑 N 輪後仍漏 catch 同類問題時觸發。
...
| 規則四：事後檢視看判讀品質                           | 可查詢性 + 欄位設計 | 失敗類型是可 grep 的分類鍵；改善方向與失敗類型分欄，避免「要更仔細」這種無欄位化結論。設計檢討類文章另外要避免 hindsight 論述汙染判讀（見 [#110 hindsight](/report/design-flaw-by-current-axes-not-hindsight/)） |
...
**Version**: 0.8.1 — WRAP 審視後降 over-claim：5 張對應 report 卡（#110-114）都加「論述基礎與限制」段揭露樣本量限制；補上 [#112 地區用語](/report/regional-terminology-alignment/) 進輪 8 keyword bank 引用；同步 [#110 hindsight](/report/design-flaw-by-current-axes-not-hindsight/) 的個人 vs 制度歸因軟化跟多面向假設性對比 disclaimer
**Version**: 0.8.0 — Multi-pass review 加輪 8-10（keyword bank / reader simulation / self-criticism）、處理「跑了 N 輪、字句層問題仍漏」的盲點；對應三張新 report 卡:[#111 口語修辭 keyword bank](/report/colloquial-rhetoric-erodes-technical-precision/) / [#113 self-contained 論述 + reader simulation](/report/prose-self-contained-without-code-reference/) / [#114 frame 顆粒度盲點 + self-criticism 三機制](/report/multi-pass-review-frame-granularity-blindspot/)
**Version**: 0.7.6 — 規則四補進階段「避免 hindsight 論述汙染判讀記錄」、輪 5 補設計檢討類另掃 hindsight 論述；對應 [#110 設計檢討用當下三軸論證](/report/design-flaw-by-current-axes-not-hindsight/)——把「設計缺陷的精準定義」從「沒預測到後來需求」改成「在當下成本對稱條件下選了限制更高的選項」、用三軸論證代替 hindsight、確保產出 portable 判斷工具

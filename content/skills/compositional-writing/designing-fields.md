...
實測有執行者把「初稿即為現況」寫成「發現後已修正」——那句話的每個字都對得上某個真實，唯一假的是它隱含了一次不存在的修正動作，而那正是欄位要求他提供的東西。**兩種失真的方向相反**（多一筆假的／少一筆真的），所以彙總時抵銷不了也看不出來。下游的代價是讀報告的人會把那一項算進「這一輪修了幾處」，而那個數目常常是後續每一個判斷的分母。

**檢查方式**：逐欄列舉它可能的值，問有沒有一種真實情形落在全部的值之外。落在外面的就是會被改寫或被跳過的。**修法是補值，不是補說明**——填表的人選的是清單裡有的那一個。

同一條紀律用在報告的結論欄上，值有兩個。**分辨看的是這項結論的真假由什麼決定**——正面命中是在某個具體對象上量到的，真假由那個對象決定；窮舉宣稱是對一個集合的全稱陳述，真假由集合劃得對不對決定。**不要用量詞當判斷標準**：「其餘」「只有」「全部」這些字是常見的表面訊號而不是判斷標準，一句「這一類的成員已經列完」沒有任何量詞而它是窮舉宣稱。
...
不分類的報告在涵蓋數被修正時只剩兩個極端可選：整份重驗，或整份照用。一次實測的七項結論裡有兩項是窮舉宣稱，所以涵蓋數被修正之後的代價是重驗那兩項、其餘五項不動（這個比例是那一份報告的分佈，換報告要重算）。

判讀與完整實例見 [field-shape-induces-false-reporting](/report/field-shape-induces-false-reporting/)。

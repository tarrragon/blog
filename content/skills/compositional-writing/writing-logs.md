...
Log 訊息描述**發生了什麼業務事件**，而不是**程式碼執行到哪一行**。讀者想知道系統的狀態，不是程式的控制流。每條 log 都是被 grep 單獨命中的消費單位——訊息本體要句內自足（事件、對象、原因在同一條裡、指涉不依賴前後條 log），見[語句要在它的消費單位內資訊自足](/report/sentence-self-sufficiency-by-consumption-unit/)的單句消費位規則。

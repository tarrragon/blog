...
短暫斷線事件（「剛斷線」）和恢復連線用 SnackBar 短暫顯示，適合網路狀態偶爾變化的場景。持續離線狀態用 Banner 持續顯示直到恢復連線，避免 SnackBar 消失後使用者忘記自己在離線中。通知形式的完整選擇標準見[通知模式選擇](/ux-design/06-interaction-feedback/notification-pattern-selection/)。不適合頻繁斷開重連的場景（通知太多會干擾使用者）。

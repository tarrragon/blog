...
行動解決問題的層級要等於錯誤發生的層級。資料層錯誤（檔案格式不合法）配資料層行動（換檔案重試）；執行環境層行動（重載 app、重啟）只配執行環境層錯誤。過重的行動不解決問題，還附帶狀態遺失的代價；過輕的行動（執行環境層崩潰只給「關閉」）讓使用者無路可走 — 兩個方向都是缺口。使用者傾向照著按鈕走，不對位的按鈕等於系統主動引導無效操作。

一個 Chrome extension 的匯入錯誤複用了為執行層錯誤設計的通用容器，帶出「重新載入擴充功能」按鈕；使用者回饋抓到不對位，修法是建匯入專屬的錯誤卡片、只留「關閉」（[U.C13](/ux-design/cases/import-error-reload-extension-mismatch/)）。錯誤 UI 的複用邊界是「行動相容」，而非「都是錯誤」。
...
三層不需要都顯示。輕微錯誤（SnackBar）可以只有標題 + 一個行動按鈕。嚴重錯誤（全螢幕）三層都需要。通知形式（SnackBar / Banner / Dialog / 全螢幕）的選擇標準見[通知模式選擇](/ux-design/06-interaction-feedback/notification-pattern-selection/)。

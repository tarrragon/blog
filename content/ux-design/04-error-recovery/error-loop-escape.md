...
這和[畫面狀態矩陣](/ux-design/knowledge-cards/screen-state-matrix/)的退出路徑要求一致 — 每個狀態至少一條退出路徑（[定義與填寫方法](/ux-design/01-screen-state-machine/state-matrix-definition/)）。Error 狀態的退出路徑包括重試（回到 connecting 狀態）和返回（離開當前畫面）。
...
- 「重試中」畫面的狀態回饋與退出路徑設計 → [互動回饋三層模型](/ux-design/06-interaction-feedback/feedback-three-layers/)的畫面級回饋段
...
- Error 事件的分類與收集 → [monitoring 模組一 監控心智模型](/monitoring/01-mental-model/)

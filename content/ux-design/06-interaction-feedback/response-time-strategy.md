...
完成宣告的誠實度與進度指示同源。「完成」的判定條件若只是「連續 N 輪沒有變化」，分不出「真的到底了」和「載入通道根本沒打開」— 一個 Chrome extension 從 lazy-load 頁面提取書目，96/928 本就因 count 穩定誤判完成，使用者帶著殘缺資料離開且很難察覺（[U.C11](/ux-design/cases/lazy-load-premature-completion/)）。宣告完成要有窮盡證據（總數對照、終止標記）；只有停滯訊號時，改為「已取得 N 筆、可能還有更多」的部分結果表達。全螢幕 loading 的問題則是阻斷範圍 — 等待指示只需要覆蓋被等待的區域，不需要凍結整個介面。

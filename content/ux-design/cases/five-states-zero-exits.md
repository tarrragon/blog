...
1. **企劃文件的「前端引導」欄位只描述顯示，不描述操作和退出**。§1 操作盤點表的「前端引導」寫了「連線失敗顯示無法連線」— 覆蓋了 error 狀態的顯示，但沒回答「能做什麼」和「怎麼離開」。從 BDD 操作盤點到 UI 實作之間，缺少把「情境」展開成「畫面 × 狀態 × 操作 × 退出」矩陣的步驟。
...
- 想用狀態矩陣設計畫面 → [模組一：畫面狀態機設計](/ux-design/)
- 想建 widget test 覆蓋導航 → [開發測試模組四：自動化 UI 驗證](/testing/)

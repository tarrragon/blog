...
date: 2026-07-17
description: "批次 / 遍歷類操作宣告完成、實際只處理了一部分時使用。「連續 N 輪沒有變化」是暫時停滯的訊號、不是窮盡的證據，lazy-load 頁面上兩者的差距可以是 10 倍"
weight: 11
tags: ["ux-design", "case-study", "interaction-feedback", "chrome-extension", "web", "lazy-load", "progress"]
---

這個案例的核心責任是說明「完成」是一個 UI 宣告，宣告的誠實度取決於判定條件的證據強度 —「暫時沒有變化」與「確認沒有更多」是強度完全不同的兩種證據，混用會讓使用者拿到殘缺資料而不自知。
...
修復：每輪同時捲動＋點擊按鈕（不再二擇一），涵蓋率從 96/928 提升到 928/944。

## 判讀

1. **count 穩定是「沒有變化」、不是「沒有更多」**。lazy-load 頁面的內容增長依賴特定觸發動作（捲動、點按鈕）— 觸發動作缺失時 count 永遠穩定，穩定訊號分不出「真的到底了」和「載入通道根本沒打開」。完成判定缺一個獨立的窮盡證據：頁面宣告的總數、終止標記（「沒有更多了」元素）、或載入觸發器消失。
...
1. **完成判定列出證據清單**：宣告完成前問「終止條件是『確認沒有更多』還是『暫時沒變化』」。只有停滯訊號時，完成宣告降級為「已取得 N 筆、可能還有更多」的誠實表達。

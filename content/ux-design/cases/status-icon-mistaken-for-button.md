...
管理模式下右格換成 Checkbox — 選取控制與兩顆動作圖示混排同列、位置隨前兩顆的寬度浮動，這是「勾選框沒有靠右對齊」的成因：它不是獨立右對齊的固定欄。
...
4. **選取控制要有固定的對齊軸**。管理模式的 checkbox 混在動作列裡隨內容浮動，每張卡的勾選位置不在同一條垂直線上 — 批次勾選的操作是「沿一條線往下點」，浮動的位置破壞這個動線。
...
3. **選取控制（checkbox）獨立成固定右欄**，不與動作圖示混排 — 對齊軸就是批次操作的動線。

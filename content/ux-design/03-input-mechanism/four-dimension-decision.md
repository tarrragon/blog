...
app_tunnel 的 terminal 輸入框用 `TextInputType.visiblePassword` — 因為 CLI 指令包含路徑分隔符、flag 縮寫等非自然語言內容，一般文字鍵盤會嘗試自動校正 `ls -la` 或 `/usr/bin/` 成其他東西（[U.C3](/ux-design/cases/terminal-input-mechanism-absent/)）。
...
- Terminal app 的輸入設計特殊需求 → [Terminal app 輸入設計](/ux-design/03-input-mechanism/terminal-input-design/)
- 安全敏感輸入框的 IME 控制 → [IME 安全 checklist](/ux-design/03-input-mechanism/ime-security-checklist/)
- 整行送出 vs 逐字元對 protocol 的影響 → [testing 模組三 協議整合測試](/testing/03-protocol-integration-test/)

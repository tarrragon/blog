...
Terminal app 需要繞過或控制 IME 層的這些行為。app_tunnel 的 TextField 用 `TextInputType.visiblePassword` + `autocorrect: false` + `enableSuggestions: false` + `enableIMEPersonalizedLearning: false` 四個參數關閉 IME 的自動行為（[U.C3](/ux-design/cases/terminal-input-mechanism-absent/)）。

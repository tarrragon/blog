---
title: "所有程式碼設計原則都指向同一件事：降低認知負擔"
slug: "cognitive-load-design-methodology"
date: 2026-03-04
draft: false
description: "設計原則背了不少而判斷仍靠感覺、或想解釋某條規則為什麼存在時，用來把函式長度、命名與拆分還原成閱讀時要記住的資訊量"
tags: ["認知負擔", "程式碼設計", "函式設計", "命名規範", "任務拆分"]
---

**所有程式碼設計原則指向的是同一個目標：降低閱讀者的認知負擔。** DRY、SOLID、Clean Code、設計模式各自規定的做法不同，而它們要省下的是同一種資源——閱讀時要同時記住的東西。

<!--more-->

以這個目標當共同基準之後，個別規則的存在理由就可以被推導，而不必逐條記住：一條規則若降不了閱讀時的記憶量，它在這個情境就不成立。

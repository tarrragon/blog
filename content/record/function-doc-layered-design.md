...
還要拆一次。這類 doc 的動機是防護（怕有人順手補上），而註解不參與執行、補上去的當下不會發聲。上面那個例子要拆成兩半：「加了 retry 會重複扣款」寫得成一條測試（模擬重試、斷言只扣一次），交給測試守；「gateway 非冪等」是外部契約、repo 裡查不到，那半才是 doc 該留的。驗證方式是當場把守衛拿掉跑測試，紅了代表已經有人接手（[#253](/report/protective-comment-signals-missing-enforcement/)）。
...
doc 警告的執行力靠使用者「願意讀並且記住」、型別約束則是編譯期強制——當失敗成本高（內部狀態被破壞）、保護機制就值得從 doc 升到型別。

型別表達不了的使用情境（例如「必須在 main isolate 呼叫」）還有一站要問，才輪到 doc：**這件事有沒有一條會紅的測試守得住？** 型別是把規則寫進程式本身，寫不進去這條路就到底了；測試則站在程式外面觀測執行結果，跨方法 invariant 與時序這類寫不進型別的約束，往往一條行為測試就守得住。判斷方式是問剩下的資訊有沒有對應的斷言——存在一條會紅的斷言，那件事的家是測試名稱；要寫出來必須引用 repo 之外的事實（法規、稽核、外部契約、當初的取捨），才輪到 doc 警告（[#253 寫註解的動機是怕被改壞時要處理的是那個約束](/report/protective-comment-signals-missing-enforcement/)）。

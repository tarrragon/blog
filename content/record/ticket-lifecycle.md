...
這套系統 除了做任務管理，其實核心目標是把所有的任務需求有邏輯的拆分拆細

所以任務進來會先做分析，然後拆分成平行的多個子任務，子任務實際執行如果評估太大，還是可以繼續往下切

然後就是往下走到不能再往下走的時候，確認有沒有平行層級的任務

平行層級任務都完成後回到上一層的父任務，依照同樣的方式執行，完成驗收，檢查平行任務，再往上驗收，直到任務都完成。

任務拆小有兩個好處，一個是降低任務執行的認知負擔，另一個是更方便做驗收檢查，避免執行的時候有疏失沒有察覺。

...
date: 2026-03-04
draft: false
description: "讀自己或別人的程式碼要反覆回上面確認變數與縮寫時，用來把命名、函式長度、變數用途還原成可逐項檢查的撰寫規則"
...
**沒有背景的讀者測試**：交給不熟悉這段程式碼的人讀。五分鐘內能理解主要邏輯算合格；需要旁邊有人解釋才理解得了，就要重寫。
...
每一行程式碼是一句話，每個函式是一個段落。這套規則要換到的是可讀性，而可讀性的收益落在維護成本上——它是工程決策，判斷標準是閱讀時省下多少記憶量，不是風格偏好。

這些規則共同的理論出發點，走 [所有程式碼設計原則都指向同一件事](../cognitive-load-design-methodology/)；註解該補的是哪一種資訊，走 [註解保護需求，不解釋程式](../comment-writing-methodology/)。

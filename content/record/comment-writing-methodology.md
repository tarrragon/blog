...
date: 2026-03-04
draft: false
description: "註解寫成函式名稱的重述、或改動一段程式時查不到當初的業務約束時，用來判斷哪些內容該進註解、哪些該進命名或文件"
tags: ["方法論", "程式設計", "軟體架構", "註解規範", "需求管理","AI協作心得"]
categories: ["軟體工程", "開發方法論"]
keywords: ["程式碼註解", "需求保護", "維護指引", "設計意圖", "業務邏輯", "事件驅動", "Flutter Widget"]
---

`processBook()` 旁邊寫著「處理書籍相關邏輯」的註解，帶進來的資訊是零：它重述了函式名稱，而背後有什麼業務限制、改了會影響哪裡、當初為什麼這樣設計，一項都沒說。

決定註解該寫什麼的，是幾個月後回到這段程式碼的人手上缺什麼——那時程式碼本身還在，消失的是它成立的條件。
...
**動機測試**：寫下這條註解是為了說明，還是為了防止有人改壞它？後者的需求該送去會發聲的機制——註解不參與執行，改壞的當下不產生任何訊號。這一問的完整判斷標準與可當場執行的驗證方式見 [#253 寫註解的動機是怕被改壞時要處理的是那個約束](/report/protective-comment-signals-missing-enforcement/)。

好的程式碼自說明的是「它在做什麼」，而「它為什麼只能這樣做」不在程式碼裡——那是需求與約束，它們在寫下的當下是常識，幾個月後就只剩註解這一個載體。

註解與其他載體的職責邊界（commit message 記變更理由、型別記約束、文件記使用方式），走 [Commit message vs source code doc](../commit-message-vs-source-doc/) 與 [型別取代 doc 的收益曲線](../types-replacing-docs/)。

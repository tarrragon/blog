...
**動機測試**：寫下這條註解是為了說明，還是為了防止有人改壞它？後者的需求該送去會發聲的機制——註解不參與執行，改壞的當下不產生任何訊號。這一問的完整判準與可當場執行的驗證方式見 [#253 寫註解的動機是怕被改壞時要處理的是那個約束](/report/protective-comment-signals-missing-enforcement/)。

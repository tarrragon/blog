...
date: 2026-03-04
draft: false
description: "同一個功能被重複實作、或難題被臨時解法蓋過去時，用來在任務建立的當下強制回答六個問題，包含執行者與分派者的職責邊界"
tags: ["方法論", "決策框架", "5W1H", "開發流程", "品質管理", "AI協作心得"]
---

AI 協作開發有兩個會重複出現的失效。第一個是重複實作：某個模組已經有 `BookValidator`，另一個任務裡又建了一個新的驗證類別——因為任務開始前沒有問「這個功能是否已經存在」。第二個是迴避：遇到難題時轉向「比較簡單的方法」，用臨時解法蓋過設計缺陷。

兩者的共同位置都在任務建立的那一刻——那時還沒有程式碼，只有一段描述，而描述沒有被要求回答任何問題。5W1H 自覺決策把六個問題設成任務建立的前置條件：任何一個答不出來，任務就不成立。
...
Who 問的不只是「誰來做」，更重要的是「有沒有人已經做過了」。這套協作模式裡，Who 要明確標記執行者與分派者兩個角色，格式如：「`parsley-flutter-developer`（執行者）由 `rosemary-project-manager`（分派者）分派」。

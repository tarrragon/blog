...
這個性質還有一個反向用途：判斷某段 doc 該不該存在。想寫的內容如果有一條會紅的斷言撐得住，那件事的家就是測試名稱而不是 doc（判斷標準是斷言存不存在、不是造不造得出句子）——尤其當寫下它的動機是「怕有人改壞」時（[#253 寫註解的動機是怕被改壞時要處理的是那個約束](/report/protective-comment-signals-missing-enforcement/)）。

...
這個架構的副作用是：**單次完整 index 速度快**（README 聲稱 Linux kernel 3 分鐘），但**增量更新採背景 git polling**（IDE-style file watcher 是即時觸發、cbm 是定期掃描），對「邊改邊查」的工作流會有秒級延遲。
...
表格列了 9 個明確 signal、README 另說有 11 個（剩 2 個是 implementation detail 沒公開細節）。實務上 11-signal 的價值在於**幾個高權重 signal 各自負責不同 query 類型**——權重分配有明顯的高低差：
...
cbm 提供的 `query_graph` 是 Cypher 的**真子集**——覆蓋大部分 read-only query 語法、省略 mutation 與部分 aggregation 語法：

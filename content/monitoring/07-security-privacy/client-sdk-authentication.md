...
這個定位在密碼學上有對應的名字：金鑰隨產出物發佈時，機制提供的是 [混淆](/backend/knowledge-cards/obfuscation/) 而非機密性，它成立的前提是 collector 端另有把關，也就是它只作為 [縱深防禦](/backend/knowledge-cards/defense-in-depth/) 的外層。選型層的完整判斷標準見 [7.28 金鑰位置決定對抗對象](/backend/07-security-data-protection/cryptographic-primitive-selection/#金鑰位置決定對抗對象)。

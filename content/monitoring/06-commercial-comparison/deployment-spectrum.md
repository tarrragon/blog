...
| Vercel Hobby       | 100GB bandwidth、10s function timeout、100K invocations/月 | 攢批 100 筆/request 可達 10M events/月、timeout 對 ingestion 足夠             |
...
**Background job 限制**：Downsample 和 purge 需要外部 cron。Vercel Cron 免費方案每天只能觸發 1 次 — 如果需要每小時 downsample，要用 Supabase pg_cron（Free 方案支援）或外部 scheduler。

**免費額度耗盡**：500K function invocations/月 ≈ 每天 16K requests。如果每個 request 攢批 100 筆事件，可處理每天 160 萬筆事件。超過後進入按量付費。
...
Railway 和 Fly.io 都支援 persistent volume（SQLite 的 WAL 檔案需要持久化）。Render 的免費方案沒有 persistent disk — SQLite 在每次 deploy 後重置，不適合需要保留歷史事件的場景。

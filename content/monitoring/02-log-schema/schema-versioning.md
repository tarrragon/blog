...
這個結論的作用範圍是 collector 的驗證層：新事件名不會讓驗證失敗，因此不需要改版。**下游的查詢與聚合是另一回事**——依賴「事件名稱有哪幾種」的儀表板、分類邏輯與彙總，會在新名稱出現時靜默漏掉它，而漏掉不產生任何錯誤。這類失效的判讀與處置（catch-all 分支、總量守恆檢查）見 [#250 資料多出一種形狀時，既有分析邏輯靜默換語意](/report/new-data-shape-silently-changes-analysis/)。

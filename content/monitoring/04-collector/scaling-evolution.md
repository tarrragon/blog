...
type BasicStorage interface {
    Store(events []Event) error  // 批次寫入，單筆場景傳 []Event{event}
...
`Downsample` / `Purge` 接受時間切點參數 — 保留策略（保留幾天、何時聚合）由排程層計算，storage 只負責執行。這讓不同 backend 共用同一套 retention 排程邏輯。

### 能力偵測的 HTTP 介面

Type assertion 解決了 collector 內部的能力判斷。當 dashboard 是獨立前端、或有外部工具想使用分析能力時，能力資訊需要一個 HTTP 出口：
...
分析能力本身走獨立的 endpoint 群（如 `/v1/analytics/aggregate`、`/v1/analytics/funnel`、`/v1/analytics/cohort`），與基礎查詢 `/v1/events` 分開 — 兩者的可用性由 backend 決定，路徑分群讓「這一群在當前 backend 不可用」有清楚的表達方式。

SQLite 模式下呼叫 analytics endpoint 回 `501 Not Implemented` 加 JSON error body，而非 404：endpoint 存在、路由有註冊，只是當前 backend 不提供這個能力。501 讓呼叫端能區分「打錯路徑」和「backend 不支援」，前者該修 client，後者該查 capabilities 或換 backend。外部工具的正確使用順序是先查 `/v1/capabilities` 決定要不要顯示分析入口，和 dashboard 的 type assertion 是同一個原則的兩種載體：能力不足時隱藏入口，優於顯示入口後報錯。
...
原始事件的保留期到期後，需要決定如何保留歷史統計。降採樣有兩種思路。**抽樣保留**是同事件名稱（`name` 欄位）同小時保留一筆原始事件、刪除其餘，保留了逐筆查詢能力但喪失準確計數。**聚合摘要**是把一小時內的事件壓成一筆計數記錄，喪失逐筆細節但保留準確統計。
...
- 水平擴展的基礎概念 → [運行期維運 水平擴展](/operations/02-horizontal-scaling/)
- Error fingerprint 的 DDL 擴充 → [Error Fingerprint 與去重分群](/monitoring/04-collector/error-fingerprint/)

...
| ---------------------- | ---------------------------------------------------------- | -------------------------------- |
| **Funnel 分析**        | 需要 JOIN 同 session 的多個事件、跨所有 session 計算轉換率 | Window functions + 高效 JOIN     |
...
| **高併發寫入**         | 多個 SDK 同時 flush                                        | 連線池 + 並行寫入                |
...
需要高併發寫入（多個 SDK 同時 flush > 100 req/s 持續） → PostgreSQL
以上都不需要 → SQLite 足夠
```
...
SQLite implementation 的 `Funnel` 和 `Cohort` 方法回傳 `ErrNotSupported`。Dashboard domain 根據 storage 的能力動態決定顯示哪些視圖 — SQLite 模式下 funnel/cohort 入口不顯示（而非顯示後報錯）。

...
本系列所有 endpoint 範例都帶 `/v1` 前綴（`/v1/events`、`/v1/events/summary`）。這個前綴是一個從第一天就該做的設計決策，值得說清楚它的依據。

### /v1 是保險，不是 v2 的規劃

版本前綴的意義常被誤解為「已經在規劃 v2」。它的真正角色是一份希望永遠用不到的保險：版本標記唯一無法事後補加的時機就是一開始。API 上線時若是 `/events`，之後要做破壞性變更，得同時發明版本機制並遷移所有消費端，兩件難事疊在一起；一開始就是 `/v1/events`，破壞性變更的逃生門已經存在，付出的成本只是 URL 多四個字元。這是一個成本高度不對稱的賭注 — 預留的代價趨近零，不預留的潛在代價極高。

成熟的 API 設計會盡量讓 v2 永遠不發生。v2 的真實成本在程式碼之外：同時維護兩個 API 面、消費端遷移期沒有明確終點（總有 client 不升級）、文件和測試翻倍。因此演化的主要路徑是在 v1 內做加法變更 — 只加欄位不改既有欄位語意、只加 endpoint 不改既有 endpoint 行為，搭配消費端的 tolerant reader（忽略不認識的欄位）。這和 [Schema 版本演進策略](/monitoring/02-log-schema/schema-versioning/)是同一個原則在兩個層次的投影：schema 層管 payload 契約的演化，API 層管 endpoint 面的演化。

### 版本化方案對照

| 做法               | 代表                  | 特點                                                    |
| ------------------ | --------------------- | ------------------------------------------------------- |
| URL 前綴 `/v1/`    | 多數 REST API         | 直白、curl 可見、cache 友善；粒度粗 — 整個 API 一起升版 |
| 日期版本（header） | Stripe                | 每個帳戶釘選一個日期，破壞性變更以日期切分，粒度細      |
| Accept header      | GitHub（早期）        | URL 乾淨，但對使用者不直觀                              |
| 不做版本           | GraphQL、多數內部 API | 純加法演化 + 欄位棄用標記；消費端可控時最省             |

### 判斷準則：消費端的可控程度

要不要放版本前綴，取決於你能不能控制所有呼叫方的升級節奏。內部 API（同一個團隊、同一次部署升級所有消費端）常直接省略版本；公開或多消費端 API 則應該放。

Monitoring 系統落在光譜的極端：SDK 隨著使用者的 app 發佈，散落在無數個裝置上，collector 完全控制不了它們什麼時候升級 — 這是「消費端不可控」的最極端情況，版本前綴是必要配備。反過來說，後續為 collector 增加新能力（例如聚合分析的 endpoint）時，沿用同一個 `/v1` 前綴就好 — 加新 endpoint 是能力擴充，不是契約破壞，不構成升版的理由。

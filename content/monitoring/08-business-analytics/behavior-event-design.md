...
設計好的行為事件直接成為 [Funnel analysis](/monitoring/08-business-analytics/funnel-analysis/) 的輸入 — funnel 的每一步對應一個行為事件。行為事件在四類事件分類中屬於 Event 類，完整的分類定義見[模組一 四類事件定義](/monitoring/01-mental-model/four-event-types/)。收集行為事件前必須完成[去識別化](/monitoring/07-security-privacy/) — 使用者行為模式本身就是可識別資訊。

...
[畫面狀態矩陣](/ux-design/knowledge-cards/screen-state-matrix/)（[ux-design 模組一](/ux-design/01-screen-state-machine/)）描述每個畫面的狀態和轉換。Funnel 描述使用者跨畫面的操作路徑。兩者的對應是：funnel 的每一步通常對應一個畫面狀態的進入事件。

...
Serverless 發布通常以函式版本為單位，並透過 [Function Alias](/ci/knowledge-cards/function-alias/) 或流量權重切換。部署步驟看起來短，但對權限、[Event Source](/ci/knowledge-cards/event-source/)、重試政策與 observability 欄位要求很高。
...
| 章節                                                                         | 主題                       | 核心責任                           |
| ---------------------------------------------------------------------------- | -------------------------- | ---------------------------------- |
| [Serverless function 版本、事件來源與回復流程](function-version-event-flow/) | Function version and event | 管理版本別名、事件來源、權限與回復 |

## 下一步路由

- Serverless 發布主流程：讀 [Serverless function 版本、事件來源與回復流程](function-version-event-flow/)。

...
| [Backfill](/ci/knowledge-cards/backfill/) | 歷史資料補算與節流                       | 是否有 checkpoint 與停損條件 |
| Recovery                                  | rerun、rollback、forward fix             | 異常資料是否可修補           |

## 常見注意事項

- schema 變更要先定義相容窗口，再切換 downstream。
- [Backfill](/ci/knowledge-cards/backfill/) 要有節流與 [Checkpoint](/ci/knowledge-cards/checkpoint/)，避免壓垮上游與儲存層。
...
| 章節                                                                                | 主題                           | 核心責任                     |
| ----------------------------------------------------------------------------------- | ------------------------------ | ---------------------------- |
| [Data pipeline backfill、checkpoint 與 rerun 流程](backfill-checkpoint-rerun-flow/) | Backfill, checkpoint and rerun | 控制歷史補算、重跑與資料修復 |

## 下一步路由

- Data pipeline 發布主流程：讀 [Data pipeline backfill、checkpoint 與 rerun 流程](backfill-checkpoint-rerun-flow/)。

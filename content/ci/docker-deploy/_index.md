...
Image 部署常出現在後端、worker、batch job 與自架服務。它把「在哪個環境跑」前移到 build 階段，但也引入 registry、tag、base image、vulnerability scan、[SBOM](/ci/knowledge-cards/sbom/) 與 promotion 流程（platform 概念可對照 [Container](/backend/knowledge-cards/container/)）。
...
| Scan     | vulnerability、secret、[SBOM](/ci/knowledge-cards/sbom/) | 是否有阻擋門檻與例外流程             |
...
Build 階段負責把 application 與 runtime 封裝成 image。Multi-stage build、dependency cache、base image 與 layer 順序會影響速度、安全性與可重現性（指令與 layer 機制見 [Dockerfile 設計](/backend/05-deployment-platform/vendors/docker/dockerfile-design/)、cache mount 與跨平台見 [BuildKit 與跨平台 build](/backend/05-deployment-platform/vendors/docker/buildkit-cross-platform/)）；CI 應能從 Dockerfile 與 lockfile 重建同一類產物。
...
- Build secret 不應留在 image layer（用 [BuildKit secret mount](/backend/05-deployment-platform/vendors/docker/buildkit-cross-platform/)、不用 ENV / COPY）。
...
| 章節                                                                      | 主題               | 核心責任                                   |
| ------------------------------------------------------------------------- | ------------------ | ------------------------------------------ |
| [Image build、scan、registry 與 promotion 流程](image-supply-chain-flow/) | Image supply chain | 建立可追溯 tag、掃描 gate 與 registry 推進 |

## 下一步路由

- Image 供應鏈流程：讀 [Image build、scan、registry 與 promotion 流程](image-supply-chain-flow/)。

...
第三步 caelestia、階段四～五環境準備、階段六第二次復現的發現統一收在這張表。狀態欄的「已回寫」都逐一對過教材原文；「未回寫」附候選落點，是目前僅存的回寫債。
...
| AUR 建置失敗三型態（`-bin` 半衰期 / sysconfig 烤入路徑 / 架構宣告） | 機制 | AUR 卡明示為獨立主題、判讀鏈已進 linux-install-debug skill；content 側缺獨立篇                     | backlog  |
...
| Widevine 兩段式 hint 註冊時序 + `/proc/<pid>/maps` 判讀             | 機制 | 候選 `install/gui-apps-install-verify.md` 或獨立 work-log                                          | 未回寫   |
...
| sshd_config 改壞先 `sshd -t` 驗證再 restart                         | 判讀 | 候選 `debug/ssh-and-terminal-troubleshooting.md`                                                   | 未回寫   |
| 宿主側：UTM console 長輸出 crash 導檔避法、macOS Local Network 權限 | 環境 | 候選 `debug/machine-unreachable.md`（No route to host 一節）                                       | 未回寫   |

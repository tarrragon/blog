...
| VMware Fusion             | 個人免費，但對 ARM Linux guest 的 Wayland + GPU 加速支援比 UTM 弱、VirGL 路徑不完整，容易出狀況。               |
...
**退 ISO 的常見誤區**：裝完選 Poweroff（不是 Reboot，Reboot 會繞回 archboot ISO）。關機後 UTM 資訊面板的 CD/DVD 顯示「(無)」= ISO 已退，再開機就從硬碟 GRUB 進新系統。GRUB 選單出現 `Arch Linux` 項 = ISO 退乾淨、bootloader 裝對。
...
字型踩到第一個實測意外。教材範例（`desktop-shell-components.md`、`color-system-theming.md`）裡 waybar / mako / hyprlock 的字型都寫 `JetBrainsMono Nerd Font`，但 VM 照 `packages-arch.txt` 裝的是 `ttf-meslo-nerd`，實際字族名是 **MesloLGS Nerd Font**：
...
字族名對不上的後果是**狀態列 icon glyph 直接變豆腐方塊**，沒有 fallback 成別的字這回事——Nerd Font 的圖示字元落在私有區（PUA），只有那支字有；指定一個沒裝的字族，Pango 找不到 PUA glyph 就畫不出 icon。所以所有 rice config 的字型統一改成 `MesloLGS Nerd Font`（實裝字族）。**回寫教材**：範例字型要嘛改成 `ttf-meslo-nerd` 對應的 `MesloLGS Nerd Font`，要嘛在 `packages-arch.txt` 改裝 `ttf-jetbrains-mono-nerd`，兩邊得對齊，不能教材寫一套、套件清單裝另一套。

#### 部署陷阱：stow 的 tree folding / unfolding
...
但測試 hyprlock 踩到這趟最嚴重的意外，根因很有教材價值：

`pkill hyprlock` 的實際效果是讓 compositor 掉進**鎖屏失效保護畫面**——client 死了、鎖還在 compositor 手上。
...
`LockedHint=no`（logind 層）會讓人以為已解鎖，但 compositor 層的 `ext-session-lock` 還鎖著——這個假象很容易誤導。
...
第二步用手動拼裝（waybar / wofi / mako / hyprlock）證明了各元件在 VM 上能獨立 render。第三步的目標是測試預組裝桌面 shell——用 Caelestia（基於 Quickshell 框架）取代手動拼裝的元件，驗證教材三篇（overview / installation / configuration）的準確性，並挖出 VM 環境特有的意外。
...
- **`qs` 的 process 名不是 `quickshell`**：`/usr/bin/qs` 是 `/usr/bin/quickshell` 的 symlink，用 `qs` 啟動時 process 的 comm 是 `qs`。`pgrep quickshell` 會找不到、誤判成 crash——要用 `pgrep -f "qs "` 或 `pgrep -x qs`。這是判讀 quickshell 是否活著的陷阱。
...
- **`loginctl` 無 `LockedHint`**：`ext-session-lock` 是 **Wayland 合成器層**的鎖，跟 **logind 的 session 鎖**是兩套獨立機制。查 `LockedHint` 查的是 logind 那套，對 Wayland 協議鎖本來就查不到——是查錯層，不是沒鎖。
...
caelestia 的招牌功能是 `dynamic` 配色：**從當前桌布圖片即時抽出一整組 Material-3 tonal palette**，色盤跟著桌布走、沒有預設色這一層。實測這條在 virtio-gpu VM 上完整運作。
...
**兩個 SSH 終端機故障（回寫教材重點，已寫進 ssh-keyless-bootstrap）**：裝了 p10k + plugins 後，從 Ghostty SSH 進 VM 打字變「一字母重複多次」的累加亂碼。陽春 shell 之前正常、是因為沒 unicode 撐得住壞掉的 locale/terminfo。
...
排查教訓：兩個故障都在裝了 unicode 重的 shell 之後才浮現，先懷疑「終端機環境層（locale/terminfo）」而不是「剛裝的 shell 配置壞了」，能省很多時間。
...
換到另一台 Mac（第一台的 VM 留在原宿主機），從零重走一次「裝 UTM → archboot 裝 Arch ARM → clone dotfiles → install.sh」，目的是驗證整套 dotfiles 能不能在全新環境一鍵復現。第一輪（step 1-3）全程互動式操作，這輪改成 SSH 非互動驅動——同一份腳本換一種執行情境，意外就換一批浮出來。
...
- **兩個 SSH 終端機故障**（macOS 送非法 LC_CTYPE → locale fallback；Ghostty 的 xterm-ghostty terminfo 新機沒有 → ZLE 重繪亂碼）→ 寫進 [ssh-keyless-bootstrap](/linux/install/ssh-keyless-bootstrap/) 的終端機故障排查段。
...
| stow tree folding/unfolding          | 機制 | `knowledge-cards/gnu-stow.md`                                                                 | 已回寫   |
...
第三步 caelestia、階段四～五環境準備、階段六第二次復現的發現統一收在這張表。狀態欄的「已回寫」都逐一對過教材原文，全表已無未回寫項。

| 發現                                                                | 類別 | 回寫落點                                                                                           | 狀態     |
| ------------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------------------- | -------- |
| `qs` 的 comm 是 `qs`、`pgrep quickshell` 誤判成掛掉                 | 判讀 | `debug/process-service-state-diagnosis.md`                                                         | 已回寫   |
| build 中斷先查資源（磁碟寫滿）、不是先疑平台相容性                  | 判讀 | `debug/diagnosis-read-authoritative-state.md`                                                      | 已回寫   |
| 通知 daemon 獨佔 D-Bus name、busctl 查擁有者是 ground truth         | 機制 | `debug/process-service-state-diagnosis.md`、`06-rice-design/desktop-shell-components.md`           | 已回寫   |
| session lock 兩層：合成器 `ext-session-lock` ≠ logind `LockedHint`  | 機制 | 新卡 `knowledge-cards/session-lock.md` + `debug/process-service-state-diagnosis.md`                | 已建卡   |
| lock-died 復原（`allow_session_lock_restore` + 接管 client）        | 機制 | `knowledge-cards/session-lock.md` + `07-desktop-maintenance/common-failures-recovery.md`           | 已回寫   |
| scheme 熱套用的啟動時序前提（watcher 只在檔案已存在時建）           | 配置 | `06-rice-design/caelestia-configuration.md` 配色段                                                 | 已回寫   |
| config watcher 與 scheme watcher 是兩個獨立機制                     | 機制 | `06-rice-design/caelestia-configuration.md`                                                        | 已回寫   |
| 假重啟：驗證重啟看 pid + 起始時間、不是指令沒報錯                   | 判讀 | `06-rice-design/caelestia-configuration.md` 維運段                                                 | 已回寫   |
| caelestia vs 手動拼裝的實測對照（足跡 / 失敗半徑 / 配色一致）       | 決策 | `06-rice-design/integrated-shell-vs-manual-assembly.md`                                            | 已回寫   |
...
| runtime 寫入穿透 folded symlink 弄髒 repo（gitignore 配套）         | 機制 | `06-rice-design/caelestia-configuration.md` 維運段                                                 | 已回寫   |
| kmscon 取代 VT getty、`chvt` 無效的三層判讀                         | 機制 | 新卡 `knowledge-cards/compositor.md`（判讀鏈另進 linux-install-debug skill）                       | 已建卡   |
| pipewire 在、wireplumber 不在的缺件症狀與權威判讀                   | 環境 | `install/gui-apps-install-verify.md`、`05-hyprland-config/hyprland-installation.md`                | 已回寫   |
| Spotify 架構牆：套件存在性差異的 CPU 架構軸                         | 環境 | `install/platform-divergence-map.md` 存在性段                                                      | 已回寫   |
| Widevine 兩段式 hint 註冊時序 + `/proc/<pid>/maps` 判讀             | 機制 | `install/gui-apps-install-verify.md` 兩段式註冊段                                                  | 已回寫   |
| 裝了 ≠ 自動啟動：`exec-once` 唯一機制 + waybar/mako 互斥            | 配置 | `06-rice-design/caelestia-installation.md`                                                         | 已回寫   |
| VLC 拆包兩層 / 首跑同意對話框 / 播放驗證鏈                          | 環境 | `install/gui-apps-install-verify.md`                                                               | 已回寫   |
| sshd_config 改壞先 `sshd -t` 驗證再 restart                         | 判讀 | `debug/machine-unreachable.md` 判讀路由（Connection refused 條）                                   | 已回寫   |
| 宿主側：UTM console 長輸出 crash 導檔避法、macOS Local Network 權限 | 環境 | `debug/machine-unreachable.md`（發起端權限段 + 虛擬機宿主側段）                                    | 已回寫   |
| idle 設定本機改未回 repo 的 dotfile 漂移案例                        | 案例 | 已以「寫回 repo shell.json」收斂；漂移原則教材已有覆蓋                                             | 已有覆蓋 |

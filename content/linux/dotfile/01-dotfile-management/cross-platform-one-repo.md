...
本文的做法基於 GNU Stow——將 dotfile repo 的檔案透過 symlink 對應到家目錄的工具（完整說明見[管理策略與選型](/linux/dotfile/01-dotfile-management/management-strategies/)）。一個 repo 跨平台的做法是三層分離，每層處理不同粒度的差異：

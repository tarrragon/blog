...
Hyprland 的安裝分三層：compositor（負責視窗管理與畫面合成的核心程式，詳見 [Wayland 顯示協議](/linux/dotfile/04-window-management/wayland-explainer/)）本身、GPU 驅動、桌面配套工具。Compositor 只管視窗排列和畫面輸出，其餘功能（status bar、launcher、通知、音訊、藍牙、網路）都需要另外裝。本篇以 Arch Linux 為主要說明對象，其他發行版在最後簡述。
...
GPU 驅動相關的故障診斷（driver hang、畫面凍結、suspend/resume 後異常）見[桌面環境維護與故障排除](/linux/dotfile/07-desktop-maintenance/)。
...
本系列後續文章（[Hyprland 核心配置](/linux/dotfile/05-hyprland-config/hyprland-core-config/)、[Workspace 與外觀](/linux/dotfile/05-hyprland-config/workspace-rules-appearance/)、[Caelestia](/linux/dotfile/06-rice-design/caelestia-overview/)）都使用 Lua 格式。如果在網路上看到 `.conf` 格式的教學，多數仍然可用，但建議儘早遷移到 Lua。
...
詳細的配置語法和設定邏輯在[核心配置](/linux/dotfile/05-hyprland-config/hyprland-core-config/)和 [Workspace、Window Rules 與外觀](/linux/dotfile/05-hyprland-config/workspace-rules-appearance/)。
...
安裝完成後，VM 環境的額外設定（環境變數、效能調整、測試矩陣）見 [VM 環境設定與測試矩陣](/linux/dotfile/05-hyprland-config/hyprland-vm-setup/)。配置檔的組織方式和 keybind 設計見 [Hyprland 核心配置](/linux/dotfile/05-hyprland-config/hyprland-core-config/)。

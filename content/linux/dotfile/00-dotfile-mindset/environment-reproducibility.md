...
這個判斷標準管的是配置該放哪個 repo。有個相鄰但不同的失敗要另外注意：**一個全域使用的工具，它的來源 / binary 若寄居在某個別專案的 checkout 裡**（例如把 CLI 用 `--from ~/project/foo/tools/bar` 這種本地路徑安裝），它就有一條隱藏的 sibling-project 依賴——換機器時那個專案不在，工具就裝不回來，重建鏈斷在那個 checkout。全域工具的來源要能獨立取得（發佈的套件、獨立 repo、或收進 dotfile），不綁在另一個專案的目錄上。

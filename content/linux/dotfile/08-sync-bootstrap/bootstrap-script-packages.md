...
**權限前綴要偵測、不要寫死 `sudo`**：上面範例的 `sudo pacman` / `sudo apt` 在兩種相反的機器上會壞。一種是有非 root 使用者但沒裝 sudo（剛裝好的 VM）——解法是補齊：裝 sudo、把使用者加進 wheel。另一種相反：**以 root 起手、且沒有 sudo**（裸 container base image 的常態）——這時裝 sudo 是多餘的，root 本就有權限，但腳本裡的 `sudo` 會直接回 `sudo: command not found`（exit 127），在裝任何套件前就掛掉。要讓同一支腳本兩邊都跑得動，把權限前綴做成偵測 root 的變數：

```bash
# root 時把 sudo shim 掉（root 本就有權限、且容器常無 sudo）；非 root 才用 sudo
SUDO=sudo; [ "$(id -u)" -eq 0 ] && SUDO=""
$SUDO pacman -Syu --noconfirm
```

這樣 root 容器直接跑、非 root 走 sudo，腳本不必先知道自己在哪種機器上。這是可攜性的一環：跟 [可除錯的 bootstrap](/linux/install/observable-bootstrap/) 的 ERR trap 互補——一個讓權限前綴跨機器可攜、一個讓真的掛了時看得出掛在哪一行。

**別硬編「你這台剛好有」的東西**：`sudo` 只是這類假設的一個。最常見的下一個是**跨作業系統的工具不一定在**——GNU coreutils 的 `timeout` 在 macOS 預設沒有（要 `gtimeout`、或缺席），`sed -i` / `readlink -f` 在 BSD 行為也不同；腳本要偵測擇一、沒有就退回不依賴它，或只用兩邊都有的 POSIX 子集（跨平台工具分歧見 [平台差異地圖](/linux/install/platform-divergence-map/)、macOS 的 BSD userland 逐工具差異見 [macOS 的 BSD userland](/macos/macos_bsd_userland_vs_gnu/)）。另一個完全不同維度的假設在**編碼**：`$var` 後面若緊跟一個多位元組字元（例如中文全形括號 `）`），多位元組解析不夠健全的 bash 可能把該字元的首位元組吞進變數名、報 unbound——實測 macOS 的 bash 3.2 在 UTF-8 locale 下、腳本裡寫 `"…（$logf）"` 就這樣掛；一律用 `${var}` 大括號標清楚變數名邊界就免疫（跟 bash 版本、locale 無關）。這幾個都是「在作者機器上跑得動、換一台就爆」的假設：寫的時候看不出來，只有在跟作者環境不同的機器上實跑才現形——所以 bootstrap 腳本要在真正乾淨、且涵蓋目標作業系統的環境各跑一次，不是在自己機器上跑過就算數。
...
**pin runtime 版本時要同時滿足兩個約束**：一是對外的（跟 prod 對齊 / 跟團隊對齊，見 [prod-parity](/linux/dotfile/10-prod-parity/)），二是對內的——script 自己要裝的工具鏈可能要求更新的 runtime。實例：pin 了某語言 runtime 的舊版，但 script 接著要裝的該語言工具（linter / language server）要求更新的最低版本，於是 pin 太舊反而讓自裝工具裝不起來。兩個約束可能衝突，pin 的版本要同時過這兩關。

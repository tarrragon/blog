...
**別硬編「你這台剛好有」的東西**：`sudo` 只是這類假設的一個，同一個陷阱還有兩張臉。其一，**跨作業系統的工具不一定在**：GNU coreutils 的 `timeout` 在 macOS 預設沒有（要 `gtimeout`、或缺席），`sed -i` / `readlink -f` 在 BSD 行為也不同——腳本要偵測擇一、沒有就退回不依賴它，或只用兩邊都有的 POSIX 子集（跨平台工具分歧見 [平台差異地圖](/linux/install/platform-divergence-map/)、macOS 的 BSD userland 逐工具差異見 [macOS 的 BSD userland](/macos/macos_bsd_userland_vs_gnu/)）。其二，**locale 相關的解析**：shell 變數 `$var` 後面若緊跟一個多位元組字元（例如中文全形括號 `）`），在非 UTF-8 locale 下 bash 可能把該字元的首位元組吞進變數名、變成 `$var` 加一個雜訊 byte 而報 unbound——實測一支腳本在中文字串裡寫 `"…（$logf）"` 就這樣掛。修法是一律用 `${var}` 大括號把變數名的邊界標清楚。這幾個都是「在作者機器上跑得動、換一台就爆」的假設：寫的時候看不出來，只有在跟作者環境不同的機器上實跑才現形——所以 bootstrap 腳本要在真正乾淨、且涵蓋目標作業系統的環境各跑一次，不是在自己機器上跑過就算數。

...
判斷標準是「這一項換 distro 會不會變」：會變的（套件名、安裝指令）進 detection 分支；不會變的（config 內容、symlink 邏輯、框架 clone）留共通層。一個常見的誤置是把套件名寫死在共通層，結果換 distro 就壞——正確做法是共通層只呼叫抽象後的安裝函式，具體套件名下沉到各平台清單。

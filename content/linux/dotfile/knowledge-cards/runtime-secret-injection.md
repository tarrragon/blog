...
- **env 注入自身也有曝露邊界**：runtime 注入解掉「進 image / 進 git」的問題，但環境變數不是零曝露——同機上的程序讀得到自己的 `/proc/<pid>/environ`、`docker inspect` 讀得到用 `-e` 傳的值。所以「注入」降低的是持久化與散佈風險，不是同機讀取風險；跑在容器裡、有網路的不受信任程式仍可能讀到並外送，長效機密要短期輪替、不長放。

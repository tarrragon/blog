...
SIGHUP 的另一個慣例用途是「請 daemon 重讀設定」——`nginx`、`sshd` 這類長駐服務收到 SIGHUP 不是終止而是 reload。同一個訊號在前景互動程序與 daemon 上語意不同：前者預設終止、後者常被程式主動接管成「重載」。這張卡講的是前者（斷線殺前景任務）。系統層「服務怎麼被叫醒 / 重載」另見服務管理主題。相鄰概念見 [TCP 連線與漫遊](/linux/dotfile/knowledge-cards/tcp-connection-roaming/)（連線本身怎麼斷）。

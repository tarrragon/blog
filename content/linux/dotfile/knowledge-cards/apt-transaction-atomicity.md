...
apt 的安裝是一筆交易（transaction）：`apt-get install a b c` 會先把所有指定套件連同它們的依賴一起解析成一份完整的安裝計畫，全部解析成功才開始下載安裝；只要其中一個套件解析不到（名字不存在、沒打包、依賴衝突），整筆交易放棄，一個都不裝。apt 是 [Package Manager 抽象層](/linux/dotfile/knowledge-cards/package-manager-abstraction/) 底下各發行版套件管理員之一，這個交易行為是 apt 特有的實作細節。

...
相關概念：[UEFI 開機鏈](/linux/dotfile/knowledge-cards/uefi-boot-chain/)、[分區識別](/linux/dotfile/knowledge-cards/partition-identification/)。安裝時 ESP 大小怎麼估，見 [Linux 安裝選項判讀](/linux/install/install-option-decisions/)。

因為它要跟 kernel 一起被 bootloader 載入，所以它跟 kernel 放在同一個地方——[UEFI 開機鏈](/linux/dotfile/knowledge-cards/uefi-boot-chain/)裡的 ESP，或獨立的 `/boot`。這也是為什麼估 ESP 大小時要把它算進去：一個 kernel 加上它的 initramfs（含 fallback 版本）大約一兩百 MB。

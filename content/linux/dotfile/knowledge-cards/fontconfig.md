...
`fc-list` 和 `fc-match` 每次執行都是新 process，讀到的是當下最新的系統快取。這跟已啟動的長駐程式不同——長駐程式的字型清單是啟動時的快照，詳見 [font-availability-at-startup](/linux/dotfile/knowledge-cards/font-availability-at-startup/)。
...
`fc-cache -f` 的 `-f` 是 force，忽略時間戳全部重建；不加 `-f` 只更新有變動的目錄。兩者都只動系統快取層——已啟動的 process 記憶體中的字型清單不受影響，那是另一個層級的問題（見 [font-availability-at-startup](/linux/dotfile/knowledge-cards/font-availability-at-startup/)）。

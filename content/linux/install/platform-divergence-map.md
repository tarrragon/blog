...
title: "平台與發行版差異的判讀地圖"
date: 2026-07-02
description: "跨 macOS / Linux 或跨發行版寫 bootstrap、或除錯時不確定該用哪個工具與套件名時回來讀"
...
Hyprland 在 Arch 官方 repo、Fedora 要 COPR、Debian stable 沒有；Quickshell 只有 Arch 打包。反過來，macOS 的 cask app（GUI 應用程式）概念在 Linux 對應的是各桌面環境自己的生態。這層差異沒有翻譯的空間——桌面層的清單天生就是平台專屬的。

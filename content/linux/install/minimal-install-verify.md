...
這篇的驗證跟 [模組八的 bootstrap script 設計](/linux/dotfile/08-sync-bootstrap/bootstrap-script-packages/) 是兩件互補的事，界線在「假設」上。bootstrap script 的套件清單假設一個前提：機器已經有能力執行安裝（有 sudo、有 package manager、清單裡的東西都能被裝上）。這篇處理的正是那個前提成立之前的階段——最小系統到底有沒有滿足那些假設，缺的補上，讓 bootstrap 的假設變成事實。

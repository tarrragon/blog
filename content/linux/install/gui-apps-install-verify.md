...
瀏覽器的外掛元件系統（DRM 模組、部分 codec 元件）有一個容易誤判成「裝壞了」的時序：元件裝進系統路徑後，第一個瀏覽器實例只做「發現預裝元件、寫下 hint 檔」，真正把元件註冊成可用要等**下一次啟動**。實測 Widevine CDM（Spotify / Netflix 類 DRM 串流的解密模組）：裝完後第一個 Chromium 實例照樣播不了，開 log（`--enable-logging=stderr --v=1`）看到的是元件發現與 hint 寫入；重啟後才出現 `Registering hinted Widevine`、串流開始工作。「裝完 DRM 元件要重啟瀏覽器」的機制層原因就是這個兩段式設計。

驗證「元件到底載入了沒」的權威位置在行程的記憶體映射：

```bash
grep widevinecdm /proc/$(pgrep -f chromium | head -1)/maps
# 有輸出 = 元件已被 dlopen 進行程；空 = 還在第一段、重啟瀏覽器
```

這個判讀把「元件沒裝對」跟「元件裝對了但還沒註冊」拆開：檔案在系統路徑上、maps 裡沒有映射，就是時序問題而非安裝問題，重啟即解，往安裝路徑跟權限查反而繞遠路。

...
weight: 2
tags: ["linux", "ssh", "terminal", "debugging"]
---
...
SSH「連不上」本身（`Permission denied`、`Host key verification failed`、`Connection refused`）的判讀與修復，見 [外部連入與無 key 的 bootstrap 路徑](../../install/ssh-keyless-bootstrap/) 的重連段落。這篇處理的是「連上了、但終端機或 session 的狀態不對」的那些情況。
...
第二類是**有些東西必須從實體圖形終端機（VT，即 `Ctrl+Alt+F1`~`F6` 切換的那些文字主控台）啟動，SSH 的 pty 起不來**。Wayland 的合成器（compositor，畫桌面、把視窗合成到螢幕、管輸入輸出的核心程式，如 Hyprland）需要一個真正的圖形 VT 上的登入 session，拿到 DRM master（對顯示卡的獨佔繪圖控制權）與 logind seat（一組綁在一起的實體螢幕／鍵鼠裝置）才能啟動；從 SSH 的 pty 起它會直接失敗（例如報 backend 建立失敗）。這不是設定問題，是它需要的資源在 SSH 這條連線上根本不存在。判讀訊號：合成器一啟動就報 seat / DRM / backend 相關的錯，而你是從 SSH 起的——那就是這個界線。
...
- SSH 連不上（拒絕 / host key / refused）→ 見 [外部連入與無 key 的 bootstrap 路徑](../../install/ssh-keyless-bootstrap/)。

這幾種分流的共同底線是先讀權威狀態（`locale`、`$TERM`、runtime 目錄、`loginctl`、`fgconsole`）再下判斷；背後的方法論見 [診斷心法](../diagnosis-read-authoritative-state/)。

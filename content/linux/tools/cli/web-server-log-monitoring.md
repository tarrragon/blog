...
本文承接 [終端機圖形化工具總覽](/linux/tools/cli/cli-graphical-tools-overview/) 的 TUI 工具脈絡，屬監控的 web 請求子題。但比起工具本身，更該先分清的是「什麼時候用終端機看請求、什麼時候不該」，這放在最後一節。
...
- 系統資源（CPU／記憶體／磁碟）的即時監控：[TUI 監控工具](/linux/tools/cli/tui-monitoring-tools/)。
- 把即時觀測擺進可持久化的多工器 pane：[tmux 基礎](/linux/tools/cli/tmux-persistence-and-basics/)。
- 這類工具在遠端工具分類中的定位：[終端機圖形化工具總覽](/linux/tools/cli/cli-graphical-tools-overview/)。

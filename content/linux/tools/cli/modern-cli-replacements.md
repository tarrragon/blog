...
最高價值的兩個內建整合：`Ctrl+R` 覆寫 shell 的歷史搜尋（模糊搜尋整個命令歷史，比預設的反向搜尋強太多）、`Ctrl+T` 把檔案路徑插進當前命令列。裝好 `fzf` 後在 shell 設定裡啟用它的 key bindings，這兩個是日常最有感的。`fd` 跟 `rg` 可以當 `fzf` 的預設來源，三者是一組。
...
| 原工具            | 替代品   | 解的痛點                                        |
| ----------------- | -------- | ----------------------------------------------- |
| `cd`              | `zoxide` | 記住常去的目錄，`z proj` 跳到最常用的 proj 目錄 |
...
**安裝**：多數在各發行版套件庫直接有（`pacman -S ripgrep fd fzf bat eza zoxide`）。它們該進你的 [dotfile 套件清單](/linux/dotfile/08-sync-bootstrap/bootstrap-script-packages/)，新機器 bootstrap 時一次裝齊。

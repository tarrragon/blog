...
| `du`              | `dust`   | 樹狀 + 視覺化長條，一眼看出誰佔空間                                                         |
| `df`              | `duf`    | 表格化、帶色、好讀的磁碟用量                                                                |
| `top`             | `btop`   | 全螢幕互動監控（詳見 TUI 監控篇）                                                           |
...
**別在腳本裡依賴它們**：這些工具是「你互動時」的省力工具，不是「腳本裡」的依賴。寫要在別台機器 / CI 跑的腳本時，用保證存在的 `grep` / `find` / `cat`，因為現代替代品不一定裝了。把它們留在你的互動 shell（alias、key binding），腳本走 POSIX 工具，兩邊分開。

**安裝**：多數在各發行版套件庫直接有。Arch：`pacman -S ripgrep fd fzf bat eza zoxide`。Debian / Ubuntu 有兩個要注意的改名坑——`fd` 的執行檔在 apt 上叫 `fdfind`、`bat` 叫 `batcat`（都是為了避免跟既有套件撞名），要自己 alias 回 `fd` / `bat`；`eza` 較舊的 apt 源可能還沒有，用 cargo 或官方 repo 裝。macOS 用 `brew install ripgrep fd fzf bat eza zoxide`。裝好後它們該進你的 [dotfile 套件清單](/linux/dotfile/08-sync-bootstrap/bootstrap-script-packages/)，新機器 bootstrap 時一次裝齊。

## 相關

- 這篇是「日常指令替代」；全螢幕 TUI 工具（監控、Git、檔案瀏覽、資料庫）見本系列其他文章。

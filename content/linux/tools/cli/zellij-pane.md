...
- 把 session 分享給沒有 SSH 連線的協作者（瀏覽器連入）：[Zellij Web Client 外網連線教學](/linux/tools/cli/zellij-remote-web-client/)。
- 純 SSH 的多工器持久化與 tmux 對照：[tmux 基礎](/linux/tools/cli/tmux-persistence-and-basics/)。
- 多工器在遠端工具選型中的定位：[終端機圖形化工具總覽](/linux/tools/cli/cli-graphical-tools-overview/)。

...
CLI 環境工具跟 tools 底下另外兩個子系列的分界：

| 情境                                       | 該看                                         |
| ------------------------------------------ | -------------------------------------------- |
| 純終端機下的指令替代與 TUI                 | 本系列                                       |
| 有圖形環境、要挑桌面 app（檔案管理員、DE） | [圖形桌面工具](../gui/)                      |
| 遠端操作、把 session 留在遠端不掉          | [遠端工具](../remote/)（多工器的遠端應用面） |

多工器（`tmux` / `zellij`）與檔案瀏覽（`broot` / `yazi`）這類工具兩邊都會提到：本系列講它們在純終端機下的用法與選型，遠端 / 圖形的情境應用則由 remote / gui 兩系列承接。這些工具的配置檔怎麼版控、跨機器同步，見 [Dotfile 管理](/linux/dotfile/) 的[終端機與編輯器](/linux/dotfile/03-terminal-ecosystem/)模組。

---

底下自動列出本系列的所有文章、依日期排序。

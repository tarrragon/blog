...
date: 2026-06-15
draft: false
description: "想讓遠端 session 脫離連線、不隨斷線消失、學 tmux 的 detach／reattach 與 window／pane 操作、或在 tmux 與 zellij 之間選時回來讀"
...
tmux 保住的是 session；連線本身的漫遊與自動重連是另一層，由 mosh / autossh 補——「mosh 還是 tmux」不是二選一，兩層常一起用（mosh 顧連線手感、tmux 顧 session 存活），見 [遠端連線與同步工具](/linux/tools/remote/connection-and-sync-tools/)。

...
工作流本身還在驗證階段時，client 端優先選免費工具、把付費決策延後。這是控制變數的延伸：[實作記錄](../agent-workstation-vm-handson/) 的 Step 9 已經把「這次用成熟現成 client 歸零變數」定為順序原則，而在成熟工具之內、免費又能滿足核心判準的優先——工作流沒驗通之前，不知道 client 的付費功能是不是真的需要，先付費是把未驗證的假設變成沉沒成本。

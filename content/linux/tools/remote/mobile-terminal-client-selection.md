...
mosh 漫遊是這三個裡的第一順位，因為它是 [連線層選型](../connection-and-sync-tools/) 把 mosh 放進技術棧的唯一理由。手機的網路狀態本來就在漂移——走出 Wi-Fi 範圍、app 被系統凍結、地鐵進出隧道——mosh 的 UDP 漫遊與本地回顯預測就是為這個設計。client 若不支援 mosh、只能退回純 SSH，斷線復原（[實作記錄](../agent-workstation-vm-handson/) 的 Step 10 情境二）就從「無感漫遊」降級成「重連再 attach」，體感差一個等級。
...
**擴充鍵列兩者都有、差在完整度與手感。** Blink 從設計上就服務重度鍵盤使用者，擴充鍵列與實體鍵盤映射完整；Termius 的鍵列可自訂、圖形化調整直覺。這一格的判讀不是「有沒有」而是「夠不夠用」：實測時走一遍 agent 操作——`Esc` 中斷一次生成、方向鍵翻 zellij 歷史、`Ctrl-C` 送中斷、`Tab` 補全——四個動作都順才算過關。
...
擴充鍵列之外還有一格常被忽略的輸入限制：**CJK 即時輸入**。本輪 Termius Android 實測，中文貼上能正常送出（編碼沒問題）、但終端裡輸入法切不到中文、無法即時組字——終端的 raw input 模式擋掉 IME 的組字流程，這是行動終端的普遍弱點、不是單一 client 的 bug，各 client 支援度要各自實測。繞法是「他處組字、貼進終端」或改用 ASCII 派任務。對要用中文下長指令的使用者、這格的權重不低——「能不能打中文」要細分成「能不能貼上」（多半可以）與「能不能即時組字」（多半不行）兩問。

**多 host 管理是 Termius 的長處、但對這個場景用不上。** Termius 的圖形化 host 管理與金鑰同步在管理機群時省事，但 agent 工作機只連一台 VM，這個長處在此場景兌現不出價值。選型時要抵抗「功能比較多所以比較好」的直覺，回到「這個場景會不會用到」。
...
- 本篇工具能力宣稱標「需實測」的項目，在裝置上驗過後回填結論、移除 draft。

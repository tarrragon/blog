...
- **mutagen**：**不在官方 repo**，且 `pacman -S mutagen` 會裝到完全無關的 `python-mutagen`（音訊 metadata 函式庫）。正確安裝是 AUR 的 `mutagen.io-bin`（`paru -S mutagen.io-bin`），提供 `mutagen` 執行檔，aarch64 有官方 binary。這是「以為一句 pacman 就有、實際會裝錯東西」的典型坑。

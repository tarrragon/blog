...
其二是 CJK 輸入：本輪派任務時發現手機在終端裡打不出中文、只能用英文描述任務——這是為什麼 agent 回報的三行是英文的系統事實、而非中文的架構重點（先前一度誤判成 agent 自行填補模糊、實際是輸入端就受限）。實測把問題定位得很乾淨：手機鍵盤有中文（其他 app 打得出）、終端也收得下中文（把中文**貼**進終端能正常送出、編碼無誤），卡的是**終端裡輸入法切不到中文**——終端的 raw input 模式擋掉了 IME 的語言切換與組字流程，所以無法即時打字，但不影響已組好的字貼進去。可靠繞法是「他處組字、貼進終端」（在筆記 app 打好中文、複製、貼上——實測有效），或直接用英文派任務（agent 認得英文）。這條把「行動端能不能派複雜任務」精確到終端能不能即時組字這一層：編碼與鍵盤中文都不是卡點、真正卡的是終端擋掉 IME 組字、要靠貼上或改用 ASCII——選型與使用形態要納入這一格。

...
下面各節按主題拆解每個工具解什麼問題。若你的目標是**從零把一整台遠端 agent 工作機架起來**、想要一條照順序走的路（先做什麼、後做什麼），先看 [把遠端 agent 工作機鋪成一條路](remote-agent-paved-road/)——它把本節的內容排成從選型到無人值守的任務序列，再往各主題的深入文章分流。
...
工具層之上還有一個上游問題：這台被連入的機器該放家裡還是租 VPS。延遲、浮動 IP、環境隔離三個常見顧慮各有軟體層的標準解、跟機器位置無關；拆完之後選型只剩「機器由誰保證活著」一個變數。已經有機器（家用機或既有 VPS）、只想把 agent 裝上去的，可跳過選型直接看實作記錄的連線 / session / 隔離三層。

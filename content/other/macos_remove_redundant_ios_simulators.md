...
description: "磁碟空間不足、發現裝了多版 iOS Simulator runtime 時的移除流程。透過 Xcode 或 simctl 指令移除，不要手動刪 ~/Library/Containers。"
tags: ["macos", "xcode", "ios-simulator", "disk-space", "troubleshooting"]
---

iOS Simulator runtime 每組約 16G（Bundle DMG + Runtime DMG），裝了兩個版本就佔 32G 以上。Flutter 和 Xcode 開發通常只需要最新版，舊版可以安全移除。移除的正確路徑是透過 Xcode 或 `simctl` 指令，不是手動刪除 `~/Library/Containers` 裡的檔案。
...
## 為什麼不能手動刪 ~/Library/Containers

模擬器的完整資料散在好幾個位置：

| 位置                                         | 內容             |
| -------------------------------------------- | ---------------- |
| `/Library/Developer/CoreSimulator/`          | Runtime 掛載點   |
| `/System/Library/AssetsV2/.../AssetData/`    | Runtime DMG 本體 |
| `~/Library/Developer/CoreSimulator/Devices/` | 模擬器裝置資料   |
| `~/Library/Caches/com.apple.dt.Xcode/`       | Xcode 建置快取   |

手動刪其中一處只會刪掉一部分，留下孤兒資料，讓 CoreSimulator 的索引跟實際檔案不一致。之後可能出現模擬器列表裡有裝置但開不了、或空間沒真正釋放的情況。`simctl runtime delete` 會同步清理所有相關位置。

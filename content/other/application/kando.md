...
`command` 型項目直接把指令交給作業系統執行，它本身不開終端機視窗。要「叫出 Ghostty 並在裡面跑腳本」，得用 Ghostty 的 `-e` 旗標開一個新視窗執行指定指令。下面這組放兩個需要看畫面的磁碟工具：`mole`（互動式清理 TUI）跟 [`disk-report`](/macos/macos_disk_space_diagnosis/)（印出硬碟空間報告，安裝方式見該篇）。

兩個寫法的共同骨架是 `ghostty -e zsh -lc "<腳本>; exec zsh"`：

- **`zsh -lc`**：用 login shell 執行，載入 `.zprofile` / `.zshrc` 的 PATH（[PATH 設定見新機基礎建設](/macos/macos_new_machine_setup/)），`mole` 跟 `~/.local/bin` 底下的腳本才解析得到。少了這層，Kando 走 `/bin/sh`、沒有自訂 PATH，會找不到指令。

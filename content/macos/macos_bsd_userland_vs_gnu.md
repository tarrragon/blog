...
date: 2026-07-09
description: "指令或腳本在 Linux 好好的、搬到 macOS 卻 command not found 或行為不同（timeout、sed -i、readlink -f、bash 語法），想理解 macOS 的命令列工具為何跟 Linux 不一樣、怎麼繞時回來讀"
tags: ["macos", "bash", "shell", "coreutils", "homebrew", "tooling", "cross-platform"]
---
...
- **`getopt` / `stat` / `grep -P` / `find`**：`getopt` BSD 版不支援長選項（`--foo`）、GNU / util-linux 版才支援，靠它解析長參數的腳本在 macOS 整個失效；`stat` 的格式旗標 GNU 是 `-c`、BSD 是 `-f`；`grep -P`（PCRE）GNU 有、BSD 沒有；`find` / `xargs` 也有零星旗標差異。
...
- **裝 GNU 工具（自己的互動環境）**：`brew install coreutils gnu-sed findutils grep gawk bash`。這些 formula 各自裝 **g-prefix** 版、刻意不覆蓋 BSD 原生（`coreutils` 給 `gtimeout` / `greadlink` / `gdate`、`gnu-sed` 給 `gsed`、依此類推）——不覆蓋是因為系統與其他軟體可能依賴 BSD 行為，蓋掉會讓那些元件出錯。要讓無前綴的名字也走 GNU 版，把 `$(brew --prefix)/opt/coreutils/libexec/gnubin` 加進 PATH（那裡有無前綴 symlink 指向 GNU 版；但這只涵蓋 coreutils 那批，`sed` / `grep` / `find` / `awk` 要各自加它們對應的 `gnubin`）。這條 PATH 讓你的 shell 跟系統其他部分的 BSD 假設分歧，所以它屬於「自己的互動環境」這格、別設成全域（具體會怎麼壞：有些 configure / autotools 腳本靠 `sed --version` 之類的字串判斷手上的 `sed` 是 GNU 還是 BSD、據此決定加哪些旗標，PATH 被 gnubin 覆蓋後這個判斷跟系統其他部分的預期不一致，可能在編譯期出岔）。新 bash 裝到 `$(brew --prefix)/bin/bash`（`brew --prefix` 在 Apple Silicon 是 `/opt/homebrew`、Intel 是 `/usr/local`），腳本要用 `#!/usr/bin/env bash`（走 PATH）才抓得到它，`#!/bin/bash` 仍是系統的 3.2。

...
Apple Silicon 的 CPU 架構（arm64）跟 iPhone/iPad 相同，所以 iOS App 的二進位可以直接在 Mac 上跑，不需要轉譯或模擬。macOS 提供一個相容層（Catalyst 的延伸），把 iOS 的觸控 API 對應到滑鼠和鍵盤，讓 iOS App 在 Mac 上有基本的操作體驗。

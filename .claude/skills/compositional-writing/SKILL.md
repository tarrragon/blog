...
**Version**: 1.3.0 — 新增 principle 卡 teaching-article-context：教材的脈絡由三件事決定（這一篇在序列裡的位置、論證用得到的材料有沒有就地寫進去、往外的連結綁在什麼上面），從兩次全分類改寫抽出。第三項有三種強度而只有一種完整——綁在本篇模型的一個變數上（「把其中一項換掉會變成什麼」），比列舉相鄰主題強、比「遇到 X 去讀 Y」強兩級；做不到最強的那一種代表本篇還沒建立帶變數的模型。另記偏移的成因固定是規則錯配，而射程限制放在原則自己身上沒有效力。writing-articles.md 加對應的動筆前檢查節

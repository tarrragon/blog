...
> **角色**：本卡是 `compositional-writing` 的支撐型原則（principle）、被 SKILL.md 第 3 原則「意圖顯性與層級貼合」引用、被 `references/writing-articles.md` 在 case 引用段落紀律段引用。
>
...
LLM 從 case 反推內容寫教學文章時、容易把 case 揭露當概念出發點、寫成「對應 [case]：揭露 N 個機制」段首結構。讀者尚未理解概念就被丟入案例細節、且跨章讀同句構會感同質。

實證觀察：跨 12 段新寫的章節、11 段都犯這個錯（11/12 比例）— 屬最大宗 systemic 違規。原因：
...
三段式只是 *引用結構* axis。案例引用的完整紀律含四個正交 axis、寫每段 case 引用時都要過一遍：

| Axis                     | 看什麼                                                                            | 違規徵兆                                     |
| ------------------------ | --------------------------------------------------------------------------------- | -------------------------------------------- |
| 引用「深度」             | Case 整體類型（10-30 行 skeleton / 30-50 行 medium / 50-200 行 rich）決定承接深度 | Skeleton case 引用補具體數字（編造）         |
| 引用「分層」             | Rich case 內部結構（觀察層 fact / 判讀層 derive）決定要不要分層標明               | 用「才是 / 必須 / 一定」強化詞合併兩層       |
| 引用「合成」             | 跨多 case 抽象 frame 時要 explicit 標「本章合成、非 case 原文」                   | 「兩 case 都揭露 X」但 X 在原文 grep 不到    |
| **引用「結構」**（本卡） | **段落順序：概念 → case → 通用三段**                                              | **段首是「對應 [case] 揭露」、沒概念定義句** |

四 axis 組合起來覆蓋 case 引用紀律完整面。本卡聚焦結構 axis、其他三 axis 屬 case-driven 寫作的特化紀律。

---

## 案例引用句構分流

即使遵守三段式紀律、跨章 case 引用句構仍會同質化。實證觀察：13 處 case 引用 11 處用同一句構「揭露 N 層失效控制面 — A、B、C」、讀者跨章連讀時把 case 引用當儀式而非論證。
...
如果 review 抓出大量「case 引用段首」issue、polish pass 的修法：
...
修法成本：每段補 1-2 句概念定義、單章約 5-10 分鐘、整篇 1-2 小時。
...
| 跨章 5+ 段用同句構「揭露 N 層失效控制面」    | Polish pass 句構分流                   |

...
| 9   | Reader simulation（換視角）                                                                        | 兩個 lens：(a) **自包含性**——拿掉所有 code block 重讀論述是否仍能 parse？跳段直接讀能拿到關鍵資訊？(b) **register/stance**——這段在「陳述概念」、還是在「管理 / 評價 / 絕對化讀者」（喊話 / 誇飾 / 必然）？register 類無穩定關鍵詞、keyword bank（輪 8）抓不到、**reader-sim 是主、keyword bank 是輔**；且這類最依賴 external cold-read、同 reviewer 模擬有限——production 教材建議刻意換視角或外部讀者。詳見 [teaching-prose-neutral-register](principles/teaching-prose-neutral-register.md)                                                                                                                                                             |

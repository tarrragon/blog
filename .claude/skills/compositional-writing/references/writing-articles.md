...
| 2   | 對意圖（[#67](principles/ease-of-writing-vs-intent-alignment.md)） | 開頭一句點題嗎？段落順序由「易讀」決定不是「好寫」決定？**〔新增〕去掉視覺標記後還能讀嗎、有無依賴 emoji/顏色/圖表作為唯一區分方式** |
...
Multi-pass review 的威力在於「每輪用不同 frame」。但常見的誤區是把**相同層次的問題檢查多輪**，導致無法發現根本問題。

技術文章中常見的三個檢查層次：

| 層次       | 檢查內容                                         | 檢查時機  | 根本問題的訊號                                                            |
| ---------- | ------------------------------------------------ | --------- | ------------------------------------------------------------------------- |
| **邏輯層** | 論證是否完整、推導是否跳過、方案是否充分列舉     | 第 1-2 輪 | 「有未回答的假設」「觀察直接跳執行」「只列一個選項」                      |
| **語意層** | 概念是否清晰、有無依賴視覺標記、概念邊界是否明確 | 第 2 輪   | 「用 emoji 作為唯一區分」「同一行表達兩個不同概念」「去掉顏色就無法理解」 |
| **視覺層** | 排版是否對齐、圖表是否正確渲染、容器寬度是否適配 | 第 3-4 輪 | 「emoji 斷行」「圖表無法顯示」「文字超出邊界」                            |

**關鍵原則**：

1. 邏輯和語意錯誤會表現成視覺問題
2. 不能用視覺修補替代邏輯或語意修正
3. 若多輪都在檢查視覺問題，表示根本問題沒被發現

**例**：emoji 在容器窄時斷行 → 看起來是「視覺層」問題 → 但根本原因可能是「語意層」（不應該用 emoji 作為唯一區分）或「邏輯層」（兩個概念本就應分開表達）。

參見知識卡片：[語意清晰度 vs 視覺區分](../../record/semantic-clarity-vs-visual-distinction.md)、[Multi-pass Review 的層次意識](../../record/multi-pass-review-layer-awareness.md)
...
**Version**: 0.7.0 — 補強 multi-pass review：第 2 輪檢查清單新增「層次意識」（去掉視覺標記後還能讀嗎、有無依賴 emoji/顏色/圖表）；新增「層次意識」段落說明邏輯層/語意層/視覺層的區別與優先順序；引用新知識卡片「語意清晰度 vs 視覺區分」和「Multi-pass Review 的層次意識」

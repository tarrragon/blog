...
- **A: 寫作規範 audit** — 專案寫作規範（如 AGENTS.md / markdown-writing-spec）/ compositional-writing 規範遵循
  - **字句層 grep（必跑）**：
    ...
    - 物理化錯配：`rg "撐得住|撐不住|撐得起|撐不起|能撐|撐住|撐起|撐不久|掛在|扛|垮|頂得住" <files>` — 同一張卡的第三種錯配，也是三種裡唯一有穩定關鍵詞的一種。抽象概念（證據 / 論證 / 結論 / 判準 / 方法 / 責任）配上承重、支撐、懸掛類動詞；判準可機械執行——問這個動詞照字面成立需要什麼物理條件，需要重量或支點的就是借來的。替換：足以支持 / 依賴 / 佐證 / 維持不久 / 負責。這一種的危害多一層：「撐得住」聽起來已經像答案，於是「支持什麼、到什麼範圍」不必被回答。例外是就地展開成可逐項對應的類比、以及主詞本來就是實體或系統負載
...
**Version**: 1.34.0 — Round 1-A 字句層 bank 加「物理化錯配」grep（`rg "撐得住|撐不住|撐得起|撐不起|能撐|撐住|撐起|撐不久|掛在|扛|垮|頂得住"`）：用詞搭配錯位的第三種、也是三種裡唯一有穩定關鍵詞的一種，判準可機械執行（問這個動詞照字面成立需要什麼物理條件）；危害多一層——借來的物理動詞聽起來已經像答案、讓「支持什麼、到什麼範圍」不必被回答。同步 compositional-writing v0.51.0。

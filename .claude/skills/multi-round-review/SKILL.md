...
**寫下這一輪要交付的是判定還是歸因，預設判定，歸因要另外授權。** 判定回答「這份產物現在讀得懂嗎」，產出是通過或不通過；歸因回答「這一句改動有沒有效」，產出是某次改動與某個結果之間的因果宣稱。判別問句是：**這一輪結束的時候，要能說出「它現在通過了」，還是要能說出「是那一句讓它通過的」。**

這一段排在工具清單之前，因為產出層級決定後面每一步的規模。歸因要單變數實驗，而單變數實驗的定義就是一次只准動一個變數——於是 N 次改動要 N 次全量驗證。**審查對象是累積式產物（題庫、案例庫、逐輪讀數）時這件事沒有收斂點**：驗證成本由既有存量決定而每次改動的價值固定，分母一直長而分子不動。一次實測的下場是單一工作階段新建 82 張票、關閉 35 張，而每一張票單獨看都是真缺陷。

判定為歸因時另外寫下要付的那幾樣：改動逐項驗、題庫要能重現、混淆因子要排除。判定為判定時驗收取整體通過、失敗才用二分定位，改動批次化。見 [審查的產出是判定還是歸因](references/principles/verdict-and-attribution-are-different-products.md)，那裡有器材凍結在第一批形狀的五個實例與完整的處置清單

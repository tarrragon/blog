...
**Version**: 1.58.0 — Round 2-B′ 冷讀 frame 的篇章層加一題「這一篇讓你具備什麼能力」。冷讀預設「讀得懂」等於「有內容」，而互相引用的集合裡有一種稿件讀得懂而說不出要交付什麼——一次三十九篇的量測裡，篇幅有一半在講鄰居的短文，冷讀把每一段的內容都答對而最後一題寫「看不出來」並自列三種互斥的可能。同一題另外驗路由承諾的形式：關於目的地的謂語才算，連結前的修飾語在實測裡一律被判為沒有說，所以這一維用關鍵詞比對會高估、要靠這個 frame 量。新增 `cross-links-eat-the-article-they-live-in` principle 卡。

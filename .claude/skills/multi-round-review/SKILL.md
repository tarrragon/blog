...
**同一個目錄底下可能兩種定位並存**：一個內容分類裡的文章是教材，而它的目錄頁是那個分類的操作手冊（成員表、路由、待辦），使用者的動作是照著它走到某一篇。目錄頁該有的正是文章不該有的——編號、清單、查表用的對照欄。所以定位按稿件要讀者做什麼判、不按目錄判，而 2-D 掃體例時要把目錄頁排除，否則會把它正確的形式報成違規。

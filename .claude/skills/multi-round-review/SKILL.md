...
規範裡的每一條規則都誕生自某一個定位，而共用一套規範之後那些規則累積成一個不記得自己來歷的池子。定位沒先定下來的時候，**規則的觸發條件會先於它自己的判斷標準生效**——要求補微案例的那一條觸發於「判讀 / 選型型內容」，於是一篇讓人理解某個現象的文章形狀對上了，照樣被要求補一則演示判錯代價的短敘事，而那條規則自己明寫著要挑後果最不直觀的。

判別定位的軸不是「內容裡有沒有可執行的東西」，而是**那個動作落在哪裡：在世界上動手，還是在腦中重新歸類**。一次五份探針的量測裡，教學控制項也給了可執行的內容（辨識訊號、可以改的東西），五份仍全部答讀一次就好——它要讀者做的是用一個框架重新詮釋已經發生的事。
...
**同一個內容集合底下可能兩種定位並存。** 一個分類裡的文章是教材，而它的目錄頁是那個分類的操作手冊（成員表、路由、待辦），使用者的動作是照著它走到某一篇。整個子集合也可能落在手冊那一側：一份規則語料的消費者是即將動手而要查一條規則的人，它解釋理由是為了讓規則套得準，不是為了讓人讀懂一個主題。這些位置該有的正是文章不該有的——編號、清單、查表用的對照欄。所以定位按稿件要讀者做什麼判、不按它放在哪裡判，而 2-D 掃體例時要把它們排除，否則會把正確的形式報成違規。
...
- **B⁷: 類型探針（受測單位是整篇的定位，不是句子也不是詞）** — 2-D 的判定由作者做，而作者知道自己想寫的是什麼，所以那個判定有結構上限。類型探針把判定移到多份獨立的理解上：**問法不能點名類型**，五個問題沒有一個提到手冊或教材——這一段寫給誰看、讀完之後的第一個動作是什麼、預期讀者會照著做嗎、會放在手邊隨時翻還是讀過一次就好、以及讓你這樣判斷的是哪幾句（逐字抄）。第五題是這個 frame 產出價值最高的一題，它直接交出裝置清單而不必事後回推。
  ...
  **同批要混入兩份控制文件且不標示**：一份無疑義的操作手冊（有編號步驟的流程）與一份無疑義的教材（概念解釋），機制與 B⁶ 的控制詞相同。控制項在「隨時翻」這一軸上分開，測試項的判讀才可歸因；控制項分不開就整批作廢。實測的分離度是五比零對五比零。

  判讀的主軸是**測試項靠哪一邊**，而兩個問題可以分開看：「會不會照著做」與「會不會放在手邊」是兩條軸，一次量測裡有一段改寫過的稿件在後者翻到了教材側而前者仍在手冊側——**部分移動要當成沒改完，不要當成改好了**。回報的引句一律回原文逐字核對，找得到就是文章自己這樣寫的，歸因結束、直接進處置。
...
- **D: 體例與定位一致** — 拿派發前寫下的那一句定位，逐節問「這一節的體例屬於哪一種定位」。這個 frame 抓的東西每一條規則都遵守了，所以其餘 frame 全部通過——它們問的是「這一條遵守了嗎」，而失配的稿件確實遵守了。可機械執行的判斷標準是**讀者讀完之後那個動作落在哪裡**：在世界上動手（轉小火、按比例準備、輸入指令）是手冊，在腦中重新歸類（用一個框架重新詮釋已經發生的事）是教材。內容裡有沒有可執行的東西不是判斷標準——教材也給辨識訊號與可以改的東西。掃的是**裝置**而不是語氣，七種形態各有樣子：表徵對映清單（「症狀：成因」冒號兩邊讓讀者對號）、檢查順序的指示（「凝固之後才看 X」）、配比與參數數字、順序關鍵加上顛倒的後果（「顛倒過來的話……就散」）、可執行的操作判準、具體器具與工具作法、**序列連接詞（先、之後、最後）**。最後一項最容易漏——把步驟編號換成序列連接詞只改掉一半，一次量測顯示「會不會放在手邊隨時翻」翻到了教材那一側而「預期讀者會照著做」仍在手冊那一側，五份全部引了那三個連接詞。產出是一張逐節表（節名 / 體例判定 / 命中哪些裝置 / 處置）。處置不是刪掉資訊——材料與手法該留就留，改的是框架：把「照著做的步驟」改寫成「這件事為什麼是這樣」，把「顛倒的後果」改寫成「順序在這裡有什麼物理意義」。見 [定位決定體例](references/principles/positioning-decides-form-before-any-rule-applies.md)

...
`description` 的體例由內容定位決定，而兩種定位共用同一條驗收。

**查閱型內容**（排錯記錄、工具筆記、術語卡的定義層）：`description` 回答「讀者在什麼情境下需要回來讀這篇」。使用者本來就帶著症狀或術語來，情境索引在這裡有用。

**教材**（規劃在序列裡的章節與文章）：`description` 說這一篇涵蓋什麼、交付哪一段觀念。**觸發條件式在這裡是缺陷**——以「⋯⋯時」收尾、「想知道 / 想確認 / 想判斷」起手、把症狀寫在前面、或寫成問句，這四種都要換掉。理由是框架會反過來決定文章：那句話寫成「遇到 X 時」，文章就得是 X 的答案，而教材照問題組織會產生沒有問就不知道的斷點。文章從標題發想、`description` 從文章發想，順序不能反。
...
- 查閱型：description 的主詞是「本文 / 這篇 / 記錄」→ 可能是摘要不是 trigger
- 教材：一個分類的 description 幾乎每一篇都以「時」收尾 → 那是規範長出來的形狀、不是各篇各自選的 → 整批重寫並回頭改規範

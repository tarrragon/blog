...
| **Multi-pass Refinement** 多輪精煉           | 第 1 輪實作不追求完美、預期會有未發現問題；設計第 2 / 3 輪用不同 frame 收斂、不是「再仔細一次」、是換角度看（[#82](/report/literal-interception-vs-behavioral-refinement/) / [#85](/report/methodology-multi-pass-embedding/)） |
...
呈現決策時的「五維度展開」（[`references/decision-dialogue.md`](references/decision-dialogue.md)）就是 multi-pass 在「決策呈現」場景的具體實現：每維度等於一輪 self-check。**「再仔細一次」≠ multi-pass — 同 frame 重看 catch 不到不同層的錯**。L4 review / pair / dogfood 才是行為錯誤的解、不是再寫一條 hook（[#82](/report/literal-interception-vs-behavioral-refinement/)）。
...
- [#74 決策呈現：選項 + 推薦 + 開放修改](/report/decision-presentation-options-recommendation/) — 不要開放問、給結構表 + 推薦、把整理問題的成本攤在 agent 而非 user
- [#75 主策略 + 補強：選擇不必互斥](/report/main-strategy-plus-supplementary/) — 多策略可疊加（structural + UX）、預設「五選一」會放掉互補可能
- [#76 分批 ship：可見性 + 風險 + 驗證三軸切分](/report/incremental-shipping-criteria/) — 「ship 順序 ≠ 重要程度」、低風險可見價值先 ship
